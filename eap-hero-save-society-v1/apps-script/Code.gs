...
/* === PATCH v3.5.0: Teacher bySession Real Sheet + Syntax Fix === */
...
function aq350PostProcessTeacherConsole_(payload){
  try{
    ...
    payload.bySessionServer = 'v3.5.0';
  }catch(err){
...
function aq350AttachBySessionToStudents_(students){
  (students || []).forEach(function(st){
    var attempts = [];
...
function aq350PostProcessTeacherConsole_(payload){
  try{
    if(payload && payload.data && payload.data.allStudents){
      payload.data.allStudents = aq350AttachBySessionToStudents_(payload.data.allStudents);
    }
    if(payload && payload.allStudents){
      payload.allStudents = aq350AttachBySessionToStudents_(payload.allStudents);
    }
    if(payload && payload.students){
      payload.students = aq350AttachBySessionToStudents_(payload.students);
    }
    payload.bySessionServer = 'v3.5.0';
  }catch(err){
    payload.bySessionServerError = String(err);
