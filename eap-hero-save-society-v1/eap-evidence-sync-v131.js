...
(function(){
  ...
  /*
     The Web App uses no-cors transport, so the browser cannot inspect
     the receiver response. Boss audio must therefore be posted only after
     a short evidence-first delay and retried. EAP_SpeakingAudio.gs v2 is
     idempotent: a repeated payload reuses its first attached Drive file.
  */
  const AUDIO_RETRY_DELAYS_MS = [1500, 3500, 7000, 12000];
  ...
  function audioPayload(file, evidenceId, state, base64){
    const person = profileFrom(state);
  ...
  function sendAudio(file, evidenceId, state){
    ...
    fileToBase64(file)
      .then(base64 => {
        ...
        AUDIO_RETRY_DELAYS_MS.forEach((delay, index) => {
          window.setTimeout(() => {
            postJson(payload);

            if (index === 0) {
              console.info(
                '[EAP Evidence v131] Boss audio upload started',
                evidenceId
              );
            }
          }, delay);
      ...
      .catch(error => {
        console.warn(
          '[EAP Evidence v131] Audio preparation failed',
          error
        );
      });
  }
  ...
  function captureSpeaking(entry, state){
    ...
    mask.querySelector('#eap-save')
      .addEventListener('click', () => {
        ...
        const evidenceQueued = submitRaw(entry, state, {
          evidenceId: evidenceId,
          output: output,
        ...
        /*
           Queue audio only after submit_evidence has been initiated.
           The delayed retry schedule gives Apps Script time to create the
           evidence row before EAP_SpeakingAudio.gs looks it up.
        */
        if (useAudio && evidenceQueued) {
          sendAudio(
            recordedFile,
            evidenceId,
            state
  ...
  window.EAPEvidenceSyncV131 = publicApi;

  /* Compatibility alias: current eap-hero.js resolves V130 first. */
  window.EAPEvidenceSyncV130 = publicApi;
})();