# EAP Hero: Save the Society v1z69 Integrated Boss Runtime Fix
...
## v1z43 Session Stars + Evidence Sync Fix
- แก้ปัญหาดาว/หลักฐานไม่ sync กับ Boss Gate
- เพิ่ม sessionEvidenceSummary() อ่าน evidence แบบลึกจาก portfolio, learningReports, reports, submissions, logs ฯลฯ
- อ่าน session จากข้อความ เช่น "Reading • Session 6" หรือ "S6"
- อ่าน skill จากข้อความ Reading/Writing/Listening/Speaking
- เพิ่ม earnedStarsHTML() และ sessionProgressBadgeHTML()
- เพิ่ม badge บน Session card: Complete/Evidence · x/2 found · earned stars
- repairLegacySessionCompletion() sync evidence/stars/progress เข้า state.sessions


## v1z44 Session Black Screen Recovery + Safe Render
- แก้ปัญหาเข้า session บางตัวแล้วจอดำ
- เพิ่ม safeOpenSession(), safeOpenSkillPath(), renderSessionRecoveryPanel()
- ครอบ renderSkillPath ด้วย try/catch
- ปรับ session card ให้เปิดผ่าน safeOpenSession
- จำกัด evidence badge sync ให้แตะเฉพาะ map/session card เพื่อลด DOM side effects
- เพิ่ม window error guard เฉพาะกรณี session render error


## v1z45 Emergency App Boot Recovery
- แก้ปัญหาหน้า app ว่าง/จอดำตั้งแต่โหลดแรก
- เพิ่ม emergencyBootWatchdog()
- เพิ่ม forceHome(), forceMap(), emergencyBootPanel()
- เพิ่ม fallback script ใน index.html หลัง window load
- ถ้า app ไม่ render จะบังคับ Home/Map หรือ Recovery Panel


## v1z46 S3/S6 Boss Notice Safe Render Fix
- แก้จอดำเมื่อกด Session 3 และ Session 6
- ทำ bossGateNoticeForSession() ให้ปลอดภัยและไม่ทำให้ทั้ง session render พัง
- เพิ่ม safeBossGateNoticeHTML() และ attachSafeBossGateNotice()
- เพิ่ม openSessionFromCard() ให้ map card เปิด session ผ่าน safe route
- ปรับ boot watchdog ไม่ให้แทรกระหว่าง render เร็วเกินไป


## v1z47 Boss Gate Naming + Unlock Clarity
- ทำให้ชัดว่า S3/S6/S9/S12/S15 ไม่ใช่ Boss
- Boss Gate 1/2/3/4/Final Boss เป็น checkpoint หลังผ่านชุด Session
- เพิ่ม sessionIsNotBossNote()
- เพิ่ม bossGateUnlockRuleText()
- เพิ่ม bossGateNamingLegendHTML() ใน Timeline
- ข้อความ locked/requirement อธิบายเงื่อนไขให้ชัดกว่าเดิม


## v1z48 Direct S3/S6 Route Hotfix
- แก้ S3/S6 ยังจอดำด้วย directOpenSession()
- เพิ่ม simpleSessionFallback() เพื่อเปิด Session แบบ safe fallback
- เพิ่ม openSkillMissionSafe() ให้เข้า Reading/Writing/Listening/Speaking ได้จาก fallback
- ใช้กับ checkpoint-adjacent sessions: S3, S6, S9, S12, S15
- S3/S6 ยังเป็น Session ปกติ ไม่ใช่ Boss


## v1z49 Hard Checkpoint Session Click Intercept
- ดักคลิก S3/S6/S9/S12/S15 ระดับ document capture ก่อน legacy handler
- บังคับเปิด forceCheckpointSessionFallback()
- patchCheckpointSessionCards() เปลี่ยน onclick หลัง render
- directOpenSession() สำหรับ checkpoint-adjacent sessions เปิด fallback ทันที


## v1z50 Unified Checkpoint Session UI
- แก้ S3/S6/S9/S12/S15 ไม่ให้ดูต่างจาก Session ปกติ
- Safe fallback เปลี่ยนเป็นหน้า Session Path แบบเดียวกับ S1/S2/S4/S5
- เพิ่ม unifiedCheckpointSessionPage()
- เพิ่ม checkpointSessionNoteHTML()
- ซ่อนภาษา emergency/recovered ที่ทำให้ผู้เรียนสับสน
- ยังกันจอดำด้วย safe route เหมือนเดิม


## v1z51 Normalized Checkpoint Session UI Polish
- ทำให้ S3/S6/S9/S12/S15 หน้าตาเหมือน Session ปกติมากขึ้น
- แก้ CEFR ที่เพี้ยนเป็น A2-Boss Gate 1 ให้กลับเป็น A2-B1+
- ลด checkpoint note ให้เป็นแถบเล็กด้านล่าง


## v1z52 Hide Four Skills Hub from Student Flow
- ซ่อน Four Skills Hub จาก Student Mode
- ผู้เรียนใช้ flow หลัก Map → Session Path → Mission เท่านั้น
- ถ้าเข้าหน้า Hub ใน Student Mode จะ redirect กลับ Session Path/Map


## v1z53 Clear Pass Criteria + Progress Diagnostics
- เกณฑ์ผ่าน Session: Core + Support Mission ต้องได้ 60+ ทั้งคู่
- Boss Gate เปิดเมื่อผ่าน Session ครบชุดและ Gate ก่อนหน้าผ่านแล้ว
- เพิ่ม Progress Diagnostics บอกว่าติดตรงไหน

## v1z54 Storage Quota Guard + Compact Save
- แก้ Submit ไม่ไปเมื่อ localStorage เต็ม (QuotaExceededError)
- saveState() ไม่ throw แล้ว แต่ prune/retry/emergency minimal save
- compactPortfolioEntry() ตัด output/prompt/log ที่ยาวเกิน
- จำกัด portfolio/reports/logs ให้ไม่โตจนเกิน quota
- เพิ่ม optimizeStorageNow() และ renderStorageDiagnostics()

## v1z55 Session Star Sync + CEFR Storage Cleanup
- แก้ S6 มี report/score แล้วแต่ดาวไม่ sync
- ดาว Session คำนวณจาก Core + Support score: avg >=85 = 3 ดาว, >=70 = 2 ดาว, >=60 = 1 ดาว
- แก้ CEFR label เพี้ยนเป็น CEFR Boss Gate 1 ให้กลับเป็น CEFR A2-B1+
- เพิ่ม syncSessionStarsNow(), deepStorageCleanupNow()
- ลด storage quota warning loop จาก runtime errors

## v1z56 Visible Session Completion Badge + CEFR Final Fix
- เพิ่ม badge ชัดเจนว่า Session Passed / Not passed บน Map และ Session Path
- แสดงดาว, avg, Core score, Support score ให้เห็นทันที
- Mark mission cards ด้วย Passed badge จากคะแนนจริง
- แก้ CEFR Boss Gate 1 ด้วย text walker ทั้งหน้าให้กลับเป็น CEFR A2-B1+
- เพิ่ม refreshVisibleProgressNow()


## v1z57 EAP Core Content + Replay Challenge Bank
- ล็อกแกนทักษะ S1-S15 แบบ Core + Support ตาม EAP ปี 2 A2-B1+
- Boss Gate 1-4 + Final Boss เป็น checkpoint รวม Reading/Writing/Listening/Speaking
- ปรับ S8 เป็น Paragraph Structure Lab ไม่ใช่ Midterm Boss
- เพิ่ม replay topic bank ราย Session: เล่นซ้ำได้และสุ่ม scenario/prompt ใหม่
- เพิ่ม question bank แบบ scenario-based ลดการเดาคำตอบ
- ปรับ scoring ให้ใช้ source keywords + skill markers + connector + specificity แทนตอบกว้าง ๆ
- เพิ่ม renderFinalContentRoadmap()


## v1z58 AI Help + Prediction + Difficulty Integration
- เพิ่ม AI Learning Layer: AI Help, AI Prediction, AI Difficulty
- AI Help เป็น scaffold เท่านั้น: hint / frame / keyword / checklist ไม่ใช่เฉลย
- AI Prediction ประเมิน readiness, risk, next action จาก score, stars, AI use, weak skills
- AI Difficulty แนะนำ easy / normal / hard / challenge ตาม readiness
- เพิ่ม AI Learning Coach บนหน้า Session Path
- เพิ่ม renderAILearningDiagnostics()


## v1z59 Mission-Specific AI Help + Question Alignment
- แก้ AI Help/คำถามไม่ตรงกับ mission scenario
- S1 academic goal audit ใช้คำถาม skill problem / stronger goal / evidence ไม่ใช่ target audience
- เพิ่ม EAP_MISSION_ALIGNMENT ครบ S1-S15
- Reading questions, help steps, sentence frames, useful vocab ตรงกับ focus ของ session
- เพิ่ม renderMissionAlignmentDiagnostics()


## v1z60 Student Mission Entry Fix
- แก้ปุ่ม Start Mission ถูกดันหายเพราะ Challenge + AI layer ทำให้ card สูงเกิน
- เพิ่ม mission-entry-strip ด้านบนหน้า Session Path ให้กด Start Reading/Writing/Speaking ได้ทันที
- บังคับ original Start buttons ให้ visible/clickable
- ซ่อน Four Skills Hub และ Debug buttons ใน Student flow
- ย่อ Challenge/AI notes ไม่ให้บังปุ่ม Start


## v1z61 Direct Student Session Path
- Replaced the native Session Path renderer, not merely a later DOM patch.
- Core and Support Start buttons render before long guidance content.
- Guidance/AI scaffold is collapsible under Mission guide.
- Removes Four Skills Hub and Debug buttons directly from Session Path markup.
- Checkpoint sessions S3/S6/S9/S12/S15 use the same Session Path renderer.
- Checkpoint click interceptor ignores mission-launch buttons.


## v1z62 Strict Pass Truth + Evidence Integrity
- Fixes false “Session Passed” caused by legacy session done/completed flags.
- A Session can pass only when both Core and Support scores are >= 60.
- Strict evidence matching uses direct session + skill fields first; text mentions cannot misclassify Writing as Reading.
- Session average includes missing skills as 0, so 90 + 0 becomes 45, not 90.
- Stars are zero unless both required skills pass.
- Mission cards distinguish Start / Retry / Replay based on exact score.
- Legacy false completion state is reconciled automatically at boot.


## v1z63 UK–Thailand Co-Teaching S/B Alignment
- Replaces all S1–S15 map/session titles with the agreed Global Campus Mission names.
- Adds an every-week British co-teacher role, Live Drop, Wildcard Question, Human Proof and Human Stamp reward.
- Replaces the Boss structure with B1–B5:
  1. Global Learner Clearance
  2. Evidence Court Live
  3. Academic Makeover Studio
  4. International Help Desk Crisis
  5. Human Override Summit
- Rebuilds Boss Gate records to align with the 3-session checkpoint sequence: S1–S3, S4–S6, S7–S9, S10–S12, S13–S15.
- Boss Gate pages now present the British co-teacher live role and four-skill checkpoint contract.


## v1z64 Real Reading Live Drop Alignment
- Reading is now a real short Live Drop passage, not a one-sentence task prompt.
- S1 has four fully written passages: Academic Goal Audit, Study Strategy Choice, Learning Reflection, and Skill Diagnosis File.
- S2–S15 create expanded short authentic reading cases from the selected replay topic.
- Every Reading Mission shows source type, British co-teacher role, silent-read → human-read → clarify → evidence-answer flow.
- Questions, sentence frames, vocabulary and placeholders align with the specific reading pack.
- Generic alignment injectors are disabled inside real reading pages to prevent question/frame mismatch.


## v1z65 Weekly Context Game Engine
- Adds `eap-hero-weekly-context.js`, which converts the 15-week co-teaching plan into actual Core/Support game missions.
- Each Session now has a theme, learning target, team activity, British co-teacher role, wildcard, human proof, source/artifact, frames, vocabulary and expected output.
- Reading, Writing, Listening and Speaking screens now follow the weekly context instead of generic prompt templates.
- AI Help now receives the same weekly skill alignment.
- Listening is audio-first: the complete script is not displayed before review.


## v1z66 Live Conversation Speaking
- Replaces the large self-transcript / self-checkbox speaking form with a two-round live conversation.
- Round 1: speak to the British co-teacher using a three-note quick plan.
- Round 2: respond to a live Wildcard question.
- Human Stamp is selected by the British co-teacher or Thai instructor: Clear, Developing, or Retry.
- Glow + Grow is recorded as short human feedback.
- Speech-to-text is moved into an optional collapsed review area and is no longer the main task.
- Evidence save requires Round 1, Wildcard response, at least two quick-plan notes and a Human Stamp.
- Existing portfolio pipeline stays in use; the saved evidence includes round duration, plan, Human Stamp, Glow/Grow, reflection and optional transcript.


## v1z67 Route Progression Unlock Fix
- Fixes the old Continue behavior that reopened the current Session after it had already passed.
- Passing S1 now unlocks S2, synchronizes the Map, and shows a direct “Continue to S2” route card.
- Route order is enforced: S1 → S2 → S3 → B1 → S4 → S5 → S6 → B2 → S7 → S8 → S9 → B3 → S10 → S11 → S12 → B4 → S13 → S14 → S15 → Final Boss.
- Boss Gate availability uses the agreed pass contract: all three prerequisite Sessions passed plus the previous Boss Gate cleared. It does not incorrectly demand all four skills before the integrated Boss.
- Completing a Boss Gate saves Boss completion state and unlocks the first Session in the next three-session block.


## v1z68 Clean Route Ledger + Map Clarity
- Creates a separate UK–Thailand Course Run ledger so historic test evidence cannot unlock future Sessions or Boss Gates.
- Safely carries the current completed S1 result into the new route when the learner upgrades while on S1.
- New portfolio evidence is tagged with the current course-route version.
- S1 pass → S2 unlock is now calculated from the clean route ledger, not all historic local data.
- Boss Gate B1–B5 unlocks only from the current route's three prerequisite Session passes and prior Boss Gate completion.
- Replaces generic Continue Session with the exact next route, such as “Continue to S2”.
- Replaces raw internal “Need Reading 0/60…” map text with compact Session/Boss status pills.


## v1z69 Integrated Boss Runtime Fix
- Fixes the Boss runtime renderer crash caused by an undefined result variable on the first Boss question screen.
- Boss metadata is now stored before rendering the battle, so B1–B5 open reliably.
- Boss questions are drawn across all three prerequisite Sessions, not only the final Session of the block.
- The runtime screen shows the Digital Round, the following Live Human Round, and the Boss Wildcard.
- A cleared Boss records its gate ID/title, awards the right Human badge, syncs the clean route, and unlocks the next Session block.
