/* =========================================================
   EAP Hero Living Mission Runtime v20260711
   Phase C-F runtime layer
   - Turns Living Mission metadata into visible gameplay conditions.
   - Safe runtime only: countdown HUD, independent-run hint lock,
     one-chance submit lock, pressure feedback, and dynamic reflection cue.
   - Does not change academic score, pass/fail, Sheet sync, teacher review,
     boss completion, or route unlock rules.
========================================================= */
(function(){
  'use strict';

  var VERSION='v20260711-EAP-LIVING-MISSION-RUNTIME-CF2';
  ...
  function isSkillPage(){
    ...
    var t=text(a.innerText);
    return /Reading Mission|Writing Mission|Listening Mission|Speaking Mission/i.test(t)&&!/Boss Gate|Boss Battle/i.test(t);
  }

  function targetHost(){
    var a=app();if(!a)return null;
    return a.querySelector('.phase-b-mission-card,#eapLivingMissionCard,.phase-a2-panel,.panel,main section,section')||a;
  }
  ...
  function lockHints(m){
    var should=m&&m.modifierId==='no-extra-hint';
    document.querySelectorAll('button,a').forEach(function(el){
      var t=text(el.textContent);
      if(/Need a 10-sec clue|AI Mentor|Hint|Clue/i.test(t)&&!/Mission condition|Independent Run/i.test(t)){
        el.classList.toggle('eap-living-hint-locked',!!should);
      }
    });
  }

  function submitButtons(){
    return Array.prototype.slice.call(document.querySelectorAll('button')).filter(function(el){
      return /Submit|Save Evidence|Finish|Check Answer|บันทึก|ส่งคำตอบ/i.test(text(el.textContent));
    });
  }

  function installOneChance(m){
    if(!m||m.modifierId!=='one-chance')return;
    submitButtons().forEach(function(btn){
      ...
      btn.addEventListener('click',function(){
        ...
        submitted=true;
        setTimeout(function(){
          submitButtons().forEach(function(b){if(b!==btn)b.classList.add('eap-living-one-chance-used');});
        },40);
      },true);
    });
  }
  ...
  function renderHud(m){
    var host=targetHost();if(!host)return;
    ...
    if(left<=0){
      ...
      hud.querySelector('.lmrt-note').textContent='Time reached. Finish carefully; your academic score is not auto-failed.';
    }
  }
  ...
  function applyReflection(m){
    ...
    if(ta&&!ta.dataset.eapLivingReflection){
      ...
      var lead=ta.parentElement&&ta.parentElement.querySelector('.lead');
      if(lead)lead.textContent=m.reflectionPrompt;
    }
  }
