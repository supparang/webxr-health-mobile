...
async function get(action,params={}){const u=new URL(endpoint);u.searchParams.set('action',action);Object.entries(params).forEach(([k,v])=>u.searchParams.set(k,String(v)));u.searchParams.set('_',Date.now());const r=await fetch(u,{redirect:'follow',signal:AbortSignal.timeout(60000)});const t=await r.text();if(!r.ok)throw new Error(`${action} HTTP ${r.status}: ${t.slice(0,300)}`);return JSON.parse(t.replace(/^[^(]+\(/,'').replace(/\);?$/,''));}
