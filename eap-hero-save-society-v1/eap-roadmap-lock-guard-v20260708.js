/* =========================================================
   EAP Hero Roadmap Lock Guard v20260709
   V9 CLASSROOM LOCAL+SHEET UNLOCK
   - ONE rule for roadmap + session grid.
   - Unlock uses best evidence from Cloud/Sheet restored rows AND fresh local
     rows just submitted in the current browser, so students do not wait for
     a slow Apps Script/Fast Resume cache before S2 opens after S1 complete.
   - Still hides future locked progress and never locks active question UI.
   - Current route is the first incomplete route. If S1 is complete, S2 opens.
...
const VERSION='v20260709-EAP-ROADMAP-LOCK-GUARD-V9-CLASSROOM-LOCAL-SHEET-UNLOCK';
const PACK='EAP_HERO_SESSION_CONTENT_PACK';
const STATE='EAP_HERO_PROGRESS_V3';
const PASS=60;
const SKILLS=['reading','listening','writing','speaking'];
const STYLE='eapUnifiedSeqLockStyleV9ClassroomLocalSheet';
const TOAST='eapUnifiedSeqLockToastV9ClassroomLocalSheet';

function clean(v){return String(v==null?'':v).replace(/\s+/g,' ').trim()}
function lower(v){return clean(v).toLowerCase()}
function num(v){const n=Number(v);return Number.isFinite(n)?n:0}
function cap(v){const s=clean(v);return s.charAt(0).toUpperCase()+s.slice(1)}
function norm(v){const raw=clean(v).toUpperCase();if(!raw)return'';if(/^\d+$/.test(raw))return'S'+Number(raw);let m=raw.match(/^S(?:ESSION)?\s*0?(1[0-5]|[1-9])$/);if(m)return'S'+Number(m[1]);m=raw.match(/^B(?:OSS)?\s*0?([1-5])$/);if(m)return'B'+Number(m[1]);return raw}
...
function isCloudState(s){s=s||readState();return !!(s.serverResume&&(s.serverResume.cloudFirst===true||num(s.serverResume.recordCount)>0||clean(s.serverResume.resumeKey)))||s.cloudResumeStatus==='ok'||clean(s.currentCloudRoute)||s.cloudVerified===true}
function isLegacyOnly(e){return e&&(e.legacyCompletion===true||clean(e.legacyCompletion).toUpperCase()==='TRUE'||/legacy evidence retained after browser-storage migration/i.test(clean(e.output)))}
...
function addCloudSessionProgress(out,s){
  ...
  Object.keys(s.sessionProgress).forEach(routeKey=>{
    const st=s.sessionProgress[routeKey];
    const rid=norm(st&&st.routeId||st&&st.sessionId||routeKey);
    if(!rid||!routeById(rid))return;
    const scores=st.scores||{};
    Object.keys(scores).forEach(sk=>{
      const score=num(scores[sk]);
      out.push({routeId:rid,sessionId:rid,skill:sk,score:score,bestScore:score,passed:score>=PASS,cloudVerified:true,serverVerified:true,restoredFromSheet:true,resumeSource:'server_sessionProgress'});
    });
    if(st.complete===true&&Array.isArray(st.required)){
      st.required.forEach(sk=>{
        if(scores[sk]==null)out.push({routeId:rid,sessionId:rid,skill:sk,score:100,bestScore:100,passed:true,cloudVerified:true,serverVerified:true,restoredFromSheet:true,resumeSource:'server_completedSessions'});
      });
    }
  });
}
...
function entries(){
  ...
  ['portfolio','attempts','evidence','summary','records'].forEach(k=>{if(Array.isArray(s[k]))out.push.apply(out,s[k])});
  if(s.sessions&&typeof s.sessions==='object'){
    Object.keys(s.sessions).forEach(k=>{
      const v=s.sessions[k];
      if(Array.isArray(v))v.forEach(x=>out.push(Object.assign({sessionId:k},x||{})));
      else if(v&&typeof v==='object')Object.keys(v).forEach(sk=>{const x=v[sk];if(x&&typeof x==='object')out.push(Object.assign({sessionId:k,skill:sk},x));});
    });
  }
...
function completedByCloudState(rid){const s=readState();const r=norm(rid),n=(r.match(/^S(\d+)$/)||[])[1];const c=s.completedSessions||{};return c[r]===true||(n&&c[n]===true)}
function routeStatus(id){
  const list=routeList();
  const r=list.find(x=>norm(x.routeId)===norm(id));
  if(!r)return{routeId:norm(id),complete:false,required:[],passed:[],missing:[],scores:{}};
  const rid=norm(r.routeId),b=best(),req=required(r),passed=[],missing=[],scores={};
  req.forEach(s=>{const item=b[rid+'|'+s];scores[s]=item?item.score:0;if(item&&(item.passed||item.score>=PASS))passed.push(s);else missing.push(s)});
  if(missing.length&&completedByCloudState(rid)){missing.splice(0,missing.length);req.forEach(s=>{if(passed.indexOf(s)<0)passed.push(s);if(!scores[s])scores[s]=100});}
  return{routeId:rid,routeType:r.routeType,title:r.title||'',complete:missing.length===0,required:req,passed:passed,missing:missing,scores:scores};
}
...
function normalizeStoredIfLocked(){const cur=currentRoute();if(!cur)return false;try{const active=norm(localStorage.getItem('EAP_HERO_ACTIVE_ROUTE')||localStorage.getItem('EAP_HERO_CURRENT_ROUTE')||'');if(!active||!isUnlocked(active)||routeStatus(active).complete){setActive(cur.routeId);return true}}catch(e){}return false}
