...
const VERSION='v20260708-EAP-ROADMAP-LOCK-GUARD-V3-UNIFIED-SEQUENTIAL';
const PACK='EAP_HERO_SESSION_CONTENT_PACK';
const STATE='EAP_HERO_PROGRESS_V3';
const PASS=60;
const SKILLS=['reading','listening','writing','speaking'];
const STYLE='eapUnifiedSeqLockStyle';
const TOAST='eapUnifiedSeqLockToast';
...
.eap-lock-note,.eap-done-note{font-size:11px;font-weight:950;line-height:1.35;margin-top:4px}.eap-lock-note{color:#9a6700}.eap-done-note{color:#087f5b}
...
function setNote(card,cls,text){let n=card.querySelector(':scope > .eap-lock-note,:scope > .eap-done-note');if(!text){if(n)n.remove();return}if(!n){n=document.createElement('div');const a=card.querySelector('.rm-actions')||card.querySelector('button')?.parentElement;if(a)a.insertAdjacentElement('beforebegin',n);else card.appendChild(n)}n.className=cls;n.textContent=text}
function routeText(routeId){const st=routeStatus(routeId);return st.missing.length?('🎯 ด่านปัจจุบัน · ต้องผ่าน '+st.missing.map(cap).join(' + ')+' ≥ 60/100'):'🎯 ด่านปัจจุบัน'}

function decorateRoadmap(){document.querySelectorAll('[data-eap-roadmap-card]').forEach(card=>{const rid=norm(card.getAttribute('data-eap-roadmap-card')),idx=routeIndex(rid),st=routeStatus(rid),open=isUnlocked(rid),done=st.complete,current=idx===maxOpenIndex()&&!done;card.classList.toggle('eap-locked',!open);card.classList.toggle('eap-done',open&&done);card.classList.toggle('eap-current',current);card.dataset.eapUnlocked=open?'1':'0';card.dataset.eapComplete=done?'1':'0';card.querySelectorAll('[data-eap-roadmap-route],[data-eap-roadmap-brief]').forEach(btn=>{btn.disabled=!open;btn.setAttribute('aria-disabled',open?'false':'true');btn.dataset.eapLocked=open?'0':'1'});if(!open)setNote(card,'eap-lock-note','🔒 ล็อกไว้ก่อน · '+reason(rid));else if(done)setNote(card,'eap-done-note','✓ ผ่านเกณฑ์แล้ว · ย้อนทบทวนได้');else setNote(card,'eap-lock-note',routeText(rid));});}
...
function decorateSessionGrid(){findSessionCards().forEach(item=>{const card=item.card,rid=item.routeId,idx=routeIndex(rid);if(idx<0)return;const st=routeStatus(rid),open=isUnlocked(rid),done=st.complete,current=idx===maxOpenIndex()&&!done;card.classList.toggle('eap-session-unified-locked',!open);card.classList.toggle('eap-session-unified-done',open&&done);card.classList.toggle('eap-session-unified-current',current);card.dataset.eapUnlocked=open?'1':'0';card.querySelectorAll('button,a,[role="button"]').forEach(btn=>{btn.disabled=!open;btn.setAttribute('aria-disabled',open?'false':'true');btn.dataset.eapLocked=open?'0':'1'});if(!open)setNote(card,'eap-lock-note','🔒 ล็อกไว้ก่อน · '+reason(rid));else if(done)setNote(card,'eap-done-note','✓ ผ่านเกณฑ์แล้ว · ย้อนทบทวนได้');else setNote(card,'eap-lock-note',routeText(rid));});}
function decorate(){addCss();normalizeStoredIfLocked();decorateRoadmap();decorateSessionGrid();}
function routeFromButton(btn){const explicit=btn.getAttribute('data-eap-roadmap-route')||btn.getAttribute('data-eap-roadmap-brief')||'';if(explicit)return norm(explicit);const card=btn.closest('[data-eap-roadmap-card]');if(card)return norm(card.getAttribute('data-eap-roadmap-card'));const sessionCard=btn.closest('.eap-session-unified-locked');if(sessionCard){const t=clean(sessionCard.textContent||'');const m=t.match(/SESSION\s+(1[0-5]|[1-9])/i);if(m)return'S'+Number(m[1])}return''}
