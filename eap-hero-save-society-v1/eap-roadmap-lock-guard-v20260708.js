...
function rememberBossCountFromDom(){
  const t=clean((document.getElementById('app')||document.body).innerText||'');
  const m=t.match(/Session\s+Bosses\s*:\s*(\d+)\s*\/\s*15/i);
  if(!m)return;
...
function addCloudSessionProgress(out,s){
  ...
  Object.keys(s.sessionProgress).forEach(routeKey=>{
    ...
    Object.keys(scores).forEach(sk=>{const score=num(scores[sk]);out.push({routeId:rid,sessionId:rid,skill:sk,score:score,bestScore:score,passed:score>=PASS,cloudVerified:true,serverVerified:true,restoredFromSheet:true,resumeSource:'server_sessionProgress'})});
    if(st.complete===true&&Array.isArray(st.required))st.required.forEach(sk=>{if(scores[sk]==null)out.push({routeId:rid,sessionId:rid,skill:sk,score:100,bestScore:100,passed:true,cloudVerified:true,serverVerified:true,restoredFromSheet:true,resumeSource:'server_completedSessions'})});
  });
}
function entries(){
  const s=readState(),out=[];rememberCompletedFromState(s);
  ['portfolio','attempts','evidence','summary','records'].forEach(k=>{if(Array.isArray(s[k]))out.push.apply(out,s[k])});
  if(s.sessions&&typeof s.sessions==='object')Object.keys(s.sessions).forEach(k=>{const v=s.sessions[k];if(Array.isArray(v))v.forEach(x=>out.push(Object.assign({sessionId:k},x||{})));else if(v&&typeof v==='object')Object.keys(v).forEach(sk=>{const x=v[sk];if(x&&typeof x==='object')out.push(Object.assign({sessionId:k,skill:sk},x))})});
...
function routeStatus(id){
  const list=routeList();const r=list.find(x=>norm(x.routeId)===norm(id));
...
[data-eap-locked-progress-detail="1"]{display:none!important}.battle-layout.eap-session-unified-locked,.challenge-card.eap-session-unified-locked{opacity:1!important;filter:none!important;pointer-events:auto!important;background:inherit!important;border-style:solid!important}.battle-layout.eap-session-unified-locked button,.challenge-card.eap-session-unified-locked button{pointer-events:auto!important;cursor:pointer!important}.eap-lock-note,.eap-done-note{display:none!important}
#${TOAST}{position:fixed;left:50%;bottom:22px;transform:translateX(-50%);z-index:999999;max-width:min(680px,calc(100vw - 28px));padding:13px 16px;border-radius:14px;background:#7c2d12;color:#fff;box-shadow:0 14px 35px rgba(0,0,0,.25);font:800 13px Arial,'Noto Sans Thai',sans-serif;text-align:center}`;document.head.appendChild(s)}
...
function findSessionCards(){const found=[];const seen=new Set();document.querySelectorAll('#app .session-tile,#app .session-card,#app .checkpoint-card').forEach(card=>{if(isActiveMissionArea(card))return;const t=clean(card.textContent||'');const m=t.match(/SESSION\s+(1[0-5]|[1-9])\b/i);if(!m||seen.has(card))return;seen.add(card);found.push({card:card,routeId:'S'+Number(m[1])})});return found}
...
function decorate(){addCss();removeOldNotes(document);rememberBossCountFromDom();normalizeStoredIfLocked();decorateRoadmap();decorateSessionGrid()}
