...
(function(){
  'use strict';
  const VERSION='v3.5.0-teacher-bysession-real-sheet-syntax-fix';
  ...
  async function loadTeacherData(){
    ...
    adapt(td,sd,hd);
    renderAll(td,sd,hd);
    console.log('[AIQuest Teacher v3.5.0] loaded', {students:state.students.length,attempts:state.attempts.length,adapter:state.adapterInfo,raw:td,summary:sd,health:hd});
  }
  ...
  function renderSheets(d,s,h){
    const version=first(h||{},['version','serverVersion'])||first(d||{},['version','serverVersion'])||'v3.5.0';
    const a=state.adapterInfo||{};
  ...
  function bind(){
    ...
    $('previewBtn').onclick=()=>window.open('./index.html?v=20260614-bysessionreal350','_blank');
