/* =========================================================
   ...
   V2 DIRECT SKILL-LAUNCH INTEGRATION
   ...
   - Works from both Phase A "next task" and native Start Reading/Listening/Writing/Speaking buttons.
   - Shows a Phase B mission preview on the Session hub and full mission card on Skill pages.
...
(function(){
  ...
  var VERSION='v20260711-EAP-PHASE-B-DYNAMIC-REPLAY-V2-DIRECT-LAUNCH';
  var STATE_KEY='EAP_PHASE_B_DYNAMIC_REPLAY_V1';
  var STYLE_ID='eap-phase-b-dynamic-replay-style-v2';
  ...
  function inferSession(){var app=document.getElementById('app'),m=text(app&&app.innerText).match(/Session\s*(1[0-5]|[1-9])\b/i);return m?'S'+Number(m[1]):'';}
  function ensurePreview(){var app=document.getElementById('app');if(!app||/Boss Gate|Boss Battle/i.test(text(app.innerText)))return;var sessionId=inferSession();if(!sessionId)return;var task=activeTaskFor(sessionId),run=currentRun(sessionId);if(!task||!run)return;var preview=app.querySelector('.phase-b-preview');if(!preview){var host=app.querySelector('.session-path-panel,.session-shell,.panel,main section,section');if(!host)return;preview=document.createElement('div');preview.className='phase-b-preview';var anchor=host.querySelector('.pass-progress,.session-progress');if(anchor&&anchor.parentNode===host)host.insertBefore(preview,anchor);else host.insertBefore(preview,host.firstChild);}var cached=read()[(run.runId||sessionId)+'::'+task.taskId],label=cached&&cached.encounter||'Fresh mission will be generated when you start',source=cached&&cached.sourceTitle||'New authored scenario';preview.innerHTML='<div class="phase-b-row"><span class="phase-b-chip">🧠 Phase B Ready</span><span class="phase-b-chip">'+esc(task.stage)+'</span><span class="phase-b-chip">'+esc(task.skill)+'</span><span class="phase-b-chip">'+esc(run.adaptiveLevel)+'</span></div><div><b>Next dynamic mission:</b> '+esc(label)+' · '+esc(source)+'</div>';}
  function decorate(){addStyle();ensurePreview();var app=document.getElementById('app'),m=current();if(!app||!m||/Boss Gate|Boss Battle/i.test(text(app.innerText)))return;var host=app.querySelector('.phase-a2-panel')||app.querySelector('.session-path-panel,.panel,main section,section');if(!host)return;var card=host.querySelector('.phase-b-mission-card');if(!card){card=document.createElement('div');card.className='phase-b-mission-card';host.insertBefore(card,host.firstChild);}card.innerHTML='<div class="phase-b-row"><span class="phase-b-chip">🧠 Phase B</span><span class="phase-b-chip">'+esc(m.encounter)+'</span><span class="phase-b-chip">'+esc(m.adaptiveLabel)+'</span>'+(m.twistLabel?'<span class="phase-b-chip">⚡ '+esc(m.twistLabel)+'</span>':'')+(m.timeLimitSec?'<span class="phase-b-chip">⏱ '+m.timeLimitSec+' sec</span>':'')+'</div><div class="phase-b-prompt">'+esc(m.missionPrompt)+'</div><div class="phase-b-source"><strong>Scenario:</strong> '+esc(m.sourceTitle||'Fresh mission')+' · <strong>Reward:</strong> '+esc(m.rewardToken)+'</div>';}
