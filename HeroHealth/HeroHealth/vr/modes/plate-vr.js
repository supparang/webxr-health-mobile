...
AFRAME.registerComponent('game-manager', {
  ...
  start(){ this._clear(); this.ui.resultPanel?.setAttribute('visible','false');
    ...
    this.running=true; this._updateHUD('PLAY'); this._stop('bgm'); this._play('bgm'); this.mq.start(this.goal); this._spawnLoop();
    this.tickT=setInterval(()=>{ if(!this.running)return; this.timeLeft--; if(!this.fever.active) this._fever(-6); this._updateHUD(); this.mq.second(); if(this.timeLeft<=0) this.end(); },1000);
  },
  ...
  _score(e,ok){ if(ok){ this.hits.good++; this.score+=10*this.combo; this.combo=Math.min(999,this.combo+1); this.maxCombo=Math.max(this.maxCombo,this.combo); this.streak++; this._fever(+14); this._play('pop'); }
    ...
    this._updateHUD(); this.mq.junk(); setTimeout(()=>e.remove(),40);
  },
