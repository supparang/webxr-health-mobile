// === /HeroHealth/modes/goodjunk.safe.js (2025-11-12 FINAL) ===
// โหมด Good vs Junk + Goal 5/สุ่มจาก 10 + Mini 3/สุ่มจาก 10 (เติมใหม่อัตโนมัติ)
// ใช้ร่วมกับ: /HeroHealth/vr/mode-factory.js, /HeroHealth/vr/quest-hud.js, /HeroHealth/vr/particles.js, /HeroHealth/vr/mission.js
...
export async function boot(cfg = {}) {
  ...
  // ---- Stat helpers (อ่านจาก deck.stats) ----
  const G = {
  ...
  // ---- Deck ----
  // (ต้องใช้ mission.js ที่มี drawGoals, draw3, getProgress, isCleared ฯลฯ)
  const deck = new MissionDeck({ goalPool: GOAL_POOL, miniPool: MINI_POOL });
  deck.drawGoals(5);
  deck.draw3();

  // ส่งให้ HUD โฟกัสทีละรายการ (ตัวแรกที่ยังไม่เสร็จ)
  ...
  function onSec(){
    ...
    // เติมเควสต์/เป้าเมื่อเคลียร์ครบ
  ...
  return factoryBoot({
    ...
    window.addEventListener('hha:time', (e)=>{ if((e.detail?.sec|0)<=0){
      ...
      window.dispatchEvent(new CustomEvent('hha:end',{detail:{
        ...
        questsTotal: (deck.currentMini||[]).length
      }}));
    }});
    // แจ้ง HUD ครั้งแรก
    pushQuest('เริ่ม');
