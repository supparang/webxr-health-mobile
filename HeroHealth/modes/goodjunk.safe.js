// === /HeroHealth/modes/goodjunk.safe.js (2025-11-13 FIX+COACH) ===
...
export async function boot(cfg = {}) {
  ...
  const G = {
    good: s=>s.goodCount|0, junk: s=>s.junkMiss|0,
    score:s=>s.score|0, comboMax:s=>s.comboMax|0, tick:s=>s.tick|0
  };
  ...
  const GOAL_POOL = [
    ...
    // ✅ “พลาดไม่เกิน 6 ครั้ง” ใช้ junkMiss อย่างเดียว (ตามการแก้ onExpire ด้านล่าง)
    { id:'g_nojunk6', label:'พลาดไม่เกิน 6 ครั้ง',     level:'normal', target:6,   check:s=>G.junk(s)<=6,      prog:s=>Math.max(0,6-G.junk(s)) }
  ];
  ...
  const MINI_POOL = [
    ...
    { id:'m_nomiss12', label:'ไม่พลาด 12 วินาที',        level:'normal', target:12,  check:s=>G.tick(s)>=12 && G.junk(s)===0, prog:s=>Math.min(12,G.tick(s)) },
  ...
  function pushQuest(hint){
    ...
    if (hint==='เริ่ม' && window.Coach?.say){
      if (focusGoal) window.Coach.say('Goal: ' + focusGoal.label);
    }
  }
  ...
  function judge(ch, ctx){
    ...
    // powerups ...
    if (ch===STAR){ const d=40*mult(); score+=d; gainFever(10); star++; Particles.burstShards(null,null,{screen:{x,y},theme:'goodjunk'}); deck.onGood(); syncDeck(); pushQuest(); return {good:true,scoreDelta:d}; }
    if (ch===DIA){  const d=80*mult(); score+=d; gainFever(30); diamond++; Particles.burstShards(null,null,{screen:{x,y},theme:'groups'});  deck.onGood(); syncDeck(); pushQuest(); return {good:true,scoreDelta:d}; }
    if (ch===SHIELD){ shield=Math.min(3, shield+1); setShield(shield); score+=20; Particles.burstShards(null,null,{screen:{x,y},theme:'hydration'}); deck.onGood(); syncDeck(); pushQuest(); return {good:true,scoreDelta:20}; }
    if (ch===FIRE){   feverActive=true; setFeverActive(true); fever=Math.max(fever,60); setFever(fever); score+=25; Particles.burstShards(null,null,{screen:{x,y},theme:'plate'}); deck.onGood(); syncDeck(); pushQuest(); return {good:true,scoreDelta:25}; }

    const isGood = GOOD.includes(ch);
    if (isGood){
      const delta = (16 + combo*2) * mult();
      score += delta; combo += 1; gainFever(7 + combo*0.5);
      deck.onGood(); syncDeck(); Particles.burstShards(null,null,{screen:{x,y},theme:'goodjunk'}); Particles.scorePop(x,y,`+${delta|0}`);
      return { good:true, scoreDelta: delta };
    } else {
      if (shield>0){ shield-=1; setShield(shield); Particles.burstShards(null,null,{screen:{x,y},theme:'goodjunk'}); syncDeck(); return {good:false,scoreDelta:0}; }
      const delta = -12;
      score = Math.max(0, score + delta); combo = 0; decayFever(16);
      deck.onJunk(); // ✅ นับ “พลาด”
      syncDeck(); Particles.burstShards(null,null,{screen:{x,y},theme:'groups'}); Particles.scorePop(x,y,`${delta}`);
      window.Coach?.say('อย่าคลิกของเสีย!');
      return { good:false, scoreDelta: delta };
    }
  }

  // ✅ นับ "พลาด" ให้ถูกต้อง: ของดีหมดอายุ = พลาด, ของเสียหมดอายุ = ไม่พลาด (ได้ fever เล็กน้อย)
  function onExpire(ev){
    if (!ev) return;
    if (ev.isGood){
      // พลาดของดี
      combo = 0; decayFever(12);
      deck.onJunk(); // นับ miss
      syncDeck(); pushQuest();
      window.Coach?.say('พลาดของดี ระวังนะ');
    }else{
      // เลี่ยงของเสียได้
      gainFever(2); syncDeck(); pushQuest();
    }
  }
  ...
  function onSec(){
    ...
    if (deck.isCleared('mini'))  { deck.draw3(); pushQuest('Mini ใหม่');   window.Coach?.say('Mini ใหม่!'); }
    if (deck.isCleared('goals')) { deck.drawGoals(5); pushQuest('Goal ใหม่'); window.Coach?.say('Goal ใหม่!'); }
  }

  window.addEventListener('hha:expired', onExpire);
  window.addEventListener('hha:time', (e)=>{ const s=e.detail?.sec|0; if(s>=0) onSec(); if(s===10) window.Coach?.say('เหลือเวลา 10 วิ!'); });
