...
export async function boot(cfg = {}) {
  ...
  const stats=()=>({score,goodCount:hits,comboMax:maxCombo,star:starCount,diamond:diamondCount,
  ...
  function end(reason='timeout'){
    ...
    clearInterval(timeId); clearTimeout(loopId); clearInterval(watchdogId); clearInterval(noMissId);
    ...
    window.dispatchEvent(new CustomEvent('hha:end',{detail:{
      ...
      questsCleared,questsTotal:3,reason
    }}));
  }
  ...
  function spawnOne(){
    ...
    el.addEventListener('click',(ev)=>{
      ...
      if(type==='good'){ const val=20+combo*2; score+=val; combo++; maxCombo=Math.max(maxCombo,combo); hits++;
        ...
        else { combo=0; score=Math.max(0,score-15); misses++; noMissSec=0; emitMiss();
