...
export async function boot(cfg = {}) {
  // รอ AFRAME.THREE ให้พร้อม (กัน THREE is not defined)
  await new Promise(res=>{
    const ok = () => (globalThis.AFRAME?.THREE) && res();
    if (globalThis.AFRAME?.THREE) return res();
    const iv = setInterval(()=>{ if (globalThis.AFRAME?.THREE){ clearInterval(iv); ok(); }}, 30);
  ...
  // State
  let running=true, score=0, combo=0, maxCombo=0;
  let misses=0, hits=0, spawns=0, shield=0, starCount=0, diamondCount=0, noMissSec=0;
  let remain = dur, tTimer=0, tLoop=0, tNoMiss=0, tGuard=0;

  // Mini-quests: 3 ใบจาก 10 ตามระดับ
  const QUESTS = drawThree('goodjunk', diff);
  let qIdx=0;
  const questText = () => `Quest ${qIdx+1}/3 — ${QUESTS[qIdx]?.label || 'กำลังสุ่ม…'}`;
  const pushQuest = () => window.dispatchEvent(new CustomEvent('hha:quest',{detail:{text:questText()}}));
  window.dispatchEvent(new CustomEvent('hha:quest',{detail:{text:'Quest 1/3 — กำลังสุ่ม…'}}));
  setTimeout(pushQuest, 300);
  ...
  const miss = ()=>window.dispatchEvent(new CustomEvent('hha:miss',{detail:{count:misses}}));

  const snapshot = () => ({score,goodCount:hits,comboMax:maxCombo,star:starCount,diamond:diamondCount,junkMiss:misses,noMissTime:noMissSec,feverCount:0});
  function tryAdvanceQuest(){
    const cur=QUESTS[qIdx]; if(!cur?.check) return;
    if (cur.check(snapshot())){ qIdx=Math.min(qIdx+1,2); pushQuest(); }
  }

  function end(reason='timeout'){
    if(!running) return; running=false;
    clearInterval(tTimer); clearInterval(tNoMiss); clearInterval(tGuard); clearTimeout(tLoop);
    host.querySelectorAll('a-image').forEach(n=>n.remove());
    const cleared = QUESTS.reduce((n,q)=> n + (q?.check && q.check(snapshot()) ? 1 : 0), 0);
    window.dispatchEvent(new CustomEvent('hha:end',{detail:{
      mode:'Good vs Junk', difficulty:diff, score, combo, comboMax:maxCombo, misses, hits, spawns,
  ...
  function spawnOne(){
    if(!running) return;
    if (host.querySelectorAll('a-image').length >= C.maxConcurrent) { tLoop=setTimeout(spawnOne,110); return; }
    ...
    const ttl=setTimeout(()=>{ if(!el.parentNode) return;
      if (type==='good'){ misses++; combo=0; score=Math.max(0, score-10); noMissSec=0; miss(); emitScore(); }
    ...
    el.addEventListener('click',(ev)=>{
      ...
      if(type==='good'){
        ...
        else{ combo=0; score=Math.max(0,score-15); misses++; noMissSec=0; miss(); burstAt(scene,wp,{color:'#ef4444',count:12,speed:0.9}); floatScore(scene,wp,'-15'); }
      ...
      emitScore(); el.remove(); sp.unmark(rec); tryAdvanceQuest();
  ...
  tTimer=setInterval(()=>{ if(!running) return; remain=Math.max(0,remain-1); window.dispatchEvent(new CustomEvent('hha:time',{detail:{sec:remain}})); if(remain<=0) end('timeout'); },1000);
  tNoMiss=setInterval(()=>{ if(running) noMissSec=Math.min(9999,noMissSec+1); },1000);
  tGuard=setInterval(()=>{ if(running && !host.querySelector('a-image')) spawnOne(); },1800);
