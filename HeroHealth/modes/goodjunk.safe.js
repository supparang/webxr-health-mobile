...
export async function boot(cfg = {}) {
  ...
  function popScore(x,y,delta,good){
    try{ if(Particles?.scorePop){ Particles.scorePop({x,y,delta,good}); } }catch(_){}
  }
  ...
  function judge(ch, ctx){
    ...
    if (ch===SHIELD){ shield=Math.min(3, shield+1); setShield(shield); score+=20;
      Particles?.burstShards?.(null,null,{screen:{x,y},theme:'hydration'}); deck.onGood(); syncDeck(); popScore(x,y,20,true); focusFromDeck(); return {good:true,scoreDelta:20}; }
    if (ch===FIRE){ feverActive=true; setFeverActive(true); fever=Math.max(fever,60); setFever(fever); score+=25;
      Particles?.burstShards?.(null,null,{screen:{x,y},theme:'plate'}); deck.onGood(); syncDeck(); popScore(x,y,25,true); focusFromDeck(); return {good:true,scoreDelta:25}; }
    ...
    if (isGood){
      ...
      if (shield>0){ shield-=1; setShield(shield);
        Particles?.burstShards?.(null,null,{screen:{x,y},theme:'goodjunk'}); syncDeck(); popScore(x,y,0,false); focusFromDeck(); return {good:false,scoreDelta:0}; }
      const delta = -12;
  ...
  function onExpire(ev){
    ...
    // เลี่ยงของเสียได้ → ให้อารมณ์บวกเล็กน้อย
    gainFever(4); deck.onJunk(); // พลาด/เลี่ยง นับฝั่ง junk เพื่อให้เควสต์บางตัวอ่านค่าได้
    syncDeck(); focusFromDeck();
  }
  ...
  function onSec(){
    ...
    // เติม mini/goal อัตโนมัติเมื่อเคลียร์หมด
