...
export async function boot(cfg = {}) {
  ...
  let qIdx=0;
  function questText(){ return `Quest ${qIdx+1}/3 — ${QUESTS[qIdx]?.label || 'กำลังสุ่ม…'}`; }
  ...
  function tryAdvanceQuest(){ const q=QUESTS[qIdx]; if(q?.check && q.check(stats())){ qIdx=Math.min(qIdx+1,2); updateQuestHUD(); } }

  function end(reason='timeout'){
    if(!running) return;
    running=false;
    [timeId,loopId,watchdogId,noMissId].forEach(id=>{ try{clearInterval(id);clearTimeout(id);}catch{} });
  ...
  function spawnOne(){
    ...
    const ttl = setTimeout(()=>{ if(el.parentNode){ if(type==='good'){ misses++; combo=0; score=Math.max(0,score-10); noMissSec=0; emitMiss(); emitScore(); } el.remove(); sp.unmark(rec); } }, lifeMs());
    ...
    el.addEventListener('click', (ev)=>{
      ...
      if (type==='good'){
        ...
        else { combo=0; score=Math.max(0,score-15); misses++; noMissSec=0; emitMiss(); burstAt(scene,wp,{color:'#ef4444',count:12,speed:0.9}); floatScore(scene,wp,'-15'); }
