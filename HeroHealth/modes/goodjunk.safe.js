// === Good vs Junk — SAFE + FEVER (fixed parenting/position) ===
...
// Fever
var FEVER_ACTIVE=false, FEVER_COMBO_NEED=10, FEVER_MS=10000, feverTimer=null;
function feverStart(){ if(FEVER_ACTIVE) return; FEVER_ACTIVE=true; emit('hha:fever',{state:'start',ms:FEVER_MS}); clearTimeout(feverTimer); feverTimer=setTimeout(feverEnd,FEVER_MS); }
function feverEnd(){ if(!FEVER_ACTIVE) return; FEVER_ACTIVE=false; emit('hha:fever',{state:'end'}); clearTimeout(feverTimer); feverTimer=null; }

// Target
function makeTarget(emoji, good, diff){
  var el=document.createElement('a-entity');

  // ตำแหน่งเป้า — วางที่คอนเทนเนอร์ el
  ...
  // glow ซ้อนใต้รูป (เป็นลูกของ img เพื่อไม่หลุดตำแหน่ง)
  var glow=document.createElement('a-plane');
  glow.setAttribute('width',0.48);
  glow.setAttribute('height',0.48);
...
export async function boot(cfg){
  host=(cfg&&cfg.host)||document.getElementById('spawnHost');
  ...
  emit('hha:quest',{text:'Mini Quest — เก็บของดีติดกัน 10 ชิ้น เพื่อเปิด FEVER!'});
