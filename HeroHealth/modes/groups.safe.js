...
export async function boot(cfg = {}) {
  ...
  const updateQuestHUD=(txt)=>window.dispatchEvent(new CustomEvent('hha:quest',{detail:{text:txt}}));
  function setNewGoal(){ target=keys[(Math.random()*keys.length)|0]; correctPicked=0;
    updateQuestHUD(`เป้า: เลือกให้ถูกหมู่ (${target.toUpperCase()}) × ${goalSize}`); }
  setNewGoal();
  ...
  function tryAdvanceQuest(){
    ...
    if(q.check(s)){ qIdx=Math.min(2,qIdx+1); updateQuestHUD(`Quest ${qIdx+1}/3 — ${QUESTS[qIdx]?.label||''}`); }
  }
  function end(reason='timeout'){ if(!running) return; running=false;
    ...
    window.dispatchEvent(new CustomEvent('hha:end',{detail:{
      mode:'Food Groups',difficulty:diff,score,comboMax:maxCombo,misses,hits,spawns,duration:dur,
  ...
  function spawnOne(){
    ...
    let ch; if(Math.random()<0.30){ const pool=GROUPS[target]; ch=pool[(Math.random()*pool.length)|0]; }
    ...
    const ttl=setTimeout(()=>{ if(!el.parentNode) return;
      if(inTarget){ misses++; combo=0; score=Math.max(0,score-10);
    ...
    el.addEventListener('click',(ev)=>{
      ...
      if(inTarget){ const val=25+combo*2; score+=val; combo++; maxCombo=Math.max(maxCombo,combo); hits++; correctPicked++;
