// === /HeroHealth/modes/hydration.quest.js (release; Water Gauge + THREE-safe) ===
import { waitAframe, worldPosOf } from '../vr/three-safe.js';
...
function ensureWaterGauge(){
  ...
  Object.assign(wrap.style,{position:'fixed',left:'50%',bottom:'56px',transform:'translateX(-50%)',
    width:'min(540px,86vw)',zIndex:'900',color:'#e8eefc',background:'#0f172a99',border:'1px solid #334155',
    borderRadius:'12px',padding:'10px 12px',backdropFilter:'blur(6px)',fontWeight:'800'});
...
export async function boot(cfg = {}) {
  ...
  function applyHit(type,wp){
    if(type==='good'){ const val=20+combo*2; score+=val; combo++; maxCombo=Math.max(maxCombo,combo); hits++;
      ...
      else { if(zone()==='HIGH'){ score+=5; floatScore(scene,wp,'+5 (High)'); }
  ...
  function tryAdvanceQuest(){
    ...
    const q=QUESTS[qIdx]; if(!q?.check) return; if(q.check(s)){ qIdx=Math.min(2,qIdx+1); updateQuestHUD(); }
  }
  function end(reason='timeout'){
    ...
    destroyWaterGauge(); Array.from(host.querySelectorAll('a-image')).forEach(n=>n.remove());
  ...
  function spawnOne(){
    ...
    const ttl=setTimeout(()=>{ if(!el.parentNode) return; if(type==='good'){ water=Math.max(0,water-4); score=Math.max(0,score-8); combo=0;
        window.dispatchEvent(new CustomEvent('hha:miss',{detail:{count:++misses}})); setWaterGauge(water); emitScore(); }
      try{el.remove();}catch{}; sp.unmark(rec);
    },lifeMs());

    el.addEventListener('click',(ev)=>{
      if(!running) return; ev.preventDefault(); clearTimeout(ttl);
      const wp=worldPosOf(el); applyHit(type,wp); try{el.remove();}catch{}; sp.unmark(rec); tryAdvanceQuest();
  ...
  timerId=setInterval(()=>{ if(!running) return; remain=Math.max(0,remain-1);
    window.dispatchEvent(new CustomEvent('hha:time',{detail:{sec:remain}})); if(remain<=0) end('timeout'); },1000);

  const dispose=()=>destroyWaterGauge(); window.addEventListener('hha:dispose-ui',dispose); disposeHandler=dispose;
