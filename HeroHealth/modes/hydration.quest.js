...
function setWaterGauge(val){
  const f=document.getElementById('waterFill'); const l=document.getElementById('waterLbl'); if(!f||!l) return;
...
export async function boot(cfg = {}) {
  ...
  let remain=dur, timerId=0, loopId=0, dispose=null;
  ...
  function updateQuestHUD(){ window.dispatchEvent(new CustomEvent('hha:quest',{detail:{text:`Quest ${qIdx+1}/3 — ${QUESTS[qIdx]?.label||'รักษาระดับน้ำให้พอดี'}`}})); }
  updateQuestHUD();
  ...
  function tryAdvanceQuest(){ const q=QUESTS[qIdx]; if(q?.check&&q.check({score,goodCount:hits,junkMiss:misses,comboMax:maxCombo,feverCount:0,star:0,diamond:0,noMissTime:0})){ qIdx=Math.min(2,qIdx+1); updateQuestHUD(); } }

  function end(reason='timeout'){
    if(!running) return; running=false; try{clearInterval(timerId);}catch{}; try{clearTimeout(loopId);}catch{};
    if(dispose){ window.removeEventListener('hha:dispose-ui',dispose); dispose=null; }
    destroyWaterGauge();
  ...
  function spawnOne(){
    ...
    if(r<0.05){ ch='⭐'; type='star'; } else if(r<0.07){ ch='💎'; type='diamond'; } else if(r<0.10){ ch='🛡️'; type='shield'; }
    ...
    const ttl=setTimeout(()=>{ if(el.parentNode){ if(type==='good'){ water=Math.max(0,water-4); score=Math.max(0,score-8); combo=0; window.dispatchEvent(new CustomEvent('hha:miss',{detail:{count:++misses}})); setWaterGauge(water); emitScore(); } el.remove(); sp.unmark(rec);} }, lifeMs());

    el.addEventListener('click',(ev)=>{ if(!running) return; ev.preventDefault(); clearTimeout(ttl);
      const wp=el.object3D.getWorldPosition(new THREE.Vector3()); applyHit(type,wp); try{el.remove();}catch{} sp.unmark(rec); tryAdvanceQuest(); loopId=setTimeout(spawnOne,nextGap()); }, {passive:false});
  ...
  dispose=()=>destroyWaterGauge(); window.addEventListener('hha:dispose-ui',dispose);
  spawnOne();
