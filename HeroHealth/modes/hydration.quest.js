// === modes/hydration.quest.js (no THREE, Water Gauge kept) ===
...
function ensureWaterGauge(){
  ...
  Object.assign(wrap.style,{position:'fixed',left:'50%',bottom:'56px',transform:'translateX(-50%)',width:'min(540px,86vw)',
    zIndex:'900',color:'#e8eefc',background:'#0f172a99',border:'1px solid #334155',borderRadius:'12px',padding:'10px 12px',backdropFilter:'blur(6px)',fontWeight:'800'});
...
function setWaterGauge(v){
  ...
  l.textContent=zone; f.style.background=(zone==='Balanced')?'linear-gradient(90deg,#06d6a0,#37d67a)':(zone==='High'?'linear-gradient(90deg,#22c55e,#93c5fd)':'linear-gradient(90deg,#f59e0b,#ef4444)');
}
...
export async function boot(cfg = {}) {
  ...
  const updateQuestHUD=()=>window.dispatchEvent(new CustomEvent('hha:quest',{detail:{text:`Quest ${qIdx+1}/3 — ${QUESTS[qIdx]?.label || 'รักษาระดับน้ำให้พอดี'}`}}));
  updateQuestHUD();
  ...
  function applyHit(type,p){
    if(type==='good'){
      const val=20+combo*2; score+=val; combo++; maxCombo=Math.max(maxCombo,combo); hits++;
      water=Math.min(100,water+6); Particles.burstShards(host,p,{theme:'hydration'}); floatScore(scene,p,'+'+val);
    }else if(type==='bad'){
      if(shield>0){ shield--; floatScore(scene,p,'Shield!'); Particles.burstShards(host,p,{theme:'hydration'}); }
      else{
        if(zone()==='HIGH'){ score+=5; floatScore(scene,p,'+5 (High)'); }
        else { score=Math.max(0,score-20); combo=0; floatScore(scene,p,'-20'); }
        water=Math.max(0,water-8); Particles.burstShards(host,p,{theme:'hydration'});
      }
    }else if(type==='star'){ score+=40; Particles.burstShards(host,p,{theme:'hydration'}); floatScore(scene,p,'+40 ⭐'); }
    else if(type==='diamond'){ score+=80; Particles.burstShards(host,p,{theme:'hydration'}); floatScore(scene,p,'+80 💎'); }
    else if(type==='shield'){ shield=Math.min(3,shield+1); Particles.burstShards(host,p,{theme:'hydration'}); floatScore(scene,p,'🛡️+1'); }
    setWaterGauge(water); emitScore();
  }
  function tryAdvanceQuest(){ const s={score,goodCount:hits,comboMax:maxCombo}; const q=QUESTS[qIdx]; if(q?.check && q.check(s)){ qIdx=Math.min(2,qIdx+1); updateQuestHUD(); } }
