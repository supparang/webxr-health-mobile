// === /HeroHealth/modes/hydration.quest.js (FIXED GREEN TIME) ===
...
function G(s){
  return {
    ...
    tick: s.tick|0,
    zone: s.zone || '',
    green: s.timeInGreen || 0   // << สำคัญที่สุด
  };
}
...
function goalsFor(diff){
  ...
  return [
    {id:'g_green',label:`โซน GREEN รวม ${K.green}s`, target:K.green,
      check:s=>G(s).green>=K.green, prog:s=>Math.min(K.green,G(s).green)},
...
function minisFor(diff){
  ...
  return [
    ...
    {id:'m_green12',label:'อยู่ GREEN 12s', target:12,
      check:s=>G(s).green>=12, prog:s=>Math.min(12,G(s).green)},
    ...
    {id:'m_green8', label:'อยู่ GREEN 8s', target:8,
      check:s=>G(s).green>=8, prog:s=>Math.min(8,G(s).green)},
