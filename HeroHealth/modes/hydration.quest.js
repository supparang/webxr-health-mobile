// === /HeroHealth/modes/hydration.quest.js (GREEN TICK BASED) ===
...
function goalsFor(diff){
  ...
  return [
    // เคยอยู่โซน GREEN สะสมครบ X วินาที (ไม่ต้อง GREEN ตอนจบด่าน)
    {id:'g_green',label:`อยู่ในโซนสมดุล (GREEN) รวม ${K.green}s`, target:K.green,
      check:s=>G(s).greenTick>=K.green,
      prog:s=>Math.min(K.green,G(s).greenTick)},
...
function minisFor(diff){
  ...
  return [
    ...
    {id:'m_combo',label:`คอมโบ ≥ ${K.combo}`, target:K.combo,
      check:s=>G(s).comboMax>=K.combo, prog:s=>Math.min(K.combo,G(s).comboMax)},
    {id:'m_good', label:`เก็บไฮเดรต ${K.good}`, target:K.good,
      check:s=>G(s).good>=K.good, prog:s=>Math.min(K.good,G(s).good)},
    {id:'m_nomiss',label:`พลาดไม่เกิน ${K.miss}`, target:K.miss,
      check:s=>G(s).miss<=K.miss, prog:s=>Math.max(0,K.miss-G(s).miss)},
    // ใช้ greenTick สะสมเหมือนกัน
    {id:'m_green12',label:'อยู่ GREEN 12s', target:12,
      check:s=>G(s).greenTick>=12, prog:s=>Math.min(12,G(s).greenTick)},
    {id:'m_time15',label:'อยู่รอด 15s', target:15,
      check:s=>G(s).tick>=15, prog:s=>Math.min(15,G(s).tick)},
    {id:'m_combo12',label:'คอมโบ ≥ 12', target:12,
      check:s=>G(s).comboMax>=12, prog:s=>Math.min(12,G(s).comboMax)},
    {id:'m_score1100',label:'คะแนน 1100+', target:1100,
      check:s=>G(s).score>=1100, prog:s=>Math.min(1100,G(s).score)},
    {id:'m_good16',label:'เก็บไฮเดรต 16', target:16,
      check:s=>G(s).good>=16, prog:s=>Math.min(16,G(s).good)},
    {id:'m_nomiss4',label:'พลาด ≤ 4', target:4,
      check:s=>G(s).miss<=4, prog:s=>Math.max(0,4-G(s).miss)},
    {id:'m_green8', label:'อยู่ GREEN 8s', target:8,
      check:s=>G(s).greenTick>=8, prog:s=>Math.min(8,G(s).greenTick)},
    {id:'m_score800',label:'คะแนน 800+', target:800,
      check:s=>G(s).score>=800, prog:s=>Math.min(800,G(s).score)},
    {id:'m_good10',label:'เก็บไฮเดรต 10', target:10,
    ...
    {id:'m_time25',label:'อยู่รอด 25s', target:25,
      check:s=>G(s).tick>=25, prog:s=>Math.min(25,G(s).tick)},
  ];
}
