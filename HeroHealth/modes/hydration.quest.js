// === /HeroHealth/modes/hydration.quest.js (release; THREE-safe + Water Gauge cleanup) ===
...
function setWaterGauge(val){
  ...
  let z='Low'; if(pct>=40 && pct<=70) z='Balanced'; else if(pct>70) z='High'; l.textContent=z;
  f.style.background = (z==='Balanced') ? 'linear-gradient(90deg,#06d6a0,#37d67a)'
    : (z==='High' ? 'linear-gradient(90deg,#22c55e,#93c5fd)' : 'linear-gradient(90deg,#f59e0b,#ef4444)');
}
...
export async function boot(cfg = {}) {
  ...
  let qIdx=0;
  const questText=()=>`Quest ${qIdx+1}/3 — ${QUESTS_POOL[qIdx]?.label || 'รักษาระดับน้ำให้พอดี'}`;
  ...
  const dispose=()=>destroyWaterGauge();
  disposeHandler=dispose; window.addEventListener('hha:dispose-ui', disposeHandler);
