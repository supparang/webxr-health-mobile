// === /HeroHealth/modes/plate.quest.js (release with HUD & Refill) ===
...
function makeHUD(){
  ...
  w.innerHTML=`<div style="display:flex;justify-content:space-between"><div id="t">Healthy Plate</div><div id="n"></div></div>
  <div id="gL" style="margin:6px 0 4px">เป้า: จัดครบ 5 หมู่</div>
...
export async function boot(cfg={}) {
  ...
  function updateHUD(){
    const p=roundProgress(); HUD.setGoal(`Goal: จัดครบ 5 หมู่ — ทำแล้ว ${p}/5`, Math.round((p/5)*100));
  ...
  function end(reason='timeout'){
    ...
    window.dispatchEvent(new CustomEvent('hha:end',{detail:{mode:'Healthy Plate',difficulty:diff,score,combo:maxCombo,misses,hits,spawns,duration:dur,questsCleared:3,questsTotal:3,reason}}));
  }
  ...
  function spawnOne(){
    ...
    if      (r<0.05){ ch=STAR; type='star'; }
      ...
      const keys=Object.keys(GROUPS); groupKey=keys[(Math.random()*keys.length)|0];
    ...
    const ttl=setTimeout(()=>{ if(!el.parentNode) return; if(type==='food'){ misses++; combo=0; score=Math.max(0,score-10); window.dispatchEvent(new CustomEvent('hha:miss',{detail:{count:misses}})); } try{ host.removeChild(el);}catch{} sp.unmark(rec); updateHUD(); }, lifeMs());
    ...
    el.addEventListener('click',(ev)=>{
      ...
      if(type==='food'){
        ...
        if(roundCleared()){ score+=100; floatScore(scene, wp, 'ROUND +100'); roundDone={veg:false,fruit:false,grain:false,protein:false,dairy:false}; }
