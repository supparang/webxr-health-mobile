// === /HeroHealth/modes/plate.quest.js (release; THREE-safe) ===
...
export async function boot(cfg = {}) {
  ...
  const updateQuestHUD=()=>window.dispatchEvent(new CustomEvent('hha:quest',{detail:{text:`Quest ${qIdx+1}/3 — ${QUESTS[qIdx]?.label||'จัดครบ 5 หมู่!'}`}}));
  updateQuestHUD();
  ...
  function end(reason='timeout'){ if(!running) return; running=false;
    ...
    window.dispatchEvent(new CustomEvent('hha:end',{detail:{
      mode:'Healthy Plate',difficulty:diff,score,combo:maxCombo,misses,hits,spawns,duration:dur,questsCleared:qIdx+1,questsTotal:3,reason
    }}));
  }
  ...
  function spawnOne(){
    ...
    else { const keys=Object.keys(GROUPS); groupKey=keys[(Math.random()*keys.length)|0];
    ...
    el.addEventListener('click',(ev)=>{
      ...
      if(type==='food'){ const val=22+combo*2; score+=val; combo++; maxCombo=Math.max(maxCombo,combo); hits++;
