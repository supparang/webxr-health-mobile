...
// เมนูหลัก (delegation ปกติ ยังคงไว้)
...
// ======= Hard Fix: ไม่ให้ canvas บังปุ่ม + เมนูไปล่างซ้าย + hard-bind =======
function __fixLayersAndMenuPos() {
  const c = document.getElementById('c');
  if (c) { c.style.pointerEvents = 'none'; c.style.zIndex = '1'; }

  // ทำให้ HUD/เมนู/โมดัล คลิกได้แน่ ๆ
...
function __bindMenuButtonsHard() {
  const byId = (id)=>document.getElementById(id);

  const onStart = ()=> {
    if (typeof preStartFlow === 'function') preStartFlow();
    else if (typeof start === 'function') start();
  };
  const onPause   = ()=> { if (typeof state!=='undefined') state.running = !state.running; };
  const onRestart = ()=> { if (typeof end==='function') end(true); if (typeof start==='function') start(); };
  const onHelp    = ()=> { const help = byId('help'); if (help) help.style.display = 'flex'; };

  const map = [
    ['btn_start',   onStart],
    ['btn_pause',   onPause],
    ['btn_restart', onRestart],
    ['btn_help',    onHelp],
    ['btn_ok',      ()=>{ const h=byId('help'); if (h) h.style.display='none'; }],
    ['btn_replay',  ()=>{ const r=byId('result'); if (r) r.style.display='none'; onRestart(); }],
    ['btn_home',    ()=>{ const r=byId('result'); if (r) r.style.display='none'; }]
  ];

  map.forEach(([id, handler])=>{
    const el = byId(id);
    if (el) {
      el.addEventListener('click', (e)=>{ e.stopPropagation(); handler(); }, {passive:true});
    }
  });

  console.log('✅ UI buttons hard-bound');
}

window.addEventListener('DOMContentLoaded', ()=>{
  __fixLayersAndMenuPos();
  __bindMenuButtonsHard();

  // re-assert หลังเฟรมแรก เผื่อ renderer/stylesheet ปรับเลย์เอาต์ทีหลัง
  setTimeout(__fixLayersAndMenuPos, 400);
  setTimeout(__bindMenuButtonsHard, 450);
