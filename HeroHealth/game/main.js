// === Hero Health Academy — game/main.js (2025-11-01 stable) ===
'use strict';
...
(function () {
  ...
  // ---------- Safe stubs (จะถูกแทนที่เมื่อ import สำเร็จ) ----------
  ...
  // spawn watchdog markers (ให้โหมดเรียก window.__HHA_MARK_SPAWNED เมื่อ spawn สำเร็จ)
  let __spawnCount = 0;
  let __spawnLastTs = 0;
  window.__HHA_MARK_SPAWNED = function(){ __spawnCount++; __spawnLastTs = performance.now(); };
  ...
  function gameTick(){
    ...
    // gentle watchdog: ถ้า 2 วินาทีไม่มี spawn ให้เตะเล็กน้อยหนึ่งครั้ง
    if (R.playing && (tNow - __spawnLastTs) > 2000) {
      __spawnLastTs = tNow;
      try {
        if (typeof R.modeAPI?.update === 'function') R.modeAPI.update(0.6, busFor());
      } catch(e){ console.warn('[watchdog kick]', e); }
    }
  ...
  function endGame(){
    ...
    try{
      ...
      hud.showResult({
        ...
        stats:[ `Score: ${score}`, `Best Combo: ${bestC}`, `Time: ${R.matchTime|0}s`, res && res.totalDone!=null ? `Quests: ${res.totalDone}` : '' ].filter(Boolean)
      });
  ...
  async function startGame(){
    ...
    // safety kick เพียงหนึ่งครั้งถ้าเริ่มแล้วไม่มี spawn
    setTimeout(()=>{
      if ((__spawnCount|0) === 0) {
        try {
          if (typeof R.modeAPI?.update === 'function') R.modeAPI.update(0.8, busFor());
        } catch(e){ console.warn('[safety kick]', e); }
      }
    }, 800);
