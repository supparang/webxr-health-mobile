...
function bootApp(){
  ...
  announceHudReady();
  var tries=0, id=setInterval(function(){ announceHudReady(); if(++tries>15) clearInterval(id); },150);
...
// === Result Modal Overlay (คะแนน + ภารกิจสำเร็จ + Quest Count Badge w/ color) ===
(function(){
  ...
  function injectStyles(){
    ...
    st.textContent = [
      ...
      /* สีตามสัดส่วนผ่านเควสต์ */
      '.hha-badge-quest.ok  { background:#052e1a; color:#86efac; border-color:#16a34a; } ',
      '.hha-badge-quest.mid { background:#2a1f08; color:#fcd34d; border-color:#d97706; } ',
      '.hha-badge-quest.low { background:#3b0a0a; color:#fda4af; border-color:#ef4444; } ',
  ...
  function show(detail){
    ...
    // ✅ อัปเดต Quest badge: QUESTS x/y + สีตามสัดส่วน
    try {
      var c = computeQuestCounts(detail); // {x,y}
      var ratio = (c.y>0) ? (c.x / c.y) : 0;
      overlay._questBadge.textContent = 'QUESTS ' + fmt(c.x) + '/' + fmt(c.y);

      overlay._questBadge.classList.remove('ok','mid','low');
      if (ratio >= 2/3)      overlay._questBadge.classList.add('ok');   // เขียว
      else if (ratio >= 1/3) overlay._questBadge.classList.add('mid');  // เหลือง
      else                   overlay._questBadge.classList.add('low');  // แดง
    } catch(_){}
