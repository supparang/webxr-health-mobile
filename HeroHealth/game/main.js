// === Hero Health Academy — main.js (stable + freeze fix + shards + reduced-motion) ===
...
// - Shards 3D on hit (budget-aware), respects prefers-reduced-motion
...
// ✅ Effects on/off ตามระบบผู้ใช้ + shards budget อัตโนมัติ
state.effectsOn = !matchMedia('(prefers-reduced-motion: reduce)').matches;
state.shardsBudget = ((navigator.hardwareConcurrency||0) <= 4 || (navigator.deviceMemory||Infinity) <= 2) ? 32 : 48;
...
function scoreWithEffects(base, x, y){
  ...
  // ถ้าปิด motion effects ให้จบที่เพิ่มคะแนน
  if (!state.effectsOn) return;
...
// ----- Spawn (✅ Freeze fix + shards call) -----
function spawnOnce(diff){
  if (!state.running || state.paused) return;

  // ✅ Freeze: หยุดสแปวน์ถ้าโดนพลังแช่แข็ง (รองรับทั้ง state.freezeUntil และ power.timers.freeze)
  const now = performance?.now?.() || Date.now();
  if ((state.freezeUntil && now < state.freezeUntil) || (power?.timers?.freeze > 0)) {
    state.spawnTimer = setTimeout(()=>spawnOnce(diff), 140);
    return;
  }
  ...
  el.addEventListener('click', function(ev){
    ...
    try{
      ...
      // ✅ แตกเศษ 3D (budget-aware) ถ้า engine รองรับและเปิดเอฟเฟกต์
      if (state.effectsOn && eng?.fx?.spawnShards){
        eng.fx.spawnShards(cx, cy, { count: state.shardsBudget });
      }
