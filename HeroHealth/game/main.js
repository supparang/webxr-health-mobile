// === Hero Health Academy — main.js (G9 In-Frame & On-Top) ===
// - Layer-safe: canvas clipped in #gameLayer (isolation + z-index)
// - Size-sync: ResizeObserver + hard sync for mobile/VR viewport changes
// - Pointer-safe: click/pointerdown/touchstart + center-gaze fallback
// - Frustum utils: ให้โหมดขอ "จุดสุ่มในกรอบกล้อง" ได้ (ctx.utils.randInView)
// - Cache-bust import + provide both cam/camera to modes
...
(function css(){
  const tag=document.createElement('style'); tag.textContent = `
    ...
    #gameLayer{
      ...
      overflow:hidden; isolation:isolate;   /* <— สร้าง stacking context ของตัวเอง */
      z-index:100;
    }
    #spawnHost{position:absolute;inset:0;z-index:8;pointer-events:auto}
...
function addScore(delta, perfect){
  ...
  const gl = $('#gameLayer');
  try{ gl.style.transition='transform 80ms ease'; gl.style.transform='translate3d(4px,-4px,0)'; setTimeout(()=>gl.style.transform='translate3d(0,0,0)',90);}catch(_){}
}
...
function ensureThree(){
  ...
  return import('https://unpkg.com/three@0.159.0/build/three.module.js').then((THREE)=>{
    ...
    // ---- Size sync: ResizeObserver + hard sync timer
    function size(){
    ...
    setInterval(size, 800);  // กันกรณีแถบ URL มือถือยุบ/ขยาย
    ...
    // ---- Utils for modes: random point in camera view (z in safe band)
    function randInView(z=0){
      // กล้องอยู่ที่ z=8 → ให้เป้าอยู่ช่วง [-0.3..0.6] ใกล้กล้องพอเห็นใหญ่
      const zv = Math.min(0.6, Math.max(-0.3, z));
    ...
    THREE_CTX = {
      ready:true, THREE, renderer, scene, camera:cam, cam,
      ray, pointer, canvas:cvs,
      utils:{ randInView }   // <— ให้โหมดเรียกเพื่อสปอว์น "ในกรอบ" เสมอ
    };
    return THREE_CTX;
  }).catch(()=>({ready:false}));
}
...
function getMode(key){
  ...
  return import(`${MODES_DIR}${key}.js?${ver}`)
    ...
    .catch(async e => {
      console.warn('[HHA] mode load failed', e);
      const m = await import(`${MODES_DIR}goodjunk.js?${ver}`);
...
function startGame(){
  ...
  getMode(STATE.modeKey).then(api=>{
    ...
    return ensureThree().then(ctx=>{
      // บอก hint ให้โหมดวางสไปรท์ให้ถูกเลเยอร์
