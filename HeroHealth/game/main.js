// === /HeroHealth/game/main.js (2025-11-13 FULL PACK) ===
'use strict';
...
function setCombo(n){ if(elCombo) elCombo.textContent = (n|0); }
...
// ---------- Quest HUD (ถ้ามีไฟล์) ----------
try{ import('../vr/quest-hud.js').catch(()=>{}); }catch(_){}

window.addEventListener('hha:quest',(e)=>{
  // bridge → quest-hud.js ใช้ event นี้ได้เหมือนกัน
  try{
...
(function setupCoachHUD(){
  ...
  if(!document.getElementById(cssId)){
    ...
    css.textContent=`
      ...
      #hhaCoachBubble{
        display:inline-block;max-width:min(320px,72vw);
        padding:6px 10px;border-radius:999px;
        background:rgba(15,23,42,.92);border:1px solid #38bdf8;
        color:#e0f2fe;font:800 11px system-ui;letter-spacing:.2px;
        opacity:0;transform:translateY(4px);
        transition:opacity .25s ease,transform .25s ease;
        box-shadow:0 6px 18px rgba(0,0,0,.45);
      }
      #hhaCoachWrap.show #hhaCoachBubble{opacity:1;transform:translateY(0);}
    `;
  ...
  wrap.innerHTML='<div id="hhaCoachBubble">แตะเริ่มเกมแล้วเก็บของดีให้ต่อเนื่อง!</div>';
  ...
  showCoach('พร้อมแล้วแตะ "เริ่มเกม" เพื่อเริ่มภารกิจ!');
})();
