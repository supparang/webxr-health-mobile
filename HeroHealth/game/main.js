...
window.__HHA_BOOT_OK = true; // ตรวจสอบว่าโหลดสำเร็จแน่นอน
...
function applyLang() {
  ...
  q('#gfxToggle').textContent = state.gfx === 'low' ? L.gfx.low : L.gfx.quality;
...
// ---------------- Game Logic ----------------
function updateHUD() {
  q('#score').textContent = score.score | 0;
  q('#combo').textContent = 'x' + (score.combo || 0);
  q('#time').textContent = state.timeLeft | 0;
}
