// --- 7. GameEngine.js (สมองใหม่ - ตัวเชื่อมต่อที่ผมเขียนขึ้น) ---
(function(exports, imports) {
    ...
    function spawnTarget() {
        if (!window.running || !targetRoot) return;
    ...
    function onHitTarget(targetEl) {
        ...
        // ป้องกันกรณี THREE ไม่มี / object3D ยังไม่พร้อม
        let pos = { x: 0, y: 1.6, z: -2.5 };
        try {
            if (targetEl.object3D && typeof THREE !== 'undefined') {
                const v = new THREE.Vector3();
                targetEl.object3D.getWorldPosition(v);
    ...
    // --- ยิง: ป้องกัน listener ซ้อน ---
    let clickHandler = null;
    let mouseHandler = null;

    function setupShootHandlers(sceneEl) {
        // 1) รองรับ VR / Gaze / Touch ด้วย 'click' บน entity
        if (!clickHandler) {
            clickHandler = (e) => {
                const t = e.target;
        ...
        // 2) รองรับ PC mouse (ยิงด้วยเมาส์ซ้าย) ผ่าน raycaster ของ cursor
        function bindMouse(canvas) {
            if (!canvas || mouseHandler) return;
            const cursor = document.getElementById('cursor');
            if (!cursor) return;
            const rayComp = cursor.components && cursor.components.raycaster;
            if (!rayComp) return;

            mouseHandler = (ev) => {
                if (!window.running) return;
                const hit = rayComp.intersectedEls && rayComp.intersectedEls[0];
        ...
        // canvas อาจจะยังไม่พร้อม → รอ 'loaded'
        const tryBind = () => {
            const canvas = sceneEl.canvas || (sceneEl.renderer && sceneEl.renderer.domElement);
            if (canvas) {
                bindMouse(canvas);
                sceneEl.removeEventListener('loaded', tryBind);
            }
        };
        if (sceneEl.hasLoaded) {
            tryBind();
        } else {
            sceneEl.addEventListener('loaded', tryBind);
        }
    }
    ...
    exports.GameEngine = {
        start(level) {
            ...
            spawnTimer = setTimeout(spawnTarget, 800); // เริ่ม spawn แรก
            
            Quest.start(); // เริ่มระบบภารกิจ
            setupShootHandlers(sceneEl); // ผูก event ยิง (ครั้งเดียว)
