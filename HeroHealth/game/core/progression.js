// === Hero Health Academy — core/progression.js (v2.2: daily-event binding) ===
//
// เพิ่มเติมจาก v2.1:
// - ภารกิจรายวันเป็นแบบมี key/need ที่ตรวจจากอีเวนต์จริง
// - Progress.notify(type,payload) สำหรับรับเหตุการณ์ระหว่างเล่น
// - นับ "จำนวนรอบวันนี้" เพื่อภารกิจ runs_any_2
// - ตรวจภารกิจตอน endRun โดยใช้ runCtx.flags + metrics ของรอบนั้น
//
// ภารกิจที่รองรับ (keys):
//   runs_any_2                : เล่นโหมดใดก็ได้วันนี้ >= 2 รอบ
//   combo_ge_15               : ทำคอมโบสูงสุด >= 15
//   score_ge_400              : คะแนนรวม >= 400
//   golden_ge_2               : เก็บ Golden >= 2
//   fever_ge_1                : เปิด FEVER >= 1
//   acc_ge_70                 : ความแม่น >= 70 (ใช้ acc ที่ส่งเข้า endRun)
//   plate_no_overfill         : รอบนี้ไม่มี overfill
//   hydration_no_high         : รอบนี้ไม่เคยหลุดขึ้นโซน HIGH/overflow
//   groups_target_rounds_ge_3 : จบ “รอบเป้าหมาย” 3 ครั้ง (นับ group_round_done)
//   goodjunk_perfect_ge_10    : โหมด goodjunk มี perfect >= 10
//
// Public API: (เดิม)
//   Progress.init(), .on(), .beginRun(), .endRun(), .getStatSnapshot()
//   .profile(), .genDaily(), .getDaily(), .markDaily(), .resetDaily()
//   .addMissionDone(), .exportProfile(), .importProfile(),
//   .setXPLevel(), .setModeStat()
//
// ใหม่:
//   Progress.notify(type, payload)   // แจ้งเหตุการณ์เฉพาะกิจระหว่างเล่น
//
export const Progress = (() => {
  ...
  function defProfile(){
    return {
      ...
      // เก็บจำนวนรอบเฉพาะ "วันนี้"
      todayRuns: { date: '', count: 0 }
    };
  }
  function mergeSchema(obj){
    ...
    out.todayRuns = { ...base.todayRuns, ...(obj?.todayRuns||{}) };
    out.version = VERSION;
    return out;
  }
  ...
  function safeLoad(){
    try{
      ...
      return mergeSchema(JSON.parse(raw));
    }catch{ return defProfile(); }
  }
  ...
  // ---------- Daily mission pool (3 ภารกิจ/วัน แบบไม่ซ้ำ) ----------
  const DAILY_POOL = [
  ...
  function roll3Distinct(arr){
    const bag = arr.slice(); const out=[];
  ...
  function ensureDaily(){
    ...
    profile.daily = {
      date: today,
      missions: picks.map((m,i)=>({
        id: `d${i+1}`,
        key: m.key,
        need: m.need,
        label: (navigator.language||'th').toLowerCase().startsWith('th') ? m.labelTH : m.labelEN
      })),
      done: []
    };
    save();
  ...
  function markDailyByKey(key){
    ensureDaily();
    const mis = profile.daily.missions || [];
    const found = mis.find(m => m.key === key);
    if (!found) return;
    markDaily(found.id);
  }
  ...
  function ensureTodayRuns(){
    const today = localDateStr();
    if (!profile.todayRuns || profile.todayRuns.date !== today) {
      profile.todayRuns = { date: today, count: 0 };
      save();
    }
  }
  function incTodayRuns(){
    ensureTodayRuns();
    profile.todayRuns.count = (profile.todayRuns.count|0) + 1;
    save();
  }
  ...
  function beginRun(mode, diff, lang){
    runCtx = {
      ...
      metrics: { bestCombo: 0, score: 0, acc: 0 } // acc จะตั้งตอน endRun
    };
  ...
  function notify(type, payload = {}){
    ...
    switch(String(type)){
      ...
      case 'combo_best':
        runCtx.metrics.bestCombo = Math.max(runCtx.metrics.bestCombo|0, payload?.value|0);
        break;
      case 'score_tick':
        runCtx.metrics.score = Math.max(runCtx.metrics.score|0, payload?.score|0);
        break;
      default: break;
    }
  }
  ...
  function endRun({ score=0, bestCombo=0, timePlayed=0, acc=0 } = {}){
    ...
    runCtx.metrics.score = Math.max(runCtx.metrics.score|0, score|0);
    runCtx.metrics.acc = acc|0;
    ...
    // XP & Level
    const gain = Math.max(5, Math.min(150, Math.round(score/20 + acc)));
    ...
    // ---- ตรวจภารกิจรายวัน (จาก runCtx + metrics) ----
    ensureDaily(); ensureTodayRuns();
    // 1) นับรอบวันนี้
    incTodayRuns();
    if ((profile.todayRuns.count|0) >= 2) markDailyByKey('runs_any_2');

    // 2) เงื่อนไขตามตัวชี้วัด
  ...
  function getStatSnapshot(){
    const rows = Object.keys(profile.modes).map(k=>{
      ...
      return { key:k, bestScore:v.bestScore|0, acc:+((v.accAvg||0).toFixed(1)), runs:v.games|0, missions:v.missionDone|0, lastPlayedAt:v.lastPlayedAt||0 };
    });
    return {
      level: profile.level|0, xp: profile.xp|0,
      totalRuns: profile.meta.totalRuns|0, bestCombo: profile.meta.bestCombo|0, lastPlayedAt: profile.meta.lastPlayedAt||0,
      todayRuns: { ...(profile.todayRuns||{}) },
      rows
    };
  }
  ...
  function importProfile(json){
    try{
      ...
      profile = mergeSchema(src); save(); emit('profile_imported', { version: profile.version }); return true;
