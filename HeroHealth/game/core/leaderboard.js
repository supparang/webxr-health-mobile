...
export class Leaderboard {
  ...
  getInfo(scope='month'){
    ...
    return { text:`Scope: ${scope.toUpperCase()} • Records: ${data.length} • Best: ${best}` };
  }

  _scoped(scope){
    if (scope === 'all') return this.entries.slice();
    const d = new Date(); let from = 0;
    if (scope === 'week') {
      const day = (d.getDay()+6)%7;
      const start = new Date(d.getFullYear(), d.getMonth(), d.getDate()); start.setDate(start.getDate()-day); start.setHours(0,0,0,0);
      from = +start;
    } else if (scope === 'month') {
      const start = new Date(d.getFullYear(), d.getMonth(), 1); start.setHours(0,0,0,0); from = +start;
    } else if (scope === 'year') {
      const start = new Date(d.getFullYear(), 0, 1); start.setHours(0,0,0,0); from = +start;
    }
  ...
  _gc(){
    ...
    this.entries = this.entries.filter(e=> e.ts >= cutoff).sort((a,b)=> b.score - a.score || a.ts - b.ts);
    if (this.entries.length > this.maxKeep) this.entries.length = this.maxKeep;
    this._save();
  }

  _load(){
    try{ const raw = localStorage.getItem(this.key); if (!raw) return []; const arr = JSON.parse(raw); return Array.isArray(arr)?arr.filter(validEntry):[]; }catch{ return []; }
  }
  _save(){
