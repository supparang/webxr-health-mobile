// === core/hud.js (production HUD: top bar, big-number, fever flame, quest chips, full result) ===
'use strict';
...
export class HUD {
  constructor() {
    ...
    this.top.innerHTML =
      ...
      '<div style="display:flex;gap:8px;align-items:center">'+
        ...
        '<span id="hudStarsWrap" style="padding:4px 8px;border-radius:10px;background:#0b1c36;color:#fcd34d;border:1px solid #134064;pointer-events:auto">★☆☆☆☆</span>'+
      '</div>';
    ...
    this.$stars = this.top.querySelector('#hudStarsWrap');

    // ----- Fever bar host (bottom-left) -----
    ...
    this.big.style.cssText = 'position:fixed;left:50%;top:42%;transform:translate(-50%,-50%) scale(.9);font:900 92px ui-rounded,system-ui;color:#fef3c7;text-shadow:0 8px 40px rgba(0,0,0,.6);pointer-events:none;opacity:0;transition:opacity .15s, transform .15s;z-index:2100';
    ...
    this.result.innerHTML =
      '<div style="width:min(600px,94vw);background:#0e1930;border:1px solid #16325d;border-radius:16px;padding:16px;color:#e6f2ff">'+
        ...
        '<p  id="resDesc"  style="margin:0 10px 12px;color:#cfe7ff;white-space:pre-line">—</p>'+
        '<div id="resStars" style="font:900 22px ui-rounded;color:#fcd34d;margin:0 0 10px">★☆☆☆☆</div>'+
    ...
    // ----- Expose HUD API for coach & others -----
    window.__HHA_HUD_API = {
      say: (m)=> this.say(m),
      setFever: (on)=> this.showFever(on),
      big: (t)=> this.showBig(t),
      toast: (t)=> this.toast(t),
      setQuestChips: (list)=> this.setQuestChips(list),
    };
  }
  ...
  setTimer(sec) {
    this.$time.textContent = Math.max(0, Math.round(sec)) + 's';
  }
  updateHUD(score, combo) {
    this.$score.textContent = String(score | 0);
    this.$combo.textContent = String(combo | 0);
  }
  setStars(n = 0) {
    const k = Math.max(0, Math.min(5, n | 0));
    this.$stars.textContent = '★★★★★'.slice(0, k) + '☆☆☆☆☆'.slice(0, 5 - k);
  }

  // ======= Quests (focus one by one) =======
  ...
  setQuestChips(list = []) {
    ...
    for (const m of list) {
      ...
      d.style.cssText =
        ...
        'background:' + (m.done ? '#0f2e1f' : m.fail ? '#361515' : '#0d1a31') + ';' +
        'color:#e6f2ff;';
      d.innerHTML =
  ...
  showFever(on) {
    if (on) {
      ...
      document.body.dataset.fever = '1';
      // subtle bg tint
      document.body.style.backgroundColor = 'rgba(255,120,0,0.06)';
    } else {
      this.$powerFill.innerHTML = '';
      document.body.dataset.fever = '';
  ...
  showBig(text) {
    ...
    setTimeout(() => {
      this.big.style.opacity = '0';
      this.big.style.transform = 'translate(-50%,-50%) scale(.9)';
    }, 380);
  }
  pulseCountdown(sec) {
    // ใช้เรียกตอนนับถอยหลัง 3-2-1- GO หรือ 10 วิสุดท้าย
    this.showBig(sec > 0 ? String(sec) : 'GO!');
  }
  say(text) {
    if (!this.coachBox) return;
    this.coachBox.textContent = String(text || '');
    this.coachBox.style.display = 'block';
    clearTimeout(this._coachTo);
    this._coachTo = setTimeout(() => { this.coachBox.style.display = 'none'; }, 1400);
  }
  ...
  showFloatingText(x, y, text) {
    ...
    el.style.cssText =
      ...
      'pointer-events:none;z-index:2100;opacity:1;transition:all .72s ease-out;';
  ...
  // ======= Result =======
  showResult({ title='Result', desc='—', stats=[], extra=[], stars=0 } = {}) {
    this.$resTitle.textContent = String(title);
    this.$resDesc.textContent  = String(desc);

    // stars visual
    const k = Math.max(0, Math.min(5, stars | 0));
    this.$resStars.textContent = '★★★★★'.slice(0, k) + '☆☆☆☆☆'.slice(0, 5 - k);
  ...
  toast(text) {
    ...
    this._toastTo = setTimeout(() => { t.style.opacity = '0'; }, 1200);
  }
}
