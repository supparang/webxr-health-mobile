// === core/hud.js — HUD v2 (Mini-Quest one-by-one + Stars + Combo + Fever) ===
export class HUD {
  constructor () {
    ...
    // TOP BAR (mode/diff/time/score/combo + fever)
    ...
    // fever border flash layer
    this.feverFx = document.createElement('div');
    this.feverFx.style.cssText = 'position:fixed;inset:0;pointer-events:none;z-index:1999;display:none;box-shadow:inset 0 0 0 6px #6ea8ff88, inset 0 0 40px 12px #7c4dff44';
    document.body.appendChild(this.feverFx);
  }

  _styleChips(){
    const css = `.chip{padding:4px 8px;border-radius:10px;background:#0b2544;color:#cbe7ff;border:1px solid #15406e;pointer-events:auto}`;
    this._inject(css);
  }
  _styleBar(){
    const css = `.bar{display:inline-block;height:10px;width:140px;border-radius:999px;background:#0a1931;border:1px solid #12325a;overflow:hidden}
  ...
  setFever(on){
    this.$fever.style.display = on ? 'inline-flex':'none';
    this.feverFx.style.display = on ? 'block':'none';
  }
  setStars(n=0){
    const k = Math.max(0, Math.min(5, n|0));
    this.$stars.textContent = '★★★★★'.slice(0,k).padEnd(5,'☆');
  }
  setQuest({icon='🎯', text='—', have=0, need=0}){
    this.$qIcon.textContent = icon;
    this.$qText.textContent = text;
    this.$qProg.textContent = `${have|0}/${need|0}`;
    const pct = need>0 ? Math.min(100, Math.round((have/need)*100)) : 0;
    this.$qBar.style.width = pct+'%';
  }
  say(text=''){
  ...
  flashMiss(){
    ...
    setTimeout(()=>document.body.classList.remove('flash-danger'), 140);
  }
  popScore(txt, x, y){
    const el=document.createElement('div');
    el.textContent=txt;
    el.style.cssText = `position:fixed;left:${x|0}px;top:${y|0}px;transform:translate(-50%,-50%);font:900 18px ui-rounded;color:#fff;text-shadow:0 2px 10px #000;pointer-events:none;z-index:2100;opacity:1;transition:all .7s ease-out`;
