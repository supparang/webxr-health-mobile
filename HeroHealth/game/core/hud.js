// === core/hud.js — HUD + stars + fever bar + result ===
'use strict';
export class HUD {
  constructor(){
    ...
    if(!this.root){
      ...
      this.root.style.cssText='position:fixed;inset:0;pointer-events:none;z-index:2000;'; document.body.appendChild(this.root);
    }
    ...
    this.top.innerHTML =
      ...
      '<div style="display:flex;gap:8px;align-items:center">'+
        ...
        '<span style="padding:4px 8px;border-radius:10px;background:#0b1c36;color:#ffd166;border:1px solid #134064;pointer-events:auto">⭐ <b id="hudStars">0</b></span>'+
      '</div>';
    this.root.appendChild(this.top);
    this.$mode=this.top.querySelector('#hudMode'); this.$diff=this.top.querySelector('#hudDiff'); this.$time=this.top.querySelector('#hudTime');
    this.$score=this.top.querySelector('#hudScore'); this.$combo=this.top.querySelector('#hudCombo'); this.$stars=this.top.querySelector('#hudStars');
    ...
    // Coach bubble container (ให้ fallback coach ใช้)
    ...
    // Quest chips
    this.chips=document.createElement('div'); this.chips.id='questChips';
    ...
    this.result.innerHTML =
      '<div style="width:min(580px,94vw);background:#0e1930;border:1px solid #16325d;border-radius:16px;padding:16px;color:#e6f2ff">'+
