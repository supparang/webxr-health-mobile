// === Hero Health Academy — core/hud.js (2025-10-29, id-stable to index.html) ===
export class HUD {
  constructor() {
    // cache DOM
  ...
  setCombo(text){
    // ถ้าอยากโชว์เป็นข้อความสั้น ๆ ใช้ toast แทน (ไม่ต้องมี element เพิ่ม)
    if (!text) return;
    // แสดงแบบสั้น ๆ แต่ไม่ถี่เกิน
    this.toast(`Combo ${String(text)}`, 420);
  }
  ...
  /* --------- Missions / Quests (แสดงแบบชิพง่าย ๆ) --------- */
  ...
  setFeverProgress(/*0..1*/){ /* เตรียมไว้—หน้า index ยังไม่มีแถบ fever */ }
  setPowerTimers(/*t*/) { /* เตรียมไว้—ยังไม่มีแถบ power ในหน้า */ }

  /* --------- Hydration HUD hooks --------- */
  showHydration(zone='OK', pct=50){
    // ใน hydration.js มี HUD เฉพาะของมันเองแล้ว (hydroWrap)
    // ตรงนี้ไม่ต้องทำอะไรเพิ่มเติม
  }
  hideHydration(){ /* noop */ }
  ...
  dispose(){
    ...
    try{ this.$coach && (this.$coach.style.display='none'); }catch{}
    try{ this.$toast && this.$toast.classList.remove('show'); }catch{}
  }
...
export const fx = {
  popText(text, { x=0, y=0, ms=720 } = {}){
    ...
    el.style.cssText = `
      ...
      background:#102038cc; border:1px solid #1a2c47; padding:6px 10px; border-radius:10px;
      z-index:120; pointer-events:none; opacity:1; transition:transform .7s, opacity .7s`;
    document.body.appendChild(el);
