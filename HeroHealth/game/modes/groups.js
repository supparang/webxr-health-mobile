...
export function init(state,hud,diff){
  const pick = groups[Math.floor(Math.random()*groups.length)] || groups[3];
  state.currentTarget = (pick && pick.key) ? pick.key : 'fruit';
  const el=document.getElementById('targetWrap'); if(el) el.style.display='block';
  const badge=document.getElementById('targetBadge');
  if(badge){ const g=groups.find(x=>x.key===state.currentTarget); badge.textContent=g?g.label:state.currentTarget; }
}
...
export function onHit(meta,systems,state){
  ...
  const delta = ok ? 7 : -2;
  systems.score.add(delta);
  systems.fx.spawn3D(null, (delta>0?'+':'')+delta, delta>0?'good':'bad');
  if(ok){
    ...
    if(!systems.power.consumeShield()){ systems.sfx.play('sfx-bad'); }
  }
}
