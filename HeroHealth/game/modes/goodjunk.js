// === modes/goodjunk.js — DOM-spawn icons + Fever hooks + Shield/Star (signals enriched) ===
...
function onMiss(bus, reason){
  if (consumeShield()){ bus?.sfx?.power?.(); return; }
  bus?.miss?.({ reason }); bus?.sfx?.bad?.();
}
...
function spawnOne(glyph, isGood, isGolden, bus){
  ...
  const killto = setTimeout(()=>{ try{ d.remove(); }catch{} onMiss(bus, isGood? 'expired.good' : 'expired.bad'); }, lifeMs);
  ...
  d.addEventListener('click', (ev)=>{
    ...
    if (isGood){
      ...
      bus?.hit?.({ kind:(perfect?'perfect':'good'), points:pts, ui:{x:ev.clientX, y:ev.clientY}, meta:{ good:true, golden:!!isGolden } });
      if (perfect) bus?.sfx?.perfect?.(); else bus?.sfx?.good?.();
    } else {
      onMiss(bus, 'clicked.bad');
    }
  }, { passive:true });
...
function spawnPower(kind, bus){
  ...
  d.addEventListener('click',(ev)=>{
    ...
    else { bus?.power?.('star'); bus?.hit?.({ kind:'perfect', points:150, ui:{x:ev.clientX,y:ev.clientY}, meta:{ good:true, golden:false, star:true } }); }
  }, { passive:true });
  host.appendChild(d);
}
