// === modes/goodjunk.js — DOM-spawn icons + FEVER hooks + Shield/Gold + strict MISS rules ===
export const name = 'goodjunk';

const GOOD = ['🥦','🥕','🍎','🍌','🥗','🐟','🥜','🍚','🍞','🥛','🍇','🍓','🍊','🍅','🍆','🥬','🥝','🍍','🍐','🍑'];
const JUNK = ['🍔','🍟','🌭','🍕','🍩','🍪','🍰','🧋','🥤','🍗','🍖','🍫','🥓','🍿','🧈','🧂'];

// power: 'gold' = 🌟/⭐ (นับ mini quest gold), 'shield' = กัน MISS good 1 ครั้ง
const POWERS = ['gold','shield'];

let host=null, alive=false, fever=false, allowMiss=0;
let diff='Normal';
let iconSizeBase=48;
let spawnIntervalS=0.70; // lower = เร็วขึ้น
let lifeS=1.60;
let _accum=0;
...
export function start(cfg={}){
  ...
  if (diff==='Easy'){ spawnIntervalS=0.85; lifeS=2.0; iconSizeBase=58; }
  else if (diff==='Hard'){ spawnIntervalS=0.52; lifeS=1.35; iconSizeBase=42; }
  else { spawnIntervalS=0.70; lifeS=1.60; iconSizeBase=50; } // Normal
}
...
function spawnOne(glyph, isGood, isGolden, bus){
  ...
  // lifetime → MISS เฉพาะ good เท่านั้น
  ...
  d.addEventListener('click',(ev)=>{
    ...
    if (isGood){
      ...
      // junk click → ไม่ใช่ MISS แต่ตัดคอมโบ (แจ้งผ่าน bus.junk)
...
function spawnPower(kind, bus){
  ...
  d.textContent = (kind==='shield'?'🛡️':'🌟'); // gold uses 🌟
  Object.assign(d.style,{ position:'absolute', border:'0', background:'transparent',
    fontSize:(iconSizeBase+6)+'px', transform:'translate(-50%,-50%)',
  ...
  d.addEventListener('click',(ev)=>{
    ...
    if (kind==='shield'){ grantShield(1); bus?.power?.('shield'); }
      ...
      bus?.power?.('gold');
      // ให้แต้มเล็กน้อยด้วย (สร้างแรงจูงใจ)
      bus?.hit?.({ kind:'perfect', points:(fever?180:120), ui:{x:ev.clientX,y:ev.clientY} });
    }
  }, { passive:true });
...
function onMissGood(bus){
  ...
  bus?.miss?.({kind:'timeout_good'});
}

// update loop driven by main
...
// legacy compatibility for main.create()
export function create(){
  return { start:(cfg)=>start(cfg), update:(dt,bus)=>update(dt,bus), cleanup:()=>stop() };
}
