// === modes/goodjunk.js — DOM-spawn icons + Fever hooks + Shield/Star (penalty only on junk click) ===
...
export function start(cfg={}){
  ...
  if (diff==='Easy'){ rate=0.82; life=1.90; } else if (diff==='Hard'){ rate=0.56; life=1.40; } else { rate=0.70; life=1.60; }
}
...
function spawnOne(glyph, isGood, isGolden, bus){
  ...
  const base = (diff==='Easy'? 56 : diff==='Hard'? 42 : 50);
  const size = isGolden ? base+10 : base;
  Object.assign(d.style,{ position:'absolute', border:'0', background:'transparent', fontSize:size+'px', transform:'translate(-50%,-50%)', filter:'drop-shadow(0 6px 16px rgba(0,0,0,.55))' });
  const pad=56, W=innerWidth, H=innerHeight;
  const x = Math.floor(pad + Math.random()*(W - pad*2));
  const y = Math.floor(pad + Math.random()*(H - pad*2 - 140));
  d.style.left = x+'px'; d.style.top = y+'px';

  // อายุไอคอน: ถ้าหมดอายุ → แค่หาย (ไม่ลงโทษ)
  ...
  d.addEventListener('click', (ev)=>{
    ...
    if (isGood){
      ...
      bus?.hit?.({ kind:(isGolden?'perfect':'good'), points:pts, ui });
      if (isGolden) bus?.sfx?.perfect?.(); else bus?.sfx?.good?.();
    } else {
      // ❌ Junk → MISS (ให้ main จัดการโทษ)
...
function spawnPower(kind, bus){
  ...
  Object.assign(d.style,{ position:'absolute', border:'0', background:'transparent', fontSize:'50px', transform:'translate(-50%,-50%)', filter:'drop-shadow(0 8px 18px rgba(10,120,220,.55))' });
  const pad=56, W=innerWidth, H=innerHeight;
  const x = Math.floor(pad + Math.random()*(W - pad*2));
  const y = Math.floor(pad + Math.random()*(H - pad*2 - 140));
  d.style.left=x+'px'; d.style.top=y+'px';
...
function explodeAt(x,y){
  ...
  for(let i=0;i<n;i++){
    ...
    p.textContent='✦';
    Object.assign(p.style,{ position:'fixed', left:x+'px', top=y+'px', transform:'translate(-50%,-50%)', font:'900 16px ui-rounded,system-ui', color:'#a7c8ff', textShadow:'0 2px 12px #4ea9ff', transition:'transform .7s ease-out, opacity .7s ease-out', opacity:'1', zIndex:2200, pointerEvents:'none' });
