...
function guessCandidates() {
  ...
  const baseA = new URL('.', document.baseURI).href;
  const baseB = new URL('.', here).href;
  ...
  for (const r of rels) {
    const a = new URL(r, baseA).href, b = new URL(r, baseB).href;
...
(async function boot(){
  try {
    ...
    showWarn([
      ...
      'ตรวจสอบ:\n• ตำแหน่งไฟล์ควรอยู่ที่ game/main.js\n• ตรวจ <base> ให้ถูกโฟลเดอร์\n• ดู Network/Console เพื่อตรวจ MIME/404',
