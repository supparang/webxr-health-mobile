...
export function boot(opts = {}) {
  ...
  function getXY(ev){
    ...
    try{
      ...
      // mobile UI offset (soft keyboard / visualViewport)
  ...
  function computeSafeTop(){
    // อ่านตำแหน่ง HUD จริง (#hudTop .score-box หรือ [data-hud="scorebox"])
    let safe = 100;
    try{
      ...
      if (el){
        ...
        safe = Math.max(60, Math.round(r.bottom + 24)); // buffer สำหรับ fever bar/ไอคอน
      }
    }catch(_){}
    return safe;
  }
  ...
  (function injectCSS(){
    ...
    st.textContent = `
      ...
      .hha-tgt{position:absolute;transform:translate(-50%,-50%);display:block;opacity:1;
        ...
        background:transparent;padding:14px 16px;border-radius:18px; /* invisible wide hitbox */
  ...
  function spawnOne(forceCenter){
    ...
    function onHit(ev){
      ...
      const pt = getXY(ev); // ✅ coords on screen
    ...
    // pointer-first with fallbacks (no passive to allow preventDefault)
  ...
  function start(){
    ...
    // first spawn after layout is stable
  ...
  function hardClearLayer(){
    try{
      layer.classList.add('off'); // stop clicks instantly
