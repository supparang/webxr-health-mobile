// === HeroHealth/vr/mode-factory.js — DOM targets + hit-screen + expired + power-ups + onFinish (2025-11-12 patched) ===
export async function boot(config){
  ...
  var onFinish  = (typeof config.onFinish === 'function') ? config.onFinish : null;   // ให้โหมดคืนสรุปเพิ่ม
  var running = true;

  function pick(arr){ return arr[(Math.random()*arr.length)|0]; }
  function clamp(n,a,b){ return Math.max(a, Math.min(b,n)); }
  function fire(name, detail){ try{ window.dispatchEvent(new CustomEvent('hha:'+name,{detail:detail||{}})); }catch(e){} }
  ...
  function dbglog(s){ if(DEBUG){ try{dbg.textContent='DEBUG: '+s;}catch(_){}} }
  ...
  function tickTime(){ if(!running) return; left=Math.max(0,left-1); fire('time',{sec:left}); if(left<=0) end(); }
  ...
  function spawnOne(forceCenter){
    ...
    function onHit(ev){
      ...
      // screen coords for judge/Particles
      var cx=vw()/2, cy=vh()/2;
      if (ev && ev.touches && ev.touches[0]) { cx=ev.touches[0].clientX; cy=ev.touches[0].clientY; }
      else if (ev && ev.clientX!=null) { cx=ev.clientX; cy=ev.clientY; }

      // call judge with coords + context
      var res   = {good:false, scoreDelta:0};
      try {
        res = judge(ch, { clientX:cx, clientY:cy, score:score, combo:combo, misses:misses, diff:diff, isGood:isGood });
      } catch(_){}
      ...
      if(!good){ misses += 1; fire('miss',{count:misses}); }

      try{ el.className='hha-tgt hit'; layer.removeChild(el); }catch(_e){}
      fire('score',{score:score, combo:combo, delta:delta, good:good});
      fire('hit-screen',{x:cx, y:cy, good:good, delta:delta, char:ch, isGood:isGood});
    ...
    setTimeout(function(){
      ...
      if(isGood){
        ...
        combo=0; misses+=1; fire('miss',{count:misses});
        fire('score',{score:score, combo:combo});
        fire('expired',{isGood:true, ch:ch});              // ✅ แจ้งหมดอายุ (GOOD)
        if(onExpire) try{ onExpire({isGood:true, ch:ch}); }catch(_e){}  // ✅ ให้โหมดรู้ด้วย
        dbglog('expire GOOD (miss)');
      }else{
        // JUNK หมดอายุ = หลบสำเร็จ
        fire('avoid',{ch:ch});
        fire('expired',{isGood:false, ch:ch});             // (JUNK)
  ...
  function end(){
    ...
    // ให้โหมดเติม field เพิ่มได้ (quests, goal ฯลฯ)
    if (onFinish) {
      try {
