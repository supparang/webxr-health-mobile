// === /HeroHealth/vr/mode-factory.js — DOM targets + hit-screen + expired + power-ups + onFinish (2025-11-12) ===
export async function boot(config){
  ...
  var powerEvery = (typeof config.powerEvery==='number') ? config.powerEvery : 7;    // อย่างน้อยทุก 7 สปอน
  ...
  function spawnOne(forceCenter){
    ...
    function onHit(ev){
      ...
      else if (ev && typeof ev.clientX === 'number') { cx=ev.clientX; cy=ev.clientY; }
      else if (ev && ev.changedTouches && ev.changedTouches[0]) { cx=ev.changedTouches[0].clientX; cy=ev.changedTouches[0].clientY; }

      // ✅ ส่ง clientX/clientY เข้า judge เพื่อให้เอฟเฟกต์คะแนนเด้งตรงเป้า
      var res = judge(ch, {
        clientX: cx,
        clientY: cy,
        score:   score,
