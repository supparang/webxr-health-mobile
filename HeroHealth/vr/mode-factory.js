...
const MIN_DIST_DEFAULT=0.36, SLOT_COOLDOWN_MS_DEFAULT=520;
...
export async function boot(config={}){
  const {
    name='mode', pools={ good:[], bad:[] }, judge,
  ...
  let host=givenHost; if(!host){ const wrap=$('a-scene')||document.body; const auto=document.createElement('a-entity'); auto.id='spawnHost'; wrap.appendChild(auto); host=auto; }
  const sfx=new SFX('../assets/audio/'); await sfx.unlock?.(); sfx.attachPageVisibilityAutoMute?.();
  const scene=$('a-scene')||document.body; const fever=new Fever(scene,null,{durationMs:10000});

  const mq=new MiniQuest( { tQmain:$(ui.questMainSel||'#tQmain') },
    { coach_start:$('#coach_start'), coach_good:$('#coach_good'), coach_warn:$('#coach_warn'), coach_fever:$('#coach_fever'), coach_quest:$('#coach_quest'), coach_clear:$('#coach_clear') } );
  mq.start(goal);
  const missions=new MissionDeck(); missions.draw3?.();

  const difficulty=givenDiff; let duration=givenDuration||timeByDiff[difficulty]||60;
  $('#hudTime')?.setAttribute('troika-text','value',`เวลา: ${duration}s`);
  const diff=new Difficulty(); const safe={ size:0.60, rate:520, life:2000 };
  const base=(diff?.config?.[difficulty]) || (diff?.config?.normal) || safe;
  let spawnRateMs=Number(base.rate)||safe.rate, lifetimeMs=Number(base.life)||safe.life;
  let sizeFactor=Math.max(0.40,(Number(base.size)||0.60)*0.80);
  let hitWBase=(difficulty==='easy'?0.50: difficulty==='hard'?0.40:0.46); const hitW=Math.min(hitWBase,minDist*0.80);

  let running=true, missionGood=0, score=0, combo=0, comboMax=0, streak=0, lastGoodAt=now();
  let MAX_ACTIVE=maxActiveByDiff[difficulty]??2; const BUDGET=budgetByDiff[difficulty]??2;

  const active=new Set(); const slots=buildSlots(); const busyCols=new Set(), busyRows=new Set();
  ...
  function spawnOne(){
    if(!running) return; if(SPAWN_LOCK) return; SPAWN_LOCK=true;
    try{
      if(active.size>=MAX_ACTIVE || issuedThisSec>=BUDGET) return;
      const slot=takeFreeSlot(slots,busyCols,busyRows,slotCooldownMs); if(!slot) return;
      ...
      if(isGold){
        ...
        const halo=document.createElement('a-ring'); halo.setAttribute('radius-inner','0.18'); halo.setAttribute('radius-outer','0.22');
        halo.setAttribute('position','0 0 0.001'); halo.setAttribute('material','color:#ffe066; opacity:0.85; shader:flat'); el.appendChild(halo);
      }

      const hit=document.createElement('a-plane'); hit.setAttribute('width',hitW); hit.setAttribute('height',hitW);
      hit.setAttribute('material','opacity:0; transparent:true; side:double'); hit.classList.add('hit','clickable'); el.appendChild(hit);
      ...
      const killer=setTimeout(()=>{ // miss
        if(typeof judge==='function'){ const res=judge(null,{type:'timeout',char,score,combo,streak,feverActive:fever.active}); if(res?.good===false){ streak=0; combo=0; mq.junk(); } }
        else{ streak=0; combo=0; mq.junk(); }
        cleanup();
      }, ttl);

      const fire=(ev)=>{ if(consumed) return; consumed=true;
        try{ ev?.stopPropagation?.(); ev?.preventDefault?.(); }catch{} clearTimeout(killer);
        let res={good:true,scoreDelta:10,feverDelta:0}; if(typeof judge==='function') res=judge(char,{type:'hit',score,combo,streak,feverActive:fever.active});
        if(res.good){
          ...
          try{ const t=document.createElement('a-entity'); t.setAttribute('troika-text',`value: +${plus}; color:#fff; fontSize:0.08; anchor:center`);
               ...
               t.setAttribute('animation__fade',`property: opacity; to: 0; dur: 520; easing: linear`); setTimeout(()=>t.remove(),560); }catch{}
          if(res.feverDelta) try{ fever.add(res.feverDelta); }catch{}
          mq.good({score,combo,streak,missionGood});
        }else{
          score=Math.max(0,score+(res.scoreDelta??-5)); combo=0; streak=0; sfx.popBad?.(); Particles.smoke?.(host,{x:slot.x,y:slot.y,z:slot.z}); mq.junk();
        }
      ...
      function cleanup(){ try{ el.remove(); }catch{} active.delete(el); busyCols.delete(slot.col); busyRows.delete(slot.row); releaseSlot(slots,slot); }
    } finally { SPAWN_LOCK=false; }
  }

  // Sweeper แก้ซ้อนทุก 200ms
      ...
      for(let j=i+1;j<arr.length;j++){
        ...
        try{
          ...
          if(d2 < (MIN_DIST_DEFAULT*MIN_DIST_DEFAULT)){
            const dest=takeFreeSlot(slots,new Set(),new Set(),SLOT_COOLDOWN_MS_DEFAULT);
            if(dest){ b.setAttribute('position',`${dest.x} ${dest.y} ${dest.z}`); b.__col=dest.col; b.__row=dest.row; }
  ...
  function loop(){ clearTimeout(spawnTicker); const tick=()=>{ if(running && issuedThisSec<BUDGET) spawnOne(); const cd=Math.max(380, spawnRateMs|0); spawnTicker=setTimeout(tick, cd); }; tick(); }
  ...
  window.addEventListener('hha:fever',(e)=>{ if(e?.detail?.state==='start'){ try{ mq.fever(); }catch{} spawnRateMs=Math.round(spawnRateMs*0.85); } else { spawnRateMs=Number(base.rate)||520; } });
  ...
  const api={ pause(){ if(!running) return; running=false; clearTimeout(spawnTicker); },
