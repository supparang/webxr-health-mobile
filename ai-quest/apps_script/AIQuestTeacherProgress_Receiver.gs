/**
 * CSAI2102 AI Quest Teacher Progress Receiver v1.1
...
AIQTEACHER.VERSION = '20260722-AIQ-TEACHER-PROGRESS-V1.1.0-DIAGNOSTIC';
...
AIQTEACHER.requireReady_ = function(){
  var d=AIQTEACHER.dependencyStatus_();
  if(!d.flowReady) throw new Error('AIQFLOW_DEPENDENCY_MISSING');
  if(!d.spreadsheetReady) throw new Error('SPREADSHEET_NOT_FOUND');
  return d;
};
AIQTEACHER.profileRows_ = function(section){
  ...
  for(var n=0;n<names.length;n++){
    ...
    if(iId<0) continue;
    d.rows.forEach(function(r){var sid=AIQTEACHER.text_(r[iId]),sec=iSec>=0?AIQTEACHER.text_(r[iSec]):section;if(!sid||sec!==section)return;out[sid]={studentId:sid,studentName:iName>=0?AIQTEACHER.text_(r[iName]):'',section:section};});
  }
  return out;
};
...
AIQTEACHER.detail_ = function(student,section){
  ...
  AIQTEACHER.ORDER.forEach(function(id){var m=AIQFLOW.missionStatus_(student.studentId,section,id),c=AIQFLOW.codingStatus_(student.studentId,section,id),r=AIQFLOW.reflectionStatus_(student.studentId,section,id),done=!!(m.completed&&c.completed&&r.completed);if(m.completed)missionCount++;if(c.completed)codingCount++;if(r.completed)reflectionCount++;if(done){completed++;lastSession=id;}sessions[id]={mission:!!m.completed,missionScore:Number(m.bestScore||0),coding:!!c.completed,codingScore:Number(c.bestScore||0),reflection:!!r.completed,completed:done};});
...
AIQTEACHER.ping_ = function(){
  var d=AIQTEACHER.dependencyStatus_();
  return {ok:d.flowReady&&d.spreadsheetReady,code:d.flowReady?(d.spreadsheetReady?'READY':'SPREADSHEET_NOT_FOUND'):'AIQFLOW_DEPENDENCY_MISSING',module:'AIQTEACHER',version:AIQTEACHER.VERSION,dependencies:d};
};
AIQTEACHER.handle = function(p){
  try {
    p=p||{};var action=AIQTEACHER.text_(p.action).toUpperCase();
    if(action==='PING'||action==='TEACHER_PING')return AIQTEACHER.ping_();
    ...
    return {ok:false,code:String(e&&e.message||e||'TEACHER_SERVER_ERROR'),message:String(e&&e.stack||e||''),version:AIQTEACHER.VERSION};
  }
};