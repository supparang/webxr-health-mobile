/**
 * CSAI2102 AI Quest Coding Receiver v3.2.0
...
AIQCODING.VERSION = '20260727-AIQ-CODING-RECEIVER-V3.2.0-SEMANTIC-ALIGNMENT';
...
/* b=Completed Code, m=Scenario Modification, c=Challenge, ca/ma=one-of */
AIQCODING.RULES = Object.freeze({
 S1:{expected:'High temperature alert',b:['temperature','if','print'],m:['38.5','38','39'],c:['if','elif','else','low','normal','high']},
 S2:{expected:'clean',b:['def','agent','percept','return'],m:['obstacle','turn'],c:['battery_low','goal_found','return']},
 S3:{expected:"['A', 'B', 'C', 'D', 'E']",b:['deque','queue','visited'],m:["'f'","'d'"],c:['goal','path'],ca:['break','return']},
 B1:{expected:'search_new_route',b:['choose_action','blocked','return'],m:['percept','return'],mc:{percept:3,return:3},c:['deque','blocked'],ca:['path','route']},
 S4:{expected:'D 6',b:['heapq','frontier','cost'],m:["'b'","'c'",'1'],c:['parent','path','cost']},
 S5:{expected:'B 4',b:['g','h','print'],m:["'d'",'3','1'],c:['astar','heapq','path'],ca:['heuristic','h[']},
 S6:{expected:'3',b:['scores','min','max'],m:['8','1','6','4'],c:['minimax','max','min'],ca:['maximizing','maximizing_player']},
 B2:{expected:'A*',b:['weighted','heuristic','strategy'],m:['false','weighted','ucs'],c:['choose_strategy','bfs','ucs','a*','minimax']},
 S7:{expected:'can_fly',b:['facts','bird','has_wings'],m:['penguin','cannot_fly'],c:['forward_chaining','facts','rules']},
 S8:{expected:'0.154',b:['prior','sensitivity','posterior'],m:['0.10'],c:['bayes','prior','likelihood','posterior']},
 S9:{expected:'recommend_checkup',b:['symptoms','fever','cough'],m:['rest_and_monitor','not','cough'],c:['inference_engine','rules','reason']},
 B3:{expected:'high_risk_with_evidence',b:['facts','probability','0.8'],m:['medium_risk','0.5','0.8'],c:['decision','confidence','explanation']},
 S10:{expected:'8 2',b:['data','train','test'],m:['0.7','train','test'],c:['split_data','ratio','seed'],ca:['leakage','overlap','set(']},
 S11:{expected:'[0, 1, 1]',b:['scores','predictions','0.5'],m:['0.8'],c:['confusion_matrix','y_true','y_pred','accuracy']},
 S12:{expected:'[0, 0, 1, 1]',b:['points','centers','clusters'],m:['5'],ma:['tie','equal','<=','>='],c:['kmeans','clusters','centroids'],ca:['iteration','range(2']},
 B4:{expected:'overfitting',b:['train_accuracy','test_accuracy','0.2'],m:['underfitting','0.6'],c:['evaluate_model','diagnosis','recommendation']},
 S13:{expected:'0.1',b:['x','w','b','relu'],ba:['max(0','relu'],m:['sigmoid','exp','z'],c:['dense_layer','weights','bias','outputs']},
 S14:{expected:'0.55',b:['q','reward','alpha','new_q'],m:['gamma','next_max_q'],c:['epsilon_greedy','explore','exploit','random']},
 S15:{expected:'RAG retrieves evidence',b:['documents','query','selected'],m:['lower'],c:['simple_rag','answer','sources','fallback']},
...
AIQCODING.pick_ = function(row,names){
  row=row||{};for(var i=0;i<names.length;i++){var n=names[i],v=row[n];if(v===undefined)v=row[AIQCODING.key_(n)];if(v!==undefined&&v!==null&&AIQCODING.text_(v)!=='')return v;}return '';
};
...
AIQCODING.validateEvidenceV31_ = function(sessionId,payload){
  ...
  if(!rule)return {mode:'TEACHING_V3_2_SEMANTIC',predictionCorrect:false,completedCodePassed:false,outputPassed:false,modifyPassed:false,challengePassed:false,quizPassed:false,quizScore:0,runScore:0,modifyScore:0,challengeScore:0,quizPoint:0,parsed:x,diagnostics:{error:'RULE_NOT_FOUND'}};
  ...
  var challengePassed=challengeBody.length>=20&&AIQCODING.containsAll_(challengeBody,rule.c)&&AIQCODING.containsAny_(challengeBody,rule.ca)&&AIQCODING.minCounts_(challengeBody,rule.cc);
  var quizPassed=x.quizScore>=4;
  return {
    mode:'TEACHING_V3_2_SEMANTIC',predictionCorrect:predictionCorrect,completedCodePassed:completedCodePassed,outputPassed:outputPassed,modifyPassed:modifyPassed,challengePassed:challengePassed,quizPassed:quizPassed,quizScore:x.quizScore,
    ...
    diagnostics:{expectedOutput:rule.expected,predictionReasonLength:x.predictionReason.length,missingCompletedKeywords:AIQCODING.missing_(x.completedCode,rule.b),missingModifyKeywords:AIQCODING.missing_(x.scenarioCode,rule.m),missingChallengeKeywords:AIQCODING.missing_(challengeBody,rule.c)}
  };
};
AIQCODING.validateEvidenceLegacy_ = function(sessionId,payload){
  var rule=AIQCODING.RULES[sessionId];if(!rule)return {mode:'LEGACY_V2',predictionCorrect:false,completedCodePassed:false,outputPassed:false,modifyPassed:false,challengePassed:false,quizPassed:false,quizScore:0,runScore:0,modifyScore:0,challengeScore:0,quizPoint:0,parsed:{}};
  ...
  var predictionCorrect=prediction===expected,outputPassed=output.indexOf(expected)>=0,modifyPassed=AIQCODING.containsAll_(modified,rule.m)&&AIQCODING.containsAny_(modified,rule.ma)&&AIQCODING.minCounts_(modified,rule.mc),challengePassed=AIQCODING.containsAll_(challenge,rule.c)&&AIQCODING.containsAny_(challenge,rule.ca)&&AIQCODING.minCounts_(challenge,rule.cc);
...
AIQCODING.submit_ = function(payload){
  payload=payload||{};var sessionId=AIQCODING.session_(payload.sessionId),studentId=AIQCODING.text_(payload.studentId),studentName=AIQCODING.text_(payload.studentName),section=AIQCODING.section_(payload.section||'101');
  ...
  if(!evidence.challengePassed)return {ok:false,code:'CHALLENGE_SEMANTIC_FAILED',message:'Challenge Code ยังไม่แสดงแนวคิดตามระดับที่เลือก',evidence:evidence,version:AIQCODING.VERSION};
...
AIQCODING.getStatus_ = function(payload){
  ...
  var all=AIQCODING.allHistoricalRows_(),rows=all.filter(function(r){return AIQCODING.matches_(r,studentId,section,sessionId);});
  rows.sort(function(a,b){var ta=Date.parse(AIQCODING.rowSubmittedAt_(a))||AIQCODING.num_(AIQCODING.pick_(a,['attempt_number','attemptNumber'])),tb=Date.parse(AIQCODING.rowSubmittedAt_(b))||AIQCODING.num_(AIQCODING.pick_(b,['attempt_number','attemptNumber']));return ta-tb;});
  var latest=rows.length?rows[rows.length-1]:null,best=null;rows.forEach(function(r){if(!best||AIQCODING.rowScore_(r)>AIQCODING.rowScore_(best))best=r;});
  var bestScore=best?AIQCODING.rowScore_(best):0,sourceSheets=[];rows.forEach(function(r){if(sourceSheets.indexOf(r._sheet)<0)sourceSheets.push(r._sheet);});
...
AIQCODING.auditStatus_ = function(payload){
  payload=payload||{};var result=AIQCODING.getStatus_(payload),target={studentId:AIQCODING.text_(payload.studentId),studentName:AIQCODING.text_(payload.studentName),section:AIQCODING.section_(payload.section||'101'),sessionId:AIQCODING.session_(payload.sessionId)},rows=AIQCODING.allHistoricalRows_();
...
AIQCODING.handle = function(payload){
  ...
  if(action==='GET_LAB_CONFIG'){var sessionId=AIQCODING.session_(payload.sessionId);if(!AIQCODING.allowedLab_(sessionId))return {ok:false,code:'LAB_NOT_AVAILABLE',version:AIQCODING.VERSION};return {ok:true,version:AIQCODING.VERSION,sessionId:sessionId,config:AIQCODING.RULES[sessionId]};}
