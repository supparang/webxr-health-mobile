/*
  CSAI2102 AI Quest
  PATCH v2.8.1 Session Roadmap + Boss Gate
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v2.8.1-roadmap-s4-ui-consistency';
  ...
  function isClickable(st, item){
    ...
    if(item.id === 'm1' || item.id === 'm2' || item.id === 'm3' || item.id === 'm4') return sessionUnlocked(st, item.id) || item.available;
    return false;
  }
  ...
  function render(){
        ...
        : b1Open && !isPassed(st,'b1')
          ...
          : isPassed(st,'b1') && !isPassed(st,'m3')
            ...
            : isPassed(st,'m3') && !isPassed(st,'m4')
              ...
              : isPassed(st,'m4')
                ? 'ผ่าน S4 แล้ว: S5 A* Rescue Mission จะเปิดใน patch ถัดไป'
    ...
    anchor.innerHTML = `
      ...
      <div class="roadmapTop">
        ...
        <div class="roadmapMetric"><div class="num">${isPassed(st,'m4') ? 'S5' : isPassed(st,'m3') ? 'S4' : 'S3'}</div><div class="label">${isPassed(st,'m4') ? 'ถัดไป: A* Rescue Mission' : isPassed(st,'m3') ? 'ถัดไป: Route Cost Challenge' : 'ถัดไป: Search Maze'}</div></div>
      </div>

      <div class="bossGateBox">
        <b>Progress Rule:</b> S1–S2 → B1 → S3 Search Maze → S4 Route Cost Challenge → S5 A*
