/* CSAI2102 AI Quest — Teacher AR + Attempt History v4.0.7
   - Displays every normal Session attempt received from Google Sheets.
   - Displays S1/S2 AR supplementary evidence from session_events.
   - Collapses exact duplicate AR delivery records while preserving real replays.
   - Keeps AR separate from normal-session score and gates.
*/
(() => {
  'use strict';
  ...
  // A real replay gets a new completedAt. Exact same evidence timing/metrics is delivery duplication.
  function arFingerprint(row){
    return [
      row.type,
      row.studentId,
      row.completedAt,
  ...
  function uniqueArEvents(rows){
    const map = new Map();

    (rows || []).forEach((row) => {
      const key = arFingerprint(row);
      const existing = map.get(key);
      if (existing) {
        existing.duplicateCount += 1;
        return;
      }
      map.set(key, Object.assign({}, row, { duplicateCount: 0 }));
    });

    return [...map.values()].sort((a, b) => (Date.parse(b.completedAt) || 0) - (Date.parse(a.completedAt) || 0));
  }
  ...
  function renderS2Ar(){
    ...
    const rawRows = allArEvents().filter((row) => row.type === 's2_ar_complete');
    const rows = uniqueArEvents(rawRows);
    const hiddenDuplicates = rawRows.length - rows.length;
    ...
    box.innerHTML = `
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap">
        <div>
          <h2 style="margin:0">S2 AR Practice</h2>
          <p class="muted" style="margin:7px 0 0">Agent Builder • ประวัติกิจกรรมเสริมจาก session_events</p>
        </div>
        <span class="pill good">✓ Real S2 AR events ${rows.length} รอบ${hiddenDuplicates ? ` • ซ่อน duplicate ${hiddenDuplicates} รายการ` : ''}</span>
      </div>
  ...
  function renderArEvidenceHistory(){
    ...
    box.innerHTML = `
      <div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start;flex-wrap:wrap">
        <div><h2 style="margin:0">AR Evidence History</h2><p class="muted" style="margin:7px 0 0">แสดงทุกครั้งที่ทำ S1/S2 AR โดยซ่อนเฉพาะรายการส่งซ้ำที่มีหลักฐานเหมือนกันทุกค่า</p></div>
        <span class="pill good">${rows.length} AR runs${hiddenDuplicates ? ` • ซ่อน duplicate ${hiddenDuplicates}` : ''}</span>
      </div>
      <div style="overflow:auto;max-height:420px;margin-top:12px">
        <table>
  ...
  function fingerprint(){
    ...
    const events = uniqueArEvents(allArEvents());
    return JSON.stringify({
      attempts: attempts.map((a) => [a.studentId, a.sessionId, a.score, a.timestamp, a.correct, a.total]),
      events: events.map((e) => [e.type, e.studentId, e.completedAt, e.score, e.correct, e.total, e.help, e.usedSec])
    });
  }
  ...
  window.AIQUEST_TEACHER_S2_AR_ANALYTICS = {
    version: 'v4.0.7-deduplicated-ar-evidence',
    getS2Records: () => uniqueArEvents(allArEvents().filter((row) => row.type === 's2_ar_complete')),
