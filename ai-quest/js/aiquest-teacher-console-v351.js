(function(){ window.FocusCsv = window.FocusCsv || ''; window.focusCsv = window.focusCsv || ''; })();
/*
  CSAI2102 AI Quest
  PATCH v3.5.1 Teacher Console Session Switch
  ------------------------------------------------------------
...
(function(){
  ...
  function displayAccuracyV325(row){
    row = row || {};
    const direct = Number(row.accuracy || row.Accuracy || row.accuracyPct || row.accuracyPercent || 0);
    if(Number.isFinite(direct) && direct > 0) return Math.round(direct);
    const correct = Number(row.correct || row.correctCount || row.correctItems || NaN);
    const total = Number(row.total || row.totalCount || row.totalItems || row.questions || NaN);
    if(Number.isFinite(correct) && Number.isFinite(total) && total > 0) return Math.round((correct / total) * 100);
    return 'N/A';
  }
const VERSION = 'v3.5.1-teacher-console-session5-astar';
  ...
  function ensureConsolePanel(){
    ...
    panel.innerHTML = `
      <div class="teacherConsoleHeader">
        ...
        <div class="teacherToolBar">
          ...
          <button class="btn secondary" id="btnTeacherExportCsv">Export Review Focus CSV</button>
      ...
      <div class="teacherGrid2">
        ...
        <div class="teacherBox">
          <h3>Students to Review / Support</h3>
          <div class="teacherSmallNote">
          ...
          <div class="teacherTableWrap" style="margin-top:10px">
            <table class="teacherTable">
              ...
              <tbody id="teacherReview FocusBody">
    ...
    $('#btnTeacherExportCsv').onclick = exportReview FocusCsv;
  ...
  function renderConsole(resp){
    ...
    renderReview Focuss(data.risks || []);
  ...
  function renderReview Focuss(risks){
    const body = $('#teacherReview FocusBody');
    if(!body) return;
  ...
  function renderDecision(stats, risks, misconceptions){
    ...
    if(gate && (active === 'all' || active === '*')){
      ...
      if(notSubmitted > 0) lines.push(`ยังไม่ส่ง ${notSubmitted} คน: ให้ตรวจรายชื่อใน Students to Review ก่อนปิดกิจกรรม`);
      if(need > 0) lines.push(`มีนักศึกษาที่ควรช่วยเพิ่ม ${need} คน: ดูตาราง Students to Review`);
  ...
  function buildCleanLink(mode){
    ...
    if(mode === 'teacher'){
      ...
      url.searchParams.set('v', '20260616-bysessionrepair351');
    }else{
      url.searchParams.set('v', '20260616-bysessionrepair351');
    }
  ...
  function exportReview FocusCsv(){
    const data = window.AIQuestTeacherConsole.lastData;
    const risks = data && data.data && data.data.risks ? data.data.risks : [];

    if(!risks.length){
      alert('ยังไม่มีข้อมูล Students to Review สำหรับ export');
      return;
    }
