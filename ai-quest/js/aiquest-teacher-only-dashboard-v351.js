...
(function(){
  'use strict';
  const VERSION='v3.5.1-teacher-bysession-smart-repair';
  ...
  async function loadTeacherData(){
    ...
    adapt(td,sd,hd);
    renderAll(td,sd,hd);
    console.log('[AIQuest Teacher v3.5.1] loaded', {students:state.students.length,attempts:state.attempts.length,adapter:state.adapterInfo,raw:td,summary:sd,health:hd});
  }
  ...
  function renderSheets(d,s,h){
    const version=first(h||{},['version','serverVersion'])||first(d||{},['version','serverVersion'])||'v3.5.1';
    const a=state.adapterInfo||{};
  ...
  function showDetail(s){
    ...
    $('detailBox').innerHTML=`
      ...
      <div class="card" style="margin-top:12px">
        ...
        <p class="muted">Source: ${esc((s.raw&&s.raw.bySessionSource)||s.bySessionSource||'-')} ${s.raw&&s.raw.unknownAttemptRows ? ' • Unknown rows: '+esc(s.raw.unknownAttemptRows) : ''}</p>
  ...
  function bind(){
    ...
    $('previewBtn').onclick=()=>window.open('./index.html?v=20260616-bysessionrepair351','_blank');
