/*
  CSAI2102 AI Quest
  PATCH v2.3.5 Teacher Console Layout
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v2.3.5-teacher-console-layout';
  ...
  function metricSkeleton(){
    const labels = ['Students','Submitted','Avg Score','Mastery','Need Support','Reflection OK'];
  ...
  function renderMetrics(stats){
    ...
    const metrics = [
      {label:'Students', value:total, cls:'', hint:'จำนวนนักศึกษาที่มี profile/attempt'},
      {label:'Submitted', value:submitted, cls:submitted >= total && total ? 'good' : 'warn', hint:'มี attempt แล้ว'},
      {label:'Avg Score', value:avg ? avg.toFixed(1) : '0.0', cls:avg >= 70 ? 'good' : avg >= 60 ? 'warn' : 'bad', hint:'คะแนนเฉลี่ย'},
      ...
      {label:'Reflection OK', value:reflPct + '%', cls:reflPct >= 90 ? 'good' : reflPct >= 70 ? 'warn' : 'bad', hint:'reflection ครบ'}
    ];
