/*
  CSAI2102 AI Quest
  PATCH v2.8.0 Teacher Console Session Switch
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v2.8.0-teacher-console-session4-route-cost';
  ...
  function renderPhaseAnalytics(rows){
    ...
    const ordered = ['AI vs Automation','Agent Foundation','PEAS Gate','Environment Gate','Rationality Gate','Final Attack','State Space','BFS/DFS Trace','Maze Path','Search Boss','Cost Concept','UCS Trace','Optimal Path','Frontier Cost','BFS vs UCS','Cost Boss'];
  ...
  function renderDecision(stats, risks, misconceptions){
    ...
    if(gate && (active === 'all' || active === '*')){
      ...
      const sessionName = active === 's1' ? 'Session 1' : active === 's2' ? 'Session 2' : active === 'b1' ? 'Boss B1' : active === 's3' ? 'Session 3 Search Maze' : active === 's4' ? 'Session 4 Route Cost Challenge' : 'Session นี้';
  ...
  function buildCleanLink(mode){
    ...
    if(mode === 'teacher'){
      ...
      url.searchParams.set('v', '20260612-s4ucs280');
    }else{
      url.searchParams.set('v', '20260612-s4ucs280');
    }
