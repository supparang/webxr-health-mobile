/*
  CSAI2102 AI Quest
  PATCH v2.9.0 Session Roadmap + Boss Gate
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v2.9.0-roadmap-session5-astar';
  ...
  function render(){
    ...
    anchor.innerHTML = `
      ...
      <div class="roadmapTop">
        ...
        <div class="roadmapMetric"><div class="num">${isPassed(st,'m5') ? 'B2' : isPassed(st,'m4') ? 'S5' : isPassed(st,'m3') ? 'S4' : 'S3'}</div><div class="label">${isPassed(st,'m5') ? 'ถัดไป: Search Arena Boss' : isPassed(st,'m4') ? 'ถัดไป: A* Rescue Mission' : isPassed(st,'m3') ? 'ถัดไป: Route Cost Challenge' : 'ถัดไป: Search Maze'}</div></div>
      </div>

      <div class="bossGateBox">
        <b>Progress Rule:</b> S1–S2 → B1 → S3 → S4 → S5 → B2
