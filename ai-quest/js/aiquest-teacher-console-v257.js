/*
  CSAI2102 AI Quest
  PATCH v2.5.7 Teacher Console Session Switch
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v2.5.7-teacher-console-session2';
  ...
  function ensureConsolePanel(){
      ...
      <div class="teacherGrid2">
        <div class="teacherBox">
          ...
          <div class="teacherSmallNote">
            โดยเฉพาะ S2: ดูว่าเด็กอ่อน Agent / PEAS / Environment / Boss ตรงไหน
          </div>
    ...
    if(sessionSelect){
      const cfg = getConfig();
      sessionSelect.value = cfg.activeSession || 's1';
  ...
  function renderHealth(resp, stats){
    ...
    box.innerHTML = `
      ...
      <span class="pillLike warn">Session: ${escapeHtml(cfg.activeSession || 's1')}</span>
  ...
  function buildCleanLink(mode){
    ...
    if(mode === 'teacher'){
      ...
      url.searchParams.set('v', '20260611-teacher257');
    }else{
      url.searchParams.set('v', '20260611-teacher257');
    }
