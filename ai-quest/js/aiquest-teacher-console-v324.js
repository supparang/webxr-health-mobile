/*
  CSAI2102 AI Quest
  PATCH v3.2.4 Teacher Console Session Switch
  ------------------------------------------------------------
...
(function(){
  ...
  function displayAccuracyV324(row){
    row = row || {};
    ...
    const score = Number(row.score || row.Score || row.best || row.Best || row.latestScore || 0);
    if(Number.isFinite(score) && score > 0) return Math.round(score);
    return 0;
  }


  const VERSION = 'v3.2.4-teacher-console-session5-astar';
  ...
  function buildCleanLink(mode){
    ...
    if(mode === 'teacher'){
      ...
      url.searchParams.set('v', '20260614-accuracy324');
    }else{
      url.searchParams.set('v', '20260614-accuracy324');
    }
