...
function normalizeAttemptPayload(payload={}){
 const p={...payload};
 const r1=clean(p.reflection1),r2=clean(p.reflection2),r3=clean(p.reflection3);
 if(r1.length>=3&&r2.length>=3&&r3.length>=3){
  const sessionId=sidOf(p.sessionId||p.missionId||'s1');
  const studentId=clean(p.studentId||'student');
  const caseId=clean(p.selectedCaseId||p.caseId||p.evidenceCaseId||p.selectedCase||'case');
  const base=clean(p.attemptId)||['attempt',studentId,sessionId,clean(p.clientTs||Date.now())].join('-');
  const refKey=hash([studentId,sessionId,caseId,r1,r2,r3].join('|'));
  p.attemptId=/-ref-[a-z0-9]+$/i.test(base)?base:base+'-ref-'+refKey;
  p.sessionId=sessionId;p.missionId=sessionId;
  p.reflectionSubmitted=true;p.reflectionStatus='submitted';p.submitStatus='reflection-submitted';
  p.clientTs=p.clientTs||new Date().toISOString();
 }
 return p;
}
const post=(kind,payload)=>{
 const normalized=kind==='attempt'?normalizeAttemptPayload(payload):{...payload};
 return fetch(U,{method:'POST',mode:'no-cors',cache:'no-store',keepalive:true,headers:{'Content-Type':'text/plain;charset=UTF-8'},body:JSON.stringify({action:'sync_v23',kind,payload:{...normalized,section:'101',clientTs:normalized?.clientTs||new Date().toISOString(),pageUrl:normalized?.pageUrl||location.href}})}).then(()=>({ok:true,queued:true,attemptId:normalized.attemptId||''}));
};
const stableKey=(action,params={})=>action+'|'+Object.keys(params).sort().map(k=>k+'='+String(params[k]??'')).join('&');
const keepCallbackTombstone=callback=>{try{window[callback]=()=>{}}catch(e){}setTimeout(()=>{try{delete window[callback]}catch(e){try{window[callback]=undefined}catch(_){}}},120000)};
const jsonpOnce=(action,params={},timeoutMs=30000)=>new Promise((resolve,reject)=>{
 ...
 url.searchParams.set('action',action);url.searchParams.set('callback',callback);url.searchParams.set('_cb',Date.now().toString());Object.keys(params).forEach(k=>url.searchParams.set(k,String(params[k]??'')));
 const finish=(ok,value)=>{if(settled)return;settled=true;clearTimeout(timer);keepCallbackTombstone(callback);try{script.remove()}catch(e){}ok?resolve(value||{}):reject(value instanceof Error?value:new Error(String(value||action+' failed')))};
 window[callback]=data=>finish(true,data);script.onerror=()=>finish(false,new Error(action+' unavailable'));timer=setTimeout(()=>finish(false,new Error(action+' timeout after '+timeoutMs+'ms')),timeoutMs);script.async=true;script.src=url.toString();document.head.appendChild(script);
});
const shared=(action,params,timeoutMs)=>{const key=stableKey(action,params);if(inflight.has(key))return inflight.get(key);const p=jsonpOnce(action,params,timeoutMs).finally(()=>inflight.delete(key));inflight.set(key,p);return p};
const jsonp=async(action,params={})=>{let lastError;for(let attempt=1;attempt<=2;attempt++){try{return await shared(action,{...params,_attempt:attempt},attempt===1?30000:45000)}catch(err){lastError=err;if(attempt<2)await new Promise(r=>setTimeout(r,1200))}}throw lastError||new Error(action+' failed')};
const jsonpFast=(action,params={})=>shared(action,params,12000);
...
console.log('[AIQuest] cloud logger v728 ready • idempotent Reflection follow-up attempt • safe JSONP');
})();