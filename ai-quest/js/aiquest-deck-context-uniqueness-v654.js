...
(()=>{'use strict';
  ...
  const labels=[
    'คัดกรองข้อมูลก่อนให้บริการ',
    'ตั้งเกณฑ์ความสำเร็จของภารกิจ',
    'ตรวจคุณภาพข้อมูลรับเข้า',
    'จัดลำดับการให้บริการผู้ใช้',
    'สื่อสารข้อจำกัดของระบบ',
    'บันทึกหลักฐานการตัดสินใจ',
    'ทดสอบกรณีข้อมูลผิดปกติ',
    'ส่งต่อกรณีความเสี่ยงสูง',
    'ทบทวนผลกระทบต่อผู้ใช้',
    'ปรับกฎเมื่อบริบทเปลี่ยน',
    'คุ้มครองสิทธิ์และความเป็นธรรม',
    'ตรวจสอบผลลัพธ์ก่อนแจ้งผู้ใช้',
    'กำกับการทำงานโดยผู้รับผิดชอบ',
    'จัดการข้อมูลไม่ครบหรือคลาดเคลื่อน',
    'เปรียบเทียบทางเลือกก่อนดำเนินการ',
    'รับมือข้อร้องเรียนของผู้ใช้',
    'ติดตามผลหลังระบบให้คำแนะนำ',
    'จำกัดขอบเขตการตัดสินใจอัตโนมัติ',
    'กำหนดทางเลือกปลอดภัยเมื่อไม่แน่ใจ',
    'ตรวจย้อนกลับผลลัพธ์สำคัญ',
    'ประเมินผลก่อนนำไปใช้จริง',
    'จัดการเป้าหมายที่อาจขัดกัน',
    'ควบคุมการเปลี่ยนแปลงของข้อมูล',
    'สรุปเหตุผลเพื่อให้ผู้ใช้ตรวจสอบได้'
  ];

  function history(){
    const all=read(key+'_'+personKey(),{});all[mid]=all[mid]||{rounds:[]};all[mid].rounds=Array.isArray(all[mid].rounds)?all[mid].rounds.slice(-WINDOW):[];return all;
  }
  function activeDeck(name){return read(name,null)?.deck||null;}
  function replaceContext(text,from,to){
    const source=String(text||'');return from&&source.includes(from)?source.split(from).join(to):source;
  }
  function baseContext(card){
    return tidy(card?.contextBase||card?.context||'สถานการณ์จำลอง');
  }
  ...
  function normalize(deck,track){
    ...
    deck.cards.forEach((card,index)=>{
      ...
      if(card.__contextUniqueV654){inside.add(String(card.contextSignature||baseContext(card)+' | '+card.scenarioFocus||''));return;}
      const base=baseContext(card);
      const picked=chooseLabel(card,index,round,previous,inside);
      const context=base+' — '+picked.label;
      card.contextBase=base;card.scenarioFocus=picked.label;card.contextSignature=picked.signature;card.context=context;
      card.prompt=replaceContext(card.prompt,old,context);
      card.__contextUniqueV654=true;inside.add(picked.signature);changed=true;
    });
    ...
    if(track&&changed){
      h[mid].rounds.push({deckId:String(deck.id||''),round,at:Date.now(),signatures:[...inside]});
      h[mid].rounds=h[mid].rounds.slice(-WINDOW);write(key+'_'+personKey(),h);
    }
    return deck;
  }
  function patchCore(){
    const api=window.AIQuestReplayFactoryV650;
    if(!api||api.__contextUniqueV654||typeof api.makeDeck!=='function')return false;
    api.__contextUniqueV654=true;
    const original=api.makeDeck.bind(api);
    api.makeDeck=function(){return normalize(original(),true)};
    normalize(activeDeck(coreActive),false);
    const snap=read(coreActive,null);if(snap?.deck)write(coreActive,snap);
    return true;
  }
  function patchS2(){
    const api=window.AIQuestS2AgentDeckV672;
    if(!api||api.__contextUniqueV654||typeof api.buildDeck!=='function')return false;
    api.__contextUniqueV654=true;
    const original=api.buildDeck.bind(api);
    api.buildDeck=function(){return normalize(original(),true)};
    normalize(activeDeck(s2Active),false);
    const snap=read(s2Active,null);if(snap?.deck)write(s2Active,snap);
    return true;
  }
