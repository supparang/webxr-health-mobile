/*
  CSAI2102 AI Quest
  PATCH v2.5.3 Session Roadmap + Boss Gate
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v2.5.3-roadmap-boss-gate';
  ...
  const ROADMAP = [
    ...
    {id:'b1', type:'boss', no:'B1', title:'Rookie AI Boss', topic:'AI Overview + Agent', unlock:'ผ่าน S1 และ S2 อย่างน้อย 1 ดาว'},
  ...
  function injectStyle(){
    ...
    style.textContent = `
      ...
      .roadmapCard{border:1px solid var(--line);border-radius:16px;padding:12px;background:rgba(255,255,255,.045);min-height:126px;position:relative;overflow:hidden}
  ...
  function cardHTML(st, item){
    ...
    return `
      <div class="roadmapCard ${item.type} ${status.cls}">
  ...
  function render(){
    ...
    if(!anchor){
      ...
      if(isTeacherMode()){
        ...
        const firstBox = consolePanel && consolePanel.querySelector('.teacherGrid2');
        if(firstBox) consolePanel.insertBefore(anchor, firstBox);
        else if(consolePanel) consolePanel.appendChild(anchor);
        else return;
      }else{
    ...
    anchor.innerHTML = `
      ...
      <div class="roadmapActions">
        <button class="btn good" id="roadmapStartS1">เริ่ม Session 1</button>
        <button class="btn good" id="roadmapStartS2">เริ่ม Session 2</button>
        <button class="btn secondary" id="roadmapBossInfo">ดูเกณฑ์ Boss</button>
      </div>
    ...
    const s1 = $('#roadmapStartS1');
    const s2 = $('#roadmapStartS2');
    const bi = $('#roadmapBossInfo');

    if(s1) s1.onclick = () => {
      if(typeof startMission === 'function') startMission('m1');
      else document.getElementById('studentStartGraded')?.click();
    };

    if(s2) s2.onclick = () => {
      const top = document.getElementById('btnSession2Top');
      if(top) top.click();
      else if(typeof startMission === 'function') startMission('m2');
    };

    if(bi) bi.onclick = () => {
      const message = b1Open
        ? 'Boss Gate B1 เปิดแล้ว: ผ่าน S1 และ S2 แล้ว'
        : 'Boss Gate B1 จะเปิดเมื่อผ่าน Session 1 และ Session 2 อย่างน้อย 1 ดาว';
