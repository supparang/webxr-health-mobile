/* AI Quest Thai-first labels v4.1.12 */
(()=>{
'use strict';
const V='v4.1.12-thai-first-core';
const L={
...
function fix(x){if(!x||typeof x!=='object')return x;if(L[x.phase])x.phase=L[x.phase];if(x.prompt)x.prompt=String(x.prompt).replace('Cost Boss Claim:','คำกล่าวของบอสต้นทุน:').replace('Boss Claim:','คำกล่าวของบอส:');return x}
...
function wrap(n){const o=window[n];if(typeof o!=='function'||o.__thai412)return;const f=d=>round(o(d));f.__thai412=true;window[n]=f}
function run(){['buildMission1Round','buildSession2Round','buildBoss1Round','buildSession3Round','buildSession4Round','buildSession5Round','buildBoss2Round'].forEach(wrap);window.AIQuestSearchKnowledgeGameplay={version:V,mode:'thai-first'}}
