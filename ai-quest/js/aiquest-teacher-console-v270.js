/*
  CSAI2102 AI Quest
  PATCH v2.7.0 Teacher Console Session Switch
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v2.7.0-teacher-console-session3-search-maze';
      ...
      <div class="teacherConsoleHeader">
        ...
        <div class="teacherToolBar">
          <select class="select" id="teacherSessionSelect" title="เลือก session ที่ต้องการดูผล">
            <option value="all">Class Gate: S1+S2+B1</option>
            <option value="s1">Session 1</option>
            <option value="s2">Session 2</option>
            <option value="b1">Boss B1</option>
          </select>
  ...
  function renderPhaseAnalytics(rows){
    ...
    const ordered = ['AI vs Automation','Agent Foundation','PEAS Gate','Environment Gate','Rationality Gate','Final Attack','State Space','BFS/DFS Trace','Maze Path','Search Boss'];
  ...
  function renderMasteryGate(gate){
    ...
    if(!gate){
      box.innerHTML = `
        <div class="masteryGateHeader">
          <div>
            <div class="masteryGateTitle">Class Mastery Gate</div>
    ...
    const ready = Number(gate.readyForS3 || 0);
    const readyPct = Number(gate.readyPct || 0);
    const badgeCls = readyPct >= 70 ? 'good' : readyPct >= 50 ? 'warn' : 'bad';
    const statusText = readyPct >= 70 ? 'พร้อมเปิด S3 (ผ่านขั้นต่ำ)' : readyPct >= 50 ? 'เปิด S3 ได้แบบมี Remedial' : 'ควรทบทวนก่อนเปิด S3';
    ...
    box.innerHTML = `
      <div class="masteryGateHeader">
        <div>
          <div class="masteryGateTitle">Class Mastery Gate: S1 → S2 → B1</div>
          <div class="teacherSmallNote">
            ใช้ตัดสินใจว่าห้องพร้อมไป S3 Search Maze หรือควร remedial ก่อน
          </div>
        </div>
        <span class="readinessBadge ${badgeCls}">${statusText} · ${readyPct}%</span>
      </div>

      <div class="gateGrid">
        <div class="gateCard"><div class="num">${total}</div><div class="label">Students</div></div>
        <div class="gateCard"><div class="num">${ready}</div><div class="label">Ready for S3<br>ผ่านขั้นต่ำครบ S1+S2+B1</div></div>
        ...
        <div class="gateCard"><div class="num">${Number(gate.challengeReady || 0)}</div><div class="label">Challenge Ready<br>Mastery/คะแนนสูงครบ</div></div>
      </div>

      <div class="gateExplain">
        <div class="gateExplainBox">
          <b>Ready for S3</b><br>
          ผ่านขั้นต่ำครบ S1, S2 และ B1 แล้ว สามารถเริ่มเรียน Search Maze ได้
        </div>
        <div class="gateExplainBox">
          <b>Challenge Ready</b><br>
          พร้อมเรียนเร็ว/ยากขึ้น เพราะคะแนนสูงหรือ mastery ตามเกณฑ์ครบ
        </div>
      </div>
  ...
  function renderDecision(stats, risks, misconceptions){
    ...
    if(gate && (active === 'all' || active === '*')){
      ...
      if(readyPct >= 70){
        lines.push(`เปิด S3 Search Maze ได้: ห้องผ่านขั้นต่ำครบ S1→S2→B1 แล้ว ${readyPct}%`);
        lines.push('ก่อนเริ่ม S3 ให้ใช้ 10 นาทีแรกทบทวน misconception เด่นจาก S1/S2/B1');
      }else if(readyPct >= 50){
        lines.push(`เปิด S3 ได้แบบมี remedial คู่ขนาน: readiness ${readyPct}%`);
        lines.push('แบ่งผู้เรียนเป็น 2 กลุ่ม: กลุ่มพร้อมเริ่ม S3 และกลุ่ม remedial S2/B1');
      }else{
        lines.push(`ยังไม่ควรเปิด S3 ทันที: readiness ${readyPct}% ต่ำกว่าเกณฑ์`);
      ...
      if(Number(gate.challengeReady || 0) === 0 && Number(gate.readyForS3 || 0) > 0){
        lines.push('หมายเหตุ: Ready for S3 คือผ่านขั้นต่ำ ส่วน Challenge Ready ต้อง mastery/คะแนนสูงครบ จึงอาจเป็น 0 ได้');
      }
    }else if(submitted === 0){
      lines.push('ยังไม่มีข้อมูลส่งผลจากนักศึกษา ให้เริ่มจากการทดสอบ Save Result 1 คนก่อน');
    }else{
      const sessionName = active === 's1' ? 'Session 1' : active === 's2' ? 'Session 2' : active === 'b1' ? 'Boss B1' : 'Session นี้';
  ...
  function buildCleanLink(mode){
    ...
    if(mode === 'teacher'){
      ...
      url.searchParams.set('v', '20260611-s3maze270');
    }else{
      url.searchParams.set('v', '20260611-s3maze270');
    }
  ...
  async function copyTeachingRecommendation(){
    ...
    if(gate){
      ...
      lines.push(`Need S1: ${Number(gate.needS1 || 0)}, Need S2: ${Number(gate.needS2 || 0)}, Need B1: ${Number(gate.needB1 || 0)}, Challenge Ready: ${Number(gate.challengeReady || 0)}`);
