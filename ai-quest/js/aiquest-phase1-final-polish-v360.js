...
(function(){
  ...
  const VERSION = 'v4.1.4-s6-direct-sync-loader';
  ...
  function loadS6Tools(){
    ...
    loadScript('aiquestS6ResultSyncV414','./js/aiquest-s6-result-sync-v414.js?v=20260701-s6sync414');
  }
  function normalizeRiskLabels(){
    if(!isTeacher()) return;
    Array.from(document.querySelectorAll('span, div, td, p, li')).forEach(el=>{
      if(/^Misconception:\s*automation$/i.test(text(el))){ el.textContent='Focus: automation'; el.title='จุดทบทวนเล็กน้อย ไม่ใช่ความเสี่ยงรุนแรง'; }
    });
  }
  ...
  if(!window.__AIQUEST_PHASE1_FINAL_OBSERVER_V328){
    window.__AIQUEST_PHASE1_FINAL_OBSERVER_V328=new MutationObserver(()=>{ clearTimeout(window.__AIQUEST_PHASE1_FINAL_TIMER_V328); window.__AIQUEST_PHASE1_FINAL_TIMER_V328=setTimeout(refresh,200); });
