/*
  CSAI2102 AI Quest
  PATCH v2.7.2 Teacher Console Session Switch
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v2.7.2-teacher-console-session3-option';
  ...
  function renderDecision(stats, risks, misconceptions){
    ...
    if(gate && (active === 'all' || active === '*')){
      ...
      const sessionName = active === 's1' ? 'Session 1' : active === 's2' ? 'Session 2' : active === 'b1' ? 'Boss B1' : active === 's3' ? 'Session 3 Search Maze' : 'Session นี้';
  ...
  function buildCleanLink(mode){
    ...
    if(mode === 'teacher'){
      ...
      url.searchParams.set('v', '20260612-s3status272');
    }else{
      url.searchParams.set('v', '20260612-s3status272');
    }
