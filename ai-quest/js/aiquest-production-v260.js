/*
  CSAI2102 AI Quest
  PATCH v2.6.0 Classroom Production Release
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v2.6.0-classroom-production-session2';
  ...
  function ensureTeacherPanel(){
    ...
    panel.innerHTML = `
      ...
      <div class="productionSmall">
        ใช้ก่อนเริ่มคาบจริง: ตรวจว่า section เป็น 101, Apps Script เป็น v2.6.0, ไม่รวม test data และ Teacher Console อ่าน Google Sheets ได้
      </div>
  ...
  function renderChecklists(){
    ...
    if(teacherBox){
      ...
      const versionOk = String((tc && tc.version) || '').includes('v2.6.0');
  ...
  function copyStudentInstructions(){
    const link = location.origin + location.pathname.replace(/index\.html.*$/, 'index.html?v=20260611-report260');
