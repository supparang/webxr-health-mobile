/*
  CSAI2102 AI Quest
  PATCH v2.6.0 Teacher Console Session Switch
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v2.6.0-teacher-actionable-report';
  ...
  function renderMasteryGate(gate){
    ...
    box.innerHTML = `
      ...
      <div class="gateGrid">
        ...
        <div class="gateCard"><div class="num">${ready}</div><div class="label">Ready for S3</div></div>
        ...
        <div class="gateCard"><div class="num">${Number(gate.challengeReady || 0)}</div><div class="label">Challenge Ready</div></div>
      </div>
  ...
  function renderDecision(stats, risks, misconceptions){
    ...
    if(gate && String((getConfig().activeSession || 'all')) === 'all'){
      const readyPct = Number(gate.readyPct || 0);
      if(readyPct >= 70){
        lines.push(`ห้องพร้อมเปิด S3 Search Maze แล้ว (${readyPct}% ready)`);
      }else if(readyPct >= 50){
        lines.push(`เปิด S3 ได้ แต่ควรทำ remedial คู่กัน (${readyPct}% ready)`);
      }else{
        lines.push(`ยังไม่ควรเปิด S3 ทันที ควร remedial S1/S2/B1 ก่อน (${readyPct}% ready)`);
      }
      ...
      (gate.recommendations || []).slice(0,3).forEach(r => lines.push(r));
    }else if(submitted === 0){
      lines.push('ยังไม่มีข้อมูลส่งผลจากนักศึกษา ให้เริ่มจากการทดสอบ Save Result 1 คนก่อน');
    }else{
      if(avg < 60){
        lines.push('คะแนนเฉลี่ยต่ำกว่า 60: ควรทบทวน concept หลักก่อนขึ้น session ถัดไป');
      }else if(avg < 70){
        lines.push('คะแนนเฉลี่ยอยู่ระดับผ่านขั้นต่ำ: ควรให้ Practice/Remedial เพิ่มก่อน Challenge');
      }else{
        lines.push('คะแนนเฉลี่ยดี: สามารถต่อยอดไปด่านถัดไปได้');
      }
  ...
  function buildCleanLink(mode){
    ...
    if(mode === 'teacher'){
      ...
      url.searchParams.set('v', '20260611-report260');
    }else{
      url.searchParams.set('v', '20260611-report260');
    }
