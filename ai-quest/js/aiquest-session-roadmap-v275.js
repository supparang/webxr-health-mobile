/*
  CSAI2102 AI Quest
  PATCH v2.7.5 Session Roadmap + Boss Gate
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v2.7.5-roadmap-before-s4-consistency';
  ...
  function isClickable(st, item){
    ...
    if(item.id === 'm1' || item.id === 'm2' || item.id === 'm3') return sessionUnlocked(st, item.id) || item.available;
    return false;
  }
  ...
  function render(){
      ...
      : !isPassed(st,'m2')
        ...
        : b1Open && !isPassed(st,'b1')
          ...
          : isPassed(st,'b1') && !isPassed(st,'m3')
            ...
            : isPassed(st,'m3')
              ? 'ผ่าน S3 แล้ว: S4 Route Cost Challenge จะเปิดใน patch ถัดไป'
    ...
    anchor.innerHTML = `
      ...
      <div class="roadmapTop">
        ...
        <div class="roadmapMetric"><div class="num">${isPassed(st,'m3') ? 'S4' : 'S3'}</div><div class="label">${isPassed(st,'m3') ? 'ถัดไป: Route Cost Challenge' : 'ถัดไป: Search Maze'}</div></div>
      </div>

      <div class="bossGateBox">
        <b>Progress Rule:</b> S1–S2 → B1 → S3 Search Maze → S4 Route Cost Challenge
