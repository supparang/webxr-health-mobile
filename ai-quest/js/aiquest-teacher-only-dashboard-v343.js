...
(function(){
  'use strict';
  const VERSION='v3.4.3-teacher-individual-detail-fix';
  ...
  function normStudent(r){
    ...
    return {
      ...
      name:first(r,['name','Name','displayName','studentName'])||DEFAULT_STUDENT_NAME,
      section:first(r,['section','Section','sec'])||DEFAULT_SECTION,
      attempts:num(first(r,['attempts','Attempts','attemptCount','count','totalAttempts']),0),
      best:num(first(r,['best','Best','bestScore','maxScore']),0),
      latest:num(first(r,['latest','Latest','latestScore','lastScore','score']),0),
      accuracy:first(r,['accuracy','Accuracy','accuracyPct','accuracyPercent']),
      reflection:first(r,['reflection','Reflection','reflectionStatus','hasReflection']),
      focus:focus?String(focus).replace(/Misconception:\s*/i,'Focus: '):'',
      raw:r
    };
  }
  ...
  async function loadTeacherData(){
    ...
    adapt(td,sd,hd);
    renderAll(td,sd,hd);
    console.log('[AIQuest Teacher v3.4.3] loaded', {students:state.students.length,attempts:state.attempts.length,adapter:state.adapterInfo,raw:td,summary:sd,health:hd});
  }
  ...
  function renderStudents(){
    ...
    set('studentsBox',`<table><thead><tr><th>Student</th><th>Section</th><th>Attempts</th><th>Best</th><th>Latest</th><th>Accuracy</th><th>Reflection</th><th>Review Focus</th><th>Detail</th></tr></thead><tbody>${rows.map((s,i)=>`<tr><td><b>${esc(s.studentId)}</b><br><span class="muted">${esc(s.name)}</span></td><td>${esc(s.section)}</td><td>${esc(s.attempts)}</td><td>${esc(s.best||'-')}</td><td>${esc(s.latest||'-')}</td><td>${esc(accuracy(s))}</td><td>${s.reflection?pill('ครบ','good'):pill('ยังไม่ครบ','warn')}</td><td>${s.focus?pill(s.focus,'blue'):pill('ปกติ','good')}</td><td><button class="btn" data-detail="${i}">View</button></td></tr>`).join('')}</tbody></table>`);
  ...
  function renderReview(){
    ...
    set('reviewBox', rows.length?`<table><thead><tr><th>Student</th><th>Latest</th><th>Accuracy</th><th>Review Focus</th></tr></thead><tbody>${rows.map(s=>`<tr><td>${esc(s.studentId)}</td><td>${esc(s.latest||'-')}</td><td>${esc(accuracy(s))}</td><td>${esc(s.focus||'score ต่ำ')}</td></tr>`).join('')}</tbody></table>`:'<div class="loading">ยังไม่มีนักศึกษาที่ต้องทบทวนเป็นพิเศษ</div>');
  }
  ...
  function renderSheets(d,s,h){
    const version=first(h||{},['version','serverVersion'])||first(d||{},['version','serverVersion'])||'v3.4.3';
    const a=state.adapterInfo||{};
  ...
  function showDetail(s){
    if(!s)return;
    $('detailBox').innerHTML=`<p>${pill('Student','blue')} <b>${esc(s.studentId)}</b> ${esc(s.name||'')}</p><div class="grid cols3"><div class="metric"><span class="muted">Attempts</span><b>${esc(s.attempts||'-')}</b></div><div class="metric"><span class="muted">Best</span><b>${esc(s.best||'-')}</b></div><div class="metric"><span class="muted">Latest</span><b>${esc(s.latest||'-')}</b></div></div><div class="grid cols3" style="margin-top:10px"><div class="metric"><span class="muted">Accuracy</span><b>${esc(accuracy(s))}</b></div><div class="metric"><span class="muted">Reflection</span><b>${esc(s.reflection?'ครบ':'-')}</b></div><div class="metric"><span class="muted">Review Focus</span><b>${esc(s.focus||'ปกติ')}</b></div></div><h3>Raw row</h3><pre style="white-space:pre-wrap;background:#0f172a;padding:12px;border-radius:12px;overflow:auto">${esc(JSON.stringify(s.raw,null,2))}</pre>`;
  ...
  function bind(){
    ...
    $('previewBtn').onclick=()=>window.open('./index.html?v=20260614-detail343','_blank');
  ...
  window.AIQUEST_TEACHER_ONLY_DASHBOARD={version:VERSION,loadTeacherData,renderAll,showRaw,loadStudent12,showDetail,state,adapt,collectArrays};
