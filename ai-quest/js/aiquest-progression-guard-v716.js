/* CSAI2102 AI Quest — Progression Guard v7.1.8
   ...
   Entry flow fails closed and rechecks Sheet immediately before start.
*/
(()=>{'use strict';
if(window.AIQuestProgressionGuardV718)return;
...
function reflectionDone(a){
 if(!a)return false;
 if(a.reflectionSubmitted===true||String(a.reflectionStatus||'').toLowerCase()==='submitted'||a.completed===true)return true;
 return hasText(a.reflection1)&&hasText(a.reflection2)&&hasText(a.reflection3);
}
function passDone(a){
 if(!a)return false;
 const s=String(a.gateStatus||a.status||'').toLowerCase();
 return a.passed===true||a.mastered===true||s==='passed'||s==='mastered'||s==='completed';
}
function officialDone(id,r){
 ...
 return (passDone(p)||attempts.some(passDone))&&(reflectionDone(p)||attempts.some(reflectionDone));
}
function renameSessionLabels(){
 const id=missionId(),title=$('title');
 if(title&&id){let t=title.textContent.replace(/^W\d+\s*•\s*/i,'').replace(/^S\d+\s*•\s*/i,'').trim();title.textContent=id.toUpperCase()+' • '+t;}
 document.querySelectorAll('h1,h2,h3,.status,.muted,.notice,#sub').forEach(el=>{if(el.childElementCount===0)el.textContent=el.textContent.replace(/\bW(\d{1,2})\s*•\s*S\1\b/gi,'S$1').replace(/\bW(\d{1,2})\b/gi,'S$1')});
}
...
function blockMissionEntry(){
 ...
 const install=()=>{
  ...
  const prev=ORDER[idx-1];let checking=false,lastStudent='';
  const lock=(text)=>{start.disabled=true;start.dataset.progressionAllowed='0';start.textContent=text||('🔒 ต้องผ่านและส่ง Reflection '+prev.toUpperCase()+' ก่อน');if(note){note.className='notice bad';note.textContent='ยังเข้า '+id.toUpperCase()+' ไม่ได้: ต้องผ่านและส่ง Reflection '+prev.toUpperCase()+' ครบก่อน ระบบตรวจจาก Google Sheet เท่านั้น';}};
  const open=()=>{start.disabled=false;start.dataset.progressionAllowed='1';start.textContent='▶ เริ่ม Challenge Deck';if(note&&/ยังเข้า|ตรวจสิทธิ์|Reflection/.test(note.textContent||'')){note.className='notice good';note.textContent='✓ ตรวจสิทธิ์จาก Google Sheet แล้ว สามารถเริ่ม '+id.toUpperCase()+' ได้';}};
  const verify=async(force=false)=>{
   ...
   checking=true;lastStudent=studentId;start.disabled=true;start.dataset.progressionAllowed='0';start.textContent='กำลังตรวจสิทธิ์จาก Sheet…';
   try{const r=await readProgress(studentId);if(!officialDone(prev,r)){lock();return false;}open();return true;}
   catch(e){start.disabled=true;start.dataset.progressionAllowed='0';start.textContent='ตรวจสิทธิ์ไม่สำเร็จ';if(note){note.className='notice bad';note.textContent='ไม่สามารถตรวจ Progress จาก Sheet ได้ จึงยังไม่เปิดด่าน';}return false;}
   finally{checking=false;}
  };
  start.disabled=true;start.dataset.progressionAllowed='0';start.textContent='กำลังรอตรวจสิทธิ์จาก Sheet…';
  sid.addEventListener('input',()=>{start.disabled=true;start.dataset.progressionAllowed='0';lastStudent='';start.textContent='กำลังรอตรวจสิทธิ์…'});
  sid.addEventListener('change',()=>verify(true));
  const save=$('saveProfile');if(save)save.addEventListener('click',()=>setTimeout(()=>verify(true),500));
  if(note){const ob=new MutationObserver(()=>{if(String(sid.value||'').trim())setTimeout(()=>verify(true),120)});ob.observe(note,{childList:true,subtree:true,characterData:true});}
  start.addEventListener('click',async e=>{
   if(start.dataset.progressionAllowed==='1')return;
   e.preventDefault();e.stopImmediatePropagation();
   const ok=await verify(true);if(!ok)return;
   start.dataset.progressionAllowed='1';start.disabled=false;start.click();
  },true);
  let bootTries=0;const bootCheck=()=>{if(String(sid.value||'').trim())verify(true);else if(++bootTries<40)setTimeout(bootCheck,250)};setTimeout(bootCheck,400);
 };
 install();
}
async function enforceMap(){
 const sid=$('sid'),load=$('load'),grid=$('grid'),note=$('note');if(!sid||!load||!grid)return;
 const apply=async()=>{const studentId=String(sid.value||'').trim();if(!studentId)return;try{const r=await readProgress(studentId);document.querySelectorAll('a[data-mission]').forEach(a=>{const id=norm(a.dataset.mission),idx=ORDER.indexOf(id);if(idx<=0)return;const prev=ORDER[idx-1],allowed=officialDone(prev,r),card=a.closest('.mission');if(!allowed){a.removeAttribute('href');a.setAttribute('aria-disabled','true');a.textContent='🔒 ต้องผ่านและส่ง Reflection '+prev.toUpperCase();a.classList.remove('good','warn');if(card){card.classList.add('locked');const st=card.querySelector('.status');if(st)st.textContent='ล็อก';const m=card.querySelector('.muted');if(m)m.textContent='ต้องผ่านและส่ง Reflection '+prev.toUpperCase()+' ก่อน';}}});}catch(e){if(note){note.className='notice bad';note.textContent='ตรวจ Reflection progression จาก Sheet ไม่สำเร็จ จึงไม่เปิดด่านถัดไป';}}};
 load.addEventListener('click',()=>setTimeout(apply,1000));const ob=new MutationObserver(()=>{clearTimeout(ob._t);ob._t=setTimeout(apply,180)});ob.observe(grid,{childList:true,subtree:true});setTimeout(apply,900);
}
...
function enforceResultFlow(){
 ...
 save.addEventListener('click',e=>{if(resultScore()<60||!reflectionFieldsDone()||!caseChosen()){e.preventDefault();e.stopImmediatePropagation();update();}else setTimeout(update,300)},true);
 ['r1','r2','r3'].forEach(id=>{const el=$(id);if(el)el.addEventListener('input',update)});document.querySelectorAll('select').forEach(el=>el.addEventListener('change',update));const ob=new MutationObserver(()=>{clearTimeout(ob._t);ob._t=setTimeout(update,80)});ob.observe(result,{childList:true,subtree:true,attributes:true,attributeFilter:['class','style','disabled']});update();setInterval(update,700);};install();
}
function boot(){renameSessionLabels();blockMissionEntry();enforceMap();enforceResultFlow();}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.AIQuestProgressionGuardV716=window.AIQuestProgressionGuardV717=window.AIQuestProgressionGuardV718={version:VERSION,officialDone,reflectionDone};
console.log('[AIQuest] progression guard v718 active • fail-closed Sheet check before start');
})();