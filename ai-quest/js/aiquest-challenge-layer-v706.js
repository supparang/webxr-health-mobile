/* CSAI2102 AI Quest — Challenge Replay Layer v7.1.2.1
   Student Anti-Guess Polish v712.1
   - fixes new bias where the shortest option becomes the correct answer
   - balances correct/wrong option length without changing v711 flow or Sheet schema
   - keeps plausible session-specific distractors, no-repeat window, balanced slots, replay audit
*/
(()=>{'use strict';
  if(window.AIQuestChallengeLayerV7121)return;
  const VERSION='v7.1.2.1';
  ...
  const correctBank={
    ...
    s11:['เลือก metric ตาม FP/FN impact','ใช้ confusion matrix ไม่ใช่ accuracy เดียว','ปรับ threshold พร้อมเหตุผล','ดู class imbalance ก่อน deploy','เทียบ precision-recall ตาม risk'],
  ...
  const decoyBank={
    ...
    s11:['ใช้ accuracy เพราะอธิบายง่ายที่สุด','เพิ่ม recall สูงสุดโดยไม่ดู FP','ตั้ง threshold จาก default library','รายงาน metric เดียวแทน confusion matrix','มองข้าม imbalance เพราะจำนวนรวมสูง','ตรวจ error เฉพาะ class หลัก'],
  ...
  const keyRecent=sid=>'CSAI2102_RECENT_FINGERPRINTS_V7121_'+sid;
  const keyWeak=sid=>'CSAI2102_WEAK_CONCEPTS_V7121_'+sid;
  ...
  function makeDistractors(sid,card,i,trap,correct,usedWrong){const near=['ใช้ '+card.concept+' ถ้า confidence สูงพอ','เชื่อผล '+card.policy+' ก่อนแล้วค่อย audit','ตรวจ '+trap+' เฉพาะเมื่อมี complaint','ถือว่า demo ผ่านจึงใช้ได้','บันทึกเหตุผลแบบสั้นโดยไม่แสดง evidence','ให้ระบบ action ก่อนแล้วค่อย review'];const pool=(decoyBank[sid]||[]).concat(near).concat(decoyBank.all);const out=[];let start=hash('wrong|'+sid+'|'+card.fingerprint+'|'+i)%pool.length;for(let k=0;k<pool.length&&out.length<3;k++){let v=clean(pool[(start+k)%pool.length]);if(v&&v!==correct&&!out.includes(v)&&!usedWrong.has(v)){out.push(v);usedWrong.add(v);}}
