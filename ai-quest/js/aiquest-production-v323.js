/*
  CSAI2102 AI Quest
  PATCH v3.2.3 Classroom Production Release
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v3.2.3-classroom-production-foundation-qc-lock';
  ...
  function ensureTeacherPanel(){
    ...
    panel.innerHTML = `
      ...
      <div class="productionSmall">
        ใช้ก่อนเริ่มคาบจริง: ตรวจว่า section เป็น 101, Apps Script เป็น v3.2.3, ไม่รวม test data และ Teacher Console อ่าน Google Sheets ได้
      </div>
  ...
  function renderChecklists(){
    ...
    if(teacherBox){
      ...
      const versionOk = String((tc && tc.version) || '').includes('v3.2.3');
      ...
      if(qcBox){
        if(window.AIQUEST_FOUNDATION_AUDIT_V276){
          ...
          qcBox.innerHTML = '<b>Foundation QC Lock:</b> ยังไม่พบ aiquest-foundation-quality-audit-v323.js';
        }
      }
    }
  }
  ...
  function copyStudentInstructions(){
    const link = location.origin + location.pathname.replace(/index\.html.*$/, 'index.html?v=20260614-label323');
