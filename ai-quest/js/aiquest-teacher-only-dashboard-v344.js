...
(function(){
  'use strict';
  const VERSION='v3.4.4-teacher-detail-field-mapping';
  ...
  async function loadTeacherData(){
    ...
    adapt(td,sd,hd);
    renderAll(td,sd,hd);
    console.log('[AIQuest Teacher v3.4.4] loaded', {students:state.students.length,attempts:state.attempts.length,adapter:state.adapterInfo,raw:td,summary:sd,health:hd});
  }
  ...
  function renderSheets(d,s,h){
    const version=first(h||{},['version','serverVersion'])||first(d||{},['version','serverVersion'])||'v3.4.4';
    const a=state.adapterInfo||{};
  ...
  function showDetail(s){
    if(!s)return;
    const latestRefl=s.raw && s.raw.latestReflection ? `<h3>Latest Reflection</h3><pre style="white-space:pre-wrap;background:#0f172a;padding:12px;border-radius:12px;overflow:auto">${esc(JSON.stringify(s.raw.latestReflection,null,2))}</pre>` : '';
    $('detailBox').innerHTML=`<p>${pill('Student','blue')} <b>${esc(s.studentId)}</b> ${esc(s.name||'')}</p><div class="grid cols3"><div class="metric"><span class="muted">Attempts</span><b>${esc(s.attempts||'-')}</b></div><div class="metric"><span class="muted">Best</span><b>${esc(s.best||'-')}</b></div><div class="metric"><span class="muted">Latest</span><b>${esc(s.latest||'-')}</b></div></div><div class="grid cols3" style="margin-top:10px"><div class="metric"><span class="muted">Accuracy</span><b>${esc(accuracyValue(s))}</b></div><div class="metric"><span class="muted">Reflection</span><b>${esc(s.reflection?'ครบ':'-')}</b></div><div class="metric"><span class="muted">Review Focus</span><b>${esc(s.focus||'ปกติ')}</b></div></div><div style="margin-top:10px">${s.mastered?pill('Mastered','good'):''} ${s.helpUsed?pill('Help '+s.helpUsed,'blue'):''}</div>${latestRefl}<h3>Raw row</h3><pre style="white-space:pre-wrap;background:#0f172a;padding:12px;border-radius:12px;overflow:auto">${esc(JSON.stringify(s.raw,null,2))}</pre>`;
  ...
  function bind(){
    ...
    $('previewBtn').onclick=()=>window.open('./index.html?v=20260614-detailmap344','_blank');
