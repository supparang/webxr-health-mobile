...
(()=>{'use strict';
  ...
  function aliases(id){return ({s1:['s1','m1'],s2:['s2','m2'],s3:['s3','m3'],b1:['b1','boss1'],s4:['s4','m4'],s5:['s5','m5'],s6:['s6','m6'],b2:['b2','boss2'],s7:['s7','m7'],s8:['s8','m8'],s9:['s9','m9'],b3:['b3','boss3']})[id]||[id]}
  function rawPass(store,id){return aliases(id).some(k=>!!((store.completed&&store.completed[k])||(store.mastered&&store.mastered[k])||number(store.stars&&store.stars[k])>0||number(store.bestScore&&store.bestScore[k])>=60||number(store.best&&store.best[k])>=60))}
  function passed(id){const core=read(CORE,{}),legacy=read(LEGACY,{}),phase2=read(PHASE2,{});return ['s7','s8','s9','b3'].includes(id)?rawPass(core,id)||rawPass(phase2,id):rawPass(core,id)||rawPass(legacy,id)}
  function previous(id){const order=['s1','s2','s3','b1','s4','s5','s6','b2','s7','s8','s9','b3'];const i=order.indexOf(id);return i>0?order[i-1]:null}
  function unlocked(){const before=previous(MID);return !before||passed(before)}
  function profile(){try{return window.AIQuestStorage?.getProfile?.()||read('CSAI2102_AIQUEST_PROFILE_V421',{})}catch(e){return {}}}
  function profileReady(){const p=profile();return !!(String(p.studentId||'').trim()&&String(p.studentName||p.name||'').trim()&&String(p.section||'101')==='101')}
  function show(id){document.querySelectorAll('.screen').forEach(x=>x.classList.remove('on'));const node=$(id);if(node)node.classList.add('on');window.scrollTo({top:0,behavior:'smooth'})}
  function toast(text){const node=$('toast');if(!node)return;node.textContent=text;node.classList.add('show');clearTimeout(toast.timer);toast.timer=setTimeout(()=>node.classList.remove('show'),2300)}
  function bank(){return window.AIQuestReplayFactoryV650?.getBank?.()||null}
