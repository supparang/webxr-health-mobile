/*
  CSAI2102 AI Quest
  PATCH v2.6.1 Teacher Console Session Switch
  ------------------------------------------------------------
...
(function(){
  ...
  const VERSION = 'v2.6.1-teacher-console-polish';
  ...
  function renderMasteryGate(gate){
    ...
    const statusText = readyPct >= 70 ? 'พร้อมเปิด S3' : readyPct >= 50 ? 'เปิด S3 ได้แบบมี Remedial' : 'ควรทบทวนก่อนเปิด S3';
  ...
  function buildCleanLink(mode){
    ...
    if(mode === 'teacher'){
      ...
      url.searchParams.set('v', '20260611-polish261');
    }else{
      url.searchParams.set('v', '20260611-polish261');
    }
