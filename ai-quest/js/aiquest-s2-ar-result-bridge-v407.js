/* =========================================================
   ...
   File: /ai-quest/js/aiquest-s2-ar-result-bridge-v407.js

   Guarantees:
   - S2 AR is supplementary and never changes S2 score or gate.
   - One completed AR run produces at most one event per browser result signature.
   - V401 and earlier local result keys are recoverable after reload.
========================================================= */
(() => {
  'use strict';

  const VERSION = 'v4.0.7-s2-ar-idempotent-event-sync';
  const RESULT_KEYS = [
  ...
  const RECEIPT_KEY = 'AIQUEST_S2_AR_EVENT_SYNC_V407';
  const LEGACY_RECEIPT_KEYS = [
    'AIQUEST_S2_AR_EVENT_SYNC_V406',
    'AIQUEST_S2_AR_EVENT_SYNC_V405',
    'AIQUEST_S2_AR_EVENT_SYNC_V403'
  ];
  const LOCK_KEY = 'AIQUEST_S2_AR_EVENT_LOCK_V407';
  const GLOBAL_GUARD = '__AIQUEST_S2_AR_EVENT_GUARD_V407__';
  ...
  function signature(ar){
    return [ar?.completedAt || '',ar?.score || 0,ar?.correct || 0,ar?.total || 0,ar?.helpUsed || 0,ar?.usedSec || 0].join('|');
  }
  ...
  function statusHost(){
    return document.getElementById('aiquestS2ArEntryV405') || document.getElementById('aiquestArLauncherV401');
  }

  function renderStatus(message, tone){
    const host = statusHost();
    if (!host) return;
    let box = document.getElementById('s2ArBridgeStatusV407');
    if (!box) {
      box = document.createElement('div');
      box.id = 's2ArBridgeStatusV407';
  ...
  function validReceipt(receipt, sig){
    return receipt?.signature === sig && receipt?.status === 'queued';
  }

  function findExistingReceipt(sig){
    const own = readJson(RECEIPT_KEY);
    if (validReceipt(own, sig)) return own;
    for (const key of LEGACY_RECEIPT_KEYS) {
      const old = readJson(key);
      if (validReceipt(old, sig)) {
        writeJson(RECEIPT_KEY, { ...old, bridge:VERSION, recoveredFrom:key });
        return old;
      }
    }
    return null;
  ...
  function claim(sig){
    const global = window[GLOBAL_GUARD] || (window[GLOBAL_GUARD] = {});
    if (global[sig] === 'sending' || global[sig] === 'queued') return false;

    const persistent = readJson(LOCK_KEY) || {};
    const claimedAt = Date.parse(persistent.claimedAt || '') || 0;
    if (persistent.signature === sig && Date.now() - claimedAt < LOCK_TTL_MS) return false;

    global[sig] = 'sending';
    writeJson(LOCK_KEY, { signature:sig, status:'sending', claimedAt:new Date().toISOString(), bridge:VERSION });
    return true;
  }

  function release(sig, queued){
    const global = window[GLOBAL_GUARD] || {};
    global[sig] = queued ? 'queued' : '';
    const lock = readJson(LOCK_KEY) || {};
    if (lock.signature === sig) {
      if (queued) writeJson(LOCK_KEY, { ...lock, status:'queued', completedAt:new Date().toISOString(), bridge:VERSION });
      else {
  ...
  function renderReceipt(){
    ...
    const sent = !!findExistingReceipt(sig);
    renderStatus(
      sent
  ...
  async function sync(){
    ...
    const sig = signature(ar);
    if (findExistingReceipt(sig)) return true;
    ...
    try {
      ...
      console.log('[AIQuest S2 AR Sync] queued one s2_ar_complete event', event.eventId);
      return true;
    } catch (error) {
  ...
  window.AIQUEST_S2_AR_RESULT_BRIDGE = Object.freeze({version:VERSION,sync,getResult,evidence});
