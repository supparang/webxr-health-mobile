/* CSAI2102 Teacher Console — Analytics Actions Stability First v7.1.7
   Loaded through the v716 filename for cache-safe replacement.
   No MutationObserver, no interval, no synthetic Inspector tab click.
*/
(()=>{'use strict';
  if(window.__AIQUEST_TEACHER_ANALYTICS_ACTIONS_STABLE_V717__)return;
  window.__AIQUEST_TEACHER_ANALYTICS_ACTIONS_STABLE_V717__=true;
  ...
  function decorateCrossFilters(){
    document.querySelectorAll('#aqLearningAnalyticsV710 .aq710-card').forEach(card=>{
      ...
      card.querySelectorAll('.aq710-barrow').forEach(row=>{
        ...
        if(!term)return;
        row.classList.add('aq717-cross');row.dataset.term=term;row.tabIndex=0;
      });
    });
  }
  ...
  function enhance(){if(document.getElementById('aqInspectorV701'))return;decorateHeatmap();decorateCrossFilters();}
  function onClick(e){
  ...
  function boot(){
    ...
    [700,2400,5200].forEach(ms=>setTimeout(enhance,ms));
    console.log('[AIQuest] Analytics Actions Stability First active',VERSION);
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
  window.AIQUEST_TEACHER_ANALYTICS_ACTIONS_V716={VERSION,enhance,openInspector,filterStudents};
})();