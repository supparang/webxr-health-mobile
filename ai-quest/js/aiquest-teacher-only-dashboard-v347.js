...
(function(){
  'use strict';
  const VERSION='v3.4.7-teacher-student-detail-session-history';
  ...
  async function loadTeacherData(){
    ...
    adapt(td,sd,hd);
    renderAll(td,sd,hd);
    console.log('[AIQuest Teacher v3.4.7] loaded', {students:state.students.length,attempts:state.attempts.length,adapter:state.adapterInfo,raw:td,summary:sd,health:hd});
  }
  ...
  function renderSheets(d,s,h){
    const version=first(h||{},['version','serverVersion'])||first(d||{},['version','serverVersion'])||'v3.4.7';
    const a=state.adapterInfo||{};
  ...
  function normalizeSessionKey(k){
    ...
    if(k==='m1' || k==='mission1' || k==='session1' || k==='1') return 's1';
    if(k==='m2' || k==='mission2' || k==='session2' || k==='2') return 's2';
    if(k==='boss1' || k==='rookieboss') return 'b1';
    if(k==='m3' || k==='mission3' || k==='session3' || k==='3') return 's3';
    if(k==='m4' || k==='mission4' || k==='session4' || k==='4') return 's4';
    if(k==='m5' || k==='mission5' || k==='session5' || k==='5') return 's5';
    if(k==='boss2' || k==='searchboss' || k==='searcharenaboss') return 'b2';
    if(['s1','s2','s3','s4','s5','b1','b2'].includes(k)) return k;
    return k;
  }
  function readScore(x){
    return num(first(x||{},['bestScore','best','Best','maxScore','score','latestScore','latest']),0);
  }
  function readLatest(x){
    return num(first(x||{},['latestScore','latest','Latest','lastScore','score','bestScore','best']),0);
  }
  ...
  function readLastSubmitted(x){
    return first(x||{},['lastSubmitted','lastSubmit','timestamp','time','createdAt','updatedAt','latestTime']) || '';
  }
  ...
  function collectSessionDataFromRaw(s){
    ...
    const containers=[r.sessions,r.sessionSummary,r.bySession,r.progressBySession,r.sessionProgress,r.missions,r.byMission];
    containers.forEach(obj=>{
      if(obj && typeof obj==='object' && !Array.isArray(obj)){
        Object.keys(obj).forEach(k=>{
          const nk=normalizeSessionKey(k);
          if(['s1','s2','s3','s4','s5','b1','b2'].includes(nk)){
            out[nk]=Object.assign({}, out[nk]||{}, obj[k]||{});
          }
        });
      }
    });

    // Some teacher summary rows store flat values. Use current row as aggregate/fallback for B2 if no session object exists.
    ...
    // Merge attempt-level data if present
    state.attempts.filter(a=>String(a.studentId)===String(s.studentId)).forEach(a=>{
      ...
      out[nk]=Object.assign({}, prev, {
        attempts: num(prev.attempts,0)+1,
        bestScore: Math.max(num(prev.bestScore||prev.best,0), score),
        latestScore: score || num(prev.latestScore||prev.latest,0),
        stars: Math.max(num(prev.stars,0), num(a.stars,0)),
        lastSubmitted: a.timestamp || prev.lastSubmitted || ''
      });
    });
  ...
  function sessionHistoryRows(s){
    ...
    return SESSION_ORDER_V347.map(([key,label,title])=>{
      const x=by[key]||{};
  ...
  function sessionCompactNote(s){
    ...
    return `${done}/${rows.length} sessions passed/mastery${need?` • Need review: ${need}`:''}`;
  }
  ...
  function bind(){
    ...
    $('previewBtn').onclick=()=>window.open('./index.html?v=20260614-sessionhistory347','_blank');
  ...
  window.AIQUEST_TEACHER_ONLY_DASHBOARD={version:VERSION,loadTeacherData,renderAll,showRaw,loadStudent12,showDetail,state,adapt,collectArrays,normStudent,sessionHistoryRows,sessionHistoryHTML};
