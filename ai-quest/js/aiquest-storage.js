(function(){
  ...
  function isProfileReady(profile){
    ...
    return !!(p.studentId && p.studentName && p.section);
  }
  ...
  function saveAttempt(attempt){
    ...
    const next = {
      ...attempt,
      attemptId:attempt.attemptId || uid('att'),
      clientTs:attempt.clientTs || nowIso(),
      userAgent:navigator.userAgent,
  ...
  function setPending(queue){
    writeJson(PENDING_KEY, Array.isArray(queue) ? queue : []);
  }
  ...
  function exportAttemptsJson(){
    return JSON.stringify(getAttempts(), null, 2);
  }

  function attemptsToCsv(attempts){
    const rows = attempts || getAttempts();
    const headers = [
      'attemptId','studentId','studentName','section','sessionId','missionId','difficulty','score','stars','mastered',
      'usedTimeSec','accuracy','maxCombo','helpUsed','trickCorrect','explainCorrect','bossWin','clientTs'
    ];
    const esc = v => '"' + String(v == null ? '' : v).replace(/"/g,'""') + '"';
    return [headers.join(',')].concat(rows.map(r => headers.map(h => esc(r[h])).join(','))).join('\n');
  }

  window.AIQuestStorage = {
    PROFILE_KEY,
    ATTEMPTS_KEY,
    PENDING_KEY,
    CONFIG_KEY,
