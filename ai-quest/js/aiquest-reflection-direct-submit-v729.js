/* CSAI2102 AI Quest — Reflection Direct Submit v7.2.9
   Purpose: eliminate the result-screen deadlock where the UI begins polling
   studentGate but the original inline save handler never persists Reflection.
   This module captures the visible submit action, writes a dedicated idempotent
   Reflection attempt through AIQuestSync, then starts bounded Sheet confirmation.
*/
(()=>{'use strict';
if(window.AIQuestReflectionDirectSubmitV729)return;
...
function parseCorrect(){
 const source=[clean($('resultText')&&$('resultText').textContent),clean($('summary')&&$('summary').textContent)].join(' ');
 const m=source.match(/(?:ถูก|Correct)\s*(\d+)\s*\/\s*(\d+)/i)||source.match(/(\d+)\s*\/\s*(\d+)/);
 return{correct:m?Number(m[1]):Math.round(score()*15/100),total:m?Number(m[2]):15};
}
...
function payload(){
 ...
 return{
  ...
  source:'reflection-direct-submit-v729',clientTs:new Date().toISOString(),pageUrl:location.href
 };
}
let busy=false;
async function submit(){
 ...
 if(!window.AIQuestSync||typeof window.AIQuestSync.submitAttempt!=='function'){
  note('ส่ง Reflection ไม่สำเร็จ: ระบบส่งข้อมูลยังไม่พร้อม','bad');return false;
 }
 busy=true;
 if(save){save.disabled=true;save.textContent='กำลังส่ง Reflection…';save.dataset.directSubmitting='1'}
 note('กำลังบันทึก Reflection ลง Google Sheet…','');
 try{
  const p=payload();
  await window.AIQuestSync.submitAttempt(p);
  try{await window.AIQuestSync.submitEvent({studentId:p.studentId,studentName:p.studentName,section:'101',sessionId:p.sessionId,missionId:p.missionId,eventType:'reflection_submitted',attemptId:p.attemptId,reflectionSubmitted:true,selectedCaseId:p.selectedCaseId,clientTs:new Date().toISOString(),pageUrl:location.href})}catch(_){ }
  if(save){save.dataset.directSubmitted='1';save.textContent='✓ ส่ง Reflection แล้ว'}
  note('ส่งข้อมูลแล้ว กำลังยืนยัน Reflection จาก Google Sheet…','');
  const confirmer=window.AIQuestReflectionSheetConfirmV727||window.AIQuestReflectionSheetConfirmV726||window.AIQuestReflectionSheetConfirmV725;
  if(confirmer&&typeof confirmer.confirmSheet==='function')setTimeout(()=>confirmer.confirmSheet(true),900);
  else note('ส่ง Reflection แล้ว กรุณากลับแผนที่เพื่อตรวจสถานะจาก Sheet','good');
  return true;
 }catch(err){
  if(save){save.disabled=false;save.textContent='ส่ง Reflection และผล';save.dataset.directSubmitting='0'}
  note('ส่ง Reflection ไม่สำเร็จ: '+clean(err&&err.message||err)+' กรุณากดส่งอีกครั้ง','bad');
  return false;
 }finally{busy=false}
}
function capture(e){
 const target=e.target&&e.target.closest?e.target.closest('#save'):null;
 if(!target||!ready()||target.dataset.sheetConfirmed==='1')return;
 e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();submit();
}
function boot(){
 document.addEventListener('pointerdown',capture,true);
 document.addEventListener('click',capture,true);
 console.log('[AIQuest] Reflection direct submit v729 active • dedicated idempotent Reflection attempt');
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.AIQuestReflectionDirectSubmitV729={version:VERSION,submit,payload,ready,caseMeta};
})();