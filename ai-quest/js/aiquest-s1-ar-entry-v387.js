...
(function(){
  ...
  function getSavedResult(){
    try{ var raw = localStorage.getItem('AIQUEST_S1_AR_RESULT_V368'); return raw ? JSON.parse(raw) : null; }
    catch(_){ return null; }
  }
  function isS1Screen(){
  ...
  function findHost(){
    ...
    return area && isS1Screen() ? area : null;
  }
  function buildCard(){
    ...
    wrap.style.cssText = 'margin:0 0 14px;padding:14px 16px;border-radius:18px;border:1px solid rgba(167,139,250,.55);background:linear-gradient(135deg,rgba(124,58,237,.18),rgba(56,189,248,.13));box-shadow:0 12px 26px rgba(0,0,0,.16);position:relative;z-index:8';
    wrap.innerHTML = ''
      + '<div style="display:flex;gap:14px;align-items:center;justify-content:space-between;flex-wrap:wrap">'
      + '<div style="min-width:0"><div style="font-weight:1000;color:#f5f3ff;font-size:16px">📷 S1 AR Practice: AI Object Scanner</div>'
      ...
      + (done ? ' • เล่นแล้ว ' + correct + '/' + total + ' = ' + score + '%' : '') + '</div></div>'
      ...
      + (done ? 'ฝึก AR อีกครั้ง' : 'เริ่ม AR Practice') + '</button></div>';
  ...
  var scheduled = false;
  function schedule(){ if(scheduled) return; scheduled = true; setTimeout(function(){ scheduled=false; inject(); },60); }
  function boot(){
    schedule();
    var area = document.getElementById('gameArea');
