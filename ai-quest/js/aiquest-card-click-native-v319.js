...
(function(){
  ...
  const VERSION = 'v3.1.9-card-click-native';
  ...
  function installGlobalCapture(){
    ...
    document.addEventListener('click', function(ev){
      ...
      if(ev.target && ev.target.closest && ev.target.closest('#roadmapClickFixPanel')) return;
  ...
  function addTopReplayButton(){
    ...
    ids.forEach(id=>{
      ...
      btn.onclick=(e)=>{e.preventDefault();e.stopPropagation();go(id);};
      bar.appendChild(btn);
    });
