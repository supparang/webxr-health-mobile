...
(function(){
  ...
  const VERSION = 'v2.6.6-constructed-remedial';
    ...
    function render(){
      ...
      host.innerHTML = `
        <div class="remedialMini">
          ...
          <div class="remedialRow">
            <button class="${btnClass()}" id="showModelAnswer">ดูคำตอบตัวอย่าง</button>
          </div>
      ...
      if(show){
        show.onclick = () => {
          const answer = String(($('#constructedAnswer') || {}).value || '').trim();

          if(answer.length < 18){
            if(window.showToast) showToast('พิมพ์เหตุผลก่อนอย่างน้อย 1 ประโยค');
            else alert('พิมพ์เหตุผลก่อนอย่างน้อย 1 ประโยค');
            return;
          }
          ...
          box.innerHTML = `
            <div class="remedialFeedback">
              ...
              ${escapeHtml(q.model || '')}
              <ul class="checklist">
                ${(q.checklist || []).map(c => `<li>${escapeHtml(c)}</li>`).join('')}
              </ul>
              <div class="remedialRow">
