...
const MARKER = 'HEROHEALTH_P5_PRODUCTION_LEARNER_ACCESS_R1';
...
function patchFirestoreSource(source) {
  ...
  return { content: `${source.slice(0, close)}${additivePatch()}${source.slice(close)}`, changed: true };
}
...
if (!patched.changed) {
  console.log(`✅ ${MARKER} มีอยู่แล้ว ไม่ deploy ซ้ำ`);
  process.exit(0);
}
...
try {
  ...
  fail(`Rules validation/create ล้มเหลว; live rules ยังไม่เปลี่ยน: ${error.message}`);
}
...
console.log('\n✅ HeroHealth P5 Production Learner Rules สำเร็จ');
console.log(`   Previous: ${liveRuleset.name}`);
console.log(`   Current:  ${created.name}`);
console.log(`   Backup:   ${backupPath}`);
console.log('   Learner IDs: H51xx-H56xx');
console.log('   students: GET only (no learner LIST)');
console.log('   progress/assessment: requires UID→student binding');
