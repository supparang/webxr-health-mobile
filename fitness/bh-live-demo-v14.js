...
const RELEASE='20260717-BALANCE-HOLD-LIVE-DEMO-ATOMIC-END-V26';
...
function demoStats(){
  ...
  return{
    ...
    tracking,
    total:Math.max(1,s.sequence?.length||0),
    completed:Math.max(s.results?.length||0,s.sequence?.length||0)
  };
}
...
function showAtomicDemoEnd(){
  ...
  screen.innerHTML=`<section style="width:min(720px,100%);max-height:92vh;overflow:auto;border-radius:30px;padding:28px;background:#fff;border:4px solid #67e8f9;box-shadow:0 30px 100px rgba(0,0,0,.65);text-align:center">
    ...
    <h1 style="font-size:clamp(30px,5vw,50px);margin:4px 0 8px">Live Demo Complete!</h1>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
      <button id="bhAtomicReplay" type="button" style="min-height:58px;border:0;border-radius:17px;background:linear-gradient(135deg,#06b6d4,#2563eb);color:#fff;font-size:17px;font-weight:1000;cursor:pointer">🎥 ทดลอง Demo อีกครั้ง</button>
      <button id="bhAtomicBack" type="button" style="min-height:58px;border:0;border-radius:17px;background:#0f172a;color:#fff;font-size:17px;font-weight:1000;cursor:pointer">🏠 กลับ Fitness</button>
    </div>
  </section>`;
...
BH.startLiveDemo=async()=>{
  ...
  s.demo=true;s.demoFinalizing=false;s.demoEndShown=false;
...
BH.submitSummary=async x=>{
  if(!s.demo)return officialSubmit(x);
};
...
console.info('[BalanceHold] Atomic Demo End v26 ready',RELEASE);
})();