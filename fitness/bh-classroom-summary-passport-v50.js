...
const RELEASE='20260731-BALANCE-CLASSROOM-SUMMARY-PASSPORT-V50';
let finished=false;
...
function makeSummary(){
  let x={};
  try{x=BH.calcSummary?.('completed_v50')||{}}catch(err){console.warn('[BalanceHold V50] calcSummary fallback',err)}
  ...
  return {
    ...x,
    score:Number(x.score||s.score||0),
    ...
    reason:'completed_v50',
    classroomSummaryVersion:RELEASE
  };
}

function renderPassportSummary(x){
  const overlay=e.resultOverlay;
  if(!overlay)return;
  overlay.innerHTML=`<section class="modal" style="width:min(92vw,520px);text-align:center;max-height:92vh;overflow:auto;padding:24px;border:4px solid #facc15;border-radius:28px;background:#fff">
    ...
    <h2 style="font-size:clamp(2rem,8vw,3rem);margin:4px 0;color:#059669">Healthy Hero!</h2>
    ...
    <p style="font-weight:900;color:#047857">เล่นครบ 1 รอบ • พร้อมกลับ Passport</p>
  ...
  if(btn)btn.onclick=()=>{location.href=passportUrl()};
}
...
function hardFinish(){
  ...
  try{BH.submitSummary?.(x)}catch(err){console.warn('[BalanceHold V50] submit deferred',err)}
...
BH.finish=reason=>{
  ...
  try{
    ...
    if(String(s.phase||'').toLowerCase()==='summary'&&!e.resultOverlay?.classList.contains('hidden'))return;
  }catch(err){console.error('[BalanceHold V50] legacy finish bypassed',err)}
  hardFinish();
};

const timer=setInterval(()=>{
  if(finished)return;
  const completeByIndex=Array.isArray(s.sequence)&&Number(s.index)>=s.sequence.length;
  const completeByUi=isPlay()&&isLastPose()&&(textNumber(e.holdText)>=99||textNumber(e.hudPose)>=6);
  if(completeByIndex){hardFinish();return}
  if(completeByUi){
    if(!lastPoseReadyAt)lastPoseReadyAt=performance.now();
    if(performance.now()-lastPoseReadyAt>=500)hardFinish();
