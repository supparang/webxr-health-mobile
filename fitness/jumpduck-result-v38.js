...
function applyReactionAnalytics(target){if(!target||typeof target!=='object')return target;const avgReactionMs=averageReaction();target.avgReactionMs=avgReactionMs;target.reactionSampleCount=state.samples.length;target.reactionSamples=state.samples.slice(0,60);target.reactionBasis=state.samples.length?'warning-to-first-detected-body-or-touch-response-v39':'no-detected-response-after-warning-v39';target.reactionStatus=state.samples.length?'measured':'not-observed';target.resultFlowVersion='jumpduck-result-v3.9';target.gameVersion='jumpduck-production-v3.9-outcome-wording';target.roundCompleted=true;target.forcedReplay=false;return target}
...
function missionProgress(){const payload=window.__JUMPDUCK_LAST_RESULT__||{};let reached=Number(payload.missionReached);if(!Number.isFinite(reached)||reached<1){const text=String(document.getElementById('resultText')?.textContent||'');const match=text.match(/ภารกิจ\s*(\d+)\s*\/\s*3/i);reached=match?Number(match[1]):0}return Math.max(0,Math.min(3,Math.round(reached||0)))}
function directSummary(){const reached=missionProgress();return `เล่นครบ 1 รอบ • พร้อมกลับ Passport\nทำสำเร็จ ${reached} จาก 3 ภารกิจ`}
...
const direct=window.parent===window||!q.get('studentId');if(direct&&sync){sync.textContent=directSummary();sync.style.whiteSpace='pre-line'}syncRetryVisibility()}
function syncRetryVisibility(){const done=document.getElementById('done'),sync=document.getElementById('syncText');if(!done||!sync)return;let text=String(sync.textContent||'');const direct=window.parent===window||!q.get('studentId');if(direct&&/เปิดแบบทดสอบตรง|ผลพร้อมส่ง|ทดสอบเกมสำเร็จ|พร้อมกลับ Passport/i.test(text)){sync.textContent=directSummary();sync.style.whiteSpace='pre-line';text=sync.textContent}
