(function(){
  ...
  function buildCooldownGateUrl(summary){
    ...
    const summaryUrl =
      `../herohealth/shadow-breaker-v1-summary.html` +
    ...
    u.searchParams.set('game', 'shadow-breaker-v1');
    u.searchParams.set('gameId', 'shadow-breaker-v1');
    u.searchParams.set('cat', 'fitness');
    u.searchParams.set('theme', 'shadow-breaker-v1');
    u.searchParams.set('zone', 'fitness');
    u.searchParams.set('pid', String(summary?.pid || qs('pid', 'anon') || 'anon'));
    u.searchParams.set('hub', hubUrl);
    u.searchParams.set('finalGame', '/webxr-health-mobile/fitness/shadow-breaker-v1.html');
    u.searchParams.set('plannedGame', '/webxr-health-mobile/fitness/shadow-breaker-v1.html');
  ...
  function buildReplayLauncherHref(){
    ...
    const url = new URL('/webxr-health-mobile/herohealth/shadow-breaker-v1.html', location.origin);
  ...
  function setView(name){
    ...
    if (DOM.msgMain) {
      DOM.msgMain.textContent = name === 'play' ? (DOM.msgMain.textContent || 'พร้อมแล้ว!') : '';
    }
  }
  ...
  function computeSummary(endReason){
    ...
    const hubUrl = String(qs('hub', '') || '../herohealth/hub.html');
    const teacherUrl = `../herohealth/teacher-panel.html?game=shadow-breaker-v1&pid=${encodeURIComponent(STATE.pid)}`;
    const summaryUrl =
      `../herohealth/shadow-breaker-v1-summary.html` +
    ...
    const runUrl =
      `../herohealth/shadow-breaker-v1.html` +
      `?pid=${encodeURIComponent(STATE.pid)}` +
      `&diff=${encodeURIComponent(STATE.diff)}` +
    ...
    const summaryBase = {
      game: 'shadow-breaker-v1',
      projectTag: 'shadow-breaker-v1',
      pid: STATE.pid,
      mode: STATE.mode,
      runMode: STATE.mode,
      diff: STATE.diff,
  ...
  function persistSummary(summary){
    ...
    const bestScoreKey = `SBV1_BEST_SCORE:${pid}`;
    ...
    localStorage.setItem(`SBV1_BEST_REACHED_BOSS:${pid}`, String(summary.bestReachedBoss || ''));
    saveJson(`SBV1_BEST_BADGE:${pid}`, summary.badge || null);
    saveJson(`SBV1_LAST_SUMMARY:${pid}`, summary);
    saveJson(`HHA_LAST_SUMMARY:shadow-breaker-v1:${pid}`, summary);
    saveJson('HHA_LAST_SUMMARY', summary);

    const histKey = `SBV1_SUMMARY_HISTORY:${pid}`;
    const hist = loadJson(histKey, []);
    saveJson(histKey, [summary, ...(Array.isArray(hist) ? hist : [])].slice(0, 200));

    const allKey = `SBV1_SUMMARY_HISTORY:ALL`;
    const allHist = loadJson(allKey, []);
    saveJson(allKey, [summary, ...(Array.isArray(allHist) ? allHist : [])].slice(0, 500));

    try{
      localStorage.setItem(`SBV1_DONE_TODAY:${pid}:${hhDayKey()}`, '1');
      localStorage.setItem(`HHA_GAME_DONE::shadow-breaker-v1::${pid}::${hhDayKey()}`, '1');
    }catch(_){}

    console.log('[SBV1] persistSummary', {
      pid: summary?.pid,
      grade: summary?.grade,
  ...
  function buildSessionId(){
    return `sbv1_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
  }
  ...
  function exportEventsCsv(){
    ...
    downloadTextFile(`shadow_breaker_v1_events_${STATE.pid || 'anon'}.csv`, rowsToCsv(STATE.events), 'text/csv;charset=utf-8');
  }

  function exportSessionCsv(){
    const sum = STATE.sessionSummary || computeSummary('manual_export');
    downloadTextFile(`shadow_breaker_v1_session_${STATE.pid || 'anon'}.csv`, rowsToCsv([sum]), 'text/csv;charset=utf-8');
  }
  ...
  function initFromQuery(){
    ...
    const finalMode =
      String(qs('mode', qs('run', 'normal'))).toLowerCase() === 'research'
        ? 'research'
        : 'normal';
    ...
    const gameId = String(qs('gameId', qs('game', ''))).toLowerCase();
    ...
    console.log('[SBV1] initFromQuery', {
      gameId,
      gateSkip,
      warmupDone,
      auto,
  ...
  setTimeout(() => {
    ...
    if (gameId === 'shadow-breaker-v1' && gateSkip === '1' && !STATE.isPlaying && !STATE.isEnding) {
      console.log('[SBV1] fallback autostart fired');
      startGame(finalMode);
    }
  }, 250);
