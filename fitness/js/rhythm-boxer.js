...
(function () {
  ...
  let engine = null;
  let lastStartCfg = null; // เก็บเพลงล่าสุดไว้สำหรับปุ่ม "เล่นเพลงเดิมอีกครั้ง"
  ...
  function updateModeUI() {
    ...
    if (mode === 'normal') {
      ...
      if (modeDescEl) {
        modeDescEl.textContent =
          'Research: ใช้เก็บข้อมูลเชิงวิจัย พร้อมดาวน์โหลด CSV (แสดง AI prediction ได้ แต่ล็อกไม่ให้ AI ปรับเกม)';
      }
      if (trackModeLbl) {
  ...
  function startGame() {
    ...
    lastStartCfg = { mode, trackKey };

    engine.start(mode, cfg.engineId, meta);
    switchView('play');
  }

  function startSameAgain() {
    if (!lastStartCfg) {
      startGame();
      return;
    }

    // คงค่า mode/track เดิม
    const mRadio = modeRadios.find(x => x.value === lastStartCfg.mode);
    if (mRadio) mRadio.checked = true;
    updateModeUI();
    setSelectedTrackKey(lastStartCfg.trackKey);
  ...
  // ==== Optional UI badge/hint from AI mode ====
  function applyAiModeHint() {
    try {
      ...
      // ใส่ข้อความเสริมที่ mode desc ถ้าหน้าเมนูพร้อม
      if (modeDescEl && getSelectedMode() === 'research' && locked) {
        modeDescEl.textContent =
          'Research: ใช้เก็บข้อมูลเชิงวิจัย พร้อมดาวน์โหลด CSV (แสดง AI prediction ได้ แต่ล็อก 100% ไม่ให้ AI ปรับเกม)';
      }

      // hint ตอน normal + ai=1
      if (getSelectedMode() === 'normal' && assist && modeDescEl) {
        modeDescEl.textContent =
          'Normal: เล่นสนุก / ใช้สอนทั่วไป (เปิด AI assist ผ่าน ?ai=1 ได้ · Research จะล็อกไม่ปรับเกม)';
      }
    } catch(_){}
  }
  ...
  if (btnAgain) btnAgain.addEventListener('click', () => startSameAgain());
