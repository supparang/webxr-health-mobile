...
// ✅ Normal/Research toggle + track selector
// ✅ Start/Stop/Again/Back
// ✅ Download Events CSV + Session CSV
// ✅ Apply ?mode=research|play
// ✅ AI HUD reads from engine.aiState (prediction only; research locked by RB_AI)
...
(function () {
  ...
  function startGame() {
    ...
    // IMPORTANT: engine reads query ?mode=research for RB_AI lock policy,
    // but we still pass mode here for session labeling.
  ...
  function handleEngineEnd(summary) {
    ...
    if (res.offsetAvg) {
      const m = summary.offsetMean;
      res.offsetAvg.textContent = (m != null && Number.isFinite(m)) ? m.toFixed(3) + ' s' : '-';
    }
    if (res.offsetStd) {
      const s = summary.offsetStd;
      res.offsetStd.textContent = (s != null && Number.isFinite(s)) ? s.toFixed(3) + ' s' : '-';
    }
