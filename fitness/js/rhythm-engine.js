...
(function(){
  ...
  function makeWarmupChart(bpm, dur){
    ...
    const seq = [2,1,3,2,1,3,2,3];  // ลำดับเลน (C, L1, R1 ...)
  ...
  // ===== CSV TABLE (ใช้ร่วมกันทั้ง Event / Session) =====
  class CsvTable {
    constructor(){
      this.rows = [];
    }
  ...
  class RhythmBoxerEngine {
    ...
    handleLaneTap(lane){
      ...
      if (judgment === 'miss'){
        this._applyMiss(note, songTime, dt, /*byTap=*/true);
      } else {
