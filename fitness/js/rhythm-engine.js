...
// ✅ PATCH: music-note icon + lane flash + hitline-sync travel calc
  ...
  class RhythmBoxerEngine{
    ...
    _bindLaneInput(){
      ...
      DOC.addEventListener('keydown', (ev)=>{
        ...
        // optional calibration hotkeys
        if(k === '['){
          this.calOffsetSec = clamp(this.calOffsetSec - 0.005, -0.200, 0.200);
    ...
    _clearNotesDom(){
      ...
      this.lanesEl.querySelectorAll('.rb-lane.is-hit').forEach(el=>el.classList.remove('is-hit'));
    }
    ...
    _flashLane(lane){
      ...
      if(!laneEl) return;
      laneEl.classList.remove('is-hit');
      void laneEl.offsetWidth;
      laneEl.classList.add('is-hit');
      clearTimeout(laneEl._rbHitTimer);
      laneEl._rbHitTimer = setTimeout(()=> laneEl.classList.remove('is-hit'), 90);
    }
    ...
    _spawnNote(note){
      ...
      // NOTE LENGTH: ให้ tail ยาวพอเห็นจังหวะหล่น
      const diff = this.track.diff || 'normal';
      const base = this.noteBaseLen || 160;
      const mul = (diff==='easy') ? 1.00 : (diff==='hard') ? 1.30 : 1.15;
    ...
    _loop(){
      ...
      if(this.audio && Number.isFinite(this.audio.currentTime) && this.audio.currentTime > 0){
    ...
    _updateNotePositions(){
      // CSS ควรตั้ง .rb-note { bottom: var(--rb-hitline-y) }
      // ตอน n.t => translateY = 0 (หัวโน้ตถึงเส้นตีล่างพอดี)
      const lead = this.noteSpeedSec || 2.2;
      if(!this.lanesEl) return;
      ...
      for(const n of this.live){
        ...
        const laneH = laneRect.height || 380;

        let noteLen = parseFloat(getComputedStyle(n.el).getPropertyValue('--rb-note-len'));
        if(!Number.isFinite(noteLen)) noteLen = this.noteBaseLen || 160;

        // travel distance: start above field -> end at hit line
        const travel = Math.max(220, laneH - 28 + noteLen * 0.18);
    ...
    _resolveTimeoutMiss(){
      ...
      for(const n of this.live){
        ...
        // DOM disappeared externally -> avoid stale live notes
        if(n.spawned && !n.el){
          n.done = true;
          continue;
        }
    ...
    _updateHud(){
      ...
      if(hud.acc){
        ...
        const acc = judged ? (((judged - this.hitMiss) / (this.totalNotes || 1)) * 100) : 0;
    ...
    _updateAI(){
      ...
      // IMPORTANT:
      // Research mode = prediction display only (LOCKED, no adaptation)
      // Normal mode = can later enable adaptive changes when ?ai=1 (not applied yet here)
    }
