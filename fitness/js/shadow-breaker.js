...
(function(){
  ...
  const BOSSES = [
    { key:'bubble', name:'Bubble Glove', emoji:'🫧', speech:'แตะฟองให้ทันนะ!', clearTitle:'ผ่านบอสแล้ว!', clearSub:'เก่งมาก เตรียมลุยด่านต่อไป', desc:'เป้ากลมใหญ่ ตีง่าย เหมาะเริ่มต้น', theme:{ theme1:'rgba(56,189,248,.22)', theme2:'rgba(59,130,246,.14)', border:'rgba(56,189,248,.26)' } },
    { key:'meteor', name:'Meteor Punch', emoji:'☄️', speech:'มาเร็วขึ้นอีกนิด!', clearTitle:'ผ่านอีกบอสแล้ว!', clearSub:'ยอดเยี่ยม ไปต่อกันเลย', desc:'เป้าเร็วขึ้น เริ่มมีบอมบ์มากขึ้น', theme:{ theme1:'rgba(245,158,11,.22)', theme2:'rgba(239,68,68,.12)', border:'rgba(245,158,11,.26)' } },
    { key:'hydra', name:'Neon Hydra', emoji:'🐉', speech:'แตะให้ทันนะ ระวังหลายเป้า!', clearTitle:'ใกล้สำเร็จแล้ว!', clearSub:'เหลืออีกนิดเดียวจะถึงบอสสุดท้าย', desc:'หลายเป้าพร้อมกัน ต้องตั้งสมาธิ', theme:{ theme1:'rgba(167,139,250,.24)', theme2:'rgba(99,102,241,.16)', border:'rgba(167,139,250,.26)' } },
    { key:'final', name:'Final Boss', emoji:'👑', speech:'ถึงด่านสุดท้ายแล้ว ลุยเลย!', clearTitle:'ชนะแล้ว!', clearSub:'หนูเป็นฮีโร่ของวันนี้ 🎉', desc:'ด่านสุดท้าย เป้าบอสเด่นที่สุด', theme:{ theme1:'rgba(250,204,21,.24)', theme2:'rgba(244,114,182,.16)', border:'rgba(250,204,21,.30)' } }
  ];
  ...
  function setUiMode(mode){
    ...
    if(DOM.wrap) DOM.wrap.dataset.viewMode = getViewMode();

    if(DOM.tabNormal) DOM.tabNormal.classList.toggle('is-active', STATE.mode !== 'research');
    if(DOM.tabResearch) DOM.tabResearch.classList.toggle('is-active', STATE.mode === 'research');
    ...
    if(getViewMode() === 'mobile' && DOM.currentBossName) DOM.currentBossName.textContent = currentBoss().name || 'Boss';
    if(getViewMode() === 'cvr' && DOM.btnPause) DOM.btnPause.checked = false;
  }
  ...
  function entityCenter(ent){
    return { x: Number(ent.x || 0) + Number(ent.size || 0) / 2, y: Number(ent.y || 0) + Number(ent.size || 0) / 2 };
  }
  ...
  function resetTargetLayer(){
    STATE.entities.forEach(e=>{ try{ e.el?.remove(); }catch(_){ } });
    STATE.entities = [];
  ...
  function makeEntity(type, opts={}){
    ...
    let spawnLeft = 12, spawnRight = w - size - 12, spawnTop = 84, spawnBottom = h - size - 24;
    if (viewMode === 'mobile') {
      spawnLeft = 14; spawnRight = w - size - 14; spawnTop = Math.max(146, h * 0.27); spawnBottom = h - size - 44;
    }
    if (viewMode === 'pc') {
      spawnLeft = 16; spawnRight = w - size - 170; spawnTop = Math.max(76, h * 0.17); spawnBottom = h - size - 20;
    }
    if (viewMode === 'cvr') {
      spawnLeft = 28; spawnRight = w - size - 28; spawnTop = Math.max(104, h * 0.20); spawnBottom = h - size - 60;
    }
    ...
    const ent = { id: `${Date.now()}_${Math.random().toString(36).slice(2,9)}`, type, el, x, y, size, born: nowMs(), ttl, alive:true, hit:false };
    ...
    logEvent('spawn', { ent_id: ent.id, ent_type: ent.type, x: Math.round(x), y: Math.round(y), size, ttl });
    return ent;
  }
  ...
  function spawnPack(dtMs){
    ...
    let goodCount = 1, badChance = 0.22, decoyChance = 0.16, utilityChance = 0.09, bossFaceChance = 0.13;
    if(STATE.phaseIndex === 1){ badChance = 0.27; decoyChance = 0.18; bossFaceChance = 0.17; }
    else if(STATE.phaseIndex === 2){ goodCount = 2; badChance = 0.29; decoyChance = 0.23; bossFaceChance = 0.21; }
    else if(STATE.phaseIndex === 3){ goodCount = 2; badChance = 0.33; decoyChance = 0.24; bossFaceChance = 0.26; }
  ...
  function computeSummary(endReason){
    ...
    return {
      ...summaryBase,
      __extraJson: JSON.stringify({ summaryUrl, teacherUrl, runUrl, hubUrl, cooldownUrl })
    };
  }
  ...
  function persistSummary(summary){
    ...
    if((summary.scoreFinal || 0) >= oldBest) localStorage.setItem(bestScoreKey, String(summary.scoreFinal || 0));
  ...
  function showInlineEndOverlay(summary){
    ...
    const msg =
      summary.end_reason === 'final_clear'
        ...
        : (Number(summary.bossesCleared || 0) >= 1 ? 'ผ่านบอสได้แล้ว เก่งมาก!' : 'เริ่มต้นได้ดี ลองอีกครั้งได้นะ');
  ...
  function goToCooldown(summary){
    ...
    console.log('[SB] goToCooldown', { pid: summary?.pid, score: summary?.scoreFinal, bossesCleared: summary?.bossesCleared, target });
    try{ location.href = target; } catch(_){ location.assign(target); }
  }
  ...
  function bindDom(){
    ...
    DOM.btnResultRetry?.addEventListener('click', ()=>{
      try{ location.href = buildReplayLauncherHref(); }
      catch(_){ location.assign(buildReplayLauncherHref()); }
    });
