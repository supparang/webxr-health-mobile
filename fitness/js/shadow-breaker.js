(function(){
  ...
  const SB_PLANNER_STRICT = sbIsPlannerStrictFlow();
  ...
  function buildCooldownGateUrl(summary){
    if (SB_PLANNER_STRICT) {
      return sbBuildPlannerReturnUrl(summary, { hub: summary?.hub || qs('hub', '') || SB_DEFAULT_HUB });
    }
  ...
  function buildReplayLauncherHref(){
    const hubUrl = sbNormalizeHubUrl(String(qs('hub', '') || SB_DEFAULT_HUB));
    const launcherUrl = sbNormalizeLauncherUrl(String(qs('launcher', '') || location.pathname || SB_DEFAULT_LAUNCHER));
    const url = new URL(launcherUrl, location.origin);
  ...
  function goBackHubNow(){
    const target = SB_PLANNER_STRICT
      ? sbPlannerReturnBase()
      : String(qs('hub', '') || '../herohealth/hub.html');
  ...
  function computeSummary(endReason){
    ...
    const summaryBase = {
      ...
      hub: hubUrl,
      view: getViewMode(),
      plannerStrict: SB_PLANNER_STRICT ? 1 : 0
    };
  ...
  function persistSummary(summary){
    ...
    console.log('[SB] persistSummary', {
      ...
      plannerStrict: SB_PLANNER_STRICT
    });
  }
  ...
  function endGame(reason = 'ended'){
    ...
    try{
      ...
      setTimeout(() => goToCooldown(summary), SB_PLANNER_STRICT ? 900 : 900);
      return;
    }catch(_){}
  ...
  function initFromQuery(){
    const hub = sbNormalizeHubUrl(String(qs('hub', '')).trim() || SB_DEFAULT_HUB);
    ...
    const plannerAuto = String(qs('autostart', '0')).toLowerCase();
    ...
    const autostart =
      ...
      ['1', 'true', 'yes', 'on'].includes(plannerAuto) ||
      fromWarmupGate ||
      SB_PLANNER_STRICT;
