...
class ShadowBreakerGame {
  ...
  makeSessionId() {
    ...
    return `SB-${t.getFullYear()}${pad(t.getMonth() + 1)}${pad(t.getDate())}-${pad(t.getHours())}${pad(t.getMinutes())}${pad(t.getSeconds())}`;
  }
  ...
  wireUI() {
    ...
    btnStopEarly.addEventListener('click', () => {
      if (this.mode === 'research' && this.running) {
        alert('โหมดวิจัยต้องเล่นจนจบเวลาเพื่อเก็บข้อมูลให้ครบค่ะ');
        return;
      }
  ...
  stopGame(reason) {
    ...
    const summary = {
      ...
      build_version: 'shadowBreaker_v7',
  ...
  _computeNormPos(t) {
    ...
    // zone_lr: L / C / R, zone_ud: T / M / B
    let zoneLR = 'C';
    if (t.x_norm < 0.33) zoneLR = 'L';
    else if (t.x_norm > 0.66) zoneLR = 'R';

    let zoneUD = 'M';
    if (t.y_norm < 0.33) zoneUD = 'T';
    else if (t.y_norm > 0.66) zoneUD = 'B';
