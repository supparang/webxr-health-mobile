// === js/engine.js — Shadow Breaker core (2025-12-04, Fever glow + layout) ===
"use strict";
...
export function initShadowBreaker() {
  ...
  // fever fullscreen glow
  const feverScreen = document.getElementById("sb-fever-fullscreen");
  ...
  const chkStopEarly = document.querySelector('[data-action="stop-early"]');
  ...
  const btnDownloadSession = document.querySelector(
    '[data-action="download-csv-session"]'
  );
  const btnDownloadEvents = document.querySelector(
    '[data-action="download-csv-events"]'
  );
  const btnResearchBegin = document.querySelector(
    '[data-action="research-begin-play"]'
  );
  const btnBackFromResearch = viewResearchForm?.querySelector(
  ...
  const BOSSES = [
    {
      ...
      name: "Bubble Glove",
      emoji: "🐣",
      hint: "โฟกัสที่ฟองใหญ่ ๆ แล้วตีให้ทัน",
    },
    {
      id: 1,
      name: "Spark Guard",
      emoji: "⚡️",
      hint: "เล็งเป้าเร็ว ๆ ระวังลูกระเบิด",
    },
    {
      id: 2,
      name: "Shadow Mitt",
      emoji: "🕶️",
      hint: "เป้าบางอันจะลวง ให้ดูจังหวะดี ๆ",
    },
    {
      id: 3,
      name: "Galaxy Punch",
      emoji: "🌌",
      hint: "ด่านสุดท้าย เป้าจะเล็กและเร็วมาก",
    },
  ];
  ...
  const FEVER_PER_HIT = 0.12; // เพิ่มขึ้นให้ง่ายขึ้น
  ...
  function showView(name) {
    ...
    switch (name) {
      ...
      case "result":
        ...
        break;
      case "research":
        viewResearchForm.classList.remove("hidden");
        break;
    }
  }
  ...
  function updateFeverUi(now) {
    ...
    const on = state.feverOn && now < state.feverUntil;

    if (on) {
      feverStatus.textContent = "ON";
      feverStatus.classList.add("on");
      feverScreen?.classList.add("on");
      feverScreen?.classList.remove("hidden");
    } else {
      state.feverOn = false;
      feverStatus.textContent = "READY";
      feverStatus.classList.remove("on");
      if (feverScreen) {
        feverScreen.classList.remove("on");
        feverScreen.classList.add("hidden");
      }
    }
  }
...
data.timeoutHandle = setTimeout(() => {
  ...
  function handleTargetHit(id, hitInfo) {
    ...
    if (hpDeltaPlayer !== 0) {
      state.playerHp = Math.max(
        0,
        Math.min(1, state.playerHp + hpDeltaPlayer)
      );
    }
    if (shieldDelta !== 0) {
  ...
  function logEvent(type, targetData, extra) {
    ...
    const row = {
      ...
      event_type: type,
      rt_ms:
        extra && extra.rtMs != null ? Math.round(extra.rtMs) : "",
  ...
  function gameLoop(now) {
    ...
    if (state.fever > 0 && !state.feverOn) {
      state.fever = Math.max(
        0,
        state.fever - FEVER_DECAY_PER_SEC * (elapsed / 1000)
      );
    }
...
for (const t of nowTargets) {
  ...
  function endGame(reason) {
    ...
    sessionSummary = {
      ...
      participant_group:
        (state.researchMeta && state.researchMeta.group) || "",
      participant_note:
        (state.researchMeta && state.researchMeta.note) || "",
      menu_latency_ms: Math.round(state.startedAt - menuOpenedAt),
      rt_normal_ms: state.rtNormalCount
        ? +(state.rtNormalSum / state.rtNormalCount).toFixed(1)
        : "",
      rt_decoy_ms: state.rtDecoyCount
    ...
    // result view
    resMode.textContent =
      state.mode === "research" ? "โหมดวิจัย" : "โหมดเล่นจริง";
    ...
    resFeverTime.textContent =
      (state.feverActiveMs / 1000).toFixed(1) + " s";
    resLowHpTime.textContent =
      (state.lowHpMs / 1000).toFixed(1) + " s";
    resBosses.textContent = String(state.clearedBosses);
    resMenuLatency.textContent =
      Math.round(state.startedAt - menuOpenedAt) + " ms";
  ...
  btnStartNormal?.addEventListener("click", (ev) => {
    ev.preventDefault();
    startGame("normal", null);
  });

  btnStartResearch?.addEventListener("click", (ev) => {
    ev.preventDefault();
    showView("research");
  });
  ...
  chkStopEarly?.addEventListener("change", () => {
    if (!state || !state.running) return;
    if (chkStopEarly.checked) {
      endGame("stop-early");
      chkStopEarly.checked = false;
    }
  });
