...
// PATCH v20260412b-SB-ENGINE-V4-BOSS-RESTORE

'use strict';

const SB_ENGINE_VERSION = 'v20260412b-SB-ENGINE-V4-BOSS-RESTORE';
...
async function runRound(engine, round) {
  ...
  ui.setCoachText('ดีมาก ไปต่อรอบหน้า');
  await sleep(800);
}
  ...
  while (nowMs() < state.runtime.roundEndMs) {
    ...
    if (round.type === 'warmup') {
      ...
      if (cueKey && cueKey !== state.runtime.passiveCueKey) {
        ...
        ui.setCue(lbl, 'info');
        ui.setCoachText(cueKey === 'jab' ? 'ต่อยเบา ๆ จับจังหวะ' : 'สลับแขนอีกข้าง');
      }
    }
    ...
    if (round.type === 'wrap') {
      ...
      ui.clearTarget();
      ui.setCue('EASY', 'info');
      ui.setCoachText('ค่อย ๆ ผ่อนแรง');
    }
...
async function runBossRound(engine, round) {
  ...
  if (state.boss.hp <= 0) {
    ...
    ui.setCoachText('เก่งมาก! ชนะบอสแล้ว');
    logger.emit('sb_boss_clear', {
      roundIndex: round.index
    });
  } else {
    ui.setCue('TIME UP', 'miss');
    ui.setCoachText('บอสยังไม่หมด ลองใหม่รอบหน้า');
...
function spawnBossExpected(state, ui, logger, round) {
  ...
  const expected = {
    ...
    expiresMs: timeoutWindowMs(state.cfg.intensity, 'boss'),
    spawnAtMs: nowMs(),
    expiresAtMs: nowMs() + timeoutWindowMs(state.cfg.intensity, 'boss'),
    spawned: true
  };
  ...
  ui.setStageText(`BOSS • ${expected.label}`);
  ...
  root.innerHTML = `
    <style>
      ...
      .sb-target{
        ...
        animation:sbTargetPop .18s ease-out;
      ...
      .sb-target-ring{
        ...
        animation:sbRingShrink var(--ring-ms, 1100ms) linear forwards;
      }
      .sb-target-core{
        position:absolute;
        inset:0;
      ...
      .sb-target-hit{
        animation:sbTargetHit .18s ease-out forwards;
      }
      .sb-target-miss{
        animation:sbTargetMiss .2s ease-out forwards;
      }
      .sb-cue{
        position:absolute;
        left:50%;
      ...
      @keyframes sbTargetHit{
        ...
        to{ opacity:0; filter:brightness(1.6); transform:translate(-50%,-50%) scale(1.18); }
      }
      @keyframes sbTargetMiss{
        from{ opacity:1; }
        to{ opacity:0; transform:translate(-50%,-50%) scale(.86); }
      }
...
function bindUiRefs(root) {
  ...
  return {
    ...
    hitTarget() {
      if (!targetLayer) return;
      const el = targetLayer.querySelector('.sb-target');
    ...
    missTarget() {
      ...
      el.classList.add('sb-target-miss');
