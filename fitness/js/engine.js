/* ==========================================================
   Shadow Breaker — Research Edition v3 (2025-11-24)
   - ใช้กับ shadow-breaker.html + engine.js v3 + dom-renderer.js
   - เป้าแตก / คะแนนเด้งตรงเป้า / รองรับ mobile
   ========================================================== */
...
/* main.wrap มาจาก rhythm-boxer.css อยู่แล้ว
   ที่นี่เพิ่มแค่ให้เต็มสูงจอมากขึ้น */
main.wrap {
  min-height: 100vh;
  display: flex;
  align-items: stretch;
...
.sb-field {
  ...
  overflow: hidden;
  /* ให้ใหญ่พอบนมือถือ/จอเล็ก */
  min-height: 320px;
  height: min(68vh, 540px);
}

/* ---------- Targets & Effects (ใช้กับ dom-renderer.js) ---------- */

/* เป้า (DOMRenderer จะตั้ง left/top + width/height + margin-+ ให้แล้ว) */
.sb-target {
  position: absolute;
  border-radius: 999px;
  display: flex;
  ...
  animation: sb-target-in 0.14s ease-out forwards;
}

/* border สีตาม diff */
.sb-wrap[data-diff="easy"]   .sb-target { border-color: #22c55e; }
.sb-wrap[data-diff="normal"] .sb-target { border-color: #38bdf8; }
.sb-wrap[data-diff="hard"]   .sb-target { border-color: #f97316; }

/* bomb/decoy (ถ้า dom-renderer ตั้ง data-type="bad") */
.sb-target[data-type="bad"] {
  background: radial-gradient(circle at 30% 20%, #fee2e2, #fecaca);
  border-color: #ef4444;
}

/* boss face ให้เด่น (ถ้า dom-renderer ตั้ง data-boss-face="1") */
.sb-target[data-boss-face="1"] {
  border-width: 5px;
  box-shadow:
    0 0 0 2px rgba(250,250,250,0.8),
    0 16px 40px rgba(148,27,128,0.9);
}

/* ตอนโดนตี engine → dom-renderer.addClass('sb-target-hit') */
.sb-target.sb-target-hit {
  animation: sb-target-hit 0.22s ease-out forwards;
}

@keyframes sb-target-in {
  0%   { opacity: 0; transform: scale(0.6); }
  100% { opacity: 1; transform: scale(1);   }
}

@keyframes sb-target-hit {
  0%   { opacity: 1; transform: scale(1);   }
  60%  { opacity: 1; transform: scale(1.3); filter: brightness(1.1); }
  100% { opacity: 0; transform: scale(0.3); }
}

/* popup คะแนนจาก dom-renderer.spawnHitEffect → class="sb-hit"
   DOMRenderer จะ set left/top ให้อยู่กลางเป้าแล้ว append เข้า .sb-field */
.sb-hit {
  position: absolute;
  ...
  white-space: nowrap;
  color: #bbf7d0;
  animation: sb-pop-score 0.6s ease-out forwards;
}

/* ถ้า dom-renderer ใส่คลาสเพิ่ม เช่น perfect/good/miss/decoy */
.sb-hit.perfect { color: #fde68a; }
.sb-hit.good    { color: #bbf7d0; }
.sb-hit.miss    { color: #fecaca; }
.sb-hit.decoy   { color: #fecaca; }

@keyframes sb-pop-score {
  0%   { opacity: 0; transform: translate(-50%, -40%); }
  20%  { opacity: 1; transform: translate(-50%, -60%); }
  100% { opacity: 0; transform: translate(-50%, -95%); }
}

/* hit particle (ใช้กับ js/particle.js → .hitParticle) */
.hitParticle {
  position: absolute;
  transform: translate(-50%, -50%);
  font-size: 32px;
  pointer-events: none;
  animation: sb-particle 0.48s ease-out forwards;
}
