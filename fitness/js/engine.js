// === js/engine.js — Shadow Breaker core (2025-12-03, HUD+FX + Miss rules) ===
'use strict';
...
export function initShadowBreaker() {
  ...
  function spawnTargetOfType(kind, extra) {
    ...
    data.timeoutHandle = setTimeout(() => {
      ...
      const isRealMiss = data.type === 'normal'; // bomb/decoy/heal/shield = ไม่ใช่ miss
      ...
      if (isRealMiss) {
        ...
        // ไม่ใช่ miss เผื่อดูใน CSV ว่ามี timeout แต่ไม่ใช่เป้าจริง
  ...
  function gameLoop(now) {
    ...
    for (const t of nowTargets) {
      if (now >= t.timeoutAt) {
        ...
        const isRealMiss = t.type === 'normal';
