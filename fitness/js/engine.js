// === Shadow Breaker — engine.js (2025-11-20 Production, 4 Boss, FEVER, DomRenderer) ===
'use strict';
...
const DIFF = {
  easy: {
    ...
    spawnBaseMs: 900,
    targetLifeMs: 1100,
    bossHP: [25, 32, 40, 50],   // ปรับให้เห็นบอสครบง่าย
    dmgPerHit: 6,
    scoreHit: 10,
    scoreDecoy: -18,
    hpLossOnMiss: 4,
...
const game = {
  ...
  rounds: 0,            // ใช้บันทึก summary เฉพาะเกมนี้
...
function updateBossHUD(){
  ...
  // อัปเดตข้อมูลบน intro overlay ด้วย
...
function endGame(reason){
  ...
  if (game.els.resRTDecoy)
    game.els.resRTDecoy.textContent  = (avgDecoy  != null ? avgDecoy.toFixed(0)  : '-') + (avgDecoy!=null?' ms':'');
