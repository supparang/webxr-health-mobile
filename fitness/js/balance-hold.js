...
/* Difficulty */
const GAME_DIFF = {
  easy:   { safeHalf:0.29,  disturbMinMs:1200, disturbMaxMs:2050, disturbStrength:0.26, passiveDrift:0.100, hp:4 },
  normal: { safeHalf:0.17,  disturbMinMs:760,  disturbMaxMs:1320, disturbStrength:0.40, passiveDrift:0.165, hp:3 },
  hard:   { safeHalf:0.135, disturbMinMs:620,  disturbMaxMs:1080, disturbStrength:0.48, passiveDrift:0.185, hp:3 }
};
...
function spawnBossWave(now, cfg, waveIndex){
  ...
  if (mode === 2){
    ...
    const steps = [-0.88,-0.50,-0.12,0.26,0.64,0.88];
    ...
    setTimeout(()=>{ if(state) state.biasDir *= -1; }, 760);
    return;
  }

  const A = [-0.84, 0.00, 0.84];
  const B = [ 0.84, 0.00,-0.84];
  const arr = (mode===0)?A:B;

  popText(`⚡ Wave ${waveIndex+1}`);
  [0,150,300].forEach((off,i)=>{
    setTimeout(()=>{
      if(state) spawnObstacle(nowMs(), cfg, {
        kind:'shock',
        xNorm: arr[i],
  ...
  setTimeout(()=>{ if(state) state.biasDir *= -1; }, 470);
}
...
function loop(now){
  ...
  const target = state.targetAngle + antiIdlePush + centerEscape;

  const lerp = (stage === 'Warm') ? 0.12 : (stage === 'Trick' ? 0.13 : 0.14);
  const effectiveLerp = lerp * (idleBoost > 0.5 ? 0.92 : 1.0);
...
function spawnObstacle(now, cfg, opt){
  ...
  setTimeout(()=>{
    ...
    if (inSafe){
      ...
      const knockBase = cfg.disturbStrength * (kind === 'gust' ? 1.08 : (kind === 'bomb' ? 1.32 : 1.72));
