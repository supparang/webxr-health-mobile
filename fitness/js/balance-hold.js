// === /webxr-health-mobile/fitness/js/balance-hold.js ===
// Balance Hold — DOM-based Balance Platform + Obstacle Avoidance
// FULL BUILD (T) — includes A–S
// PATCH v20260304a-BH-RESULTLOCK-NODOUBLESTOP
// ✅ Guard stopGame double-call
// ✅ Result-first hard lock: no navigation to HUB unless user presses Back HUB
'use strict';
...
/* ✅ PATCH: stop guard */
let hasEnded = false;
/* ✅ PATCH: allow leaving result only by user action */
...
function goHubOrMenu(){
  allowNavigate = true; // ✅ only now allow navigation
...
function stopGame(endedBy){
  ...
  // open modal on timeout (still no redirect)
  if (endedBy !== 'manual'){
 ...
 * ------------------------------------------------------------ */
function init(){
  // ✅ Result-first hard lock: prevent accidental unload while viewing result (unless user pressed Back HUB)
  window.addEventListener('beforeunload', (e)=>{
    const isResult = viewResult && !viewResult.classList.contains('hidden');
    if (isResult && !allowNavigate){
      e.preventDefault();
      e.returnValue = '';
      return '';
    }
