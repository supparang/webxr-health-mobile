...
if(window.BH_UNIFIED_POSE_COMPLETION_V66)return;
const s=BH.state,e=BH.el,C=BH.clamp||((v,a,b)=>Math.max(a,Math.min(b,v)));
const RELEASE='20260808-BALANCE-UNIFIED-POSE-COMPLETION-V66';
let committing=false,lastCommittedIndex=-1;
...
function coherent(){
  ...
  // results are the authority for completed-pose count.
...
function synthesizeResult(ev,req,indexBefore){
  ...
  const n=Math.max(1,Number(a.samples)||0);
...
BH.completePose=(ev,req)=>{
  ...
  try{
    ...
    if(done>=before.total){
      ...
      try{BH.setPoseUI?.()}catch(err){console.warn('[Balance V66] setPoseUI fallback',err)}
    }
    lastCommittedIndex=done-1;
    syncHud();
...
BH.updateGameUI=(ev,p)=>{
  ...
  const nearComplete=ev?.valid===true&&(Number(p)>=.99||Number(s.holdMs||0)>=req-80);
  if(nearComplete&&lastCommittedIndex!==st.done){
...
if(typeof baseReset==='function')BH.resetRoundState=()=>{committing=false;lastCommittedIndex=-1;const r=baseReset();syncHud();return r};

window.BH_UNIFIED_POSE_COMPLETION_V66={release:RELEASE,requiredMs,coherent,syncHud};
document.documentElement.dataset.bhUnifiedPoseCompletion='v66';
console.info('[BalanceHold] Unified Pose Completion V66 ready',RELEASE);
})();
