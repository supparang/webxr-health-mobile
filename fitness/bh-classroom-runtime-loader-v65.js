(()=>{
'use strict';
const RELEASE='20260908-BALANCE-CLASSROOM-RUNTIME-LOADER-V67-SIXTH-POSE-RECOVERY';
...
function load(src,id,onload){if(document.getElementById(id)){onload?.();return}const script=document.createElement('script');script.id=id;script.src=src;script.async=false;if(onload)script.onload=onload;script.onerror=()=>{console.error('[BalanceHold V67 physical] failed to load',src);onload?.()};document.head.appendChild(script)}
if(directSmoke){document.documentElement.dataset.bhLegacyStack='skipped-direct-smoke-v67-physical';console.info('[BalanceHold] legacy runtime stack skipped for direct smoke',RELEASE);return}
...
console.info('[BalanceHold] Classroom V67 physical runtime loader ready',RELEASE);
})();