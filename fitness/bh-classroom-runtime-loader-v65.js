(()=>{
'use strict';
const RELEASE='20260908-BALANCE-CLASSROOM-RUNTIME-LOADER-V66-FIREBASE-DIRECT-RECEIPT';
...
function load(src,id,onload){if(document.getElementById(id)){onload?.();return}const script=document.createElement('script');script.id=id;script.src=src;script.async=false;if(onload)script.onload=onload;script.onerror=()=>{console.error('[BalanceHold V66 physical] failed to load',src);onload?.()};document.head.appendChild(script)}
if(directSmoke){document.documentElement.dataset.bhLegacyStack='skipped-direct-smoke-v66-physical';console.info('[BalanceHold] legacy runtime stack skipped for direct smoke',RELEASE);return}
function loadFirebaseDirectReceipt(){load('./bh-firebase-direct-receipt-v66.js?v=20260908-v66','bh-firebase-direct-receipt-v66')}
function loadCanonicalSummaryUi(){load('./bh-summary-ui-canonical-v57.js?v=20260818-v57','bh-summary-ui-canonical-v57',loadFirebaseDirectReceipt)}
...
console.info('[BalanceHold] Classroom V66 physical runtime loader ready',RELEASE);
})();