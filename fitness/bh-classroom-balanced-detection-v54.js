...
const RELEASE='20260808-BALANCE-CLASSROOM-BALANCED-DETECTION-V54';
...
BH.evaluatePose=(lm,key)=>{
  ...
  const poseFloor=boss?62:Math.max(cfg.poseThreshold-2,level==='easy'?64:level==='normal'?69:74);
  const safeFloor=boss?58:Math.max(cfg.safeThreshold-3,level==='easy'?47:level==='normal'?52:57);
  ...
  r.valid=valid;
  r.threshold=Math.max(Number(r.threshold||0),poseFloor);
  r.safeThreshold=Math.max(Number(r.safeThreshold||0),safeFloor);
  r.requiredStability=stabilityFloor;
  r.requiredControl=controlFloor;
  r.requiredConfidence=confidenceFloor;
  r.balancedDetection=true;
  r.balancedDetectionVersion=RELEASE;
  if(!valid){
...
if(typeof BH.calcSummary==='function'){
  ...
  BH.calcSummary=reason=>{
    ...
    x.balanceDetectionVersion=RELEASE;
    x.balanceDetectionProfile='grade5-balanced';
    x.holdProfile='easy:2200|normal:2800|hard:3300';
    x.poseThresholdProfile='easy:64|normal:69|hard:74|boss:62';
    x.stabilityThresholdProfile='easy:52|normal:56|hard:60|advanced:58/62/66|boss:58';
    x.controlThresholdProfile='easy:46|normal:50|hard:54|boss:50';
    return x;
  };
}

window.HH_BALANCE_DETECTION_V54={release:RELEASE,profile:'grade5-balanced'};
console.info('[BalanceHold] final Grade 5 balanced detection policy ready',RELEASE);
})();