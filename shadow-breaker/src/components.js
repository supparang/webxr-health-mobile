/*  Shadow Breaker – 4 Modes: combat / dash / impact / flow
    - Unified i18n (src/i18n.json)
    - Combat = เวอร์ชัน B
    - Dash  = Hyper + M2 warnings
    - Impact = รองรับ 5 โหมด A–E (count|timed|boss|endless|fitness)
    - Flow  = Rhythm A/S/D
*/
(function(){
  ...
  function tr(dict, key, fallback, vars){ let s=(dict&&dict[key])||fallback||key; if(vars){ for(const k in vars){ s=s.replaceAll('{'+k+'}', String(vars[k])); } } return s; }

  // SFX
  const SFX={ctx:null, beep(f=880,d=.08,v=.2){try{this.ctx=this.ctx||new (window.AudioContext||window.webkitAudioContext)();const o=this.ctx.createOscillator(),g=this.ctx.createGain();o.type='square';o.frequency.value=f;g.gain.value=v;o.connect(g);g.connect(this.ctx.destination);o.start();o.stop(this.ctx.currentTime+d);}catch(e){}}, hit(){this.beep(920,.05,.2)}, ok(){this.beep(660,.07,.18)}, warn(){this.beep(160,.12,.25)}, crit(){this.beep(1400,.06,.25)} };
  ...
  // Warning (Dash M2)
  function ensureWarnDOM(){ let w=document.getElementById('warnWrap'); if(!w){ w=document.createElement('div'); w.id='warnWrap'; w.className='warn-wrap'; w.innerHTML=`<div id="warnL" class="warn-arrow" style="visibility:hidden">← DASH</div><div id="warnU" class="warn-arrow" style="visibility:hidden">JUMP ↑</div><div id="warnD" class="warn-arrow" style="visibility:hidden">DUCK ↓</div><div id="warnR" class="warn-arrow" style="visibility:hidden">DASH →</div>`; document.body.appendChild(w); const mid=document.createElement('div'); mid.id='warnMid'; mid.className='warn-mid'; mid.innerHTML=`<div id="warnShift" class="warn-arrow" style="visibility:hidden">SHIFT – BURST!</div>`; document.body.appendChild(mid);} }
  function showWarn(which,ms=800){ ensureWarnDOM(); const el={left:document.getElementById('warnL'),right:document.getElementById('warnR'),up:document.getElementById('warnU'),down:document.getElementById('warnD'),burst:document.getElementById('warnShift')}[which]; if(!el) return; el.style.visibility='visible'; el.classList.add('flash'); setTimeout(()=>{ el.style.visibility='hidden'; el.classList.remove('flash'); },ms); }

  // Difficulty
  const DIFF={ easy:{spawnMs:950,lifeMs:3800,timedSec:75,missHP:0,bossHit:3, clickMissHP:0}, normal:{spawnMs:800,lifeMs:3200,timedSec:60,missHP:3,bossHit:6, clickMissHP:1}, hard:{spawnMs:650,lifeMs:2800,timedSec:50,missHP:5,bossHit:10, clickMissHP:2} };
  ...
  AFRAME.registerComponent('shadow-breaker-game',{
    init:function(){
      ...
      // Impact submodes A–E
      this.ibMode=(q.get('ib')||'count').toLowerCase(); // count|timed|boss|endless|fitness
      const IB_DEFAULTS={ count:{targetCores:10}, timed:{seconds:60}, boss:{preCores:5,bossHP:600}, endless:{}, fitness:{reps:50,minHold:0.3} };
      this.ibCfg=IB_DEFAULTS[this.ibMode]||IB_DEFAULTS.count;
      ...
      fetch('src/i18n.json').then(r=>r.json()).then(data=>{
        ...
        this.st={ playing:false, timeLeft:(this.mode==='timed')?this.cfg.timedSec:9999, score:0, combo:1, arcane:0, overload:0, hp:100, last:performance.now(), spawnEveryMs:this.cfg.spawnMs, spawnTimer:performance.now()-this.cfg.spawnMs-1, dmgCD:0, olTick:0, idleTimer:0, recoverCD:0,
          // dash
        ...
        window.addEventListener('keydown',(e)=>{
          ...
          if(this.game==='impact' && (e.key==='m'||e.key==='M')){
            ...
            this.ibMode=order[(i+1)%order.length];
            this.st.cores=0; this.st.reps=0; this.st.bossHP=0; this.st.bossAlive=false;
    ...
    updateHUD:function(){
      ...
      const modeTimed=(this.mode==='timed');
      $('hudScore') && ($('hudScore').textContent=`${L('score','Score')}: ${s.score}`);
      $('hudCombo') && ($('hudCombo').textContent=`${L('combo','Combo')}: x${s.combo}`);
      $('hudTime')  && ($('hudTime').textContent = `${L('time','Time')}${modeTimed?' ⏱':''}: ${modeTimed?Math.ceil(s.timeLeft):'∞'}`);
    ...
    loop:function(){
      ...
      this.st.idleTimer=(this.st.idleTimer||0)+dt; if(this.st.idleTimer>2 && this.st.overload>0){ this.st.overload=Math.max(0,this.st.overload-0.35*dt); }
    ...
    tickCombat:function(now,dt){
      ...
      if(this.st.phase!=='boss' && this.st.score>=needScore){ this.st.phase='boss'; this.spawnMiniBoss(); }
    },
    spawnTarget:function(){
      ...
      e.addEventListener('click',()=>{ if(!this.st.playing) return; this.st.combo=Math.min(20,this.st.combo+1); const add=100+(this.st.combo-1)*10; this.st.score+=add; this.st.arcane=Math.min(100,this.st.arcane+3); this.st.overload=Math.min(130,this.st.overload+0.9); this.st.idleTimer=0; if(this.st.boss){ this.damageBoss(3); } SFX.hit(); e.remove(); this.updateHUD(); });
      setTimeout(()=>{ if(e.parentNode){ e.remove(); this.st.combo=1; if(this.diff!=='easy') this.takeDamage(this.cfg.missHP, T(this.dict,'miss','Miss')); this.st.overload=Math.max(0,this.st.overload-0.6); this.updateHUD(); } }, this.cfg.lifeMs);
      sp.appendChild(e);
    },
    spawnMiniBoss:function(){
      ...
      const atk=()=>{ if(!this.st.boss||!this.st.playing||this.st.hp<=0) return; const dmg=this.cfg.bossHit + Math.floor(this.st.overload/50); this.takeDamage(dmg,T(this.dict,'bossAttack','Boss attack')); if(this.st.hp>0 && this.st.boss) setTimeout(atk,1800); };
      setTimeout(atk,1200);
    },
    damageBoss:function(amount){ if(!this.st.boss) return; this.st.boss.hp-=amount; if(this.st.boss.hp<=0){ try{this.st.boss.el.remove();}catch(_){ } this.st.boss=null; toast(T(this.dict,'missionClear','Mission Clear')); } this.updateHUD(); },
    manualRay:function(evt){
      const sceneEl=qs('a-scene'), camEl=$('camera'); const renderer=sceneEl&&sceneEl.renderer, camera=camEl&&camEl.getObject3D('camera'); if(!renderer||!camera||!this.st.playing) return;
      const rect=renderer.domElement.getBoundingClientRect(); const mouse=new THREE.Vector2(((evt.clientX-rect.left)/rect.width)*2-1,-((evt.clientY-rect.top)/rect.height)*2+1);
      const raycaster=new THREE.Raycaster(); raycaster.setFromCamera(mouse,camera);
      const nodes=[...document.querySelectorAll('.clickable')].map(el=>el.object3D).filter(Boolean);
      const hits=raycaster.intersectObjects(nodes,true); if(hits.length){ let obj=hits[0].object; while(obj && !obj.el) obj=obj.parent; if(obj&&obj.el) obj.el.emit('click'); }
    },
    showCombatTutorial:function(){
      try{ const el=document.createElement('div'); el.id='tutorialTip'; el.style.position='fixed'; el.style.left='50%'; el.style.top='14%'; el.style.transform='translateX(-50%)'; el.style.zIndex='10010'; el.style.background='rgba(0,0,0,.65)'; el.style.border='1px solid rgba(255,255,255,.15)'; el.style.padding='10px 14px'; el.style.borderRadius='12px'; el.style.color='#cfefff'; el.style.fontWeight='700'; el.style.letterSpacing='.3px'; el.textContent=T(this.dict,'missionStart','Mission Start: Tutorial'); document.body.appendChild(el); setTimeout(()=>{ el.style.opacity='0'; },18000); setTimeout(()=>{ try{el.remove();}catch(_){ } },20000);}catch(_){}
    },
    ...
    tickDash:function(now,dt){
      ...
      const rig=$('#rig'); if(rig){ const baseY=1.6; const y=(this.st.jumpT>0)?baseY+0.42*Math.sin((0.6-this.st.jumpT)/0.6*Math.PI):(this.st.crouch?baseY-0.38:baseY); const x=(this.st.side!==0?this.st.side*0.6*(this.st.sideT/0.35):0)+(this.st.burst>0?(this.st.side||1)*0.25:0); rig.setAttribute('position',`${x.toFixed(3)} ${y.toFixed(3)} 0`); }
      const spawnGap=Math.max(520,this.cfg.spawnMs); if(now-this.st.spawnTimer>spawnGap){ this.st.spawnTimer=now; this.spawnDashObstacle(now); }
      document.querySelectorAll('.hazard').forEach(h=>{
        const z=Number(h.dataset.z||-3.5); const speed=2.4; const nz=z+speed*dt*(this.st.burst>0?1.25:1.0);
        h.dataset.z=nz; h.setAttribute('position',`${h.dataset.x||0} ${h.dataset.y||0} ${nz.toFixed(3)}`);
        if(!h.dataset.warned && nz>-2.6){ h.dataset.warned='1'; const t=h.dataset.type; if(t==='low')showWarn('up',700); if(t==='high')showWarn('down',700); if(t==='leftBeam')showWarn('right',700); if(t==='rightBeam')showWarn('left',700); if(t==='shock')showWarn('burst',700); }
        if(nz>-0.2 && !h.dataset.hit){ h.dataset.hit='1'; const ok=this.checkDashPose(h.dataset.type);
    ...
    spawnDashObstacle:function(){
      const e=document.createElement('a-entity'); e.classList.add('hazard'); const r=Math.random();
      let type='mid'; if(r<0.25)type='low'; else if(r<0.5)type='high'; else if(r<0.7)type='leftBeam'; else if(r<0.9)type='rightBeam'; else type='shock';
      e.dataset.type=type; let x=0,y=0,geo='',mat='';
      ...
      e.dataset.x=x; e.dataset.y=y; e.dataset.z=-3.5; e.setAttribute('geometry',geo); e.setAttribute('material',mat); e.setAttribute('position',`${x} ${y} -3.5`);
      e.setAttribute('animation__pulse','property: scale; dir: alternate; loop:true; dur:600; to:1.08 1.08 1.08'); qs('#spawner').appendChild(e);
    },
    ...
    spawnImpactCore:function(){
      ...
      if(this.ibMode==='boss' && this.st.bossAlive){ try{e.remove();}catch(_){ } this._impactAlive=false; } // ไม่ซ้อนแกนตอนบอสอยู่
    },
    releaseImpact:function(){
      if(this.ibMode!=='boss' && !this._impactAlive) return;
      const e=qs('.impact-core'); const held=this.st.chargeStart>0?(performance.now()-this.st.chargeStart)/1000:0; this.st.chargeStart=0;
      const clamped=Math.max(0,Math.min(2,held)); const dmg=Math.round(200*clamped); const olGain=1.2*(clamped/2); const good=clamped>=0.1;
      ...
      if(this.ibMode==='boss' && this.st.bossAlive){
        const dealt=Math.max(20,Math.round(dmg*0.8));
        this.st.bossHP=Math.max(0,this.st.bossHP-dealt);
        this.st.score+=Math.round(dealt*0.4);
        this.st.overload=Math.min(130,this.st.overload+olGain); this.st.idleTimer=0;
        $('hudBoss').textContent=`${T(this.dict,'bossHP','Boss HP')}: ${this.st.bossHP}`;
      ...
      if(good){
        ...
        setTimeout(()=>this.spawnImpactCore(),700); this.updateHUD();
      }
    },
    summonImpactBoss:function(){
      this.st.bossAlive=true; this.st.bossHP=(this.ibCfg.bossHP||600);
      const b=document.createElement('a-entity');
      b.setAttribute('geometry','primitive: icosahedron; radius: 0.9');
      ...
      qs('#spawner').appendChild(b);
      toast(tr(this.dict,'ibBossSummoned','Boss summoned!'),1200);
      $('hudBoss').textContent=`${T(this.dict,'bossHP','Boss HP')}: ${this.st.bossHP}`;
    },
    ...
    tickFlow:function(now,dt){
      ...
      document.querySelectorAll('.flow-note').forEach(n=>{ const born=Number(n.dataset.t0||0); const age=now-born; const life=900; const y=Math.min(100,(age/life)*100); n.style.top=(10+y*0.6)+'%'; if(age>life+200){ n.remove(); this.st.combo=1; this.takeDamage(0.5,'Late'); }});
    },
    spawnFlowNote:function(lane,now){
      let wrap=$('#flowWrap'); if(!wrap){ wrap=document.createElement('div'); wrap.id='flowWrap'; wrap.style.position='fixed'; wrap.style.left='50%'; wrap.style.transform='translateX(-50%)'; wrap.style.top='12%'; wrap.style.zIndex='10006'; wrap.style.pointerEvents='none'; wrap.style.width='360px'; wrap.innerHTML=`<div style="display:flex;gap:14px;justify-content:center"><div class="badge">A</div><div class="badge">S</div><div class="badge">D</div></div>`; document.body.appendChild(wrap); }
      const n=document.createElement('div'); n.className='flow-note badge'; n.dataset.t0=String(now); n.dataset.key=lane; n.style.position='absolute'; n.style.left=lane==='A'?'20%':(lane==='S'?'45%':'70%'); n.style.top='10%'; n.style.background='rgba(0,0,0,.55)'; n.style.border='1px solid #49b'; n.textContent=lane; wrap.appendChild(n);
    ...
    checkFlowHit:function(key){
      ...
      notes.sort((a,b)=>parseFloat(b.style.top)-parseFloat(a.style.top)); const n=notes[0]; const now=performance.now(); const born=Number(n.dataset.t0||0);
      const age=now-born, life=900, delta=Math.abs(age-life);
      ...
      else{ this.st.combo=1; this.takeDamage(0.3,'Off-beat'); this.st.overload=Math.max(0,this.st.overload-0.2); SFX.warn(); this.updateHUD(); }
    },
