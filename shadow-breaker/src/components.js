  ...
  AFRAME.registerComponent('shadow-breaker-game',{
    init:function(){
      ...
      fetch('src/i18n.json').then(r=>r.json()).then(data=>{
        ...
        this.st={playing:false,timeLeft:(this.mode==='timed')?60:9999,score:0,combo:1,arcane:0,last:performance.now(),spawnEveryMs:1000,spawnTimer:performance.now()};
        this.updateHUD();
        this.startGame();
      });
    ...
    updateHUD:function(){
      const s=this.st;
      const scoreLabel = (this.dict['score']||'Score');
      const comboLabel = (this.dict['combo']||'Combo');
      const timeLabel  = (this.dict['time']||'Time');
      const arcaneLabel= (this.dict['arcane']||'Arcane');
      const backLabel  = (this.dict['back']||'Back to Menu');
      document.getElementById('hudScore').textContent = `${scoreLabel}: ${s.score}`;
      document.getElementById('hudCombo').textContent = `${comboLabel}: x${s.combo}`;
      document.getElementById('hudTime').textContent  = `${timeLabel}: ${this.mode==='timed'?Math.ceil(s.timeLeft):'∞'}`;
      document.getElementById('hudArcane').textContent= `${arcaneLabel}: ${s.arcane}%`;
      const back = document.getElementById('backLink'); if(back) back.textContent = backLabel;
    },
    loop:function(){
