  ...
  AFRAME.registerComponent('shadow-breaker-game',{
    ...
    init:function(){
      ...
      fetch('src/i18n.json').then(r=>r.json()).then(data=>{
        ...
        this.updateHUD(); this.applyOverloadFX(this.st.overload); this.startGame();
    ...
    updateHUD:function(){
      ...
      updateOverloadUI(s.overload); this.applyOverloadFX(s.overload);
    },
    ...
    loop:function(){
      ...
      this.updateHUD(); this.applyOverloadFX(this.st.overload);
    },
    ...
    spawnMiniBoss:function(){
      ...
      this.st.boss=b; this.updateHUD(); this.applyOverloadFX(this.st.overload); toast(this.dict['missionBoss']||'Mini Boss!');
    },
    damageBoss:function(amount){
      if(!this.st.boss) return; this.st.boss.hp -= amount;
      if(this.st.boss.hp<=0){ try{this.st.boss.el.remove();}catch(e){} this.st.boss=null; this.updateHUD(); this.applyOverloadFX(this.st.overload); toast(this.dict['missionClear']||'Mission Clear'); SFX.ok(); }
      else { this.updateHUD(); this.applyOverloadFX(this.st.overload); }
    },
