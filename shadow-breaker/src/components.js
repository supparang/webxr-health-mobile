// Shadow Breaker – Overload Part 1 (QB-3 Quantum Blue, BAR-C, POS-2)
(function(){
  ...
  AFRAME.registerComponent('shadow-breaker-game',{
    ...
    spawnShockwave:function(pos){
      ...
      try{
        ...
        sw.setAttribute('look-at','#camera');
    ...
    applyOverloadFX:function(val){
      const fx=document.getElementById('overlayFX'); if(!fx) return;
    ...
    startHeartbeat:function(bpm){
      try{
        ...
        setTimeout(()=>{ this.startHeartbeat(this._hbBpm||bpm); }, int(interval*1000));
      }catch(e){}
    },
    stopHeartbeat:function(){
      this._hbBpm = 0;
      // nothing else needed; the recursive timeouts will calm once <50%
    },
    // === Part 3 FX staging ===
    applyOverloadFX3:function(val){
      const canvas=document.querySelector('canvas');
      const haze=document.getElementById('heatHaze');
      if(canvas){ canvas.classList.remove('distort1','distort2'); }
      if(haze){ haze.classList.remove('stage-on','stage-strong'); }
      if(val>=50 && val<90){
        if(canvas) canvas.classList.add('distort1');
        if(haze) haze.classList.add('stage-on');
      }else if(val>=90 && val<100){
        if(canvas) canvas.classList.add('distort1');
        if(haze) haze.classList.add('stage-strong');
      }else if(val>=100){
        if(canvas) canvas.classList.add('distort2');
        if(haze) haze.classList.add('stage-strong');
      }
      // ensure aura rig exists when overloaded
    ...
    init:function(){
      ...
      fetch('src/i18n.json').then(r=>r.json()).then(data=>{
        ...
        this.updateHUD(); this._checkStages && this._checkStages(); this.applyOverloadFX(this.st.overload); this.applyOverloadFX3(this.st.overload); this.startGame();
    ...
    updateHUD:function(){
      ...
      updateOverloadUI(s.overload); this.applyOverloadFX(s.overload); this.applyOverloadFX3(s.overload);
    },
    ...
    loop:function(){
      ...
      if(this.st.idleTimer >= 3){
        this.st.overload = Math.max(0, this.st.overload - 0.5*dt); // decay 0.5/sec after 3s idle
      }
      ...
      this.updateHUD(); this._checkStages && this._checkStages(); this.applyOverloadFX(this.st.overload); this.applyOverloadFX3(this.st.overload);
    },
    ...
    // Skills quick (Pulse/Bind) + Overload integration
    openSkillWheel:function(){
      ...
      // Z2 at 100: misfire 25% + input lag 250ms
      const exec=()=>{
        if(choice.k==='pulse') this.castArcanePulse(); try{ this.spawnShockwave({x:0,y:1.4,z:-1.8}); }catch(e){}
        if(choice.k==='bind')  this.castShadowBind(); try{ this.spawnShockwave({x:0,y:1.6,z:-2}); }catch(e){}
        if(choice.k==='burst') this.castArcaneBurst(); try{ this.spawnShockwave({x:0,y:1.5,z:-2.2}); }catch(e){}
      };
      if(this.st.overload>=100){
        if(Math.random()<0.25){ setTimeout(()=>{ try{ this.castShadowBind(); try{ this.spawnShockwave({x:0,y:1.6,z:-2}); }catch(e){} }catch(e){} },200); return; }
        setTimeout(exec,250);
        this.st.arcane -= cost;
    ...
    spawnMiniBoss:function(){
      ...
      this.st.boss=b; this.updateHUD(); this._checkStages && this._checkStages(); this.applyOverloadFX(this.st.overload); this.applyOverloadFX3(this.st.overload); toast(this.dict['missionBoss']||'Mini Boss!');
    },
    damageBoss:function(amount){
      if(!this.st.boss) return; this.st.boss.hp -= amount;
      if(this.st.boss.hp<=0){ try{this.st.boss.el.remove();}catch(e){} this.st.boss=null; this.updateHUD(); this._checkStages && this._checkStages(); this.applyOverloadFX(this.st.overload); this.applyOverloadFX3(this.st.overload); toast(this.dict['missionClear']||'Mission Clear'); SFX.ok(); }
      else { this.updateHUD(); this._checkStages && this._checkStages(); this.applyOverloadFX(this.st.overload); this.applyOverloadFX3(this.st.overload); }
    },
