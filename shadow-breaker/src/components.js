/* Shadow Breaker – Mixed Signature Fitness Build (G5 Glow)
   Components: A-Frame game runtime for 4 games
   Games: combat / dash / impact / flow
   Key fixes:
   - Default mode = 'timed' (all games) so time always shows & decrements
   - HUD time forced visible (anti-hide)
   - Arcane Combat boss spawn via multi-gate (score OR kills OR assist-time)
   - Spawn pacing near time end
   - Daily quests (simple localStorage)
   - Fitness: calories & level
*/
(function(){
  // --------- helpers ---------
  ...
  const toast=(m,ms=900)=>{
    const t=$('toast'); if(!t) return;
    t.textContent=m; t.style.display='block';
    setTimeout(()=>t.style.display='none',ms);
  };

  // Minimal SFX (web audio)
  const SFX={ctx:null,
  ...
  const genDaily=()=>{
    ...
    const ex=JSON.parse(localStorage.getItem(QUEST_KEY)||'{}');
    if(ex.date===today) return ex;
  ...
  function updateQuestUI(dict,q){
    ...
    box.innerHTML=''; q.list.forEach(it=>{
      const nm=it.name[(localStorage.getItem('sb_lang')||'th')];
      const line=document.createElement('div'); line.textContent=`• ${nm} — ${it.cur}/${it.goal} ${it.done?'✓':''}`; box.appendChild(line);
    });
  }

  // Fitness estimate
  function kcalFromActivity(ptsPerSec, bodyMass=40){
    const MET = Math.min(8, 2 + ptsPerSec*0.04); // simple cap
    return (MET * bodyMass / 3600);
  }
  function levelFromScore(score){
    if(score<800) return 1; if(score<1800) return 2; if(score<3200) return 3;
    if(score<5200) return 4; if(score<8000) return 5; return 6;
  }
  ...
  AFRAME.registerComponent('shadow-breaker-game',{
    init:function(){
      ...
      // HOTFIX: default to 'timed' when unspecified
    ...
    takeDamage:function(amount,label){
      ...
      toast(`-${Math.round(amount)} HP${label?(' ('+label+')'):''}`);
      this.updateHUD();
    },
    ...
    loop:function(){
      ...
      let dt=(now-this.st.last)/1000;
      if(dt<0 || dt>0.25) dt=0.016; // clamp big frame gaps
      this.st.last=now;
    ...
    tickCombat:function(now,dt){
      ...
      // boss gates (multi-gate)
      const gates={
        easy:   { score:300, kills:8,  assistTime:18, assistScoreRatio:0.55 },
        normal: { score:360, kills:10, assistTime:18, assistScoreRatio:0.60 },
        hard:   { score:420, kills:12, assistTime:20, assistScoreRatio:0.65 }
      }[this.diff] || { score:360, kills:10, assistTime:18, assistScoreRatio:0.60 };

      // slight spawn acceleration when time is running out
      if(this.mode==='timed' && this.st.timeLeft <= gates.assistTime+10){
        this.st.spawnEveryMs = Math.max(520, this.cfg.spawnMs*0.88);
      }
      ...
      if(this.st.phase!=='boss' && (scoreGate || killGate || assistGate)){
        this.st.phase='boss';
        this.spawnMiniBoss();
      }
    },
    ...
    spawnMiniBoss:function(){
      ...
      this.st.boss=b;
      toast(T(this.dict,'missionBoss','Mini Boss!'));
    ...
    damageBoss:function(amount){
      ...
      if(this.st.boss.hp<=0){
        ...
        // เควสต์
        if(this.daily){
          const it=this.daily.list.find(x=>x.id==='boss1');
          if(it && !it.done){ it.cur=1; it.done=true; saveDaily(this.daily); }
        }
      }
      this.updateHUD();
    },
    ...
    showCombatTutorial:function(){
      try{
        const el=document.createElement('div'); el.id='tutorialTip';
    ...
    tickDash:function(now,dt){
      ...
      document.querySelectorAll('.hazard').forEach(h=>{
        ...
        if(!h.dataset.warned && nz>-2.6){ h.dataset.warned='1'; const t=h.dataset.type;
          if(t==='low')showWarn('up',700); if(t==='high')showWarn('down',700);
          if(t==='leftBeam')showWarn('right',700); if(t==='rightBeam')showWarn('left',700); if(t==='shock')showWarn('burst',700);
        }
        if(nz>-0.2 && !h.dataset.hit){
          h.dataset.hit='1';
          const ok=this.checkDashPose(h.dataset.type);
          if(ok){
            this.st.score+=150; this.st.combo=Math.min(20,this.st.combo+1);
            this.st.overload=Math.min(130,this.st.overload+1.0); this.st.idleTimer=0; SFX.ok();
          }else{
            this.st.combo=1; const dmg=this.cfg.missHP+1; this.takeDamage(dmg,'Rift Hit');
            this.st.overload=Math.max(0,this.st.overload-0.5); SFX.warn();
          }
    ...
    tickImpact:function(now,dt){
      ...
      if(this.st.chargeStart>0){
        const t=(now-this.st.chargeStart)/1000;
        const pct=Math.min(100,Math.round(t/2*100));
    ...
    spawnImpactCore:function(){
      ...
      e.setAttribute('position','0 1.3 -2.2');
      e.classList.add('impact-core');
      this._impactAlive=true;
      qs('#spawner').appendChild(e);
      if(this.ibMode==='boss' && this.st.bossAlive){ try{e.remove();}catch(_){ } this._impactAlive=false; }
    },

    releaseImpact:function(){
      const e=qs('.impact-core');
      ...
      const dmg=Math.round(200*clamped);
      const olGain=1.1*(clamped/2);
      const good=clamped>=0.12;
      ...
      if(good){
        ...
        if(e){ try{e.remove();}catch(_){ } }
        this._impactAlive=false; this.st.cores++;
        ...
        setTimeout(()=>this.spawnImpactCore(),650);
        this.updateHUD();
      }
    },
    ...
    summonImpactBoss:function(){
      ...
      b.setAttribute('position','0 1.6 -2.4');
      b.id='impact-boss';
    ...
    killImpactBoss:function(){
      const b=$('impact-boss'); if(b){ try{b.remove();}catch(_){ } }
      this.st.bossAlive=false; this.st.bossHP=0;
      toast(T(this.dict,'missionClear','Mission Clear'),1200);
    },

    finishImpact:function(){
      this.st.playing=false;
      const msg=tr(this.dict,'ibFinished','Finished • Score: {SCORE}',{SCORE:this.st.score});
      toast(msg,2200);
      this.updateHUD();
    },
    ...
    tickFlow:function(now,dt){
      ...
      if(now>=this.st.flowNext){
        ...
        this.spawnFlowNote(L,now); this.st.flowNext=now+900;
      }
      document.querySelectorAll('.flow-note').forEach(n=>{
        const born=Number(n.dataset.t0||0); const age=now-born; const life=900;
        const y=Math.min(100,(age/life)*100); n.style.top=(10+y*0.6)+'%';
        if(age>life+200){ n.remove(); this.st.combo=1; this.takeDamage(0.4,'Late'); }
      });
    },
    ...
    spawnFlowNote:function(lane,now){
      ...
      Object.assign(n.style,{position:'absolute',left:lane==='A'?'20%':(lane==='S'?'45%':'70%'),top:'10%',background:'rgba(0,0,0,.55)',border:'1px solid #49b'});
      n.textContent=lane; wrap.appendChild(n);
    ...
    checkFlowHit:function(key){
      ...
      const n=notes[0];
      const now=performance.now(); const born=Number(n.dataset.t0||0);
      ...
      if(delta<=200){
        ...
        this.st.idleTimer=0; SFX.hit(); n.remove(); this.updateHUD();
      }else{
        this.st.combo=1; this.takeDamage(0.25,'Off-beat');
        this.st.overload=Math.max(0,this.st.overload-0.2); SFX.warn(); this.updateHUD();
      }
    },
    ...
    endGame:function(){
      this.st.playing=false;
      toast(`${T(this.dict,'finished','Finished')} • ${T(this.dict,'score','Score')}: ${this.st.score}`,2000);
    }
  });
})();
