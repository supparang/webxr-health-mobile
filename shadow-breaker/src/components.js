...
(function(){
  ...
  function showWarn(which,ms=800){ ensureWarnDOM();
  ...
  function updateQuestUI(dict,q){ const box=$('qList'),title=$('qTitle'); if(!box) return;
    title.textContent=T(dict,'dailyQuests','Daily Quests'); box.innerHTML='';
  ...
  kcal:0, level:1, boss:null
    ...
    spawnTarget:function(){
      ...
      e.addEventListener('click',()=>{ if(!this.st.playing) return;
        this.st.combo=Math.min(20,this.st.combo+1); const add=100+(this.st.combo-1)*10;
    ...
    showCombatTutorial:function(){
      try{ const el=document.createElement('div'); el.id='tutorialTip'; Object.assign(el.style,{position:'fixed',left:'50%',top:'14%',transform:'translateX(-50%)',zIndex:'10010',background:'rgba(0,0,0,.65)',border:'1px solid rgba(255,255,255,.15)',padding:'10px 14px',borderRadius:'12px',color:'#cfefff',fontWeight:'700',letterSpacing:'.3px'}); el.textContent=T(this.dict,'missionStart','Mission Start: Tutorial'); document.body.appendChild(el); setTimeout(()=>{el.style.opacity='0';},18000); setTimeout(()=>{ try{el.remove();}catch(_){ } },20000);}catch(_){}
    },
    ...
    tickDash:function(now,dt){
      ...
      const rig=$('#rig'); if(rig){ const baseY=1.6; const y=(this.st.jumpT>0)?baseY+0.42*Math.sin((0.6-this.st.jumpT)/0.6*Math.PI):(this.st.crouch?baseY-0.38:baseY);
    ...
    spawnDashObstacle:function(){
      ...
      e.dataset.x=x; e.dataset.y=y; e.dataset.z=-3.5; e.setAttribute('geometry',geo); e.setAttribute('material',mat);
      e.setAttribute('position',`${x} ${y} -3.5`); e.setAttribute('animation__pulse','property: scale; dir: alternate; loop:true; dur:600; to:1.08 1.08 1.08'); qs('#spawner').appendChild(e);
    },
    ...
    releaseImpact:function(){
      ...
      if(this.ibMode==='boss' && this.st.bossAlive){
        ...
        this.st.score+=Math.round(dealt*0.4); this.st.overload=Math.min(130,this.st.overload+olGain); this.st.idleTimer=0;
    ...
    tickFlow:function(now,dt){
      ...
      document.querySelectorAll('.flow-note').forEach(n=>{ const born=Number(n.dataset.t0||0); const age=now-born; const life=900; const y=Math.min(100,(age/life)*100); n.style.top=(10+y*0.6)+'%'; if(age>life+200){ n.remove(); this.st.combo=1; this.takeDamage(0.4,'Late'); }});
    },
    spawnFlowNote:function(lane,now){
      let wrap=$('#flowWrap'); if(!wrap){ wrap=document.createElement('div'); wrap.id='flowWrap'; Object.assign(wrap.style,{position:'fixed',left:'50%',transform:'translateX(-50%)',top:'12%',zIndex:'10006',pointerEvents:'none',width:'360px'}); wrap.innerHTML=`<div style="display:flex;gap:14px;justify-content:center"><div class="badge">A</div><div class="badge">S</div><div class="badge">D</div></div>`; document.body.appendChild(wrap); }
