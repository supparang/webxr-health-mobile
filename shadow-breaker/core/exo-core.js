...
window.EXO=(function(){
  ...
  function beep(freq=520,dur=0.06,vol=0.15){
    AC.ensure(); if(!AC.ctx) return;
    const c=AC.ctx; const o=c.createOscillator(), g=c.createGain();
  ...
  function startOverlay(onStart){
    ...
    wrap.innerHTML=`<div class="panel"><div class="title">EXO TRAINING PROTOCOL</div><p class="note">Tap/Click to start · Audio unlock · Enter WebXR anytime</p><div style="display:flex;gap:.6rem;justify-content:center;margin-top:.5rem"><button id="exoStart" class="btn">▶ Start</button></div></div>`;
  ...
  function attachBasicInput({onLeft,onRight,onPause}){
    const L=()=>onLeft&&onLeft(); const R=()=>onRight&&onRight();
    ...
    window.addEventListener('keydown',e=>{
      if(e.key==='ArrowLeft'||e.key==='a') L();
      else if(e.key==='ArrowRight'||e.key==='d') R();
      else if(e.key==='Escape') onPause&&onPause();
    });
