...
// ✅ Default endpoint updated to your Apps Script Web App
// ✅ Accepts: {kind:'HHA_ROWS_V21', rows:[...]} from client logger
// ✅ Also accepts legacy single-row payloads (fallback)
// ✅ Queue + localStorage + dedup + sendBeacon/fetch keepalive
...
(function(){
  ...
  // ---------- Context (auto hydrated from URL) ----------
  const ctx = {
    sessionId: null,
    startTimeIso: null,
    projectTag: null,
  ...
  function sendBeacon(url, bodyStr){
    try{
      ...
      const blob = new Blob([bodyStr], { type:'text/plain' }); // ✅ avoid CORS preflight
  ...
  async function sendFetchKeepalive(url, bodyStr){
    try{
      await fetch(url, {
        method:'POST',
        headers:{ 'Content-Type':'text/plain' }, // ✅ avoid preflight in many cases
        body: bodyStr,
        keepalive: true,
        mode: 'cors',
        credentials: 'omit',
  ...
  ROOT.addEventListener('pagehide', ()=>{
    try{ persistMemQ(); flushAll(); }catch(_){}
  }, { passive:true });
