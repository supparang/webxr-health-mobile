...
(function(){
  ...
  (function injectCss(){
    ...
    st.textContent = `
      ...
      @keyframes hhaEndBlink{
        0%{ filter:none; }
        30%{ filter: brightness(1.14) contrast(1.06); }
        100%{ filter:none; }
      }
    `;
  ...
  function fxCelebrate(){
    ...
    if (P?.celebrate) P.celebrate();
      ...
      for(let i=0;i<7;i++){
        setTimeout(()=>fxBurst(innerWidth/2 + (Math.random()*2-1)*170, innerHeight*0.35 + (Math.random()*2-1)*90, 26 + Math.random()*42), i*55);
      }
    }
  }
  ...
  DOC.addEventListener('hha:judge', (e)=>{
    ...
    if (t === 'good'){
      ...
      if (String(d.label||'').toUpperCase().includes('WEAK')) fxPop(x,y,'WEAK!','big');
    } else if (t === 'perfect'){
      addBodyCls('fx-hit-good', 220);
      addBodyCls('fx-kick', 120);
      fxShock(x,y, 72);
      fxPop(x,y, 'PERFECT!', 'perfect big');
    } else if (t === 'bad'){
      addBodyCls('fx-hit-bad', 220);
      addBodyCls('fx-kick', 120);
      fxShock(x,y, 66);
      if (String(d.label||'').toUpperCase().includes('FAKE')) fxPop(x,y,'FAKE!','bad big');
    } else if (t === 'miss'){
