// === /herohealth/vr/mode-factory.js ===
// HHA Mode Factory — PRODUCTION (DOM spawner)
// ✅ export boot (named)
// ✅ FIX: no “controller before init”
// ✅ Supports decorateTarget(el, target)
// ✅ Supports shoot via vr-ui.js: hha:shoot {x,y,lockPx,source}
// ✅ stop() clears interval + listeners + timeouts + DOM targets (no “แว๊บๆ”)
// Notes:
// - Requires mount to be in DOM
// - Targets are absolutely positioned; mount can be fixed/relative
// - Safe-zone vars (optional): --hha-top-safe/--hha-bottom-safe/--hha-left-safe/--hha-right-safe
//   (fallback to --plate-*-safe if you prefer)

'use strict';

const WIN = window;
const DOC = document;

function seededRng(seed){
  let t = (Number(seed) || Date.now()) >>> 0;
  return function(){
    t += 0x6D2B79F5;
    let r = Math.imul(t ^ (t >>> 15), 1 | t);
    r ^= r + Math.imul(r ^ (r >>> 7), 61 | r);
    return ((r ^ (r >>> 14)) >>> 0) / 4294967296;
  };
}

function now(){
  return (performance && performance.now) ? performance.now() : Date.now();
}

function clamp(n, a, b){
  n = Number(n) || 0;
  return n < a ? a : (n > b ? b : n);
}

function readCssNum(varName, fallback=0){
  try{
    const cs = getComputedStyle(DOC.documentElement);
    const v = parseFloat(cs.getPropertyValue(varName));
    return Number.isFinite(v) ? v : fallback;
  }catch{
    return fallback;
  }
}

function readSafeVars(){
  // Prefer generic HHA vars; fallback to plate vars if set
  const top    = readCssNum('--hha-top-safe',    readCssNum('--plate-top-safe', 0));
  const bottom = readCssNum('--hha-bottom-safe', readCssNum('--plate-bottom-safe', 0));
  const left   = readCssNum('--hha-left-safe',   readCssNum('--plate-left-safe', 0));
  const right  = readCssNum('--hha-right-safe',  readCssNum('--plate-right-safe', 0));
  return { top, bottom, left, right };
}

function pickWeighted(rng, arr){
  const a = Array.isArray(arr) ? arr : [];
  if(!a.length) return { kind:'good', weight:1 };

  let sum = 0;
  for(const it of a) sum += (Number(it.weight) || 1);

  let x = rng() * sum;
  for(const it of a){
    x -= (Number(it.weight) || 1);
    if(x <= 0) return it;
  }
  return a[a.length - 1];
}

// ✅ Named export 'boot' (fixes: "does not provide an export named 'boot'")
export function boot({
  mount,
  seed = Date.now(),

  // spawn tuning
  spawnRate = 900,          // ms between spawns
  sizeRange = [44, 64],     // px [min,max]
  kinds = [{ kind:'good', weight:0.7 }, { kind:'junk', weight:0.3 }],

  // TTL tuning
  ttlMsGood = 2100,
  ttlMsJunk = 1700,

  // input
  cooldownMs = 90,
  lockPxDefault = 28,

  // callbacks
  onHit = ()=>{},
  onExpire = ()=>{},
  decorateTarget = null,     // (el, target) => void
}){
  if(!mount) throw new Error('mode-factory: mount missing');

  const rng = seededRng(seed);

  // ✅ ensure controller is initialized BEFORE any usage (fix “controller before init”)
  const controller = new AbortController();

  const state = {
    alive: true,
    lastSpawnAt: 0,
    spawnTimer: null,
    targets: new Set(),
    cooldownUntil: 0
  };

  // Ensure mount is a positioning context if needed
  try{
    const st = getComputedStyle(mount);
    if(st.position === 'static'){
      mount.style.position = 'relative';
    }
  }catch{}

  function computeSpawnRect(){
    const r = mount.getBoundingClientRect();
    const safe = readSafeVars();
    const left = r.left + safe.left;
    const top = r.top + safe.top;
    const right = r.right - safe.right;
    const bottom = r.bottom - safe.bottom;
    const w = Math.max(0, right - left);
    const h = Math.max(0, bottom - top);
    return { left, top, right, bottom, w, h };
  }

  function removeTarget(target){
    if(!target) return;
    if(target._to) { try{ clearTimeout(target._to); }catch{} }
    state.targets.delete(target);
    try{ target.el && target.el.remove(); }catch{}
  }

  function hit(target, meta){
    if(!state.alive) return;
    if(!state.targets.has(target)) return;
    removeTarget(target);
    try{
      onHit({ kind: target.kind, groupIndex: target.groupIndex, ...meta });
    }catch(err){
      console.error('[mode-factory] onHit error', err);
    }
  }

  function onShoot(e){
    if(!state.alive) return;

    const d = e.detail || {};
    const t = now();
    if(t < state.cooldownUntil) return;
    state.cooldownUntil = t + cooldownMs;

    const x = Number(d.x);
    const y = Number(d.y);
    const lockPx = clamp(Number(d.lockPx ?? lockPxDefault), 6, 120);

    if(!Number.isFinite(x) || !Number.isFinite(y)) return;

    let best = null;
    let bestDist = Infinity;

    for(const target of state.targets){
      const r = target.el.getBoundingClientRect();
      const cx = r.left + r.width / 2;
      const cy = r.top + r.height / 2;
      const dist = Math.hypot(cx - x, cy - y);
      if(dist <= lockPx && dist < bestDist){
        bestDist = dist;
        best = target;
      }
    }

    if(best) hit(best, { source: d.source || 'shoot' });
  }

  // ✅ listener auto-removed by controller.abort() in stop()
  WIN.addEventListener('hha:shoot', onShoot, { signal: controller.signal });

  function spawnOne(){
    if(!state.alive) return;

    const rect = computeSpawnRect();
    if(rect.w < 80 || rect.h < 80) return;

    const minS = Math.min(sizeRange[0], sizeRange[1]);
    const maxS = Math.max(sizeRange[0], sizeRange[1]);
    const size = Math.round(minS + rng() * (maxS - minS));

    const pad = Math.max(10, Math.round(size * 0.55));
    const x = rect.left + pad + rng() * Math.max(1, (rect.w - pad * 2));
    const y = rect.top + pad + rng() * Math.max(1, (rect.h - pad * 2));

    const chosen = pickWeighted(rng, kinds);
    const kind = (chosen && chosen.kind) ? String(chosen.kind) : 'good';

    const el = DOC.createElement('div');
    el.className = 'plateTarget';
    el.dataset.kind = kind;

    // ✅ IMPORTANT: absolute positioning (fix “เป้าไม่โผล่” ในหลายเคส)
    el.style.position = 'absolute';
    el.style.left = `${Math.round(x)}px`;
    el.style.top  = `${Math.round(y)}px`;
    el.style.width = `${size}px`;
    el.style.height = `${size}px`;
    el.style.transform = 'translate(-50%,-50%)';

    const target = {
      el,
      kind,
      bornAt: now(),
      ttlMs: (kind === 'junk') ? ttlMsJunk : ttlMsGood,
      groupIndex: Math.floor(rng() * 5), // 0..4 (game can override in decorateTarget)
      size,

      // expose rng for deterministic emoji picks
      rng,

      // internal timeout handle
      _to: null
    };

    el.__hhaTarget = target;

    // ✅ allow game to customize target content (emoji/icon/label/etc.)
    try{
      if(typeof decorateTarget === 'function') decorateTarget(el, target);
    }catch(err){
      console.warn('[mode-factory] decorateTarget error', err);
    }

    // tap/click
    el.addEventListener('pointerdown', (ev)=>{
      ev.preventDefault();
      hit(target, { source: 'tap' });
    }, { passive:false, signal: controller.signal });

    mount.appendChild(el);
    state.targets.add(target);

    // TTL expire
    target._to = setTimeout(()=>{
      if(!state.alive) return;
      if(!state.targets.has(target)) return;
      removeTarget(target);
      try{
        onExpire({ ...target });
      }catch(err){
        console.error('[mode-factory] onExpire error', err);
      }
    }, target.ttlMs);
  }

  // main loop
  state.spawnTimer = setInterval(()=>{
    if(!state.alive) return;
    const t = now();
    if(t - state.lastSpawnAt >= spawnRate){
      state.lastSpawnAt = t;
      spawnOne();
    }
  }, 60);

  return {
    stop(){
      if(!state.alive) return;
      state.alive = false;

      try{ clearInterval(state.spawnTimer); }catch{}
      try{ controller.abort(); }catch{} // ✅ removes shoot + pointerdown listeners

      for(const target of Array.from(state.targets)){
        removeTarget(target);
      }
      state.targets.clear();
    }
  };
}