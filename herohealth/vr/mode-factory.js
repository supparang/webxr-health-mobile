...
// ✅ export boot()
// ✅ No "controller before init"
// ✅ decorateTarget(el,target)
// ✅ hha:shoot hit-test (center-lock)
// ✅ Hit meta includes {x,y,source} for UI feedback
// ✅ stop() clears timers/listeners/targets safely
...
export function boot({
  ...
  function onShoot(e){
    ...
    if(best) hit(best, { source:'shoot', x, y });
  }
  ...
  function spawnOne(){
    ...
    // ✅ important: ensure visible placement
    ...
    el.addEventListener('pointerdown', (ev)=>{
      ev.preventDefault();
      const px = Number(ev.clientX), py = Number(ev.clientY);
      hit(target, { source:'tap', x: px, y: py });
    }, { passive:false });
