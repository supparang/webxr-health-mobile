...
// Universal VR UI — SAFE UNIVERSAL — PATCH v20260304-VRUI-NOWFIX-UXSAFE
// ✅ FIX: now() defined (แก้ Uncaught ReferenceError: now is not defined)
// ✅ Prevent shooting when clicking UI buttons/links/input (กันยิงซ้อน/กันนับมั่ว)
...
(function(){
  ...
  const VIEW = String(qs('view','') || DOC.documentElement?.dataset?.view || DOC.body?.dataset?.view || '').toLowerCase();
  const IS_CVR = (VIEW === 'cvr' || VIEW === 'cardboard' || DOC.documentElement?.dataset?.view === 'cvr' || DOC.body?.dataset?.view === 'cvr');
  ...
  // ✅ ignore taps on UI / buttons / links / inputs to avoid accidental shot
  function isUiTarget(el){
    if(!el) return false;
    if(el.closest && el.closest('#hha-vrui')) return true;
    if(el.closest && el.closest('.br-btn')) return true;
    const tag = (el.tagName||'').toLowerCase();
    return tag === 'button' || tag === 'a' || tag === 'input' || tag === 'select' || tag === 'textarea' || tag === 'label';
  }
