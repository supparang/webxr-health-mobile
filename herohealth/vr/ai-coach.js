...
export function createAICoach(opts={}){
  ...
  function onUpdate(ctx){
    ...
    if (ctx.inStorm && (ctx.shield|0) > 0 && ctx.waterZone === 'GREEN'){
  ...
  function onEnd(sum){
    ...
    if (g === 'SSS' || g === 'SS'){
      say('end_top', `สุดยอด! เกรด ${g} 🏆`, `Accuracy ${acc.toFixed(1)}% • รอบหน้าลอง diff harder ได้`, 'happy');
    } else if (g === 'S' || g === 'A'){
      say('end_good', `ดีมาก! เกรด ${g} ✅`, `จุดโฟกัส: เก็บ 🛡️ ก่อนเข้าพายุ`, 'happy');
    } else {
      say('end_train', `รอบนี้เกรด ${g} ยังไหว! ซ้อมอีกนิด 💪`, `โฟกัส: อย่ารัว • ยิงให้แม่นก่อน`, 'neutral');
    }
  }
