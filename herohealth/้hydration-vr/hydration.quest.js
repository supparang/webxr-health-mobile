...
// ใช้ greenTick นับ "เวลาอยู่โซน GREEN" จริง ๆ (อัปเดตจาก hydration.safe.js)
// state ที่ MissionDeck ส่งเข้า check/prog ควรมีอย่างน้อย:
//   - score      : คะแนนรวม
//   - combo      : คอมโบปัจจุบัน (ถ้ามี)
//   - comboMax   : คอมโบสูงสุด
//   - goodCount  : จำนวนที่เก็บ "ไฮเดรตดี" สำเร็จ
//   - junkMiss   : จำนวน miss / เลือกผิด
//   - tick       : เวลารวมที่เล่นไป (วินาที) — MissionDeck.second() ดูแลเพิ่ม
//   - greenTick  : เวลาที่อยู่ในโซน GREEN สะสม (วินาที) — อัปเดตจาก hydration.safe.js

import { MissionDeck } from '../vr/mission.js';

// map state → โครงเดียวกัน ใช้ง่ายใน goal / mini
function G(s){ 
  return {
    score:    s.score      | 0,
    combo:    s.combo      | 0,
...
// ปรับ diff ให้ปลอดภัย (ถ้าไม่ได้ easy/hard ให้เป็น normal)
...
function goalsFor(diff){
  ...
  return [
...

    {
      ...
      // ใช้ pattern เดียวกับโหมดอื่น: prog เป็น quota ที่ "เหลืออยู่"
...
function minisFor(diff){
  ...
  return [
    ...
    // ✅ mini quest ด้วย greenTick: เวลา GREEN
    {
      id:'m_green12',
