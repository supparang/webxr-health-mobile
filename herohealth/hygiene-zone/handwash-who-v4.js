...
Promise.all(files.map(url=>fetch(url,{cache:'no-store'}).then(r=>{
if(!r.ok)throw new Error(`load failed ${r.status}: ${url}`);
return r.text();
}))).then(parts=>{
const source=parts.join('');
const valid=source.trimStart().startsWith('(() => {')&&source.trimEnd().endsWith('})();')&&source.includes(RUNTIME_MARKER)&&source.includes(DOM_HOOK)&&source.includes(PHASE_HOOK)&&source.length>40000;
if(!valid)throw new Error('WHO runtime integrity check failed');
let runtime=source.replace(DOM_HOOK,DOM_FIX).replace(PHASE_HOOK,PHASE_FIX);
...
script.src=blobUrl;script.async=false;script.dataset.handwashWhoRuntime=RELEASE;
script.onload=()=>{document.documentElement.dataset.handwashRuntime='ready';enableStart();URL.revokeObjectURL(blobUrl)};
script.onerror=()=>{URL.revokeObjectURL(blobUrl);showFailure('compiled runtime could not start')};
...
console.error('Handwash WHO loader',message);document.documentElement.dataset.handwashRuntime='failed';
const s=document.getElementById('detectStatus');if(s)s.textContent='โหลดเกมไม่สำเร็จ';
const b=document.getElementById('startBtn');if(b){b.disabled=false;b.textContent='แตะเพื่อลองโหลดใหม่';b.onclick=()=>location.reload()}
const t=document.getElementById('toast');if(t){t.textContent='โหลด WHO Technique ไม่สำเร็จ กรุณารีเฟรช';t.classList.add('show')}
}
function installLayout(){
const style=document.createElement('style');style.id='handwashCanonicalR3Layout';style.textContent=`
...
#waterZone{top:34%!important;left:50%!important;width:140px!important;height:108px!important}
#soapZone{left:var(--hw-side)!important;bottom:156px!important;width:126px!important;height:112px!important}
#towelZone{right:var(--hw-side)!important;bottom:156px!important;width:126px!important;height:112px!important}
.coach{top:42%!important;right:var(--hw-side)!important;width:min(300px,28vw)!important;max-height:34vh;overflow:auto}
.who-strip{min-height:48px}.phase-chip{min-width:82px!important;font-size:10px!important;line-height:1.2!important;padding:7px 8px!important}
html[data-handwash-runtime="loading"] #startBtn{opacity:.72;cursor:wait}
