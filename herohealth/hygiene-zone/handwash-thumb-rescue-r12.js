(() => {
  ...
  const RELEASE = '20260718-HANDWASH-WHO10-ATOMIC-FINISH-R18';
...
const finishNow=()=>{
if(state.phase!=='towelFaucet'||state.towelFaucetPassed)return true;
state.phaseProgress=1;
state.towelFaucetPassed=true;
coach('WHO 10 สำเร็จ ✅ กำลังสรุปผล','good');
showToast('WHO 10 ผ่านแล้ว • กำลังเปิดสรุปผล');
completeProcess(phase);
return true;
};
if(!state.waterOn){finishNow();return;}
if(inWater>=1){
state.phaseProgress+=dt/Math.max(.70,phase.targetSec*.34);
hitZone(el.waterZone);
coach(state.phaseProgress<.72?'ขั้น 2/3 กระดาษแตะก๊อกแล้ว ✅ • ค้างไว้':'ขั้น 3/3 กำลังปิดก๊อก ✅','good');
if(state.phaseProgress>=.86||finalElapsed>5.5){setWater(false);finishNow();return;}
}else if(finalElapsed>7){
setWater(false);
coach('Final Rescue ✅ ปิดก๊อกด้วยกระดาษสำเร็จ','good');
finishNow();
return;
}else{
coach('ถือกระดาษแล้วเลื่อนไปแตะกรอบก๊อกน้ำ','towel');
}
}`;
  ...
  function patchSource(source) {
    ...
    console.info('[Handwash R18] WHO10 atomic finish installed; patches=' + patchedCount);
    return patched;
  }
