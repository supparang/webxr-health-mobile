...
function restartClean(){
 ...
 const url=new URL(location.href);url.searchParams.set('replay',String(Date.now()));url.searchParams.set('v','20260729-grade5-adaptive-r40');location.replace(url.href);
}

function render(result){
 installStyle();
 const rows=Array.isArray(result?.steps)?result.steps:[];
 const completedRows=rows.filter(done),strictRows=rows.filter(row=>String(row.passMode||'')==='strict'),coachRows=rows.filter(row=>String(row.passMode||'')==='grace'),assistRows=rows.filter(row=>String(row.passMode||'')==='assist'),pendingRows=rows.filter(row=>!done(row));
 const completed=result?.completed===true||completedRows.length>=12;
 const skill=result?.skillCriteriaMet===true||result?.passed===true;
 const panel=ensurePanel();panel.className=skill?'skill':completed?'completed':'';
 const headline=skill?'🌟 ทำครบและผ่านเกณฑ์ทักษะ':completed?'🎉 ทำครบทุกขั้นแล้ว':'💪 ทำได้ดีแล้ว มาฝึกต่ออีกนิด';
 const note=skill?'ระบบยืนยันว่าทำครบด้วยตนเองและมีความแม่นยำตามเกณฑ์':completed?'ภารกิจจบแล้ว ระบบเก็บความแม่นยำจริงและขั้นที่ใช้ Adaptive Assist ไว้สำหรับการวิเคราะห์ โดยไม่บังคับให้เล่นซ้ำ':'ผลนี้ใช้เพื่อบอกว่าควรฝึกขั้นใดต่อ ไม่ได้หมายความว่าทำผิดทั้งหมด';
 ...
 const title=document.getElementById('summaryTitle'),sub=document.getElementById('summarySub'),hero=document.getElementById('summaryHero');
 ...
 if(sub)sub.textContent=completed?`ทำครบ ${completedRows.length}/${rows.length||12} ขั้น • ด้วยตนเอง ${strictRows.length} • Coach/Assist ${coachRows.length+assistRows.length} • ความแม่นยำ ${Number(result?.accuracy||0)}%`:`ทำครบ ${completedRows.length}/${rows.length||12} ขั้น • ยังเหลือ ${pendingRows.length} ขั้น • ความแม่นยำ ${Number(result?.accuracy||0)}%`;

 const list=document.getElementById('resultList');
 if(list){
  [...list.querySelectorAll('.result-row')].forEach((node,index)=>{const row=rows[index]||{},mode=String(row.passMode||'');node.dataset.grade5Status=!done(row)?'pending':mode==='assist'?'assist':mode==='grace'?'coach':'strict';const status=node.querySelector('span:last-child');if(status)status.textContent=!done(row)?'ยังไม่ได้ลองครบ':mode==='assist'?'ผ่านด้วย Adaptive Assist':mode==='grace'?'ผ่านด้วย Coach':'ทำได้ด้วยตนเอง';});
  let toggle=document.getElementById('handwashResultToggleR40');if(!toggle){toggle=document.createElement('button');toggle.id='handwashResultToggleR40';toggle.type='button';list.parentNode?.insertBefore(toggle,list);}
  list.classList.add('g40-collapsed');toggle.textContent=`ดูผลรายขั้นทั้งหมด (${rows.length||12})`;toggle.onclick=()=>{const hidden=list.classList.toggle('g40-collapsed');toggle.textContent=hidden?`ดูผลรายขั้นทั้งหมด (${rows.length||12})`:'ซ่อนผลรายขั้น';if(!hidden)setTimeout(()=>toggle.scrollIntoView({behavior:'smooth',block:'start'}),40)};
 }

 const delivery=document.getElementById('deliveryText'),zone=document.getElementById('summaryZoneBtn'),replay=document.getElementById('replayBtn');
 ...
 if(replay){replay.textContent=completed?'ฝึกเพิ่มอีกครั้ง':'เล่นใหม่พร้อม Step Guide';replay.onclick=restartClean;}
