...
function byStep(source){
 const map=new Map();
 (Array.isArray(source)?source:[]).forEach((row,index)=>{
  const rawKey=row?.whoStep??row?.who_step??row?.stepId??row?.step??row?.id??index;
  map.set(String(rawKey),row||{});
...
function normalizeSteps(raw){
 ...
 return STEPS.map(def=>{
  ...
  return {
   stepId:def.stepId,id:def.id,label:String(row.label||def.label),group:def.group,completed,
   passMode:String(row.passMode||row.mode||''),quality,durationSec,evidence:row.evidence||{},
   retryCount:num(row.retryCount??row.retries),errorCount:num(row.errorCount??row.errors)
  };
 });
}
function completeness(result){
 const required=['score','accuracy','durationSec','steps','events','waterUseSec','waterWasteSec','waterSaveScore','twoHandsVisibleRate','handSeenRate','trackingLostCount','trackingRecoveryCount','calibrationTimeSec','avgFps','motionSmoothness','landmarkConfidence','germRemoval','foamPeak','completedRubSteps','completedProcessSteps'];
 const missing=required.filter(key=>result[key]===undefined||result[key]===null||result[key]==='');
 return {required,missing,pct:Math.round((required.length-missing.length)/required.length*100)};
}
function enrich(input){
 const raw={...(input||{})};
 const steps=normalizeSteps(raw);
 const rubRows=steps.filter(x=>x.group==='who_rub');
 const processRows=steps.filter(x=>x.group==='process');
 const completedRubSteps=rubRows.filter(x=>x.completed).length;
 const completedProcessSteps=processRows.filter(x=>x.completed).length;
 ...
 const handSeenRate=num(raw.handSeenRate??raw.trackingRate);
 const waterUseSec=num(raw.waterUseSec),waterWasteSec=num(raw.waterWasteSec);
 ...
 const result={
  ...raw,
  analyticsSchemaVersion:'HH-UNIFIED-GAME-ANALYTICS-V2',analyticsRelease:RELEASE,
  gameId:'handwash',zone:'hygiene',gameVersion:String(raw.version||raw.gameVersion||RELEASE),
  ...
  gestureAccuracy,twoHandsVisibleRate,twoHandVisibilityPct:twoHandsVisibleRate,handSeenRate,
  trackingLostCount:num(raw.trackingLostCount),trackingRecoveryCount:num(raw.trackingRecoveryCount),
  calibrationTimeSec:num(raw.calibrationTimeSec),avgFps:num(raw.avgFps??raw.fpsAverage),
  motionSmoothness:num(raw.motionSmoothness??gestureAccuracy),landmarkConfidence:num(raw.landmarkConfidence),
  ...
  assistUsed,assistTapCount:num(raw.assistTapCount),inputMode:String(raw.inputMode||raw.mode||(assistUsed?'camera-ar-with-assist':'camera-ar')),
  retryCount:num(raw.retryCount),wrongStepCount:num(raw.wrongStepCount)||num(raw.processErrors?.length),
  hintUsed:hintCount>0,hintCount,coachCount,coachType:assistUsed?'adaptive_assist':'standard',pauseCount,resumeCount,exitCount,
  steps,stepResults:steps,events,processErrors:Array.isArray(raw.processErrors)?raw.processErrors:[],
  research:{
...
document.addEventListener('click',event=>{
 const id=String(event.target?.id||''),text=String(event.target?.textContent||'');
