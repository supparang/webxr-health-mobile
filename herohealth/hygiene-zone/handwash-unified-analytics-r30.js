(()=>{
'use strict';
const RELEASE='20260729-HANDWASH-UNIFIED-ANALYTICS-R40';
...
function enrich(input){
 ...
 const assistedSteps=Array.isArray(raw.assistedSteps)?raw.assistedSteps.map(String):steps.filter(x=>x.passMode==='assist').map(x=>x.id);
 const autoAssistCount=num(raw.autoAssistCount),assistTapCount=num(raw.assistTapCount);
 const assistUsed=bool(raw.assistUsed)||bool(raw.fallbackUsed)||assistTapCount>0||autoAssistCount>0||assistedSteps.length>0||String(raw.mode||'').includes('assist')||String(raw.mode||'').includes('fallback');
 const procedureCompleted=completedRubSteps===7&&completedProcessSteps===5&&wristsPassed;
 const skillPassed=procedureCompleted&&bool(raw.skillCriteriaMet??raw.passed);
 ...
 const strictStepCount=num(raw.strictStepCount??raw.strictPassCount)||steps.filter(x=>x.passMode==='strict').length;
 const coachStepCount=num(raw.coachStepCount??raw.gracePassCount)||steps.filter(x=>x.passMode==='grace').length;
 const assistedStepCount=num(raw.assistedStepCount)||steps.filter(x=>x.passMode==='assist').length;
 const completionLevel=String(raw.completionLevel||(!procedureCompleted?'incomplete':skillPassed?'independent':assistUsed?'completed_with_adaptive_assist':'completed_with_coach'));
 const result={...raw,analyticsSchemaVersion:'HH-UNIFIED-GAME-ANALYTICS-V2',analyticsRelease:RELEASE,gameId:'handwash',zone:'hygiene',gameVersion:String(raw.version||raw.gameVersion||RELEASE),eventId:raw.eventId||raw.attemptId||`HH-handwash-${Date.now()}`,finishedAt:raw.finishedAt||raw.timestamp||raw.endedAt||new Date().toISOString(),durationSec,score:num(raw.score),scoreAvailable:raw.score!=null,accuracy,masteryPct:accuracy,passed:skillPassed,completed:procedureCompleted,skillCriteriaMet:skillPassed,procedureCompleted,progressionEligible:procedureCompleted,completionLevel,completionPolicy:'grade5-adaptive-complete-12-steps-skill-separate',singleAttemptPolicy:true,totalSteps:12,completedSteps:completedRubSteps+completedProcessSteps,totalWhoRubSteps:7,whoStepsTotal:7,whoStepsCompleted:completedRubSteps,completedRubSteps,totalProcessSteps:5,completedProcessSteps,wristsPassed,processCompliancePct:Math.round((completedRubSteps+completedProcessSteps)/12*100),gestureAccuracy,twoHandsVisibleRate,twoHandVisibilityPct:twoHandsVisibleRate,handSeenRate,trackingLostCount:num(raw.trackingLostCount),trackingRecoveryCount:num(raw.trackingRecoveryCount),calibrationTimeSec:num(raw.calibrationTimeSec),avgFps:num(raw.avgFps??raw.fpsAverage),motionSmoothness:num(raw.motionSmoothness??gestureAccuracy),landmarkConfidence:num(raw.landmarkConfidence),waterUseSec,waterWasteSec,waterSaveScore,soapDurationSec:num(raw.soapDurationSec),foamPeak:num(raw.foamPeak),finalGermLoad:num(raw.finalGermLoad),germRemoval,strictStepCount,coachStepCount,assistedStepCount,strictPassCount:strictStepCount,gracePassCount:coachStepCount,assistUsed,assistTapCount,autoAssistCount,assistedSteps,inputMode:String(raw.inputMode||raw.mode||(assistUsed?'camera-ar-with-assist':'camera-ar')),retryCount:num(raw.retryCount),wrongStepCount:num(raw.wrongStepCount)||num(raw.processErrors?.length),hintUsed:hintCount>0,hintCount,coachCount,coachType:autoAssistCount>0?'grade5_auto_assist':assistUsed?'adaptive_assist':'standard',pauseCount,resumeCount,exitCount,steps,stepResults:steps,events,processErrors:Array.isArray(raw.processErrors)?raw.processErrors:[],research:{hygieneMasteryPct:accuracy,proceduralCompliancePct:Math.round((completedRubSteps+completedProcessSteps)/12*100),waterConservationPct:Math.round(waterSaveScore),trackingQualityPct:Math.round((twoHandsVisibleRate+handSeenRate)/2),assistDependency:assistUsed?1:0,autoAssistDependency:autoAssistCount,assistedStepCount,strictStepCount,coachStepCount,sevenRubCompletion:completedRubSteps===7?1:0,wristsCompletion:wristsPassed?1:0,skillPass:skillPassed?1:0,procedureCompletion:procedureCompleted?1:0}};
 const audit=completeness(result);result.metricCompletenessPct=audit.pct;result.progressionEligible=procedureCompleted&&audit.pct>=90;result.analyticsAudit={release:RELEASE,procedureCompleted,skillPassed,progressionEligible:result.progressionEligible,completionLevel,requiredRubSteps:7,completedRubSteps,requiredProcessSteps:5,completedProcessSteps,wristsPassed,assistUsed,autoAssistCount,assistedSteps,missingMetrics:audit.missing,metricCompletenessPct:audit.pct};return result;
}
...
window.addEventListener('herohealth:game-result',event=>{const result=enrich(event.detail||readStored()||{});if(event.detail&&typeof event.detail==='object')Object.assign(event.detail,result);save(result);console.info('[Handwash Analytics R40] unified result saved',result.analyticsAudit);},{capture:true});
window.addEventListener('beforeunload',()=>{exitCount++;});window.HH_HANDWASH_ANALYTICS={version:RELEASE,enrich,read:readStored};document.documentElement.dataset.handwashUnifiedAnalytics=RELEASE;console.info('[Handwash Analytics R40] installed');
})();