(() => {
  ...
  const RELEASE = '20260731-HANDWASH-RUNTIME-COMPATIBILITY-R43';
  ...
  const normalizedFetch = async (input, init) => {
    ...
    if (partNumber === 4 && !source.includes('HH-R43-COMPATIBILITY-MANIFEST')) {
      source += compatibilityManifest;
      console.info('[Handwash R43] compatibility manifest appended to WHO runtime');
    }
