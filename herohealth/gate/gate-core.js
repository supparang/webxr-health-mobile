...
// FULL PATCH v20260319-GATE-CORE-CANONICAL
import * as GateGames from './gate-games.js?v=20260319a';

const PATCH = 'v20260319-GATE-CORE-CANONICAL';
...
function readCtx() {
  ...
  const gameRaw = params.get('game') || params.get('gameId') || params.get('theme') || '';
  const game = normalizeGameId(gameRaw);
  const meta = getGameMeta(game);

  const phaseRaw = String(params.get('phase') || params.get('gatePhase') || 'warmup').toLowerCase();
...
function ensureCoreStyle() {
  ...
  style.textContent = `
    ...
    .gate-hero{ padding:22px 22px 14px; border-bottom:1px solid rgba(148,163,184,.10); }
    .gate-kicker{ font-size:12px; font-weight:800; letter-spacing:.08em; text-transform:uppercase; color:#93c5fd; margin-bottom:8px; }
    .gate-title{ margin:0 0 8px; font-size:clamp(24px,4vw,40px); line-height:1.05; font-weight:900; }
    .gate-sub{ margin:0; color:#cbd5e1; font-size:14px; line-height:1.55; }
    .gate-meta{ display:flex; flex-wrap:wrap; gap:8px; margin-top:14px; }
    .gate-chip{ padding:8px 12px; border-radius:999px; border:1px solid rgba(148,163,184,.14); background:rgba(15,23,42,.72); color:#e2e8f0; font-size:12px; font-weight:800; }
    .gate-topstats{ display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:10px; margin-top:14px; }
    .gate-topstat{ border:1px solid rgba(148,163,184,.14); border-radius:16px; padding:12px; background:linear-gradient(180deg, rgba(15,23,42,.72), rgba(15,23,42,.52)); }
    .gate-topstat-label{ font-size:12px; font-weight:800; color:#cbd5e1; }
    .gate-topstat-value{ margin-top:6px; font-size:20px; font-weight:1000; }
    .gate-stage{ min-height:360px; padding:18px; position:relative; }
    .gate-stage-inner{ min-height:324px; border-radius:22px; border:1px solid rgba(148,163,184,.12); background:linear-gradient(180deg, rgba(15,23,42,.74), rgba(15,23,42,.56)); padding:18px; position:relative; overflow:hidden; }
    .gate-info{ text-align:center; padding:28px 18px; }
    .gate-info h3{ margin:0 0 10px; font-size:clamp(22px,4vw,30px); line-height:1.1; }
    .gate-info p{ margin:0 auto; color:#cbd5e1; max-width:720px; line-height:1.65; }
    .gate-lines{ margin:16px auto 0; max-width:720px; text-align:left; display:grid; gap:8px; }
    .gate-line{ border:1px solid rgba(148,163,184,.14); border-radius:14px; background:rgba(15,23,42,.62); padding:10px 12px; color:#e5e7eb; }
    .gate-actions{ display:flex; flex-wrap:wrap; gap:12px; justify-content:center; margin-top:18px; }
    .gate-btn{ appearance:none; border:0; border-radius:18px; padding:14px 18px; font-size:15px; font-weight:900; cursor:pointer; min-width:170px; transition:transform .15s ease; }
    .gate-btn:hover{ transform:translateY(-1px); }
    .gate-btn-primary{ background:linear-gradient(135deg, #38bdf8, #2563eb); color:#fff; box-shadow:0 12px 26px rgba(37,99,235,.32); }
    .gate-btn-ghost{ background:rgba(15,23,42,.85); color:#e5e7eb; border:1px solid rgba(148,163,184,.16); }
    .gate-footer{ padding:0 18px 18px; }
    .gate-toast{ position:fixed; left:50%; bottom:18px; transform:translateX(-50%); z-index:9999; max-width:min(90vw,720px); background:rgba(15,23,42,.94); color:#fff; border:1px solid rgba(148,163,184,.16); border-radius:16px; padding:12px 14px; box-shadow:0 16px 40px rgba(0,0,0,.35); font-size:14px; line-height:1.45; }
    .gate-error{ color:#fecaca; background:rgba(127,29,29,.35); border:1px solid rgba(239,68,68,.25); border-radius:16px; padding:14px; margin-top:14px; white-space:pre-wrap; overflow:auto; }
    .gate-mini-note{ margin-top:10px; color:#93c5fd; font-size:13px; font-weight:700; }
...
function phaseTitle(ctx) {
  if (ctx.phase === 'cooldown') return ctx.meta?.cooldownTitle || `${ctx.meta?.label || 'Game'} Cooldown`;
...
function linesHtml(lines = []) {
  ...
  return `<div class="gate-lines">${lines.map(line => `<div class="gate-line">${esc(line)}</div>`).join('')}</div>`;
}
...
function mountFallbackPhase(stage, ctx, api) {
  const title = ctx.phase === 'cooldown' ? 'พร้อมสรุปและกลับ HUB' : 'พร้อมเข้าเกมหลัก';
  ...
  renderInfo(stage, title, desc, `<div class="gate-mini-note">fallback mode • ${esc(ctx.game)} • ${esc(ctx.phase)}</div>`);
  if (ctx.phase === 'cooldown') api.complete({ source: 'fallback-cooldown' });
  else setTimeout(() => api.complete({ source: 'fallback-warmup' }), 600);
}
...
async function bootPhase(stage, ctx, api) {
  ...
  try {
    ...
    if (result && typeof result.start === 'function') {
      queueMicrotask(() => {
        try { result.start(); } catch (err) { console.warn('[gate-core] phase start() failed:', err); }
      });
    }

    if (typeof result === 'function') {
      api._destroy = () => { cleanupEvents(); try { result(); } catch {} };
      return;
    }

    if (result && typeof result.destroy === 'function') {
      api._destroy = () => { cleanupEvents(); try { result.destroy(); } catch {} };
      return;
    }
...
function showAlreadyDone(stage, footer, ctx) {
  if (ctx.phase === 'cooldown') {
    ...
    setActions(footer, [{ label: 'กลับ HUB', primary: true, onClick: () => goHub(ctx) }]);
...
function showInvalidGame(stage, footer, ctx) {
  renderInfo(stage, 'ไม่พบเกมที่ต้องการ', `game id ไม่ถูกต้องหรือไม่มีใน registry: <code>${esc(ctx.gameRaw || '(empty)')}</code>`);
  setActions(footer, [{ label: 'กลับ HUB', primary: true, onClick: () => goHub(ctx) }]);
}
...
export async function bootGate(root = document.getElementById('gate-app')) {
  ...
  const api = {
    ...
    setStats(stats = {}) { applyStats(refs, stats); },
    setSub(text = '') { setHeroSub(refs, text); },
    setTitle(text = '') { setHeroTitle(refs, text); },
    ...
    async complete(payload = {}) {
      ...
      const title = payload.title || (ctx.phase === 'cooldown' ? 'Cooldown เสร็จแล้ว' : 'พร้อมแล้ว ไปต่อกัน');
  ...
  renderInfo(stage, ctx.phase === 'cooldown' ? 'กำลังโหลด cooldown...' : 'กำลังโหลด warmup...', 'โปรดรอสักครู่ ระบบกำลังเตรียมมินิเกมของ gate');
