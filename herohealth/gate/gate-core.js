...
// FULL REPLACEMENT v20260622-GATE-CORE-FRR-V17-LOADER
...
import * as GateGames from './gate-games.js?v=20260622-gate-games-frr-v19-loader';

const PATCH = 'v20260622-GATE-CORE-FRR-V19-LOADER';
...
function saveSummary(ctx, payload = {}) {
  try {
    ...
    const history = JSON.parse(
      localStorage.getItem(`${LAST_SUMMARY_KEY}_HISTORY`) || '[]'
    );
...
function readCtx() {
  ...
  const rawPhase = text(
    params.get('phase') || params.get('gatePhase'),
    'warmup'
  ).toLowerCase();

  const phase = rawPhase === 'cooldown'
    ? 'cooldown'
    : 'warmup';
  ...
  return {
    ...
    plannerReturn: text(
      params.get('plannerReturn') ||
      params.get('plannerReturnUrl')
    ),
...
function hubHref(ctx) {
  if (ctx.hub) {
    return withContext(ctx.hub, ctx);
  }

  if (ctx.hubRoot) {
    return withContext(ctx.hubRoot, ctx);
  }
...
function runHref(ctx) {
  const explicit =
    ctx.params.get('runFile') ||
    ctx.params.get('runUrl') ||
    '';

  const candidates = [
    explicit,
    ...getRunCandidates(ctx.game)
  ].filter(Boolean);
...
function completionHref(ctx, payload = {}) {
  if (payload && payload.nextHref) {
    return withContext(payload.nextHref, ctx);
  }

  if (ctx.phase === 'cooldown' && ctx.cdnext) {
    return withContext(ctx.cdnext, ctx);
  }

  if (ctx.next) {
    return withContext(ctx.next, ctx);
  }

  if (ctx.plannerReturn) {
    return withContext(ctx.plannerReturn, ctx);
  }

  return ctx.phase === 'warmup'
    ? runHref(ctx)
    : hubHref(ctx);
}
...
function ensureStyle() {
  ...
  style.textContent = `
    .hha-gate-shell{
      ...
      place-items:center;
      background:
        radial-gradient(circle at 50% -10%,rgba(56,189,248,.18),transparent 35%),
    ...
    <section class="hha-gate-shell">
      <article class="hha-gate-card">
        <header class="hha-gate-hero">
          <div class="hha-gate-kicker">
            HeroHealth Gate • ${esc(ctx.phase)}
          </div>

          <h1 class="hha-gate-title">
            ${ctx.phase === 'cooldown'
              ? 'Cooldown & Recovery'
              : 'Warmup Ready'}
          </h1>

          <p class="hha-gate-sub">
            เตรียมความพร้อมสำหรับ
            ${esc(
              ctx.meta && ctx.meta.label
                ? ctx.meta.label
                : ctx.game || 'เกม'
            )}
            ก่อนเข้าสู่ด่านถัดไป
          </p>
        ...
        <main class="hha-gate-stage">
          <div class="hha-gate-panel" id="hhaGateStage"></div>
        </main>
...
async function loadPhaseModule(ctx) {
  ...
  // Force the current V17 module, independent of the old cached V10 URL.
...
function addGameStyle(ctx) {
  ...
  // CSS must receive the same cache-busting version as the module.
  href.searchParams.set('v', PATCH);

  const hrefText = href.toString();

  if (document.querySelector(`link[data-gate-style="${hrefText}"]`)) {
    return;
  }
  ...
  link.rel = 'stylesheet';
  link.href = hrefText;
  link.dataset.gateStyle = hrefText;
...
function bindEvents(stage, api) {
  const completed = event => {
    api.complete(event && event.detail ? event.detail : {});
  };

  const skipped = event => {
    api.skip(event && event.detail ? event.detail : {});
  };

  const failed = event => {
    api.fail(event && event.detail ? event.detail : event);
  };
...
async function mountPhase(stage, ctx, api) {
  ...
  try {
    ...
    console.warn(
      '[gate-core] phase import failed; fallback enabled',
      error
    );
  ...
  try {
    ...
    if (instance && typeof instance.start === 'function') {
      try {
        ...
        console.warn(
          '[gate-core] instance start failed',
          error
        );
      }
    }
...
export async function bootGate(root = document.getElementById('gate-app')) {
  ...
  if (!root) {
    throw new Error('Gate root (#gate-app) not found.');
  }
