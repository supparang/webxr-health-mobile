...
// ✅ Starts immediately (no "start" gating)
// ✅ Imports engine from ./plate.safe.js
// ✅ Wires HUD listeners (hha:score, hha:time, quest:update, hha:coach, hha:end)
// ✅ End overlay: aria-hidden only (no remove)
// ✅ Back HUB + Restart
// ✅ Pass-through research context params: run/diff/time/seed/studyId/... etc.
// ✅ Watchdog: if targets never appear -> coach hint + console
...
function getViewAuto() {
  // No menu override. Allow query for experiments.
...
function buildEngineConfig() {
  ...
  return {
    ...
    // research passthrough (optional fields used by cloud logger)
    studyId: qs('studyId', '') || '',
    phase: qs('phase', '') || '',
...
// ---- global trap (helps “ค้าง” debugging) ----
WIN.addEventListener('error', (e) => {
  try { console.error('[PlateVR] window.error', e?.error || e); } catch {}
});
WIN.addEventListener('unhandledrejection', (e) => {
  try { console.error('[PlateVR] unhandledrejection', e?.reason || e); } catch {}
});
...
ready(() => {
  ...
  // always start with overlay closed
  ...
  // ✅ watchdog: if nothing appears, tell what to check
  clearTimeout(WIN.__PLATE_WATCHDOG__);
  WIN.__PLATE_WATCHDOG__ = setTimeout(() => {
    const any = mount.querySelector('.plateTarget');
    if (!any) {
      showCoach('ยังไม่เห็นเป้า: เช็ก console error (mode-factory / css safe-zone)', 'Debug');
      console.warn('[PlateVR] No targets after 1500ms. Check: mode-factory boot(), export, safe vars, mount rect.');
    }
  }, 1500);
});