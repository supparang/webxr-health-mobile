...
function showPhaseBanner(kind){
  let text = 'เริ่มเลย';
  if(kind === 'warm') text = 'WARM • ผู้เล่น A เริ่มก่อน';
  else if(kind === 'trick') text = 'TRICK • ระวังตัวหลอก';
  else if(kind === 'boss') text = 'BOSS • ช่วยกันเติมจานให้ครบ';
...
function setPhase(index){
  ...
  if(S.phase === 'warm'){
    ...
    setCoach('boss', { target: S.targetGroup.label, who: activePlayer() });
...
function switchPlayer(manual = false){
  ...
  if(manual){
    try{
      const d = DOC.createElement('div');
      d.className = 'phaseBanner';
      d.textContent = `TURN • ผู้เล่น ${activePlayer()}`;
      DOC.getElementById('app')?.appendChild(d);
      setTimeout(()=>{ try{ d.remove(); }catch(_){} }, 850);
    }catch(_){}
  }
}
...
function awardCorrect(t){
  ...
  S.score += add;
  S.players[p].score += add;
  S.players[p].ok += 1;
  S.fever = clamp(S.fever + 14, 0, 100);

  if(t.kind === 'shield'){
    S.shield = Math.min(3, S.shield + 1);
    setCoach('shield', { who: p });
  } else {
    setCoach('good', { target: S.targetGroup?.label, who: p });
  }
  ...
  if(t.groupId >= 1 && t.groupId <= 5){
    ...
    if(beforePlateHave < 5 && S.plateHave === 5){
      S.score += 30;
      S.players[p].score += 30;
      showPop(t.x, t.y, 'ครบ 5 หมู่! +30');
      setCoach('phase-clear', { who: p });
    }
  }
...
function awardWrong(){
  ...
  if(S.shield > 0){
    ...
    S.players[p].score = Math.max(0, S.players[p].score - penalty);
    S.miss++;
  }

  setCoach('wrong', { target: S.targetGroup?.label, who: p });
  switchPlayer(false);
}
...
function updateHud(){
  ...
  setText('uiP1', S.players.A.score);
  setText('uiP2', S.players.B.score);
...
function phaseAdvance(){
  ...
  if(S.phaseIndex < S.phaseList.length - 1){
    setCoach('phase-clear', { who: activePlayer() });
  ...
  for(const t of Array.from(S.targets.values())){
    if(now - t.born >= t.ttlMs){
      if(t.good && t.groupId === S.targetGroup.id){
        ...
        setCoach('miss', { target: S.targetGroup?.label, who: p });
        switchPlayer(false);
      }
...
function endGame(reason){
  ...
  setText('endTitle', 'Duet จบแล้ว 🎉');
  setText('endSub', `mode=duet • diff=${S.ctx.diff} • view=${S.ctx.view} • phase=${friendlyPhaseLabel(S.phase)}`);
  setText('endGrade', sum.grade);
  setText('endScore', sum.scoreFinal);
  setText('endP1', S.players.A.score);
  setText('endP2', S.players.B.score);
...
reasonDetail=${sum.reasonDetail}`;
    try{
      ...
      setCoach('good', { who: activePlayer() });
    }catch(_){}
  });
...
function boot(ctx){
  ...
  S.players.active = 'A';
  S.players.A = { score:0, ok:0, wrong:0 };
  S.players.B = { score:0, ok:0, wrong:0 };
...
WIN.PlateDuet = WIN.PlateDuet || {};
WIN.PlateDuet.boot = boot;
