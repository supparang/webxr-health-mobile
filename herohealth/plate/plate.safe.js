...
// Balanced Plate VR — Full Engine (ES Module)
...
// ---------- Mini quests ----------
const MINI_POOL = [
  { key:'rush',    label:'Plate Rush: เก็บหมู่ให้ครบ 5',          target: 1 },
...
function emitQuestUpdate() {
  emit('quest:update', {
    ...
    minisAll: Array.from({length:Math.max(miniHistory,1)}).map((_,i)=>({done: i < miniCleared}))
  });
}
