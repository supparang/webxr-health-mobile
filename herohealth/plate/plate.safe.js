...
// PATCH: use HHA_EMOJI pack to decorate good targets (variety per group)
...
function endGame(reason='timeup'){
  ...
  emit('hha:end', {
    ...
    misses: STATE.miss,
    goalsCleared: STATE.goal.done ? 1 : 0, goalsTotal: 1,
    miniCleared: STATE.mini.done ? 1 : 0, miniTotal: 1,
...
/* ✅ Decorate target with emoji (variety) */
function decorateTarget(el, t){
  // Keep ring look; just add an emoji center for GOOD only (optional)
  if(t.kind === 'good'){
    ...
    // junk: could show small junk emoji or keep empty ring (เลือกได้)
    // ถ้าอยากให้ “junk มี emoji ด้วย” ให้ปลด comment 3 บรรทัดล่าง
...
export function boot({ mount, cfg }){
  ...
  STATE.rng = (cfg.runMode === 'research' || cfg.runMode === 'study')
    ? seededRng(cfg.seed || Date.now())
    : Math.random;

  STATE.timeLeft = Number(cfg.durationPlannedSec) || 90;

  emit('hha:start', { game:'plate', runMode: cfg.runMode, diff: cfg.diff, seed: cfg.seed, durationPlannedSec: STATE.timeLeft });
