...
// ✅ End overlay: aria-hidden only (no display none)
// ✅ Back HUB + Restart
// ✅ Pass-through research ctx params
// ✅ Save last summary: localStorage.HHA_LAST_SUMMARY (and back link hub param)
...
function getViewAuto() {
  // No UI menu. Allow force via query only (experiments).
...
function buildHubBackUrl(hubUrl, summaryObj) {
  try {
    if (!hubUrl) return '';
    const u = new URL(hubUrl, location.href);

    // Standard: attach a lightweight hint so HUB can show "resume/last summary"
    // (HUB can choose to read localStorage.HHA_LAST_SUMMARY as main data)
    u.searchParams.set('hha_last', 'plate');

    // Optional: include a short token (avoid long query)
    if (summaryObj?.sessionId) u.searchParams.set('sid', String(summaryObj.sessionId));
...
function wireEndControls(getCfg) {
  ...
  if (btnBackHub) {
    btnBackHub.addEventListener('click', () => {
      ...
      const hub = (cfg && cfg.hub) ? cfg.hub : (qs('hub', '') || '');

      if (hub) {
        // flush-hardened: overlay already shown; go hub
        const back = buildHubBackUrl(hub, null);
        location.href = back || hub;
      } else {
        history.back();
      }
    });
...
function wireEndSummary(getCfg) {
  ...
  WIN.addEventListener('hha:end', (e) => {
    ...
    // ✅ HHA Standard: save last summary for HUB/teacher dashboard
    try {
      const cfg = (typeof getCfg === 'function') ? getCfg() : null;
      const payload = {
        ts: Date.now(),
...
function buildEngineConfig() {
  ...
  // ✅ default time = 90 (if missing). clamp to prevent accidental huge.
  const time = clamp(qs('time', '90'), 10, 999);

  // seed: if missing, use Date.now; research uses seeded RNG inside engine.
  ...
  const cfg = {
    ...
    // endpoint passthrough (logger/engine can use it)
    logEndpoint: qs('log', '') || '',
...
ready(() => {
  ...
  // set view class (does not block Enter VR, vr-ui handles it)
