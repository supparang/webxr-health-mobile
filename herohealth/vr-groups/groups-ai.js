...
(function(root){
  ...
  const AI = {
    reset(opts={}){
      ...
      AI.accE = NaN; AI.rtE = NaN; AI.missE = NaN; AI.comboE = NaN;
      AI.lastCoachAt = 0;
      AI.lastAdaptAt = 0;
      AI.assistLevel = 0;
    ...
    observeFrame(state){
      ...
      const accS = clamp((AI.accE - 0.55) / 0.40, 0, 1);
      const rtS  = isFinite(AI.rtE) ? clamp((850 - AI.rtE) / 520, 0, 1) : 0.4;
      const missS= clamp(1 - (AI.missE/2.4), 0, 1);
      ...
      const struggle = clamp((0.62 - (AI.accE||0)) / 0.20, 0, 1);
      ...
      if (t - AI.lastCoachAt > 6000){
        ...
        if (AI.assistLevel > 0.65){
          ...
          emit('hha:coach', { mood:'happy', text:'โหดได้! 🎯 AI เร่งสปีดให้นิดนะ… ระวัง DECOY/JUNK!' });
        } else {
