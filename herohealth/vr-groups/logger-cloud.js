...
(function (ns) {
  ...
  function send(rawSession, rawEvents) {
    ...
    if (navigator.sendBeacon) {
      try {
        ...
        if (CONFIG.debug)
          console.warn('[GroupsVR Logger] sendBeacon error → fallback', err);
      }
    }

    // 2) ถ้าไม่ได้ → ใช้ fetch no-cors
    try {
      fetch(CONFIG.endpoint, {
        ...
        if (CONFIG.debug)
          console.log('[GroupsVR Logger] sent payload (fetch no-cors)');
      });
    } catch (err) {
  ...
  ns.foodGroupsCloudLogger = { send: send };
