...
Food Groups VR — GameEngine (CONNECTED to groups-quests.js + audio.js + groups-fx.js)
✅ Emits:
...
✅ Boss Pattern (ลำดับหมู่): hit boss ถูกลำดับ = dmg, ผิด = STUN (freeze+shake)
✅ Perfect Chain x3 -> Overdrive buff (score x2) via groups:buff
✅ Cardboard-ready: aim highlight + tap anywhere to hit aimed target (body.cardboard)
✅ Target CSS classes: fg-good/fg-wrong/fg-junk/fg-decoy/fg-boss (สำคัญ!)
*/
...
(function(root){
  ...
  function diffParams(diff){
    ...
    if (diff === 'easy') return { spawnMs:900, ttl:1750, size:1.05, powerThr:thr, junk:0.10, decoy:0.08, stormDur:6, bossHp:3, stunMs:700 };
    if (diff === 'hard') return { spawnMs:680, ttl:1450, size:0.92, powerThr:thr, junk:0.16, decoy:0.12, stormDur:7, bossHp:4, stunMs:950 };
    return                 { spawnMs:780, ttl:1600, size:1.00, powerThr:thr, junk:0.12, decoy:0.10, stormDur:6, bossHp:3, stunMs:850 };
  }
  ...
  function setupView(){
    ...
    function bind(){
      ...
      layer.addEventListener('pointerdown', (e)=>{
        // cardboard mode: tap anywhere hits aimed target (ถ้าไม่ได้กดทับบนเป้าโดยตรง)
        if (DOC.body.classList.contains('cardboard')){
          const target = e.target;
          if (target && target.classList && target.classList.contains('fg-target')) return;
          if (engine._aimEl && engine._aimEl.isConnected){
            e.preventDefault?.();
            hitTarget(engine._aimEl);
            return;
          }
        }
      ...
      // Buff listener (overdrive/freeze)
      root.addEventListener('groups:buff', (ev)=>{
        const d = ev.detail || {};
        const type = String(d.type||'').toLowerCase();
        const durSec = clamp(d.durSec ?? 6, 1, 15);
        if (type === 'overdrive'){
          engine.overUntil = Math.max(engine.overUntil, now() + durSec*1000);
          DOC.body.classList.add('groups-overdrive');
          root.setTimeout(()=> DOC.body.classList.remove('groups-overdrive'), durSec*1000 + 100);
          emit('hha:judge', { kind:'good', text:'OVERDRIVE x2!' });
        }
        if (type === 'freeze'){
          const ms = clamp(d.ms ?? 800, 200, 2000);
          engine.freezeUntil = Math.max(engine.freezeUntil, now() + ms);
          DOC.body.classList.add('groups-freeze');
          emit('groups:stun', { ms, strength: 1.0 });
          root.setTimeout(()=> DOC.body.classList.remove('groups-freeze'), ms + 120);
          emit('hha:judge', { kind:'MISS', text:'STUN!' });
        }
      }, { passive:true });
    }
  ...
  function makeTarget(type, emoji, x, y, s){
    ...
    el._ttlTimer = root.setTimeout(()=>{
      ...
      if (type === 'good'){
        ...
        updateScore();
        emitFever();
        emitProgress({ kind:'hit_bad', reason:'expire' });
      }
  ...
  function setGroup(id){
    ...
    emit('groups:group', { groupId: engine.groupId|0 });
  }
  ...
  // ---------- Boss Pattern ----------
  function bossSeqText(seq){
    try{
      return seq.map(g=> (GROUPS[g] ? GROUPS[g].label : ('หมู่ '+g))).join(' → ');
    }catch{
      return '';
    }
  }

  function makeBossSeq(len){
    const out = [];
    let last = 0;
    for (let i=0;i<len;i++){
      let g = 1 + ((engine.rng()*5)|0);
      if (g === last) g = (g % 5) + 1;
      out.push(g);
      last = g;
    }
  ...
  function tryBossSpawn(){
    ...
    engine.bossSeq = makeBossSeq(engine.bossHpMax);
    engine.bossSeqIdx = 0;
    ...
    el.dataset.seq = JSON.stringify(engine.bossSeq);
    el.dataset.step = String(engine.bossSeqIdx);
    ...
    // coach hint: pattern
    emitCoach(`👑 BOSS: ยิงตอนอยู่ ${bossSeqText(engine.bossSeq)} (ผิด=STUN!)`, 'neutral');

    engine.nextBossAtMs = now() + (engine.runMode==='research' ? 20000 : clamp(engine.adapt.bossEvery, 14000, 26000));
  }

  function stun(ms, strength){
    ms = clamp(ms||800, 250, 1600);
    strength = clamp(strength||1, 0.4, 2.2);
    engine.freezeUntil = Math.max(engine.freezeUntil, now() + ms);
    DOC.body.classList.add('groups-freeze');
    emit('groups:stun', { ms, strength });
    root.setTimeout(()=> DOC.body.classList.remove('groups-freeze'), ms + 120);
  }

  function hitBoss(el){
    // blocked by freeze
    if (now() < engine.freezeUntil) return;

    // ต้องยิงตามลำดับหมู่: bossSeq[seqIdx] ต้องเท่ากับ groupId ปัจจุบัน
    const need = (engine.bossSeq && engine.bossSeq.length) ? (engine.bossSeq[engine.bossSeqIdx]||0) : 0;
    const ok = (!need) ? true : (engine.groupId === need);

    if (!ok){
      // WRONG -> STUN
      emitProgress({ type:'hit', correct:false });
      emitProgress({ kind:'hit_bad', reason:'boss_wrong' });
      ...
      emit('hha:judge', { kind:'MISS', text:`STUN! ต้องอยู่ ${GROUPS[need]?.label || ('หมู่ '+need)}` });

      const dp = diffParams(engine.diff);
      stun(dp.stunMs, 1.25);

      try{ el.classList.add('fg-boss-hurt'); }catch{}
      root.setTimeout(()=>{ try{ el.classList.remove('fg-boss-hurt'); }catch{} }, 220);
      return;
    }
    ...
    engine.bossSeqIdx = Math.min(engine.bossHpMax, engine.bossSeqIdx + 1);
    el.dataset.step = String(engine.bossSeqIdx);
    ...
    if (engine.bossHp <= 0){
      ...
      removeTarget(el);
      emitCoach('สุดยอด! ปราบบอสได้แล้ว! 🏆', 'happy');
    } else {
      // hint next step
      const nextNeed = engine.bossSeq[engine.bossSeqIdx] || 0;
      if (nextNeed){
        emitCoach(`ต่อไป ยิงบอสตอนอยู่ ${GROUPS[nextNeed]?.label || ('หมู่ '+nextNeed)}!`, 'neutral');
      }
  ...
  function hitTarget(el){
    ...
    // freeze disables all hits (stun window)
    if (now() < engine.freezeUntil) return;
    ...
    if (badLike){
      // shield blocks junk = no miss and NOT hit_bad (no fail nojunk)
  ...
  function spawnOne(){
    ...
    // during freeze: ลดความรก (ไม่สปอว์นใหม่ช่วง stun)
    if (now() < engine.freezeUntil) return;
  ...
  // ---------- Aim helper (cardboard highlight) ----------
  function loopAim(){
    if (!engine.running || engine.ended) return;
    if (!DOC.body.classList.contains('cardboard')){
      engine.aimTimer = root.setTimeout(loopAim, 160);
      return;
    }
    const layer = engine.layerEl;
    if (!layer){
    ...
    list.forEach(el=>{
      const x = Number(el.dataset._x||0) + engine.vx;
      const y = Number(el.dataset._y||0) + engine.vy;
      const dx = x - cx;
      const dy = y - cy;
    ...
    // threshold
    if (best && bestD <= (90*90)){
      if (engine._aimEl && engine._aimEl !== best){
        try{ engine._aimEl.classList.remove('aim'); }catch{}
      }
      engine._aimEl = best;
      try{ best.classList.add('aim'); }catch{}
    } else {
      if (engine._aimEl){
        try{ engine._aimEl.classList.remove('aim'); }catch{}
      }
    ...
    engine.aimTimer = root.setTimeout(loopAim, 120);
  }
  ...
  function endGame(reason){
    ...
    try{ root.clearTimeout(engine.aimTimer); }catch{}
    clearAllTargets();
    questStop();

    DOC.body.classList.remove('groups-storm','groups-storm-urgent','groups-overdrive','groups-freeze','mini-urgent');
