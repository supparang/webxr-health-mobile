...
// 2025-12 — FULL FIX: target "แว๊บๆ" solved
// - destroy() only place to remove
// - activeTargets splice correct
// - MIN_VISIBLE enforced (easy เล่นทัน)
// - camera-relative 3D anchor -> project to 2D each frame
// - uses difficulty.foodgroups.js (HeroHealth.foodGroupsDifficulty.get)
...
(function (ns) {
  ...
  // ---------- Quest design ----------
  const GOALS = [
    { id: 'G1', label: 'เก็บอาหารดีจากหมู่ 1–3 ให้ครบ', target: 11 },
    { id: 'G2', label: 'เก็บอาหารดีจากหมู่ 4–5 ให้ครบ', target: 9 }
  ];
  ...
  const MINIS = [
    ...
    { id: 'M3', label: 'เก็บอาหารดีครบทั้ง 5 หมู่ อย่างน้อยหมู่ละ 1 ชิ้น', type: 'groups', needGroups: 5 }
  ];
  ...
  let activeTargets = []; // {el,posWorld,timeout,alive,...}
  ...
  let maxActive = 4;
  let lifetimeBase = 2400;
  let baseScale = 1.0;
  let feverGainHit = 7;
  let feverLossMiss = 16;

  // IMPORTANT: minimum visible time (fix "แว๊บๆ")
  let MIN_VISIBLE_MS = 1100; // will be adjusted by diff
  ...
  function nowFromStartMs () { return startMs ? (performance.now() - startMs) : 0; }
  ...
  function applyDifficulty (key) {
    ...
    spawnInterval = (D.spawnInterval|0) || 950;
    maxActive = (D.maxActive|0) || 4;
    lifetimeBase = (D.lifetime|0) || 2600;
    baseScale = Number(D.scale) || 1.0;
    feverGainHit = Number(D.feverGainHit) || 7;
    feverLossMiss = Number(D.feverLossMiss) || 16;

    // ✅ minimum visible time per difficulty
    if (diffKey === 'easy') MIN_VISIBLE_MS = 1400;
    else if (diffKey === 'hard') MIN_VISIBLE_MS = 950;
    else MIN_VISIBLE_MS = 1100;
  ...
  function setFeverValue (next, stateHint) {
    ...
    setFever(fever);
    emit('hha:fever', { state: stateHint || (feverActive ? 'active' : 'charge'), value: fever, max: FEVER_MAX });
  }
  function startFever () {
  ...
  function pushQuest (hintText) {
    const meta = questMeta();
    const goal = meta.goalsAll[currentGoalIdx] || null;
    const mini = meta.minisAll[currentMiniIdx] || null;
  ...
  function updateGoalOnHit (ch) {
    ...
    if (goalProg >= g.target) {
      ...
      coach(`Goal ${idxNow + 1}/${total} สำเร็จแล้ว! 🎯`, 2800);
  ...
  function updateMiniOnHit (ch, isGood) {
    ...
    if (!miniFlags.groupsDone && seenGroups.size >= MINIS[2].needGroups) {
      ...
      coach('เก็บครบทั้ง 5 หมู่แล้ว 🥦🍚🍎', 3000);
    }
    ...
    if (meta.goalsCleared >= GOALS.length && meta.minisCleared >= MINIS.length) {
      ...
      coach('เคลียร์ทุกภารกิจแล้ว! 🎉', 3500);
      stop('quest-complete');
      return;
    }
  }
  ...
  // ---------- Adaptive+ ----------
  function currentScale () { return (runMode === 'research') ? baseScale : adaptiveScale; }
  ...
  function adaptive_onHit () {
    ...
    if (combo >= 6) {
      adaptiveScale *= 0.975;
      adaptiveScale = clamp(adaptiveScale, baseScale * 0.75, baseScale * 1.35);

      adaptiveSpawn *= 0.965;
      adaptiveSpawn = clamp(adaptiveSpawn, 0.80, 1.20);
  ...
  function adaptive_onMiss () {
    ...
    if (missStreak >= 3) {
      adaptiveScale *= 1.10;
      adaptiveScale = clamp(adaptiveScale, baseScale * 0.75, baseScale * 1.55);

      adaptiveSpawn *= 1.10;
      adaptiveSpawn = clamp(adaptiveSpawn, 0.80, 1.45);

      adaptiveMaxActive = clamp(adaptiveMaxActive - 1, -1, 0);

      adjustCooldownUntil = now + 1700;
  ...
  function spawnAnchorWorldPos () {
    ...
    // distance & spread (easy = easier spread)
    const dist = (diffKey === 'easy') ? 2.25 : 2.35;
    const spreadX = (diffKey === 'easy') ? 1.45 : 1.75;
    const spreadY = (diffKey === 'easy') ? 0.95 : 1.15;
    ...
    return new THREE.Vector3().copy(tmpCamPos)
  ...
  function projectToScreen (posWorld) {
    ...
    // padding clamp (avoid spawning offscreen)
    const pad = 30;
    return {
      x: clamp(x, pad, w - pad),
      y: clamp(y, 90, h - 90)
  ...
  // ---------- Target lifecycle (FIX "แว๊บๆ") ----------
  function destroyTarget (t, isHit, reason) {
    if (!t || !t.el || !t.alive) return;
    t.alive = false;

    // clear timers
    if (t.timeout) { clearTimeout(t.timeout); t.timeout = null; }
    if (t.minTimer) { clearTimeout(t.minTimer); t.minTimer = null; }
  ...
  function createTarget (kindWanted) {
    ...
    if (kindWanted === 'good') {
      // power chance (easy lower, hard higher)
      const powerChance = (diffKey === 'easy') ? 0.055 : (diffKey === 'hard' ? 0.10 : 0.08);
    ...
    // ✅ minimum visible guard (fix "แว๊บๆ")
    tObj.minTimer = setTimeout(() => {
      tObj.allowExpire = true;
    }, MIN_VISIBLE_MS);

    // click handler (pointerdown only is enough)
    const onDown = (ev) => {
      if (!running || !tObj.alive) return;
      ev.stopPropagation();
      ev.preventDefault();
    ...
    // lifetime
    const life = Math.max(900, lifetimeBase + (Math.random() * 520 - 260));
    tObj.timeout = setTimeout(() => {
      if (!running || !tObj.alive) return;

      // ✅ do not expire before minimum visible time
      if (!tObj.allowExpire) {
        // try again after minimum visible
        const remain = Math.max(60, MIN_VISIBLE_MS - (performance.now() - tObj.bornAt));
        setTimeout(() => {
          if (!running || !tObj.alive) return;
          expireTarget(tObj);
        }, remain);
  ...
  function expireTarget (tObj) {
    if (!tObj || !tObj.alive) return;
    const kind = tObj.type;
    const emoji = tObj.emoji;

    destroyTarget(tObj, false, 'expire');
  ...
  function handleHit (tObj, x, y) {
    ...
    if (type === 'power') {
      if (ch === POWER_STAR) {
        ...
        emit('hha:judge', { label: 'STAR BONUS' });
        emit('hha:score', { score, combo, misses });

        emitGameEvent({ type:'hit', eventType:'hit', itemType:'power', emoji: ch, isGood:true, rtMs, targetId:tObj.targetId, judgment:'STAR', totalScore: score, combo, feverState: feverActive ? 'active' : 'charge', feverValue: fever });
      ...
      if (ch === POWER_FIRE) {
        ...
        coach('โหมดไฟ! เลือกอาหารดีรัว ๆ แล้วเลี่ยงของขยะนะ 🔥', 3200);
        emit('hha:judge', { label: 'FEVER' });
        emit('hha:score', { score, combo, misses });

        emitGameEvent({ type:'hit', eventType:'hit', itemType:'power', emoji: ch, isGood:true, rtMs, targetId:tObj.targetId, judgment:'FEVER', totalScore: score, combo, feverState:'active', feverValue: fever });
      ...
      if (ch === POWER_SHIELD) {
        ...
        coach('ได้เกราะกันของขยะแล้ว 🛡️ เผลอแตะจะไม่ถือว่าพลาด 1 ครั้ง', 3400);
        emit('hha:judge', { label: 'SHIELD' });
        emit('hha:score', { score, combo, misses });

        emitGameEvent({ type:'hit', eventType:'hit', itemType:'power', emoji: ch, isGood:true, rtMs, targetId:tObj.targetId, judgment:'SHIELD', totalScore: score, combo, feverState: feverActive ? 'active' : 'charge', feverValue: fever });
    ...
    if (type === 'good') {
      ...
      emit('hha:judge', { label: combo >= 8 ? 'PERFECT' : 'GOOD' });
      emit('hha:score', { score, combo, misses });

      emitGameEvent({
        type:'hit', eventType:'hit', itemType:'good',
        emoji: ch, isGood:true, rtMs, targetId:tObj.targetId,
      ...
      if (combo === 3) coach('คอมโบ x3 แล้ว 💪', 2600);
      if (combo === 6) coach('คอมโบยาวขึ้นแล้ว ระวังของขยะนะ ✨', 2600);
    ...
    if (type === 'junk') {
      if (shield > 0) {
        ...
        emitGameEvent({
          type:'hit', eventType:'hit', itemType:'junk',
          emoji: ch, isGood:false, rtMs, targetId:tObj.targetId,
          judgment: 'BLOCK',
          totalScore: score, combo, misses,
          feverState: feverActive ? 'active' : 'charge',
          feverValue: fever
        ...
        coach('เกราะช่วยกันไว้แล้ว 🛡️ แต่พยายามเลี่ยงนะ', 3000);
        pushQuest();
        return;
      }
      ...
      coach('โดนของขยะแล้ว ลองโฟกัสอาหารดีมากขึ้นนะ 🍔🍟🍩', 3200);
  ...
  function tickSpawn () {
    ...
    const kind = Math.random() < 0.80 ? 'good' : 'junk';
    createTarget(kind);
  }
  ...
  function scheduleNextSpawn () {
    ...
    const nextMs = Math.max(260, Math.round(spawnInterval * mult));
  ...
  function start (diff, opts = {}) {
    ...
    // runMode
    const runParam = String(opts.runMode || (new URL(window.location.href)).searchParams.get('run') || 'play').toLowerCase();
    ...
    // reset gameplay
    score = 0; combo = 0; comboMax = 0; misses = 0; goodStreak = 0;
    ...
    // reset analytics
    nTargetGoodSpawned = 0; nTargetJunkSpawned = 0; nTargetStarSpawned = 0; nTargetShieldSpawned = 0; nTargetDiamondSpawned = 0;
    nHitGood = 0; nHitJunk = 0; nHitJunkGuard = 0; nExpireGood = 0;
    rtGoodArr = []; fastHitCount = 0;

    // clear targets
    activeTargets.forEach(t => destroyTarget(t, false, 'reset'));
    activeTargets = [];

    applyDifficulty(String(diff || 'normal').toLowerCase());

    coach(runMode === 'research'
      ? 'โหมดวิจัย: ค่าคงที่ตามระดับความยาก ✅'
      : 'โหมดธรรมดา: เกมจะปรับ (adaptive+) ตามการเล่น ✅', 2400);
  ...
  function stop (reason) {
    ...
    if (ns.__statTimer) { clearInterval(ns.__statTimer); ns.__statTimer = null; }

    activeTargets.forEach(t => destroyTarget(t, false, 'stop'));
    activeTargets = [];
