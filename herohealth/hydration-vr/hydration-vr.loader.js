...
// HydrationVR Folder Loader (AUTO-DETECT VIEW)
// ✅ Auto-detect PC/Mobile/Quest-like -> cVR
// ✅ Keeps compatibility if hub already passes ?view=...
// ✅ Maps layers via window.HHA_VIEW.layers
...
(function(){
  ...
  function detectView(){
    // If hub already sets view, keep it
    const forced = String(q.get('view') || '').toLowerCase();
    if (forced) return forced;

    const ua = String(navigator.userAgent || '');
    const isTouch = ('ontouchstart' in window) || (navigator.maxTouchPoints > 0);
    const isQuestLike = /OculusBrowser|Quest|Meta Quest|PicoBrowser|VIVE|XRBrowser/i.test(ua);

    // Quest / XR browser -> cVR strict
    if (isQuestLike) return 'cvr';
  ...
  function showFail(err, tried){
    ...
    el.innerHTML = `
      <div style="max-width:900px;margin:0 auto">
        ...
        <div style="opacity:.9;margin-bottom:10px">Detected view: <b>${escapeHtml(view)}</b></div>
