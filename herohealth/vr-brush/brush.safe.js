...
// PATCH v20260304-BRUSH-SAFE-ABC-AI-FULL
// ✅ Stage A/B/C
// ✅ Stage B: Evidence 3 types (🍬🌙🚫🪥) — MUST complete BEFORE entering C
// ✅ Stage C: Quiz Analyze to finish (clean 100 + quiz done)
// ✅ AI prediction-only: emits brush:ai {risk/miss_streak/combo_hot/stage/quiz/boss/time}
// ✅ cVR aim assist + no double-shot
// ✅ Stable menu/end/quiz (never auto-jump to quiz on start)
(function(){
  'use strict';
  ...
  function aiPredict(){
    ...
    if(S.stage==='B') tip = `B: เก็บ 🍬🌙🚫🪥 ให้ครบ 3 (ตอนนี้ ${S.eviTotal}/3)`;
    if(S.stage==='C') tip = 'C: ตอบ Quiz วิเคราะห์ 1 ข้อ แล้วปิดเกม';
  ...
  function renderHud(force){
    const t = now();
    if(!force && (t - S._lastHud) < 70) return;
    S._lastHud = t;
  ...
  // ---------- Stage logic (LOCK C behind evidence) ----------
  function stageFromProgress(){
    if(S.clean < 40) return 'A';
    if(S.clean < 80) return 'B';
    if(S.eviTotal < S.eviNeed) return 'B'; // ✅ MUST finish evidence before entering C
    return 'C';
  }
  ...
  function applyHitRewards(t, remainMs, weakHit){
    ...
    // B gate: if evidence not complete, slow down near 80 so player must go find evidence
    if(S.stage==='B' && S.eviTotal < S.eviNeed && S.clean >= 79) gain *= 0.18;
  ...
  function spawnOne(){
    ...
    if(S.stage==='B' && S.eviTotal < S.eviNeed){
      const chance = 0.30 + (S.aiRisk > 0.65 ? 0.06 : 0);
      if(rng() < chance){
  ...
  function handleHit(t, x, y, source){
    ...
    // ✅ Open Quiz only when C is truly reached (evidence complete) and clean high enough
    if(S.stage==='C' && S.eviTotal >= S.eviNeed && !S.quizDone && !S.quizOpen && S.clean >= 92){
      openQuiz();
    }
  ...
  function startGame(){
    ...
    // ✅ HARD RESET (prevents starting in C/Quiz)
  ...
  function endGame(reason){
    ...
    const summary = {
      ...
      seed: ctx.seed, diff: ctx.diff, view: ctx.view, ai: ctx.ai ? 1 : 0,
  ...
  btnHow?.addEventListener('click', ()=>{
    toast('A: ยิงให้โดน • B: เก็บ 🍬🌙🚫🪥 ให้ครบ 3 • C: ตอบ Quiz วิเคราะห์เพื่อปิดเกม');
  }, { passive:true });
  ...
  WIN.BrushVR.boot = function(){
    // engine already reads QS; boot exists for compatibility
    if(ctx.debug) console.log('[BrushVR] boot(ctx) ok');
  };
