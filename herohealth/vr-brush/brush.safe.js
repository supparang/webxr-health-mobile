...
(function(){
  ...
  const STUDY_ID = String(qs('studyId',''));
  const PHASE = String(qs('phase',''));
  const CONDITION = String(qs('conditionGroup',''));
  ...
  const UI = {
    ...
    cam:   D.getElementById('cam'),
    spawnRoot: D.getElementById('spawnRoot'),
    tooth: D.getElementById('tooth')
  };
  ...
  const S = {
    ...
    // target map
    targets: new Map(), // id -> { el, kind, bornAt, ttlAt, good }
    seq: 0,
  ...
  function hud(){
    ...
    const acc = (S.goodHit + S.goodExpire) ? Math.round((S.goodHit / Math.max(1, (S.goodHit + S.goodExpire))) * 100) : 0;
  ...
  /* -------------------------
   * A-Frame ready / camera ready (NO CRASH)
   * ------------------------- */
  function getCam3(){
    try{
  ...
  function spawnTarget(kind='plaque'){
    ...
    const good = (kind === 'plaque'); // plaque = good, germ = junk (for brush)
    if (good) S.goodSpawn++; else S.junkSpawn++;

    // position: around tooth front area (world-ish)
    ...
    el.setAttribute('class', 'brush-target');
    ...
    S.targets.set(id, { el, kind, bornAt, ttlAt, good });

    // log spawn (non-blocking)
    safePost(API, {
      table: 'events',
  ...
  function hitTarget(id){
    ...
    if (t.good){
      ...
      S.score += (10 + Math.min(10, S.combo)); // arcade feel
      S.rtGood.push(rt);
    }else{
      // junk hit penalty
      S.junkHit++;
    ...
    // remove entity
    try{ t.el && t.el.parentNode && t.el.parentNode.removeChild(t.el); }catch(e){}
    S.targets.delete(id);

    hud();

    // log hit (non-blocking)
    safePost(API, {
      table: 'events',
   ...
   * cVR shoot (hha:shoot)
   * ------------------------- */
  function findNearestTargetToCenter(lockPx=44){
    // A-Frame click from center: use raycaster from camera
    try{
      const camEl = UI.cam || D.querySelector('#cam') || D.querySelector('[camera]');
      if (!camEl) return null;

      // build ray from camera forward
      const cam3 = getCam3();
      if (!cam3) return null;
      ...
      // collect objects from targets
      const objs = [];
      for (const t of S.targets.values()){
        const o = t.el && t.el.object3D;
        if (o) objs.push(o);
      }
      ...
      // find first hit target entity ancestor id
      let obj = hits[0].object;
      while (obj && obj.el == null && obj.parent) obj = obj.parent;
      const el = hits[0].object.el || hits[0].object.parent?.el;
      if (!el) return null;

      // climb to entity with id in targets map
      let cur = el;
      while (cur && cur.id && !S.targets.has(cur.id)){
        cur = cur.parentEl;
      }
      if (cur && S.targets.has(cur.id)) return cur.id;
  ...
  function onShoot(){
    ...
    const id = findNearestTargetToCenter(W.HHA_VRUI_CONFIG?.lockPx || S.lockPx);
    if (id) hitTarget(id);
  }
  ...
  function endGame(){
    ...
    UI.panelEnd && UI.panelEnd.classList.remove('hidden');

    // remote session (non-blocking)
    safePost(API, {
      table: 'sessions',
  ...
  function loop(ts){
    ...
    if (tnow - lastSpawn >= S.spawnEveryMs){
      lastSpawn = tnow;
      // mostly plaque (good), sometimes germ (junk)
   ...
   * ------------------------- */
  function bindUI(){
    UI.btnHelp && UI.btnHelp.addEventListener('click', ()=> UI.panelHelp && UI.panelHelp.classList.remove('hidden'));
    UI.btnCloseHelp && UI.btnCloseHelp.addEventListener('click', ()=> UI.panelHelp && UI.panelHelp.classList.add('hidden'));

    UI.btnStart && UI.btnStart.addEventListener('click', async ()=>{
      if (S.started || S.ended) return;
      UI.btnStart.textContent = 'กำลังเริ่ม...';

      // wait scene/cam ready (critical)
    ...
    UI.btnReplay && UI.btnReplay.addEventListener('click', ()=>{
      UI.panelEnd && UI.panelEnd.classList.add('hidden');
      resetGame();
      UI.btnStart && (UI.btnStart.textContent = 'เริ่มเล่น');
    });

    UI.btnBack && UI.btnBack.addEventListener('click', ()=>{
      // go hub safely
      const hub = qs('hub','../hub.html');
      location.href = hub;
    });

    // cVR shoot (from vr-ui.js)
    W.addEventListener('hha:shoot', ()=>{
      if (!IS_CVR) return;
      onShoot();
    }, { passive:true });

    // allow click/tap center in cVR to shoot
  ...
  async function boot(){
    ...
    // make sure camera ready ASAP (no crash)
    await waitForSceneAndCamera();

    // NOTE: we do NOT auto-start; user clicks Start
    console.log('[Brush] booted', { run: RUN, diff: DIFF, time: TIME, pid: PID, cvr: IS_CVR });
  }
