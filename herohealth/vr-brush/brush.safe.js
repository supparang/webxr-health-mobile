...
// FULL v20260302-BRUSH-SAFE-ALLINONE (targets + camera-ready + 403-safe + FX+missions+AI + schema log)
'use strict';
...
export function bootGame(){
  ...
  // ---------- schema helpers (events/sessions minimal but aligned) ----------
  ...
  // EVENT schema (exact column set you provided)
  ...
  function logEvent(eventType, payload={}){
    if(!LOG_ON) return;
    safePost(API, buildEventRow(eventType, payload)); // non-blocking
  }

  // SESSION schema (fill what brush has + extras)
  ...
  function spawnTarget(kind='plaque'){
    ...
    logEvent('target_spawn', { targetId: id, itemType: kind, judgment: 'spawn', isGood: good });
  }
    ...
    for (const [id, t] of S.targets.entries()){
      if (tnow >= t.ttlAt){
        if (t.good){
          ...
          S.miss++;
          S.combo = 0;
          logEvent('target_expire', { targetId:id, itemType:t.kind, isGood:1, judgment:'expire' });
        } else {
          logEvent('target_expire', { targetId:id, itemType:t.kind, isGood:0, judgment:'expire' });
        }
        try{ t.el?.parentNode?.removeChild(t.el); }catch(e){}
        S.targets.delete(id);
      }
