...
// PATCH v20260303-brush-ABC-AI-PRED-EVENTS-FULL
// ✅ Stage A/B/C
// ✅ Evidence 3 types (B)
// ✅ Quiz Analyze (C)
...
(function(){
  ...
  function stageFromProgress(){
    // A: 0-40, B: 40-80, C: 80-100 (but with constraints)
  ...
  function canFinishC(){
    // ต้องตอบ quiz C ก่อนถึงจะ “ชนะ” (ถ้าอยู่ stage C)
    if(S.stage !== 'C') return true;
    return !!S.quizDone;
  }

  function openQuiz(){
    if(!quiz) { S.quizDone = true; return; } // safety
  ...
  function applyHitRewards(t, remainMs, weakHit){
    ...
    if(t.type==='evi') gain *= 0.85; // evidence is less “clean” but helps stage B
    if(weakHit) gain *= 1.22;

    // Stage B: ต้องเก็บ evidence ให้ครบ 3 ก่อนจะเร่ง clean เกิน 80
    if(S.stage==='B' && S.eviTotal < S.eviNeed && S.clean >= 79){
      gain *= 0.18; // ชะลอไว้ให้ไปหา evidence
    }

    S.clean = clamp(S.clean + gain, 0, 100);

    // Stage B evidence collect
  ...
  function spawnOne(){
    ...
    if(!S.bossActive && S.clean >= S.nextBossAt && S.clean < 100){
      ...
      toast('💎 BOSS PLAQUE!');
      aiEmit('boss', { state:'start', hp:'?', stage:S.stage });
      return;
    }

    // Stage B: spawn evidence sometimes until all 3 collected
  ...
  function handleHit(t, x, y, source){
    ...
    // Stage C: เปิด quiz เมื่อ clean แตะ 92+ และยังไม่ทำ quiz
    advanceStageIfNeeded();
    ...
    // win condition: clean 100 + quiz (if C)
  ...
  function bindQuiz(){
    ...
    btnQuizSubmit?.addEventListener('click', ()=>{
      const a = quizAnswer();
      // Correct answer: “เพราะน้ำตาลทำให้แบคทีเรียสร้างกรด กัดฟัน เกิดคราบ”
      // เรา set value="b" เป็นคำตอบที่ถูกใน HTML
      const ok = (a === 'b');
      applyQuizResult(ok);
    }, { passive:true });

    btnQuizSkip?.addEventListener('click', ()=>{
      // skip counts as done but not correct
