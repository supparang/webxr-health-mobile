...
// PATCH v20260303e-brush-ABC-AI-PRED-EVENTS-FULL + FIX DOUBLE FIRE
...
// ✅ stable menu/end + daily gate + last summary + history
(function(){
  'use strict';
  ...
  function emit(type, detail){ try{ WIN.dispatchEvent(new CustomEvent(type,{detail})); }catch(_){ } }
  ...
  const ctx = {
    ...
    view: (qs.get('view')||getViewAuto()).toLowerCase(),
  ...
  function onShoot(ev){
    // ✅ ignore in non-cVR to prevent double fire with pointer targets
    if(!IS_CVR) return;
    if(!S.running || S.paused || S.ended || S.quizOpen) return;
  ...
  // layer fallback: only for non-cVR, and avoid double count if clicking target
