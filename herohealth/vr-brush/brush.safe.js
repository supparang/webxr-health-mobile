...
✅ Fever bar + Fever mode (via FunBoost if available)
...
✅ Emits: brush:ai (major beats) for HUD
*/
(function(){
  'use strict';
  ...
  function boot(passedCtx){
    ...
    // ---------- context (merge URL + passed ctx) ----------
    const qs = getQS();
    const ctx = {
      hub: qs.get('hub') || passedCtx?.hub || '../hub.html',
      run: (qs.get('run') || qs.get('mode') || passedCtx?.run || 'play'),
      view: (qs.get('view') || passedCtx?.view || getViewAuto()),
      diff: ((qs.get('diff') || passedCtx?.diff || 'normal')).toLowerCase(),
      time: safeNum(qs.get('time'), safeNum(passedCtx?.time, 60)),
      seed: safeNum(qs.get('seed'), safeNum(passedCtx?.seed, Date.now())),
      pid: (qs.get('pid') || qs.get('participantId') || passedCtx?.pid || '').trim(),
      studyId: (qs.get('studyId') || passedCtx?.studyId || '').trim(),
      phase: (qs.get('phase') || passedCtx?.phase || '').trim(),
      conditionGroup: (qs.get('conditionGroup') || passedCtx?.conditionGroup || '').trim(),
      log: (qs.get('log') || passedCtx?.log || '').trim()
    };
    ...
    const st = {
      ...
      uid:0,
      targets: new Map(), // id -> {el, kind, born, die, hpMax, hp, fillEl}
      fired10s:false
    };
    ...
    function normalizeShoot(d){
      // Accept:
      // - page coords: {x,y} in client space
      // - normalized coords: 0..1
      // - already-local coords: {lx,ly}
      const r = layerRect();

      if(Number.isFinite(d.lx) && Number.isFinite(d.ly)){
        return { x: d.lx, y: d.ly };
      }
    ...
    function hud(force){
      ...
      const elapsed = st.running ? ((t - st.t0 - st.pauseAccum)/1000) : 0;
    ...
    function mkTarget({x,y,kind,hpMax}){
      ...
      // pointer tap: use LOCAL coords (critical)
      el.addEventListener('pointerdown', (ev)=>{
        ev.preventDefault();
        const r = layerRect();
        const lx = (x); // already local
        const ly = (y);
        onHitAt(lx, ly, { source:'tap', targetId:id, clientX: ev.clientX, clientY: ev.clientY, rect:r });
      }, { passive:false });
    ...
    function onHitAt(lx,ly, meta){
      ...
      // boss phase AI
      if(it.kind==='boss'){
        const hpPct = Math.round((it.hp / it.hpMax) * 100);
        emit('brush:ai', { type:'boss_phase', hp: hpPct, ts: Date.now() });
      }
      ...
      if(it.hp <= 0){
        ...
        if(it.kind==='boss'){
          ...
          emit('brush:ai', { type:'boss_break', ts: Date.now() });
        }
      }
    ...
    function tick(){
      ...
      // fever AI beat
      if(!!director.feverOn && !lastFeverOn){
        emit('brush:ai', { type:'fever_on', ts: Date.now() });
      }
      lastFeverOn = !!director.feverOn;
    ...
    function startGame(){
      ...
      st.uid = 0;
      st.targets.forEach((_,id)=> removeTarget(id,false));
      st.targets.clear();
    ...
    function endGame(reason){
      ...
      const elapsed = Math.min(ctx.time, (now() - st.t0 - st.pauseAccum)/1000);
    ...
    function togglePause(){
      ...
      if(st.paused){
        ...
        const t = now();
        if(st.pauseAt) st.pauseAccum += (t - st.pauseAt);
        st.pauseAt = 0;
        if(btnPause) btnPause.textContent = 'Pause';
        toast('▶ Resume');
        startLoops();
    ...
    // layer pointer: convert to LOCAL coords (critical)
    layer?.addEventListener('pointerdown', (ev)=>{
      if(ctx.view==='cvr') return; // cvr uses crosshair; avoid double fire
      if(!st.running || st.paused || st.over) return;
      const p = toLocal(ev.clientX, ev.clientY);
      onHitAt(p.x, p.y, { source:'layer' });
    }, { passive:true });
