...
// PATCH v20260304a-brush-ABC-AI-PRED-EVENTS-FULL
// ✅ Stage A/B/C
// ✅ Evidence 3 types (B)
// ✅ Quiz Analyze (C)
// ✅ AI Risk (prediction-only): emits brush:ai signals
// ✅ cVR aim assist + stable menu/end + no auto-summary
(function(){
  'use strict';
  ...
  const S = {
    ...
    clean:0, cleanGainPerHit:1.2, cleanLosePerMiss:0.6,
    baseSpawnMs:760, ttlMs:1650, perfectWindowMs:220,
    bossEveryPct:28, nextBossAt:28, bossActive:false,

    stage:'A',
    eviTotal:0, eviNeed:3,
    eviFlags:{ sugar:0, night:0, no_brush:0 },

    quizOpen:false, quizDone:false, quizCorrect:false,

    aiRisk:0, aiTip:'—', aiRiskBand:'low',
    missStreak:0, lastAiEmit:0,
  ...
  // start/end
  function startGame(){
    S.running=true; S.paused=false; S.ended=false; S.t0=now();
    S.score=0; S.combo=0; S.comboMax=0; S.miss=0; S.shots=0; S.hits=0; S.clean=0;
    S.stage='A'; S.eviTotal=0; S.eviFlags={ sugar:0, night:0, no_brush:0 };
    S.quizOpen=false; S.quizDone=false; S.quizCorrect=false;
    S.aiRisk=0; S.aiTip='—'; S.aiRiskBand='low'; S.missStreak=0; S.lastAiEmit=0;
    S.nextBossAt=S.bossEveryPct; S.bossActive=false;
    ...
    emit('hha:start', {
      ...
      timePlannedSec: ctx.time, ts: Date.now()
    });
  ...
  // layer fallback (avoid double count when clicking target)
