...
// FULL v20260302-BRUSH-SAFE-EMOJI-FALLBACK-VIS
'use strict';
...
export function bootGame(){
  ...
  // ✅ Fallback visuals: plate + dot + ring + emoji text
  function spawnTarget(kind='plaque'){
    ...
    const x = (Math.random() * 1.05 - 0.525);
    const y = (Math.random() * 0.75 - 0.22);
    const z = -3.0 + (Math.random() * 0.45 - 0.18);
    ...
    plate.setAttribute('geometry', 'primitive: plane; width: 0.62; height: 0.62');
    plate.setAttribute('material',
      good
        ? 'shader: flat; color:#22c55e; opacity:0.38; transparent:true'
        : 'shader: flat; color:#ef4444; opacity:0.38; transparent:true'
    );
    el.appendChild(plate);

    const dot = D.createElement('a-entity');
    dot.setAttribute('geometry', 'primitive: sphere; radius: 0.10');
    dot.setAttribute('material',
      good
        ? 'shader: flat; color:#86efac; opacity:0.95; transparent:true'
        : 'shader: flat; color:#fecaca; opacity:0.95; transparent:true'
    );
    dot.setAttribute('position', '0 0 0.02');
    el.appendChild(dot);

    const ring = D.createElement('a-entity');
    ring.setAttribute('geometry', 'primitive: ring; radiusInner: 0.26; radiusOuter: 0.33');
    ring.setAttribute('material',
      good
        ? 'shader: flat; color:#86efac; opacity:0.65; transparent:true; side:double'
        : 'shader: flat; color:#fecaca; opacity:0.65; transparent:true; side:double'
    );
  ...
  function expireTick(){
    ...
    for (const [id, t] of S.targets.entries()){
      if (tnow >= t.ttlAt){
        ...
        logEvent('target_expire', { targetId:id, emoji:t.emoji||'', itemType:t.kind, lane:t.lane||'', isGood:t.good?1:0, judgment:'expire' });
        try{ t.el?.parentNode?.removeChild(t.el); }catch(e){}
        S.targets.delete(id);
      }
    }
  ...
  function endGame(reason='complete'){
    ...
    const es = D.getElementById('endSummary');
    if (es){
      es.innerHTML =
    ...
    D.getElementById('panelEnd')?.classList.remove('hidden');
  ...
  async function start(){
    ...
    D.getElementById('btnStart') && (D.getElementById('btnStart').textContent = 'กำลังเล่น...');
    S.started = true;
    S.startMs = now();
  ...
  function bindUI(){
    ...
    D.getElementById('btnStart')?.addEventListener('click', async ()=>{
      if (S.started || S.ended) return;
      D.getElementById('btnStart').textContent = 'กำลังเริ่ม...';
      await start();
    });
    ...
    D.getElementById('btnReplay')?.addEventListener('click', ()=>{
      ...
      resetGame();
      D.getElementById('btnStart') && (D.getElementById('btnStart').textContent = 'เริ่มเล่น');
    });
  ...
  // expose
  const api = { start, state:()=>({ ...S, targetsSize:S.targets.size }), end:()=>endGame('debug'), reset:()=>resetGame() };
  W.HHBrush_SAFE = api;
  return api;
}