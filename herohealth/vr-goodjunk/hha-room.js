/* === /herohealth/vr-goodjunk/hha-room.js ===
   PATCH: align writes with current Firebase RTDB rules
   - presence/$uid: uid,pid,state,updatedAt
   - players/$uid: uid,pid,displayName,lastSeenAt
   - rooms/.../results/$uid: uid,pid,score,miss,bestStreak,finished,updatedAt
*/
(function (W) {
  'use strict';
  ...
  function log(...args) {
    console.log('[HHA_ROOM]', ...args);
  }

  function warn(...args) {
    console.warn('[HHA_ROOM]', ...args);
  }
  ...
  H.attachPresence = async function attachPresence(ctx) {
    ...
    try {
      ...
      warn('presence onDisconnect failed', err);
    }
    ...
    log('presence attached', { uid, pid: ctx.pid || 'anon', state: payload.state });
    return payload;
  };
  ...
  H.createRoom = async function createRoom(opts) {
    ...
    try {
      ...
      warn('createRoom onDisconnect failed', err);
    }
    ...
    log('room created', { roomId, game, mode, uid });
  ...
  H.joinRoom = async function joinRoom(opts) {
    ...
    try {
      ...
      warn('joinRoom onDisconnect failed', err);
    }
    ...
    log('room joined', { roomId, game, mode, uid });
  ...
  H.startMatch = async function startMatch(opts) {
    ...
    setTimeout(async () => {
      try {
        ...
        warn('startMatch post-countdown update failed', err);
      }
    }, countdownMs);
    ...
    log('match started', { roomId: opts.roomId, matchId, uid });
  ...
  H.updateProgress = async function updateProgress(opts) {
    ...
    // MUST match rules for rooms/.../progress/$uid and matches/.../players/$uid
    const updates = {};
    updates[base + '/progress/' + uid] = {
      updatedAt: now()
    };
  ...
  H.submitResult = async function submitResult(opts) {
    ...
    // MUST match rules for rooms/.../results/$uid exactly
    const row = {
      uid,
      pid: opts.pid || 'anon',
      score: Number(opts.score || 0),
