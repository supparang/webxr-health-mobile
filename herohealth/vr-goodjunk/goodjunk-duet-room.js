// === /herohealth/vr-goodjunk/goodjunk-duet-room.js ===
// FULL PATCH v20260409d-GJ-DUET-ROOM-GUARD
...
export function saveRecentRoom(mode='duet', roomId=''){
  if (!mode || !roomId) return;
  storageSet(`${STORAGE.recentRoomPrefix}${mode}`, roomId);
}

export function loadRecentRoom(mode='duet'){
  return storageGet(`${STORAGE.recentRoomPrefix}${mode}`, '');
}

export function classifyDuetRoom(ctx = {}, room = null){
  ...
  if (mode !== 'duet') {
    return {
      ...
      message: 'หน้านี้ใช้สำหรับ duet เท่านั้น'
    };
  }
  ...
  if (!roomId) {
    return {
      ...
      message: 'ต้องมี room ก่อนเข้าเล่นโหมด duet'
    };
  }
  ...
  if (clean(room.mode, '') !== 'duet') {
    return {
      ok: false,
      code: 'ROOM_MODE_MISMATCH',
      title: 'ห้องนี้ไม่ใช่ duet',
  ...
  if (count > 2) {
    return {
      ...
      message: `ห้อง duet ต้องมี 2 คนพอดี แต่ตอนนี้มี ${count} คน`
    };
  }
  ...
  if (clean(room.status, 'waiting') !== 'started') {
    return {
      ...
      message: 'ครบ 2 คนแล้ว แต่ห้องยังไม่ถูกสั่งเริ่ม'
    };
  }

  const players = getPlayers(room);
  const teammate = players.find(p => clean(p.pid, '') !== pid) || null;
...
export async function ensureDuetSession(ctx = {}, options = {}){
  ...
  const result = classifyDuetRoom(ctx, room);
...
export function watchDuetSession(ctx = {}, callback, options = {}){
  const adapter = options.adapter || makeRoomAdapter();
  const roomId = clean(ctx.room, '');
  ...
  return adapter.subscribeRoom(roomId, (room) => {
    callback(classifyDuetRoom(ctx, room));
  });
}

export function buildDuetEngineQuery(ctx = {}, extra = {}){
  const payload = {
    mode: 'duet',
    entry: 'duet',
