// === /HeroHealth/vr/aframe-particles.js ===
// ระบบเอฟเฟกต์เศษชิ้นส่วน + ตัวเลขลอย สำหรับ A-Frame / Three.js
// ใช้กับ Good vs Junk VR (GameEngine.js)
//
// ES Module: export { SHARDS, setShardMode, burstAt, floatScore }

'use strict';

// พาเลตสีแต่ละโหมด (match กับชื่อ mode ที่ GameEngine ใช้)
const MODE_PALETTES = {
...
class ShardSystem {
  ...
  attach(sceneEl) {
    ...
    this.scene = sceneEl || (typeof document !== 'undefined'
  ...
  burst(worldPos, opts = {}) {
    ...
    const pal = Array.isArray(opts.palette)
      ? opts.palette
      : (MODE_PALETTES[this.mode] || MODE_PALETTES.default);

    const count = Math.max(4, Math.min(48,
      Math.floor(opts.count == null ? 18 : opts.count)
    ));
    const speed = Number.isFinite(opts.speed)  ? opts.speed  : 2.0;
    const size  = Number.isFinite(opts.size)   ? opts.size   : 1.0;
    const life  = Math.max(300, Math.floor(
      opts.life == null ? this.defaultLife : opts.life
    ));
    ...
    for (let i = 0; i < count; i++) {
      ...
      shard.setAttribute(
        'rotation',
        `0 0 ${Math.floor(Math.random()*360)}`
      );
      ...
      shard.setAttribute(
        'material',
        `color:${col}; transparent:true; opacity:1; side:double`
      );
    ...
    if (this.active.length) {
      ...
      for (let i = 0; i < this.active.length; i++) {
        ...
        if (el.object3D) {
          ...
          // ถ้าไม่มี object3D แล้ว ให้คืน pool
          this._free(el);
        }
      }
      this.active = keep;
...
// singleton ของระบบเศษชิ้นส่วน
export const SHARDS = new ShardSystem();

// sync กับ window เพื่อ debug ง่าย ๆ (ไม่จำเป็น แต่เผื่อใช้ใน console)
if (typeof window !== 'undefined') {
  window.SHARDS = SHARDS;
}

// เปลี่ยนโหมดสี เช่น 'goodjunk', 'hydration'
export function setShardMode(mode) {
  SHARDS.setMode(mode);
}

// สร้าง burst ที่ world position ที่กำหนด
// - sceneEl: a-scene (ถ้าไม่ส่ง จะใช้ตัวแรกใน DOM)
// - worldPos: THREE.Vector3 หรือ {x,y,z}
// - opts: {mode, color, count, speed, size, life, gravity, palette}
export function burstAt(sceneEl, worldPos, opts = {}) {
  if (!worldPos) return;
  if (!SHARDS.root) {
    SHARDS.attach(sceneEl);
  }
  if (opts && opts.mode) {
    SHARDS.setMode(opts.mode);
  }
  SHARDS.burst(worldPos, opts);
}

// แสดงตัวเลขคะแนนลอยขึ้นใน 3D HUD
export function floatScore(sceneEl, worldPos, text = '+10', color = '#ffffff') {
  const scene = sceneEl || (typeof document !== 'undefined'
    ? document.querySelector('a-scene')
    : null);
  if (!scene || !worldPos || typeof document === 'undefined') return;
  ...
  if (hasTroika) {
    el.setAttribute(
      'troika-text',
      `value:${text}; color:${color}; fontSize:0.10;`
    );
  } else {
    el.setAttribute(
      'text',
      `value:${text}; color:${color}; align:center; width:2.2`
    );
  }
  ...
  setTimeout(() => {
    try { scene.removeChild(el); } catch (e) {}
  }, 560);
}
