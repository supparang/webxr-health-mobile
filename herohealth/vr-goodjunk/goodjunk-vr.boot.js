...
export function boot(){
  ...
  function defaultAim(){
    // ค่า default: ต่ำกว่ากลางจอเล็กน้อย (ไม่ชน HUD มาก)
    setAimPoint(window.innerWidth*0.5, window.innerHeight*0.62);
  }

  // tap-anywhere (บน layer) = set aim point
  function bindAimListeners(){
    ...
    // อัปเดต aim ตอน resize
    window.addEventListener('resize', ()=>{
      const ap = window.__GJ_AIM_POINT__;
      if (!ap) defaultAim();
      // ถ้าเคยตั้งไว้แล้ว ไม่บังคับเปลี่ยน (แต่ถ้าหลุดจอค่อย clamp)
      else{
