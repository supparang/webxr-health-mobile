...
const DUET_UI = window.__gjDuetUi || null;

const STYLE_ID = 'goodjunk-safe-duet-style-20260320-v1';
const ROOT_ID = 'gjDuetRoot';
...
function boot() {
  ...
  bootWithGate(async () => {
    ...
    if (!ok) {
      showGate('เปิดห้อง Duet ไม่สำเร็จ', '...', 'ตรวจสอบ room แล้วลองใหม่');
      return;
    }
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .gj-stage{position:relative;width:100%;height:100%;min-height:320px;overflow:hidden;border:1px solid rgba(148,163,184,.18);border-radius:26px;background:radial-gradient(circle at 50% 0%, rgba(244,114,182,.10), transparent 30%),linear-gradient(180deg, rgba(15,23,42,.72), rgba(2,6,23,.78));box-shadow:0 24px 64px rgba(0,0,0,.22)}
    ...
    .gj-pair{display:grid;gap:10px;width:min(100%,420px)}
    .gj-pair-card{border:1px solid rgba(148,163,184,.18);border-radius:16px;background:rgba(255,255,255,.04);padding:10px}
    .gj-pair-row{display:flex;justify-content:space-between;gap:10px;align-items:center;font-size:13px;font-weight:900;color:#e5e7eb;margin-bottom:8px}
    ...
    .gj-meter-fill{width:100%;height:100%;transform-origin:left center;transition:transform .12s linear;background:linear-gradient(90deg, rgba(244,114,182,.95), rgba(56,189,248,.95))}
    ...
    .gj-progress-bar{width:100%;height:100%;background:linear-gradient(90deg, rgba(244,114,182,.85), rgba(14,165,233,.85));transform-origin:left center;transition:transform .12s linear}
  `;
...
function buildShell() {
      ...
      <div class="gj-shell">
        <header class="gj-topbar">
          <div class="gj-chip-row">
            ...
            <div class="gj-chip"><span>Pair Score</span><strong id="gjPairScore">0</strong></div>
        ...
        <div class="gj-stage-wrap">
          <div class="gj-stage" id="gjStage">
            <div class="gj-center-tip" id="gjCenterTip">ช่วยกันเก็บของดีให้ได้มากที่สุดเป็นคู่</div>
            ...
            <div class="gj-bottom-top">
              <div>
                ...
                <div class="gj-legend" id="gjHintText" style="margin-top:8px;">
                  <div>Tip: คะแนนของคุณจะรวมกับคะแนนเพื่อนเป็นคะแนนคู่</div>
                </div>
              </div>

              <div class="gj-pair">
                <div class="gj-pair-card">
                  <div class="gj-pair-row">
                    <span>PAIR GOAL</span>
                    <strong id="gjPairGoal">0</strong>
                  </div>
                  <div class="gj-meter"><div class="gj-meter-fill" id="gjPairFill"></div></div>
                </div>
              </div>
            </div>
  ...
  ui.pairScore = document.getElementById('gjPairScore');
  ui.pairGoal = document.getElementById('gjPairGoal');
  ui.pairFill = document.getElementById('gjPairFill');
...
function bindShell() {
  ...
  document.getElementById('btnDuetRematch')?.addEventListener('click', async () => {
    const nextSeed = await resetRoomForRematch();
    location.replace(buildLobbyUrl(nextSeed));
  });

  document.getElementById('btnDuetBackLobby')?.addEventListener('click', () => {
    location.replace(buildLobbyUrl());
  });

  document.getElementById('btnDuetExport')?.addEventListener('click', () => {
    downloadJson(lastSummary, `goodjunk-duet-${safeFilePart(GJ_ROOM_ID)}-${Date.now()}.json`);
  });

  document.getElementById('btnDuetBackHub')?.addEventListener('click', () => {
...
function makeFreshSeed() { return `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`; }
...
function startGame() {
  ...
  if (ui.centerTip) {
    ...
    ui.centerTip.textContent = 'ช่วยกันเก็บของดีให้ได้มากที่สุดเป็นคู่';
    setTimeout(() => ui.centerTip?.classList.add('hide'), 1800);
  }

  if (DUET_UI?.setStatus) DUET_UI.setStatus('running');
...
function hitTarget(id) {
  ...
  if (target.type === 'good') {
    ...
    state.score += gain;
    createFx(target.x + target.size / 2, target.y + target.size / 2, `+${gain}`, '#f9a8d4');
    updateHint('เก่งมาก คะแนนของคุณจะรวมเป็นคะแนนคู่');
  } else {
    state.hitsBad += 1;
    state.miss += 1;
    state.streak = 0;
    ...
    updateHint('ระวัง junk! อย่าให้คะแนนคู่ตก');
  }
...
function registerMissGood(target) {
  ...
  updateHint('ของดีหลุดไปแล้ว รีบช่วยเพื่อนเก็บชิ้นต่อไป');
...
function getPairScore(room = liveRoom) {
  const players = getDuetPlayers(room);
  let total = 0;

  players.forEach((p) => {
    if (p.id === GJ_PID && localRunActive && !state.ended) total += Number(state.score || 0);
    else total += Number(p.finalScore || 0);
  });

  return total;
}

function getPairGoal(room = liveRoom) {
  return 360;
}
...
function renderHud() {
  ...
  const pairScore = getPairScore(liveRoom);
  const pairGoal = getPairGoal(liveRoom);

  if (ui.pairScore) ui.pairScore.textContent = String(pairScore);
  if (ui.pairGoal) ui.pairGoal.textContent = String(pairGoal);
  if (ui.pairFill) ui.pairFill.style.transform = `scaleX(${clamp01(pairScore / Math.max(1, pairGoal))})`;
...
function toast(message) {
  if (window.__gjShowDuetRunMessage) {
    window.__gjShowDuetRunMessage(message);
    clearTimeout(toast._t);
    toast._t = setTimeout(() => {
      window.__gjHideDuetRunMessage?.();
    }, 2200);
  }
}
...
function showGate(msg = 'กำลังรอเริ่ม', count = '-', sub = '') {
  if (DUET_UI?.showGate) DUET_UI.showGate(msg, count, sub);
}

function hideGate() {
  if (DUET_UI?.hideGate) DUET_UI.hideGate();
}
...
function waitUntilStart(startAt) {
  return new Promise((resolve) => {
    const tick = () => {
      ...
      if (left <= 0) {
        showGate('เริ่ม Duet', 'GO!', 'กำลังเข้าสู่เกม...');
...
function sanitizeRoom(room) {
  ...
  const rawDuet = rawMatch.duet && typeof rawMatch.duet === 'object' ? rawMatch.duet : {};

  return {
    roomId: __normalizeRoomId(room.roomId || GJ_ROOM_ID || ''),
    hostId: __normalizePid(room.hostId || ''),
    mode: String(room.mode || 'duet'),
    seed: String(room.seed || RUN_CTX.seed || Date.now()),
    minPlayers: 2,
    maxPlayers: 2,
    ...
    match: {
      ...
      status: ['idle', 'countdown', 'running', 'finished'].includes(rawMatch.status)
        ...
        : 'idle',
      duet: {
        finishedAt: Number(rawDuet.finishedAt || 0)
      }
    }
  };
}
...
function roomToFirebase(room) {
  const out = {
    ...
    match: {
      ...
      duet: {
        finishedAt: Number(room.match?.duet?.finishedAt || 0)
      }
    }
  };
...
function buildDuetSummaryPayload(room, players, pending = false, reason = '') {
  const pairScore = players.reduce((sum, p) => sum + Number(p.finalScore || 0), 0);
  return {
    version: '20260320-duet-v1',
    source: 'goodjunk-duet',
    gameId: GJ_GAME_ID,
    mode: 'duet',
    pid: GJ_PID,
    name: GJ_NAME,
    ...
    pairScore,
    pairGoal: getPairGoal({ players, match: { participantIds: players.map((p) => p.id) } }),
    updatedAt: Date.now(),
...
function hideSummary() {
  const wrap = document.getElementById('duetSummary');
...
function downloadJson(payload, filename = `goodjunk-duet-${Date.now()}.json`) {
  if (!payload) return;
...
async function loadRoom() {
  ...
  try {
    ...
    console.error('[goodjunk.safe.duet] loadRoom failed:', err);
    return null;
  }
}
...
async function saveRoom(room) {
  ...
  try {
    ...
    console.error('[goodjunk.safe.duet] saveRoom failed:', err);
    return false;
  }
}
...
async function markPresence(patch = {}) {
  ...
  try {
    ...
    console.error('[goodjunk.safe.duet] markPresence failed:', err);
  }
}
...
async function maybeFinalizeRoom(force = false) {
  ...
  const duetPlayers = set.size ? players.filter((p) => set.has(p.id)) : players.slice(0, 2);

  if (!duetPlayers.length) return;
  ...
  const nextDuetPlayers = set.size ? nextPlayers.filter((p) => set.has(p.id)) : nextPlayers.slice(0, 2);
  const allFinished = nextDuetPlayers.length === 2 && nextDuetPlayers.every((p) => p.finished);
  ...
  if (room.status === 'finished') {
    ...
    stopHeartbeat();
    stopWatchdog();
    showSummary(room, nextDuetPlayers, false, 'finished');
  }
}
...
async function publishFinish(reason = 'finished') {
  ...
  const players = getDuetPlayers(room);
  const allFinished = players.length === 2 && players.every((p) => p.finished);
...
function showSummary(room, players, pending = false, reason = '') {
  const wrap = document.getElementById('duetSummary');
  const rowsBox = document.getElementById('duetSummaryRows');
  const sub = document.getElementById('duetSummarySub');
  const hint = document.getElementById('duetSummaryHint');
  const myScoreEl = document.getElementById('duetMyScore');
  const partnerScoreEl = document.getElementById('duetPartnerScore');
  const pairScoreEl = document.getElementById('duetPairScore');

  if (!wrap || !rowsBox) return;
  if (!amIParticipant(room)) return;

  const ordered = [...players].sort((a, b) => {
    if (a.id === GJ_PID) return -1;
    if (b.id === GJ_PID) return 1;
    return 0;
  });

  const me = ordered.find((p) => p.id === GJ_PID);
  const partner = ordered.find((p) => p.id !== GJ_PID);
  const pairScore = ordered.reduce((sum, p) => sum + Number(p.finalScore || 0), 0);

  rowsBox.innerHTML = ordered.map((p) => {
      ...
      <div class="result-row ${isMe ? 'is-me' : ''}">
        <div>
          <div style="font-weight:800;">
            ...
            ${isMe ? '<span style="color:#f9a8d4;"> • คุณ</span>' : '<span style="color:#cbd5e1;"> • เพื่อน</span>'}
          </div>
  ...
  if (myScoreEl) myScoreEl.textContent = String(Number(me?.finalScore || 0));
  if (partnerScoreEl) partnerScoreEl.textContent = String(Number(partner?.finalScore || 0));
  if (pairScoreEl) pairScoreEl.textContent = String(pairScore);

  if (sub) {
    sub.textContent = pending
      ? 'ผลชั่วคราว • คุณจบรอบแล้ว แต่ยังรอเพื่อนอีกคน'
      : 'ผลสุดท้าย • ผู้เล่นทั้งคู่จบรอบแล้ว';
  }

  if (hint) {
    hint.textContent = pending
      ? 'summary นี้ยังไม่ final จนกว่าเพื่อนอีกคนจะจบหรือถูกตัดสิทธิ์'
      : `คะแนนรวมคู่ ${pairScore} / เป้าหมาย ${getPairGoal(room)}`;
  }

  lastSummary = buildDuetSummaryPayload(room, ordered, pending, reason);
  wrap.hidden = false;
  if (DUET_UI?.setStatus) DUET_UI.setStatus(pending ? 'pending' : 'finished');
}
...
function buildLobbyUrl(seedOverride = '') {
  const nextSeed = String(seedOverride || liveRoom?.seed || RUN_CTX.seed || Date.now());
  ...
  return `./goodjunk-duet-lobby.html?${q.toString()}`;
}
...
async function bootWithGate(startFn) {
  ...
  if (!getEffectiveStartAt()) {
    showGate('กำลังรอเริ่ม Duet', '...', 'ยังไม่มีสัญญาณเริ่มจากห้อง');
    return;
  }
  ...
  ensureFirebase().then((ok) => {
    ...
    roomRef.on('value', async (snap) => {
      ...
      if (room.status === 'finished') {
        ...
        stopHeartbeat();
        stopWatchdog();
        showSummary(room, getDuetPlayers(room), false, 'finished');
        return;
      }

      if (room.status === 'running' && DUET_UI?.setStatus) {
        DUET_UI.setStatus('running');
      }
