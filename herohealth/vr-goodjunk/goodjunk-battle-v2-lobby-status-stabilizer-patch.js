(function GoodJunkBattleV2LobbyStatusStabilizerPatch(){
  ...
  const PATCH_VERSION = 'v2.4.36-lobby-status-stabilizer';
  ...
  function getRoomFromLobby(){
    if (
      ...
      window.GJ_BATTLE_LOBBY.state.room
    ){
      return window.GJ_BATTLE_LOBBY.state.room;
    }

    if (
      window.GJ_BATTLE_LOBBY_HOST_REPAIR &&
      window.GJ_BATTLE_LOBBY_HOST_REPAIR.state
    ){
      return window.GJ_BATTLE_LOBBY_HOST_REPAIR.state.lastRoom || null;
    }
  ...
  function normalizeRoom(room){
    ...
    const me = players.find(function(p){
      return String(p.id) === mePid || String(p.pid) === mePid;
    }) || null;

    const meIsHost = !!(
      me &&
      (
        String(me.id) === hostPid ||
        String(me.pid) === hostPid ||
        me.host ||
        me.role === 'host'
      )
    );
  ...
  function decideStableStatus(){
    ...
    const dbReady = !!(
      ...
      (
        ...
        typeof window.GJ_BATTLE_FIREBASE_BRIDGE.isReady === 'function' &&
        window.GJ_BATTLE_FIREBASE_BRIDGE.isReady()
      )
    );
    ...
    if (!dbReady){
      return {
        key:'db-wait',
        cls:'warn',
        text:'DB กำลังเชื่อมต่อ • รอสักครู่',
        btnText:'⚠️ DB ยังไม่พร้อม',
        disabled:true,
        ready:false
      };
    }
    ...
    if (!nr.code){
      return {
        key:'ready-no-room',
        cls:'good',
        text:'DB พร้อมแล้ว • สร้างห้องหรือเข้าห้องได้',
    ...
    if (nr.phase === 'play' || nr.phase === 'playing' || nr.phase === 'battle'){
      return {
        key:'play-phase',
        cls:'good',
        text:'Battle เริ่มแล้ว • กำลังเข้าเกม',
        btnText:'⚔️ Battle เริ่มแล้ว',
        disabled:true,
        ready:false
      };
    }
    ...
    if (onlineCount >= 2 && nr.meIsHost){
      return {
        key:'ready-host-' + nr.hostPid + '-' + onlineCount,
        cls:'good',
        text:'พร้อมแล้ว • มีผู้เล่นออนไลน์ ' + onlineCount + ' คน • คุณเป็น Host เริ่ม Battle ได้',
        btnText:'⚔️ Host เริ่ม Battle',
        disabled:false,
        ready:true
      };
    }
    ...
    if (onlineCount >= 2 && !nr.meIsHost){
      ...
      return {
        key:'ready-wait-host-' + nr.hostPid + '-' + onlineCount,
        cls:'warn',
        text:'พร้อมแล้ว • มีผู้เล่นออนไลน์ ' + onlineCount + ' คน • รอ ' + hostName + ' เริ่ม Battle',
        btnText:'⏳ รอ Host เริ่ม Battle',
        disabled:true,
        ready:false
      };
    }
    ...
    if (onlineCount === 1){
      return {
        key:'need-one-' + (nr.me ? nr.me.id : 'me'),
        cls:'warn',
        text:'อยู่ในห้องแล้ว • รอคู่แข่งอีก 1 คน',
        btnText:'⏳ รอผู้เล่นอีก 1 คน',
        disabled:true,
        ready:false
      };
    ...
    return {
      key:'sync-empty',
      cls:'warn',
      text:'กำลัง sync ผู้เล่นในห้อง...',
      btnText:'⏳ ยังไม่พร้อมเริ่ม',
      disabled:true,
      ready:false
    };
  }
  ...
  function applyWanted(force){
    ...
    if (!force && key !== state.lastAppliedKey && age < STABLE_MS){
      return;
    }
  ...
  function patchSetStatus(){
    if (
      !window.GJ_BATTLE_LOBBY ||
      window.GJ_BATTLE_LOBBY.__statusStabilized
    ){
      return;
    }

    window.GJ_BATTLE_LOBBY.__statusStabilized = true;

    /*
     * ไม่ไปแก้ source function เดิมโดยตรง แต่ให้ patch นี้ทับผลลัพธ์ทุก 350ms
     * เพื่อกัน setStatus จากหลาย interval เขียนสลับกัน
     */
  }
