...
// GoodJunk Duet Lobby
// FULL PATCH v20260320-DUET-LOBBY-V1
...
function buildInviteUrl() {
  const currentSeed = String(room?.seed || ctx.seed || Date.now());
...
function getJoinBlockReason(r, pid = ctx.pid) {
  ...
  if (existing) return '';
  if ((activePlayers(r).length || 0) >= 2) return 'ห้อง Duet เต็มแล้ว';
...
function sanitizeRoom(r) {
  ...
  safe.status = ['waiting', 'countdown', 'running', 'finished'].includes(safe.status)
    ? safe.status
    : 'waiting';
  safe.seed = String(safe.seed || ctx.seed || Date.now());
...
function roomToFirebase(r) {
  const out = {
    ...
    hostId: r.hostId,
    mode: r.mode,
    seed: String(r.seed || ctx.seed || Date.now()),
    minPlayers: 2,
    maxPlayers: 2,
    status: r.status,
    ...
    match: {
      ...
      duet: {
        finishedAt: Number(r.match?.duet?.finishedAt || 0)
      }
    }
  };
...
function renderButtons(r = room) {
  ...
  if (els.btnStart) {
    ...
    els.btnStart.textContent = isHost(r) ? 'เริ่ม Duet' : 'รอ host เริ่ม';
  }
}
...
function renderStatus(r = room) {
  ...
  if (r.status === 'waiting') {
    ...
    if (readyCount < 2) {
      ...
      setHint('ผู้เล่นพร้อมครบแล้ว กดเริ่ม Duet ได้');
    } else {
      setHint('ผู้เล่นพร้อมครบแล้ว รอ host กดเริ่ม');
    }

    setCopyState('ส่ง room code หรือลิงก์นี้ให้เพื่อนอีกคนเข้าร่วมได้');
    if (els.countdown) els.countdown.textContent = '';
  }

  if (r.status === 'countdown') {
    if (amIMatchParticipant(r)) {
      setHint('กำลังนับถอยหลังก่อนเริ่ม Duet');
    } else {
      setHint('รอบนี้เริ่มแล้ว แต่คุณไม่ได้อยู่ใน participant');
    }
    setCopyState('ห้องถูกล็อกแล้ว กำลังจะเริ่ม Duet', false);
  }

  if (r.status === 'running') {
    if (amIMatchParticipant(r)) {
      setHint('Duet กำลังดำเนินอยู่');
    } else {
      setHint('Duet กำลังดำเนินอยู่ และคุณไม่ได้อยู่ใน participant ของรอบนี้');
    }
...
async function beginCountdown() {
  ...
  if (participantIds.length !== 2) {
    setHint('Duet ต้องมีผู้เล่นพร้อมครบ 2 คน');
    return;
  }

  const startAt = now() + 4000;
  const roundSeed = String(cur.seed || ctx.seed || Date.now());
...
async function enterRun(startAt) {
  ...
  const roundSeed = String(room?.seed || ctx.seed || Date.now());
  ...
  location.href = `./goodjunk-duet-run.html?v=20260320-duet-lobby-v1&${q.toString()}`;
}
...
async function boot() {
  try {
    ...
    setHint('กำลังสร้าง/เชื่อมห้อง Duet...');
