...
// FULL v20260304-SOLO-DUALWIN-3STAGE-PRO
'use strict';
...
export function boot(cfg){
  ...
  // ---------- MODE ----------
  // mode is a friendly wrapper for launcher (solo/battle/race/coop/coop2)
  ...
  const WIN_TARGET = (function(){
    ...
    if(view==='cvr' || view==='vr'){ scoreTarget = Math.round(scoreTarget * 0.96); } // a bit kinder in VR
  ...
  // NEW: Reasonable coach (top 2 risk factors) — simple, explainable for Grade 5
  function coachTop2(){
    const facts = [];
  ...
  // NEW: goal object (HUD uses it, but we show dual win)
  const goal = { name:'WIN', desc:'ถึง “คะแนน” หรือ “ของดี” ก่อน', cur:0, target:0 };
  const mini = { name:'—', t:0 };
  ...
  function emitScoreEvent(){
    try{
      const payload = {
        ...
        // NEW: solo progression
        goodCount: goodCount|0,
        winScoreTarget: WIN_TARGET.scoreTarget|0,
        winGoodTarget: WIN_TARGET.goodTarget|0,
        stage: stage|0,
        pro: !!PRO,
        mode
      };
  ...
  function buildEndSummary(reason){
    ...
    const acc = accPct();
    const winBy = (score >= WIN_TARGET.scoreTarget) ? 'score' : ((goodCount >= WIN_TARGET.goodTarget) ? 'good' : '');
    return {
      projectTag: 'GoodJunkVR',
      gameKey: HH_GAME,
      pid,
      zone: HH_CAT,
      gameVersion: 'GoodJunkVR_SAFE_2026-03-04_SOLO_DUALWIN_3STAGE_PRO',
      device: view,
      runMode: runMode,
      diff: diff,
      seed: seedStr,
  ...
  function showEnd(reason){
    ...
    if(endOverlay){
      ...
      if(endTitle){
        endTitle.textContent = (summary.reason==='win') ? 'CLEAR!' : 'Game Over';
      }
      if(endSub){
        endSub.textContent =
          `reason=${summary.reason} | mode=${mode} | diff=${diff}${PRO?'(PRO)':''} | acc=${summary.accPct}% | good=${summary.goodCount}/${summary.winGoodTarget} | score=${summary.scoreFinal}/${summary.winScoreTarget}`;
      }
  ...
  function spawnTick(dt){
    ...
    // --- NEW: 3-stage progression (Warm -> Trick -> Boss) ---
    ...
    // Stage probabilities (Grade 5 tuned)
    ...
    while(spawnAcc >= 1){
      ...
      // boss unlock rule (existing-ish): only stage 2, and only when time is later
      if(stage===2 && !bossActive && tLeft <= plannedSec*0.35 && tLeft > 6){
        bossActive = true;
      ...
      if(stage===2 && bossActive && (r01() < 0.22)){
        kind = 'boss';
      }else if(p < pGood){
        kind = 'good';
      }else if(p < (pGood + pJunk)){
        kind = 'junk';
      }else if(p < (pGood + pJunk + pBonus)){
        kind = 'bonus';
      }else{
        kind = 'shield';
      }

      if(kind==='good') makeTarget('good', rPick(GOOD), TUNE.ttlGood + (stage===0 ? 0.18 : 0));
  ...
  function checkEnd(){
    ...
    // ✅ NEW: Dual-WIN condition (score OR goodCount)
  ...
  sayCoach(PRO ? 'โหมด PRO: ของดีหายไวขึ้น แต่แฟร์! 🥦🍎' : 'แตะ “ของดี” เลี่ยงของเสีย! 🥦🍎');
  setHUD();
  requestAnimationFrame(tick);
}