...
// FULL v20260304-SAFE-HELPPAUSE-AIHUD-ACC-MEDRT-BATTLE-SPAWNSAFE
'use strict';
...
export function boot(cfg){
  ...
  function emitScoreEvent(force=false){
    const t = nowMs();
    if(!force && (t - lastScoreEmitMs) < 250) return; // 4 Hz
    lastScoreEmitMs = t;
  ...
  function buildEndSummary(reason){
    ...
    return {
      ...
      pid,
      zone: HH_CAT,
      gameVersion: 'GoodJunkVR_SAFE_2026-03-04_HELPPAUSE_AIHUD_ACC_MEDRT_BATTLE_SPAWNSAFE',
      device: view,
      runMode: runMode,
      diff: diff,
      seed: seedStr,
