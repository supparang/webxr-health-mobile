...
// GoodJunkVR — SAFE Engine (PRODUCTION) — HHA Standard (DUAL-EYE READY) — v2.1 Rect-Bounds Spawn
...
/**
 * ✅ IMPORTANT FIX:
 * spawn based on the REAL layer rect (not window innerHeight),
 * so stage offsets (HUD/controls) won't push targets off-screen.
 * Returns { x, y } local to layer.
 */
function randPosInLayer(rng, layerRect, safeMargins){
  const avoid = buildAvoidRects();

  let left = safeMargins?.left ?? 14;
  let right = safeMargins?.right ?? 14;
  let top = safeMargins?.top ?? 14;
  let bottom = safeMargins?.bottom ?? 14;

  const W = layerRect.width;
  const H = layerRect.height;

  if ((W - left - right) < 160){ left = 10; right = 10; }
  if ((H - top - bottom) < 180){ top = 10; bottom = 10; }

  // try multiple times avoiding UI rects (global coords)
  for (let i=0;i<22;i++){
    const gx = layerRect.left + left + rng() * (W - left - right);
    const gy = layerRect.top  + top  + rng() * (H - top - bottom);

    let ok = true;
    for (const r of avoid){
      if (pointInRect(gx, gy, { left:r.left-10, right:r.right+10, top:r.top-10, bottom:r.bottom+10 })){
        ok = false; break;
      }
    }
    if (ok){
      return { x: gx - layerRect.left, y: gy - layerRect.top };
    }
  }
...
function updateFever(shield, fever){
  ...
  const s = clamp(shield, 0, 9);
  try{ FeverUI.set({ value: f, shield: s }); }catch(_){}
  try{ if (typeof FeverUI.setShield === 'function') FeverUI.setShield(s); }catch(_){}
  try{
    ROOT.document?.documentElement?.style?.setProperty('--fever', String(f));
  }catch(_){}
}
...
export function boot(opts = {}) {
  ...
  const dual = !!layerR;
  const safeMargins = opts.safeMargins || { top: 14, bottom: 14, left: 14, right: 14 };
  ...
  function hitJunk(el){
    ...
    // little shake FX
    try{
      DOC.body.classList.add('shake');
      setTimeout(()=> DOC.body.classList.remove('shake'), 230);
    }catch(_){}
  ...
  function spawnOne(){
    ...
    const pL = randPosInLayer(S.rng, rectL, safeMargins);
    ...
    if (dual && layerR){
      const rectR = getRect(layerR);
      if (rectR){
        const pR = randPosInLayer(S.rng, rectR, safeMargins);
  ...
  function adaptiveTick(){
    ...
    if (S.runMode === 'play'){
      ...
      // high fever: help fairness a bit
      if (S.fever >= 70){
