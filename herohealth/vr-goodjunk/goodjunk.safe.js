...
// GoodJunkVR SAFE — PRODUCTION (PACK-FAIR | Boss++ A+B+C)
// ✅ Mobile / PC / VR(Cardboard) / cVR
// ✅ HUD-safe spawn via CSS vars: --gj-top-safe / --gj-bottom-safe (set by goodjunk-vr.html)
...
// ✅ AI hooks placeholder (OFF by default) — do not enable in research
...
export function boot(payload = {}) {
  ...
  // ----------------------- AI hooks placeholder (OFF) -----------------------
  // (เก็บจุดเสียบไว้ แต่ไม่ทำอะไรใน PACK-FAIR)
  const AI = {
    enabled: false,
    emit(){},
    tip(){},
  };
  // เปิดใช้เมื่อพร้อมเท่านั้น (และห้ามเปิดใน research)
  // if (payload.aiEnabled && payload.run !== 'research') AI.enabled = true;
  ...
  const GAME_VERSION = 'GoodJunkVR_SAFE_PACKFAIR_2026-01-20';
  ...
  function makeTargetKind(){
    ...
    // fairness: give a little good too
