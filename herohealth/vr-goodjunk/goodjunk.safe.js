...
// GoodJunkVR SAFE — v4.2 + GatePatch v20260210 (RUNNABLE)
...
// ✅ Fixes: dx*dx/dy*dy, template strings, aria-label strings, adaptive math, coach/mini msg
'use strict';
...
const emit = (n, d) => {
  try { WIN.dispatchEvent(new CustomEvent(n, { detail: d })); }
  catch {}
};
...
function badgeMeta(extra){
  let pid = '';
  try { pid = (typeof getPid === 'function') ? (getPid() || '') : ''; }
  catch {}

  let q;
  try { q = new URL(location.href).searchParams; }
...
function decorateTarget(el, t){
  ...
  if (t.kind === 'good'){
    ...
    el.setAttribute('aria-label', `${labelForGroup(gid)} ${emo}`); // ✅ FIX
  } else if (t.kind === 'junk'){
...
function dailyKey(prefix, category, pid){
  ...
  return `${prefix}:${category}:${p}:${day}`; // ✅ FIX
}
