...
// ✅ กันเป้าเกิดกลาง crosshair ง่ายเกิน (เพิ่มความท้าทาย)
function pickSpawnXY(rng){
  const safe = getSafeRect();
  const r = DOC.documentElement.getBoundingClientRect();
  const cx = r.width/2;
  const cy = r.height/2;

  const excludeR = 110; // ปรับ 90-140 ได้

  for(let i=0;i<12;i++){
    const x = safe.x + rng()*safe.w;
    const y = safe.y + rng()*safe.h;
    const dx = x - cx, dy = y - cy;
    if((dx*dx + dy*dy) >= excludeR*excludeR) return { x, y };
  }
  return { x: safe.x + rng()*safe.w, y: safe.y + rng()*safe.h };
}
