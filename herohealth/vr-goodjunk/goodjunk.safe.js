...
// GoodJunkVR SAFE — v4.2 + GatePatch v20260210 (FIXED FULL)
...
// ✅ Cooldown routing uses cdGateUrl/cdur but enforces per-category key
// ✅ FIX: dxdx/dydy -> dx*dx + dy*dy
// ✅ FIX: template strings, aria-label, coach/mini strings
// ✅ FIX: adaptive formula typos (played0.002 etc.)
// ✅ FIX: safe defaults + no crash in missing UI nodes
...
function showSummaryOverlay(summary, ctx, onNext) {
  ...
  try {
    ...
    const msg =
      ((summary.miss ?? 99) <= 3 && (summary.scoreFinal ?? 0) >= 150) ? 'สุดยอด! ความแม่นยำดีมาก 👍' :
        ((summary.miss ?? 99) <= 6) ? 'ดีมาก! รอบหน้าลองลด MISS ลงอีกนิดนะ ✨' :
    ...
    if (btnHub) {
      ...
      n.addEventListener('click', () => {
        location.href = (ctx && ctx.hub) ? ctx.hub : '../hub.html';
      });
    }
    if (btnNext) {
...
export function boot(opts = {}) {
  ...
  function onShoot(ev) {
    ...
    if (aiOn) AI.onEvent('shoot', { t: (performance.now ? performance.now() : Date.now()) });
  ...
  function endGame(reason = 'timeup') {
    ...
    // ✅ Show summary first, then route when user clicks "ต่อไป"
  ...
  function tick(ts) {
    ...
    if (ts - S.lastSpawn >= (Number(D.spawnMs) || 900)) {
      S.lastSpawn = ts;
      const r = S.rng();
