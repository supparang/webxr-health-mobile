/* =========================================================
   EAP Word Quest • Core-Aligned Local AI — Student Scoped
   File: /herohealth/eap-word-quest/eap-word-engine-v195-core-ai-student-scoped.js
   Version: v1.9.5-CORE-AI-STUDENT-SCOPED-122
...
(() => {
  ...
  const VERSION = "v1.9.5-CORE-AI-STUDENT-SCOPED-122";
  const GROUP = "122";
  const MAX_HINTS_PER_ITEM = 3;
  const NO_REPEAT_WINDOW = 5;
  const STORAGE = {
    state: "EAP_CORE_AI_STATE_V195",
    weak: "EAP_CORE_AI_WEAK_V195",
    recent: "EAP_CORE_AI_RECENT_V195"
  };

  if (window.__EAP_CORE_AI_V195__) {
    console.info("[EAP Word Quest] Core AI v195 already loaded.");
    return;
  }
  window.__EAP_CORE_AI_V195__ = true;
  ...
  function studentScope(){
    ...
    return `${GROUP}_${id.replace(/[^a-z0-9_-]/gi,"_")}`;
  }

  function scopedKey(root){
    return `${root}_${studentScope()}`;
  }

  function readMetrics() {
    const state = safeJsonRead(scopedKey(STORAGE.state), {});
    return {
  ...
  function writeMetrics(next) {
    ...
    safeJsonWrite(scopedKey(STORAGE.state), safe);
  ...
  function readWeakStore() {
    const raw = safeJsonRead(scopedKey(STORAGE.weak), {});
  ...
  function recordWeakTarget(target, sessionId) {
    ...
    safeJsonWrite(scopedKey(STORAGE.weak), store);
  }
  ...
  function readRecent() {
    const value = safeJsonRead(scopedKey(STORAGE.recent), []);
  ...
  function recordRecent(sessionId, key) {
    ...
    safeJsonWrite(scopedKey(STORAGE.recent), next);
    return next;
  }
  ...
  function injectStyle() {
    if ($("eapV195CoreAiStyle")) return;

    const style = document.createElement("style");
    style.id = "eapV195CoreAiStyle";
    style.textContent = `
      #eapV195Panel {
        margin: 12px 0;
      ...
      #eapV195Panel .eap190-chip.ai {
        border-color:#fed7aa;
        background:#fff7ed;
        color:#9a3412;
      }
      #eapV195Panel .eap190-chip.good {
        border-color:#bbf7d0;
        background:#ecfdf5;
        color:#047857;
      }
      ...
      #eapV195HintBox.active { display:block; }
      #eapV195Panel .eap190-note {
        color:#475569;
        line-height:1.45;
        font-size:13px;
        font-weight:750;
  ...
  function ensurePanel() {
    if ($("eapV195Panel")) return $("eapV195Panel");

    const panel = document.createElement("div");
    panel.id = "eapV195Panel";
    panel.innerHTML = `
      <div class="eap190-row">
        <span class="eap190-chip ai" id="eapV195Difficulty">AI Difficulty: A2+</span>
        <span class="eap190-chip good" id="eapV195Prediction">AI Prediction: Collecting evidence</span>
        <span class="eap190-chip" id="eapV195Hints">AI Help: 0</span>
      </div>
      <div id="eapV195Note" class="eap190-note"></div>
      <div id="eapV195HintBox"></div>
    `;
  ...
  function renderPanel() {
    ...
    const d = $("eapV195Difficulty");
    const p = $("eapV195Prediction");
    const h = $("eapV195Hints");
  ...
  function setQuestionHints(key, count) {
    const metrics = readMetrics();
    const current = safeJsonRead(scopedKey(STORAGE.state), {});
  ...
  function appendCoachFeedback(result, sessionId, target) {
    const box = $("feedbackBox");
    if (!box || box.hidden) return;
    if ($("eapV195Feedback")) return;

    const coach = SESSION_COACH[sessionId] || {};
    const line = document.createElement("p");
    line.id = "eapV195Feedback";
  ...
  function newQuestionDetected() {
    ...
    const oldCoach = $("eapV195Feedback");
  ...
  function appendSummary() {
    ...
    let box = $("eapV195Summary");
    if (!box) {
      box = document.createElement("div");
      box.id = "eapV195Summary";
  ...
  function wrapLogger() {
    ...
    console.info("[EAP Word Quest] v195 Core AI logger bridge ready.");
  }
  ...
  window.EAP_CORE_AI_STUDENT_SCOPE = studentScope;
