/* =========================================================
   EAP Word Quest • Stability + Score Fix
   File: /herohealth/eap-word-quest/eap-word-engine-v194-stability-score-fix.js
   Version: v1.9.4-STABILITY-SINGLE-LOGGER-BRIDGE-122

   Why this patch exists:
   - v190 and v193 both wrapped the logger on recurring timers.
   ...
   - Prevents v190 from re-wrapping the logger after v194 is installed.
   ...
   Important:
   - Remove v193 from index.html. Do not load v193 and v194 together.
========================================================= */

(() => {
  "use strict";

  const VERSION = "v1.9.4-STABILITY-SINGLE-LOGGER-BRIDGE-122";
  const GROUP = "122";
  const LEDGER_KEY = "EAP_WORD_QUEST_CORE_V194_REWARD_LEDGER";

  if (window.__EAP_WORD_V194_STABILITY__) {
    console.info("[EAP Word Quest] v194 stability patch already loaded");
    return;
  }
  window.__EAP_WORD_V194_STABILITY__ = true;
  ...
  function safeWrite(key, value) {
    try {
      ...
      console.warn("[EAP Word Quest] v194 cannot save reward ledger", err);
      return false;
    }
  }
  ...
  function installSingleLoggerBridge() {
    ...
    if (current.__eapV194Wrapped) return true;
    ...
    */
    ...
    console.info("[EAP Word Quest] v194 single logger bridge ready");
    return true;
  }
  ...
  function inspect() {
    ...
    return {
      version: VERSION,
      loggerBridgeInstalled: Boolean(logger && logger.__eapV194Wrapped),
  ...
  function boot() {
    ...
    console.info("[EAP Word Quest] v194 stability + score fix ready", inspect());
  }
  ...
  window.EAP_WORD_V194_VERSION = VERSION;
