/* =========================================================
   ...
   File: /herohealth/eap-word-quest/eap-word-engine-v195-core-truth-progress-controller.js
   Version: v1.9.5-CORE-TRUTH-PROGRESS-CONTROLLER-122
...
(() => {
  ...
  const VERSION = "v1.9.5-CORE-TRUTH-PROGRESS-CONTROLLER-122";
  const GROUP = "122";

  if (window.__EAP_WORD_V195_CORE_CONTROLLER__) {
    console.info("[EAP Word Quest] v195 core controller already loaded");
    return;
  }
  window.__EAP_WORD_V195_CORE_CONTROLLER__ = true;

  const STATE_KEY_PREFIX = "EAP_WORD_QUEST_CORE_V195_STATE";
  const MIGRATE_STATE_KEY = "EAP_WORD_QUEST_CORE_V192_STATE";
  ...
  function migrateV192State(profile){
    const old = readJson(MIGRATE_STATE_KEY,null);
    if (!old || typeof old !== "object") return null;
    const migrated = blankState();
    const targetId = norm(profile && profile.studentId);

    Object.entries(old.sessions || {}).forEach(([sessionId,row]) => {
      const result = row && row.lastResult;
      const sameStudent = result && targetId && norm(result.studentId) === targetId;
      const realCoreRun = result && /^core-bank-v192$/i.test(norm(result.source));
      if (sameStudent && realCoreRun && row && row.played) migrated.sessions[sessionId] = row;
    });

    migrated.runs = (old.runs || []).filter(row =>
      row && /^core-bank-v192$/i.test(norm(row.source)) && targetId && norm(row.studentId) === targetId
    ).slice(0,120);
    migrated.weakTargets = old.weakTargets || {};
    migrated.recentItemIds = Array.isArray(old.recentItemIds) ? old.recentItemIds.slice(-80) : [];
    migrated.migratedFrom = MIGRATE_STATE_KEY;
  ...
  function readState(){
    ...
    let base = readJson(key,null);
    if (!base) {
      base = migrateV192State(profile) || blankState();
      writeJson(key,base);
    }
    ...
    base.recentItemIds = Array.isArray(base.recentItemIds) ? base.recentItemIds : [];
    base.weakTargets = base.weakTargets || {};
  ...
  function writeState(state){
    ...
    return writeJson(coreStateKey(),state);
  }

  /* v195 deliberately ignores legacy bank results. Only core-aligned runs may unlock the Core path. */
  ...
  function markRecent(itemId){
    const state = readState();
    state.recentItemIds = [itemId, ...state.recentItemIds.filter(id => id !== itemId)].slice(0,80);
    writeState(state);
  }
  ...
  function updateHeader(){
    ...
    if (pill && !pill.querySelector(".eap192-core-badge")) {
      pill.insertAdjacentHTML("afterend",`<span class="eap192-core-badge">Core AI v195</span>`);
    }
  }
  ...
  function boot(){
    ...
    console.info("[EAP Word Quest] v195 core truth progress controller ready", inspect());
  }
