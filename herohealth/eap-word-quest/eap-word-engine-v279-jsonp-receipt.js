/* =========================================================
   EAP Word Quest • JSONP Submit + Explicit Sheet Receipt
   Version: 20260728-EAPWQ-V279-JSONP-RECEIPT

   Critical rules
   - Google Sheet is the sole authority for official progress.
   - The browser receives an explicit Apps Script response for every submit.
   - Next/Home remain locked until Player Resume confirms the saved session.
   - No hidden iframe POST, no MutationObserver, no continuous polling.
========================================================= */
(function () {
  'use strict';

  var VERSION = '20260728-EAPWQ-V279-JSONP-RECEIPT';
  ...
  function readOfficialProfile() {
    ...
    try {
      if (typeof window.getEapWordOfficialProfileV278 === 'function') {
        live = window.getEapWordOfficialProfileV278();
      }
    } catch (ignore) {}

    saved = live || readJson(PROFILE_KEY);
    id = text((idInput && idInput.value) || saved.studentId || saved.id || '');
    name = text((nameInput && nameInput.value) || saved.studentName || saved.name || '');

    if (saved.official !== true || saved.authority !== 'google_sheet_roster') return null;
    if (!/^\d{10}$/.test(id) || !name) return null;
    if (text(saved.studentId) && text(saved.studentId) !== id) return null;
    if (text(saved.studentName) && text(saved.studentName) !== name) return null;
  ...
  function exactSummaryRecord() {
    ...
    record.fingerprint = [
      GROUP,record.studentId,record.studentName,record.sessionId,
      record.correct,record.total,record.accuracy,String(record.playedAt).slice(0,19)
    ].join('|');
    return record;
  }
  ...
  function jsonp(action,params,timeoutMs) {
    return new Promise(function (resolve,reject) {
      var callback = '__eapwq_v279_' + Date.now() + '_' + Math.random().toString(36).slice(2,8);
      ...
      timer = setTimeout(function () { finish(new Error(action + '_timeout')); },timeoutMs || 18000);
      document.head.appendChild(script);
    });
  }

  function submitReceipt(record) {
    var payload = {
    ...
    return jsonp('eap_word_submit_jsonp',{payload:JSON.stringify(payload)},20000);
  }
  ...
  function receiptMatches(record,resume) {
    ...
    if (recordTime && sheetTime && sheetTime < recordTime - 15000) return false;
    return true;
  }

  async function confirmResume(record) {
    var delays = [500,1200,2400,4200];
    var i;
    var resume;
    for (i = 0; i < delays.length; i += 1) {
      await new Promise(function (resolve) { setTimeout(resolve,delays[i]); });
      resume = await jsonp('eap_word_player_resume',{studentId:record.studentId},16000);
      if (receiptMatches(record,resume)) return resume;
    }
    return null;
  }
  ...
  function refreshAuthority() {
    try {
      ...
      console.warn('[EAP Word Quest] V279 authority refresh failed',error);
    }
  }

  function ackLabel(ack) {
    if (!ack) return 'ไม่ได้รับใบตอบรับจาก Apps Script';
    if (ack.ok) return 'Apps Script รับผลแล้ว';
    return 'Apps Script ปฏิเสธ: ' + text(ack.error || ack.message || 'unknown_error');
  }
  ...
  async function submit(record,reason) {
    ...
    if (!record || record.error === 'official_profile_required') {
      ...
      show('ยังส่งผลไม่ได้: Official Profile ไม่พร้อม กรุณากลับหน้าแรกและโหลดข้อมูลจาก Google Sheet ใหม่','error',false);
      return;
    }
    ...
    pending.add(key);
    setNavigationLocked(true);
    show('กำลังส่ง ' + record.sessionId + ' ไปยัง Apps Script และรอใบตอบรับ…','working',false);

    try {
      ack = await submitReceipt(record);
      lastAck = ack;
      if (!ack || !ack.ok) {
        show(ackLabel(ack) + ' ปุ่มไปต่อยังถูกล็อก','error',true);
        return;
      }
      if (!ack.receipt || text(ack.receipt.sessionId) !== record.sessionId || text(ack.receipt.studentId) !== record.studentId) {
        show('Apps Script ตอบกลับ แต่ใบรับผลไม่ตรงกับผู้เรียนหรือ Session ปุ่มไปต่อยังถูกล็อก','error',true);
        return;
      }

      show('Apps Script รับ ' + record.sessionId + ' แล้ว (appended ' + Number(ack.appended || 0) + ', duplicate ' + Number(ack.duplicate || 0) + ') กำลังตรวจ Player Resume…','working',false);
      resume = await confirmResume(record);
      lastResume = resume;
      if (!resume) {
        show('Apps Script รับผลแล้ว แต่ Player Resume ยังไม่พบ ' + record.sessionId + ' ใน Google Sheet ปุ่มไปต่อยังถูกล็อก','warning',true);
        return;
      }

      confirmed.add(key);
      setNavigationLocked(false);
      show(record.sessionId + ' บันทึกและยืนยันจาก Google Sheet แล้ว ✓ เล่นต่อที่ ' + (resume.currentSession || resume.nextMission || '-') + ' • ความก้าวหน้า ' + Number(resume.progressPercent || 0) + '%','success',false);
      refreshAuthority();
      window.dispatchEvent(new CustomEvent('eap-word-sheet-confirmed',{
        detail:{version:VERSION,record:record,ack:ack,resume:resume,reason:reason || 'summary_ready'}
      }));
    } catch (error) {
      show('ส่งผลไม่สำเร็จ: ' + text(error && error.message || error) + ' ปุ่มไปต่อยังถูกล็อก','error',true);
    } finally {
      pending.delete(key);
    }
  }
  ...
  function submitExactSummary(reason) {
    ...
    record = exactSummaryRecord();
    if (!record) return;
    if (record.error) { submit(record,reason); return; }
  ...
  function scheduleSubmit(reason) {
    [180,650,1400].forEach(function (delay) {
      setTimeout(function () { if (summaryActive()) submitExactSummary(reason); },delay);
    });
  }

  window.addEventListener('eap-core-run-finished',function () { scheduleSubmit('core_finished'); });
  document.addEventListener('click',function (event) {
    if (!summaryActive()) return;
    if (event.target && event.target.closest && event.target.closest('#eapWordExactSummaryStatus button')) return;
    setTimeout(function () { submitExactSummary('summary_interaction'); },120);
  },true);
  ...
  [500,1400,2800].forEach(function (delay) {
    setTimeout(function () { if (summaryActive()) submitExactSummary('boot_summary'); },delay);
  });

  console.info('[EAP Word Quest] V279 JSONP receipt sender ready',{version:VERSION});
})();
