/* =========================================================
   EAP Word Quest • Core Bank Play Controller
   File: /herohealth/eap-word-quest/eap-word-engine-v192-core-bank-controller.js
   Version: v1.9.2-CORE-BANK-PLAY-CONTROLLER-122
...
(() => {
  ...
  const VERSION = "v1.9.2-CORE-BANK-PLAY-CONTROLLER-122";
  const GROUP = "122";

  if (window.__EAP_WORD_V192_CORE_CONTROLLER__) {
    console.info("[EAP Word Quest] v192 core controller already loaded");
    return;
  }
  window.__EAP_WORD_V192_CORE_CONTROLLER__ = true;

  const STATE_KEY = "EAP_WORD_QUEST_CORE_V192_STATE";
  ...
  function readState(){
    const base = readJson(STATE_KEY,null) || {
      version: VERSION,
      group: GROUP,
      sessions: {},
      recentItemIds: [],
  ...
  function readLegacyPass(sessionId){
    for (const key of LEGACY_STAT_KEYS) {
      const stats = readJson(key,null);
      const s = stats && stats.sessions && stats.sessions[sessionId];
      if (s && (s.passed || num(s.bestAccuracy) >= threshold(sessionId))) {
        return {
          passed: true,
          accuracy: Math.round(num(s.bestAccuracy) || (num(s.correct) / Math.max(1,num(s.total))) * 100 || threshold(sessionId)),
          source: key
        };
      }
    }
  ...
  function bestRecord(sessionId){
    ...
    if (core && core.played) return core;
    const legacy = readLegacyPass(sessionId);
    if (legacy) {
      return {
        played:true,
        passed:true,
        accuracy:legacy.accuracy,
        bestAccuracy:legacy.accuracy,
        legacyPass:true,
  ...
  function injectStyle(){
    if ($("eapV192Style")) return;
    const style = document.createElement("style");
    style.id = "eapV192Style";
    style.textContent = `
      .eap192-core-badge{
  ...
  function renderSessionGrid(){
    ...
    const html = ARCS.map((arc,arcIndex) => {
      ...
      const cards = [...arc.sessions, arc.boss].map(sessionId => {
        ...
        const status = passed ? (rec && rec.legacyPass ? "Legacy Pass" : "Passed") : (unlocked ? "Open" : "Locked");
  ...
  function updateHeader(){
    ...
    if (pill && !pill.querySelector(".eap192-core-badge")) {
      pill.insertAdjacentHTML("afterend",`<span class="eap192-core-badge">Core AI v192</span>`);
    }
  }
  ...
  function showSummary(result){
    ...
    let box = $("eapV192SummaryBox");
    if (!box) {
      box = document.createElement("div");
      box.id = "eapV192SummaryBox";
    ...
    if (box) {
      ...
      box.innerHTML = `
        ...
        Source: ${escapeHtml(result.source)}<br>
  ...
  function resetState(){
    if (!confirm("Reset Core v192 local progress? ข้อมูล logger เก่าจะไม่ถูกลบ แต่ progress ของ controller นี้จะเริ่มใหม่")) return;
    localStorage.removeItem(STATE_KEY);
    toast("Core v192 progress reset");
    renderAllHome();
  }
  ...
  function boot(){
    ...
    console.info("[EAP Word Quest] v192 core bank controller ready", inspect());
  }
  ...
  window.EAP_WORD_V192_VERSION = VERSION;
})();
