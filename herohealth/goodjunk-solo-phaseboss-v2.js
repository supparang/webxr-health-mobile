(function () {
  ...
  try {
    ...
    const ctx = window.__GJ_RUN_CTX__ || {
      pid: q.get('pid') || 'anon',
      name: q.get('name') || q.get('nickName') || q.get('nick') || 'Hero',
      studyId: q.get('studyId') || '',
      diff: q.get('diff') || 'normal',
      time: q.get('time') || '80',
      seed: q.get('seed') || String(Date.now()),
      hubRoot: q.get('hubRoot') || q.get('hub') || new URL('./hub.html', location.href).toString(),
      launcher: q.get('launcher') || new URL('./goodjunk-launcher.html', location.href).toString(),
      hub: q.get('launcher') || new URL('./goodjunk-launcher.html', location.href).toString(),
    ...
    const state = {
      ...
      timeTotal: Math.max(80, Number(ctx.time || 80)) * 1000,
      timeLeft: Math.max(80, Number(ctx.time || 80)) * 1000,
            ...
            <div class="gjsb-pause" id="pauseOverlay">
              <div class="gjsb-pauseCard">
                ...
                <div class="gjsb-pauseActions">
                  ...
                  <button class="gjsb-pauseBtn hub" id="btnPauseHub" type="button">↩ กลับ Launcher</button>
                </div>
              </div>
            </div>
            ...
            <div class="gjsb-summary" id="summary">
              <div class="gjsb-summary-card" id="summaryCard">
                ...
                <div class="gjsb-actions">
                  ...
                  <button class="gjsb-btn hub" id="btnHub">↩ กลับ Launcher</button>
                </div>
              </div>
            </div>
          </div>
    ...
    function computeAccPct() {
      const correct = state.hitsGood + state.powerHits;
      const wrong = state.hitsBad;
    ...
    function saveLastSummary(payload) {
      try {
        ...
        storageSet(`HHA_LAST_SUMMARY:goodjunk:${ctx.pid || 'anon'}`, JSON.stringify(item));
    ...
    function buildLauncherUrl() {
      return ctx.launcher || new URL('./goodjunk-launcher.html', location.href).toString();
    }
    ...
    function buildReplayUrl() {
      ...
      u.searchParams.set('hub', ctx.hubRoot || new URL('./hub.html', location.href).toString());
      u.searchParams.set('hubRoot', ctx.hubRoot || new URL('./hub.html', location.href).toString());
      u.searchParams.set('launcher', buildLauncherUrl());
    ...
    function buildCooldownUrl() {
      ...
      u.searchParams.set('hub', buildLauncherUrl());
      u.searchParams.set('hubRoot', ctx.hubRoot || new URL('./hub.html', location.href).toString());
      u.searchParams.set('launcher', buildLauncherUrl());
      u.searchParams.set('view', ctx.view || 'mobile');
      u.searchParams.set('run', ctx.run || 'play');
    ...
    function markLastRun() {
      try {
        localStorage.setItem(`GJ_LAST_MODE:${ctx.pid || 'anon'}`, 'solo');
        localStorage.setItem(`GJ_LAST_RUN_URL:${ctx.pid || 'anon'}`, buildReplayUrl());
      } catch (_) {}
    }
    ...
    function endGame(bossClear) {
      ...
      ui.sumGrid.innerHTML = `
        ...
        <div class="gjsb-stat"><div class="k">แม่นยำ</div><div class="v">${accPct}%</div></div>
