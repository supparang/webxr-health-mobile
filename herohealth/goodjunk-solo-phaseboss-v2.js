...
(function () {
  ...
  try {
    ...
    const ctx = window.__GJ_RUN_CTX__ || {
      ...
      nick: q.get('nick') || q.get('name') || q.get('nickName') || 'Hero',
      studyId: q.get('studyId') || '',
      diff: q.get('diff') || 'normal',
      time: q.get('time') || '150',
      seed: q.get('seed') || String(Date.now()),
      hub: GJ_CANONICAL_LAUNCHER,
      hubRoot: GJ_CANONICAL_HUB_ROOT,
      launcher: GJ_CANONICAL_LAUNCHER,
      cdnext: GJ_CANONICAL_LAUNCHER,
    ...
    function getHubRootHref() {
      return GJ_CANONICAL_HUB_ROOT;
    }

    function getLauncherHref() {
      return GJ_CANONICAL_LAUNCHER;
    }

    function getCooldownReturnHref() {
      return GJ_CANONICAL_LAUNCHER;
    }
    ...
    function buildReplayUrl() {
      ...
      u.searchParams.set('nick', ctx.nick || ctx.name || 'Hero');
      ...
      if (ctx.pro) u.searchParams.set('pro', ctx.pro);
      if (ctx.research) u.searchParams.set('research', ctx.research);
      if (ctx.roomId) u.searchParams.set('roomId', ctx.roomId);
      ...
      u.searchParams.set('hub', getLauncherHref());
      u.searchParams.set('hubRoot', getHubRootHref());
      u.searchParams.set('launcher', getLauncherHref());
      u.searchParams.set('cdnext', getCooldownReturnHref());
    ...
    function buildCooldownUrl() {
      ...
      u.searchParams.set('pid', ctx.pid || 'anon');
      u.searchParams.set('name', ctx.name || ctx.nick || 'Hero');
    ...
    function endGame(bossClear) {
      ...
      saveLastSummary({
        ...
        launcher: getLauncherHref(),
        cdnext: getCooldownReturnHref()
      });
