...
// v20260514-PLATE-SOLO-FINAL-PORTION-COOLDOWN-FIX1
// ✅ เข้าไฟล์ตรงจาก plate-v1.html ได้
// ✅ Summary ไม่เด้งเองตอนโหลด
// ✅ เล่นได้ / spawn อาหารได้ / update HUD ได้
// ✅ เป้าหมายแต่ละหมู่ไม่เท่ากัน ไม่ใช่ x/4 เหมือนกันหมด
// ✅ อาหาร 1 ชิ้นเติมแบบ portion scale ไม่เต็มเร็วเกิน
// ✅ Cooldown เข้า warmup-gate.html?phase=cooldown แน่นขึ้น
// ✅ หลัง cooldown กลับ Nutrition Zone ตาม URL ที่กำหนด
// ✅ emoji-only ไม่พึ่ง asset รูป
// ✅ DOM-safe ไม่พังถ้า HTML บางตัวไม่มี

(() => {
  'use strict';

  var VERSION = '20260514-PLATE-SOLO-FINAL-PORTION-COOLDOWN-FIX1';
  var DOC = window.document;
  var WIN = window;

  try {
    console.info('[Plate Solo]', VERSION, 'loaded');
  } catch(e) {}
  ...
  const CFG = ({
    easy:{ spawn:1250, life:6800, junk:.10, target:3.8, bossAtk:6500 },
    normal:{ spawn:1150, life:6200, junk:.16, target:4.0, bossAtk:5550 },
    hard:{ spawn:850, life:5000, junk:.25, target:4.2, bossAtk:4650 },
    challenge:{ spawn:700, life:4100, junk:.32, target:4.4, bossAtk:3820 }
  })[DIFF] || { spawn:1150, life:6200, junk:.16, target:4.0, bossAtk:5550 };
  ...
  // เป้าหมายรายหมู่ไม่เท่ากัน และใช้สัดส่วนเล็กลงเพื่อไม่ให้เต็มเร็วเกิน
  const GROUP_TARGETS = {
    protein: 3.6,
    carb: 3.8,
    veg: 5.2,
  ...
  function groupTarget(g){
    return Number(GROUP_TARGETS[g] || CFG.target || 4);
  }

  function groupLimit(g){
    return groupTarget(g) + .55;
  }

  function portionValue(g, v){
    return Number(v || 0) * Number(GROUP_PORTION_SCALE[g] || .55);
  }

  function groupAlmostFull(g){
    return (state.fill[g] || 0) >= groupTarget(g) * .82;
  }
  ...
  function resolvePlateEls(){
    ...
    els.plateFoods =
      els.plateFoods ||
      byId('plateFoods') ||
      ensureEl('plateFoods', 'div', els.plate, 'plate-foods');

    els.plateLabel =
      els.plateLabel ||
      byId('plateLabel') ||
      ensureEl('plateLabel', 'div', els.plate, 'plate-label', 'วางอาหารให้ครบ 5 หมู่');
    ...
    els.startOverlay =
      els.startOverlay ||
      byId('startOverlay') ||
      ensureEl('startOverlay', 'div', els.app, 'hidden');
    ...
    els.btnHint =
      els.btnHint ||
      byId('btnHint') ||
      ensureEl('btnHint', 'button', DOC.querySelector('.controls-right') || els.app, 'btn blue', 'คำใบ้');

    els.btnSkill =
      els.btnSkill ||
      byId('btnSkill') ||
      ensureEl('btnSkill', 'button', DOC.querySelector('.controls-right') || els.app, 'btn warn', 'สกิลช่วย');

    els.missions =
      els.missions ||
      ensureEl('missions', 'div', DOC.querySelector('.goal-panel') || els.app, 'goal-list');

    els.meters =
      els.meters ||
      ensureEl('meters', 'div', DOC.querySelector('.plate-side') || els.app, 'meters-list');

    els.hintBox =
      els.hintBox ||
      ensureEl('hintBox', 'div', DOC.querySelector('.plate-side') || els.app, 'panel-note');

    els.log =
      els.log ||
      ensureEl('log', 'div', DOC.querySelector('.plate-side') || els.app, 'panel-note');

    els.timerFill =
      els.timerFill ||
      ensureEl('timerFill', 'i', DOC.querySelector('.timer-card') || els.app, 'timer-fill');

    els.phaseText =
      els.phaseText ||
      byId('phaseText') ||
      ensureEl('phaseText', 'div', DOC.querySelector('.combo-card') || els.app, 'stat-value', 'Ready');

    els.juiceLayer =
      els.juiceLayer ||
      ensureEl('juiceLayer', 'div', els.arena || els.app, 'juice-layer');

    els.flash =
      els.flash ||
      ensureEl('flash', 'div', els.arena || els.app, 'flash');
  ...
  function foodVisualHtml(food, cls = ''){
    ...
    return `
      <div class="foodVisual assetFallback emojiOnly ${cls}">
        <span class="foodEmojiFallback" aria-hidden="true">${emoji}</span>
        <span class="assetPackDebug">emoji-only</span>
      </div>
    `;
  }
  ...
  function plateFoodVisualHtml(emoji){
    ...
    return `
      <span class="plateFoodIcon assetFallback emojiOnly">
        <span class="foodEmojiFallback" aria-hidden="true">${safeEmoji}</span>
      </span>
    `;
  }
  ...
  function setGameplayViewportVars(){
    try {
      const h = Math.max(
        420,
        Math.round(WIN.innerHeight || DOC.documentElement.clientHeight || 720)
      );
  ...
  WIN.addEventListener('orientationchange', () => {
    setTimeout(setGameplayViewportVars, 120);
  }, { passive:true });
  ...
  const state = {
    ...
    bossWarnExpire:0,
    bossWarningStarted:0,
    bossCounterCount:0,
    bossAttackCount:0,

    bossMechanic:null,
    bossMechanicStats:null,
    bossMechanicExpire:0,
    lastBossMechanicAt:0,
    ...
    sessionId:
      ...
      '-' +
      Math.floor(rand()*1e6).toString(36),
  ...
  function installV403Styles(){
    ...
    st.textContent = `
      .boss-warning #arena,
      .boss-warning .arena,
      .boss-warning #gameStage{
        box-shadow:0 0 0 5px rgba(255,82,82,.18),
        0 0 42px rgba(255,82,82,.28) inset!important;
      }

      .boss-enraged #arena,
      .boss-enraged .arena,
      .boss-enraged #gameStage{
        animation:plateBossRumble .42s infinite alternate;
      }
      ...
      .nearMissFx{
        ...
        text-align:center;
        pointer-events:none;
        box-shadow:0 20px 48px rgba(25,55,80,.22);
        animation:platePopToast 1.05s both;
      }

      .counterFx{
        background:linear-gradient(135deg,#fff8d7,#dfffea);
        border:3px solid rgba(255,204,64,.75);
        color:#215a40;
        font-size:clamp(24px,4vw,46px);
      }

      .milestoneToast{
        background:linear-gradient(135deg,#fff,#ffe8b5);
        border:3px solid rgba(255,180,40,.55);
        color:#7a4500;
      ...
      .nearMissFx{
        ...
        border:3px solid rgba(80,180,255,.55);
        color:#1e5a78;
        font-size:clamp(18px,3vw,30px);
      }

      .foodCard.comboGlow{
        box-shadow:0 18px 44px rgba(255,187,45,.38),
        0 0 0 4px rgba(255,228,120,.45)!important;
      }
      ...
      .replayChallenge{
        ...
        border-radius:18px;
        padding:12px 14px;
        background:linear-gradient(135deg,#f5fbff,#fff8df);
        border:1px solid rgba(98,164,210,.28);
        font-weight:850;
        color:#31546a;
      }

      @keyframes plateBossRumble{
        from{transform:translateX(-1px)}
        to{transform:translateX(1px)}
      }

      @keyframes platePopToast{
  ...
  function practiceElapsedSec(){
    return Math.max(0, (now() - state.practiceStarted) / 1000);
  }

  function practiceLeftSec(){
    return Math.max(0, PRACTICE_SEC - practiceElapsedSec());
  }
  ...
  function mostMissingGroup(){
    return GROUPS
      .map(g => ({
        ...g,
        need:groupTarget(g.id) - (state.fill[g.id] || 0)
      }))
  ...
  function mostOverGroup(){
    return GROUPS
      .map(g => ({
        ...g,
        over:(state.fill[g.id] || 0) - groupTarget(g.id)
      }))
  ...
  function wouldOverload(food){
    return !!(
      ...
      Object.entries(food.effects).some(([g,v]) =>
        (state.fill[g] || 0) + portionValue(g,v) > groupTarget(g) + .18
      )
    );
  }
  ...
  function applyEffects(effects){
    Object.entries(effects || {}).forEach(([g,v]) => {
      if (g in state.fill) {
        state.fill[g] = clamp(
          (state.fill[g] || 0) + portionValue(g,v),
          0,
          groupLimit(g)
        );
      }
  ...
  function updatePlateHealthUI(){
    ...
    if (els.plateHealthFill) {
      els.plateHealthFill.style.width = clamp(hp,0,100) + '%';
    }

    if (els.plateHealthText) {
      els.plateHealthText.textContent = 'Plate ' + hp + '%';
    }

    if (els.plateHealthIcon) {
      els.plateHealthIcon.textContent =
        hp <= 25 ? '🚨' : hp <= 55 ? '💛' : '❤️';
    }
  ...
  function feedback(txt, kind = 'good'){
    ...
    if (kind === 'good') d.style.background = 'rgba(36,144,80,.82)';
    if (kind === 'bad') d.style.background = 'rgba(214,55,69,.82)';
    if (kind === 'perfect') d.style.background = 'rgba(120,80,220,.86)';
  ...
  function coinBurst(kind = 'good', count = 7){
    ...
    for (let i = 0; i < count; i++) {
      ...
      const a = Math.PI * 2 * i / count + rand() * .55;
      const dist = 54 + rand() * 94;
  ...
  function logLine(txt){
    ...
    els.log.innerHTML = state.logs
      .map(x => `<div class="logLine">${esc(x)}</div>`)
      .join('');
  }
  ...
  function addScore(p, label = '', kind = 'good'){
    ...
    if (n >= 45) {
      coinBurst(kind, Math.min(12, Math.max(4, Math.ceil(n / 28))));
    }
  }
  ...
  function showHint(force = false){
    ...
    if (state.lastSaveActive && state.lastSaveStats) {
      msg = `
        🚨 Last Save:
        รีบเติม <b>${state.lastSaveStats.needIcon}
        ${state.lastSaveStats.needLabel}</b>
        และห้ามโดน junk!
      `;
    } else if (o && o.over > .35) {
    ...
    if (state.boss && !state.bossDefeated) {
      msg += ' ทำถูกต่อเนื่องเพื่อลด HP บอส!';
    }
  ...
  function dailyStatus(){
    ...
    if (d.id === 'vegfruit') {
      ...
      return {
        ...
        label:`ผัก ${Math.min(v,4)}/4 ผลไม้ ${Math.min(f,3)}/3`
      };
    }
          ...
          ? (() => {
              ...
              return `
                <span id="mis-daily" class="mission ${ds.done ? 'done' : ''}">
                  ${ds.done ? '✅' : '📅'}
                  Daily: ${esc(state.dailyChallenge.text)}
  ...
  function renderDailyBox(){
    ...
    els.dailyBox.innerHTML = `
      ...
      <span>Best เดิม: ${Number(b.score || 0)} คะแนน • ${Number(b.stars || 0)} ดาว</span>
    `;
  }
  ...
  function classifyFoodCard(food){
    ...
    if (state.lastSaveActive && state.lastSaveStats) {
      ...
      if (food.junk) {
        return { cls:'bossForbidden junk', tag:'ห้ามโดน!' };
      }

      if (food.effects && food.effects[g] && !wouldOverload(food)) {
        return { cls:'bossWanted goodNow', tag:'กู้จาน!' };
      }
    }
    ...
    if (state.bossMechanic) {
      ...
      if (m.type === 'avoidJunk' && food.junk) {
        return { cls:'bossForbidden junk', tag:'ห้ามโดน!' };
      }

      if (m.type === 'avoidGroup' && food.effects && food.effects[m.group]) {
        return { cls:'bossForbidden dangerFood', tag:'ห้ามตอนนี้!' };
      }

      if (m.type === 'quickFix' && food.effects && food.effects[m.group] && !wouldOverload(food)) {
        return { cls:'bossWanted goodNow', tag:'รีบเก็บ!' };
      }
    }
    ...
    if (food.effects && food.effects[m.id]) {
  ...
  function chooseMovement(food){
    if (state.mini && state.mini.type === 'healthyRain' && !food.junk) {
      return { cls:'move-rain-fast', lifeScale:.72, label:'rain-fast' };
    }

    if (state.mini && state.mini.type === 'junkInvasion' && food.junk) {
      return {
        cls:chance(.5) ? 'move-junk-dash' : 'move-junk-wobble',
        lifeScale:.68,
        label:'junk-invasion'
      };
    }
    ...
    if (food.type === 'power') {
      return { cls:'move-power-float', lifeScale:1.18, label:'power-float' };
    }

    if (food.junk) {
      return state.boss || state.rush
        ? {
            cls:chance(.55) ? 'move-junk-dash' : 'move-junk-wobble',
            lifeScale:.72,
            label:'junk-fast'
          }
        : {
            cls:chance(.5) ? 'move-junk-wobble' : 'move-drift-right',
            lifeScale:.9,
            label:'junk-wobble'
          };
    }

    if (state.boss && chance(.38)) {
      return { cls:'move-boss-swipe', lifeScale:.78, label:'boss-swipe' };
    }

    if (state.rush && chance(.45)) {
      return {
        cls:chance(.5) ? 'move-zigzag' : 'move-drift-left',
        lifeScale:.82,
        label:'rush-move'
      };
    }
    ...
    if (chance(.28)) {
      return {
        cls:chance(.5) ? 'move-drift-left' : 'move-drift-right',
        lifeScale:1,
        label:'drift'
      };
    }
    ...
    if (chance(.18)) {
      return { cls:'move-zigzag', lifeScale:.95, label:'zigzag' };
    }
  ...
  function chooseSpawnPlacement(food, m){
    ...
    let cs = lanes.map((pct,i) => ({
      ...
      if (m && (m.label === 'boss-swipe' || m.label === 'junk-fast')) {
        x.score += Math.abs(x.pct - 50) / 70;
      }
      ...
      if (isAimMode()) {
        x.score += (1 - Math.abs(x.pct - 50) / 50) * .28;
      }
    ...
    if (ph === 1) {
      ...
      return {
        ...pick(FOODS.filter(f =>
          !f.junk &&
          f.effects &&
          (f.effects.veg || f.effects.fruit || f.effects.protein)
        ))
      };
    }
  ...
  function chooseFood(){
    ...
    if (state.mini && state.mini.type === 'healthyRain' && chance(.82)) {
      const pool = FOODS.filter(f =>
        ...
        (
          f.effects[m.id] ||
          GROUPS.some(g =>
            state.fill[g.id] < groupTarget(g.id) * .82 &&
            f.effects[g.id]
          )
        ) &&
        !wouldOverload(f)
    ...
    const missingIds = GROUPS
      .filter(g => state.fill[g.id] < groupTarget(g.id) * .82)
      .map(g => g.id);

    if (missingIds.length && chance(.68)) {
    ...
    if (chance(.45)) return { ...pick(POWERS) };
  ...
  function handleHealthy(f){
    ...
    if (overload) {
      ...
      addScore(
        gain,
        state.combo >= 5 ? `Combo +${gain}` : `+${gain}`,
        state.combo >= 5 ? 'combo' : 'good'
      );
      ...
      if (state.boss && !state.bossDefeated) {
        tryBossCounter(f, overload) || damageBoss(5 + Math.min(10, state.combo));
      }
    ...
    postLog('target_hit', {
      ...
      plateHealth:Math.round(state.plateHealth),
      combo:state.combo,
  ...
  function handleJunk(f){
    ...
    if (lastSaveWasActive) {
      checkLastSaveOnJunk();
    } else {
      damagePlate(16, 'โดน junk');
    }
  ...
  function startLastSave(){
    ...
    if (els.lastSaveText) {
      els.lastSaveText.textContent =
        `จานใกล้พัง! รีบเติม ${m.icon} ${m.label} ให้ถูก 2 ครั้ง และห้ามโดน junk`;
    }
  ...
  function checkLastSaveOnHealthy(food, overload){
    ...
    if (!overload && food.effects && food.effects[g]) {
      ...
      if (state.lastSaveStats.hits >= 2) {
        completeLastSave(true, 'กู้จานสำเร็จ!');
      }
    } else if (overload) {
  ...
  function animateBoss(kind){
    ...
    setTimeout(() => {
      if (els.bossAvatar) els.bossAvatar.classList.remove(kind);
    }, 520);
  }
  ...
  function pulseBossArena(){
    ...
    setTimeout(() => {
      if (els.arena) els.arena.classList.remove('boss-pulse');
    }, 520);
  }
  ...
  function bossAttackDelay(){
    return CFG.bossAtk * clamp(
      (1 - state.directorLevel * .06) *
      (1 + state.assistLevel * .10) *
      (state.bossEnraged ? .72 : 1),
      .58,
      1.32
    );
  }
  ...
  function isBossWarning(){
    return (
      state.boss &&
      !state.bossDefeated &&
      state.bossWarnActive &&
      now() < state.bossWarnExpire
    );
  }
  ...
  function startBossWarning(){
    ...
    const txt = state.bossEnraged
      ? `🔥 ${b.skillName || 'Boss Skill'} กำลังมา! COUNTER เร็ว!`
      : `⚠️ ${b.skillName || 'Boss Skill'} กำลังมา! เลือกของดีเพื่อ COUNTER`;
    ...
    postLog('boss_warning', {
      bossType:b.id,
      enraged:!!state.bossEnraged,
      warnMs:bossWarningDuration()
    });
  }
  ...
  function tryBossCounter(food, overload){
    ...
    const dmg = clamp(
      18 +
      Math.min(18, state.combo * 1.25) +
      (state.bossEnraged ? 8 : 0),
      18,
      46
    );
    ...
    addScore(
      state.bossEnraged ? 125 : 90,
      state.bossEnraged ? 'ENRAGE COUNTER +125' : 'COUNTER +90',
      'boss'
    );

    popFx(
      'counterFx',
      `⚡ COUNTER!<br><small>บอสเสีย HP -${Math.round(dmg)}</small>`,
      1150
    );
  ...
  function checkComboMilestone(){
    ...
    for (const m of marks) {
      if (state.combo >= m.n && !state.comboMilestones.has(m.n)) {
        ...
        popFx(
          'milestoneToast',
          `${m.txt}<br><small>โบนัส +${m.score}</small>`,
          1150
        );

        if (m.n >= 10 && now() - state.lastFever > 3500) {
          triggerFever();
        }

        sfx('perfect');

        postLog('combo_milestone', {
          combo:m.n,
          scoreBonus:m.score
        });
      }
  ...
  function tryNearMissRescue(food){
    ...
    popFx(
      'nearMissFx',
      `💙 NEAR MISS!<br><small>คอมโบสูงช่วยกันพลาด 1 ครั้ง</small>`,
      1100
    );
  ...
  function bossAttack(){
    ...
    if (now() - state.lastBossMechanicAt > (state.bossEnraged ? 6500 : 9000)) {
      startBossMechanic();
    }

    if (b.id === 'greasy') {
      state.fill.fat = clamp(
        state.fill.fat + portionValue('fat', state.bossEnraged ? 1.15 : .9),
        0,
        groupLimit('fat')
      );

      state.fill.veg = clamp(
        state.fill.veg - portionValue('veg', state.bossEnraged ? .35 : .25),
        0,
        groupLimit('veg')
      );
      ...
      state.fill.carb = clamp(
        state.fill.carb + portionValue('carb', state.bossEnraged ? .85 : .65),
        0,
        groupLimit('carb')
      );

      const junk = FOODS.filter(f =>
        f.junk &&
        ['🍰','🧃','🍩','🥤'].includes(f.emoji)
      );
      ...
      setTimeout(() => {
        if (state.running && !state.ended) spawnFood({ ...pick(junk) });
      }, state.bossEnraged ? 240 : 350);
      ...
      state.fill.carb = clamp(
        state.fill.carb + portionValue('carb', state.bossEnraged ? 1.28 : 1.05),
        0,
        groupLimit('carb')
      );

      state.fill.protein = clamp(
        state.fill.protein - portionValue('protein', state.bossEnraged ? .3 : .2),
        0,
        groupLimit('protein')
      );
      ...
      state.fill.veg = clamp(
        state.fill.veg - portionValue('veg', state.bossEnraged ? .5 : .35),
        0,
        groupLimit('veg')
      );

      state.fill.fruit = clamp(
        state.fill.fruit - portionValue('fruit', state.bossEnraged ? .5 : .35),
        0,
        groupLimit('fruit')
      );
    ...
    postLog('boss_attack_hit', {
      bossType:b.id,
      enraged:!!state.bossEnraged,
      attackCount:state.bossAttackCount,
      balancePct:balanceScore(),
      plateHealth:Math.round(state.plateHealth)
    });
  }
  ...
  function damageBoss(dmg){
    ...
    if (state.bossHp <= 0 && !state.bossDefeated) {
      ...
      postLog('boss_defeated', {
        bossType:state.bossType ? state.bossType.id : '',
        counters:state.bossCounterCount,
        attacks:state.bossAttackCount,
        score:Math.round(state.score)
      });
    }
  }
    ...
    let m =
      b.id === 'greasy'
        ? {
            ...
            icon:'🥜',
            title:'👾 Oil Warning!',
            text:'น้ำมันกำลังล้น! ห้ามเติมไขมันดีช่วงนี้',
            sec:6,
            reward:105,
            penalty:30
          }
        : b.id === 'sugar'
          ? {
              type:'avoidJunk',
              title:'🍭 Sugar Storm!',
              text:'พายุน้ำตาลมาแล้ว! ห้ามโดน junk',
              sec:6,
              reward:110,
              penalty:35
            }
          : b.id === 'carbzilla'
            ? {
                ...
                icon:'🍚',
                title:'🍚 Carb Lock!',
                text:'Carbzilla ป่วนจาน! ห้ามเติมข้าว/แป้ง',
                sec:6,
                reward:105,
                penalty:30
              }
            : {
                ...
                icon:m0.icon,
                title:'🌀 Chaos Fix!',
                text:`รีบเติม ${m0.icon} ${m0.label} ให้ถูก 2 ครั้ง`,
                sec:7,
                target:2,
                reward:125,
                penalty:28
  ...
  function updateBossMechanic(){
    ...
    if (left <= 0) {
      if (m.type === 'avoidGroup' || m.type === 'avoidJunk') {
        completeBossMechanic(true, 'หลบสกิลบอสสำเร็จ!');
      } else {
        completeBossMechanic(false, 'แก้เกมไม่ทัน!');
      }
    }
  }
  ...
  function checkBossMechanicOnHealthy(food, overload){
    ...
    if (m.type === 'avoidGroup' && food.effects && food.effects[m.group]) {
      return completeBossMechanic(false, `เผลอเติม ${m.icon} ระหว่างสกิลบอส!`);
    }

    if (m.type === 'quickFix' && !overload && food.effects && food.effects[m.group]) {
      state.bossMechanicStats.hits++;

      if (state.bossMechanicStats.hits >= (m.target || 2)) {
        completeBossMechanic(true, 'แก้เกม Chaos สำเร็จ!');
      } else {
        feedback(`ดีมาก! อีก ${(m.target || 2) - state.bossMechanicStats.hits} ครั้ง`, 'good');
      }
    }
  }

  function checkBossMechanicOnJunk(){
    if (state.bossMechanic && state.bossMechanic.type === 'avoidJunk') {
      completeBossMechanic(false, 'โดน junk ระหว่าง Sugar Storm!');
    }
  }
    ...
    setTimeout(() => {
      ...
      if (els.app) {
        els.app.classList.remove(
          'boss-mechanic-active',
          'boss-mechanic-success',
          'boss-mechanic-fail'
        );
      }
    }, success ? 520 : 700);
  ...
  function checkMiniOnHealthy(food, overload){
    ...
    if (state.mini.type === 'healthyRain') {
      if (!overload && ++state.miniStats.hits >= state.mini.target) {
        finishMiniEvent(true);
      }
    } else if (state.mini.type === 'missingAlert') {
      const g = state.mini.group;

      if (!overload && food.effects && food.effects[g] && ++state.miniStats.groupHits >= state.mini.target) {
        finishMiniEvent(true);
      }
    }
  }
  ...
  function finishMiniEvent(success){
    ...
    if (success) {
      const reward =
        mini.type === 'healthyRain'
          ? 130
          : mini.type === 'missingAlert'
            ? 115
            : 125;
  ...
  function startOrder(){
    ...
    if (o && o.over > .2) {
      pool.push({
        ...
        group:o.id,
        icon:o.icon,
        text:`ห้ามเติม ${o.icon} ${o.label} เพิ่ม!`,
        sec:7,
        reward:90
      });
    }
  ...
  function startDuel(){
    ...
    const riskyPool = FOODS.filter(f =>
      f.junk ||
      (o && o.over > .1 && f.effects && f.effects[o.id]) ||
      wouldOverload(f)
    );
    ...
    if (els.duelLayer) {
      els.duelLayer.innerHTML = `
        <div class="duelBox">
          <div class="duelTitle">
            ⚡ อาหาร 2 ชิ้นกำลังพุ่งเข้าจาน! เลือกตัวที่เหมาะกว่า!
          </div>
  ...
  function recordPerf(kind, good){
    ...
    state.recentPerf = state.recentPerf
      .filter(x => t - x.t <= 16000)
      .slice(-18);
  ...
  function updateAIDirector(){
    ...
    let mood = 'normal';
    let icon = '🤖';
    let msg = 'AI Coach: คุมจานให้พอดีต่อไป';
    ...
    if (struggling) {
      ...
      mood = 'assist';
      icon = '💙';
      msg = 'AI Coach: ช่วยชะลอเกมนิดหนึ่ง ดูป้าย “ควรเก็บ”';

      if (state.shield <= 0 && state.badStreak >= 4) {
        state.shield = 1;
        feedback('💙 AI Coach ให้ Shield ช่วย 1 ครั้ง', 'good');
      }
    } else if (strong) {
      state.directorLevel = clamp(state.directorLevel + 1, 0, 4);
      state.assistLevel = clamp(state.assistLevel - 1, 0, 3);
      ...
      msg = state.directorLevel >= 3
        ? 'AI Coach: เก่งมาก! เพิ่มความท้าทายแล้ว'
        : 'AI Coach: จังหวะดีขึ้น เกมจะเร็วขึ้นนิดหนึ่ง';
    }
    ...
    if (els.aiDirectorChip) {
      ...
      updateAIDirector._timer = setTimeout(() => {
        if (els.aiDirectorChip) els.aiDirectorChip.classList.add('hidden');
      }, 2600);
    }
    ...
    if (els.bossMood) {
      els.bossMood.textContent = state.bossDefeated
        ...
        : state.boss
          ? (
              isBossWarning()
                ? '⚠️ Boss Warning: เลือกอาหารดีเพื่อ COUNTER!'
                : state.bossEnraged
                  ? '🔥 บอสโกรธแล้ว! Counter จะได้โบนัสแรงขึ้น'
                  : 'Perfect Pick จะลด HP บอส / junk ทำให้บอสฟื้น'
            )
          : 'บอสยังไม่มา';
    }
  }
  ...
  function updatePracticeCoach(){
    ...
    if (ph === 1) {
      state.fill.carb = Math.max(state.fill.carb, groupTarget('carb'));
    }
  }
  ...
  function updatePractice(){
    ...
    const delay =
      left > PRACTICE_SEC * .66
        ? 1200
        : left > PRACTICE_SEC * .33
          ? 1000
          : 850;

    if (!isFreeze() && now() - state.lastSpawn > delay) {
      spawnFood();
    }
  ...
  function resetForMainRound(){
    ...
    [
      ...
      if (els[k]) els[k].classList.remove('on');
    });
  ...
  function startGame(){
    ...
    try {
      DOC.body.classList.add('plate-playing','plate-immersive');
    } catch(e) {}
  ...
  function loop(){
    ...
    if (state.boss && !state.bossDefeated && now() - state.lastBossAtk > bossAttackDelay()) {
      bossAttack();
    }
  ...
  function endGame(){
    ...
    try {
      DOC.body.classList.remove('plate-playing','plate-immersive');
    } catch(e) {}
    ...
    if (els.app) {
      els.app.classList.remove('playing','immersive-gameplay','hud-target-safe');
    }
    ...
    const fairScore = Math.max(
      rawScore,
      Math.round(
        (Number(state.hits) || 0) * 42 +
        (Number(state.bestCombo) || 0) * 18 +
        done * 80 +
        (state.bossDefeated ? 220 : 0) +
        (state.lastSaveSuccess ? 120 : 0) -
        (Number(state.junkHits) || 0) * 25 -
        (Number(state.overloads) || 0) * 12
      )
    );

    if (fairScore > rawScore) {
      state.score = fairScore;

      try {
        console.info('[Plate Solo] fair summary score applied', {
          rawScore,
          fairScore
        });
      } catch(e) {}
    ...
    const rank =
      bal >= 90 && done >= 3 && state.bossDefeated
        ? {
            icon:'👑',
    ...
    if (els.badgeRow) {
      els.badgeRow.innerHTML = badges.map(b => `
        <span class="badge ${news.includes(b) ? 'new' : ''}">
          ${esc(b)}${news.includes(b) ? ' • NEW' : ''}
        </span>
      `).join('');
    }
      ...
      els.recommend.innerHTML = `
        <div class="childSummary">
          <div class="childLine">
            ...
            <b>${
              bal >= 85
                ? 'คุมจานสมดุลได้ยอดเยี่ยม'
                : state.bestCombo >= 8
                  ? 'ทำคอมโบได้ดีมาก'
                  : 'เลือกอาหารได้ดีขึ้นแล้ว'
            }</b>
          </div>
          <div class="childLine">
            🎯 ฝึกต่อ:
            <b>${
              state.junkHits > 2
                ? 'หลบ junk เช่น 🍟 🍰 🧃 ให้มากขึ้น'
                : `เติม ${mostMissingGroup().icon} ${mostMissingGroup().label} ให้เร็วกว่านี้`
            }</b>
          </div>
          <div class="childLine">
            ❤️ พลังจานสุดท้าย:
            <b>${Math.round(state.plateHealth)}%</b>
          </div>
          <div class="childLine">
            🔁 เป้าหมายรอบหน้า:
            <b>${stars >= 2 ? 'รอบหน้าลุ้น 3 ดาว' : 'รอบหน้าลองทำ 2 ดาว'}</b>
          </div>
    ...
    const summary = {
      ...
      mode:HHA.mode,
      version:VERSION,
      summaryPolishHint:'v20260514-final-portion-cooldown',
      ...
      bossEnraged:state.bossEnraged,
      bossCounterCount:state.bossCounterCount,
      bossAttackCount:state.bossAttackCount,
      nearMissUsed:state.nearMissUsed,
  ...
  function buildNutritionZoneUrl(fromTag){
    ...
    // กลับ Nutrition Zone แล้ว hub ภายในเป็น Hub หลัก
    u.searchParams.set(
      'hub',
  ...
  function goCooldown(){
    ...
    // หลัง cooldown กลับ Nutrition Zone ตามที่กำหนด
    ...
    gate.searchParams.set('return', zoneUrl.toString());
    gate.searchParams.set('returnUrl', zoneUrl.toString());

    // บังคับเข้า cooldown ไม่ให้ gate ข้ามเพราะจำค่า once/day เดิม
    gate.searchParams.set('force','1');
    gate.searchParams.set('once','0');
  ...
  function postLog(eventType, extra = {}){
    ...
    if (
      ...
      now() - state.lastFlushAt > 5000
    ) {
      flushLogs(eventType === 'session_end');
    }
  }
  ...
  function flushLogs(force = false){
    ...
    const batch = state.eventQueue.splice(
      ...
      force
        ...
        : Math.min(8, state.eventQueue.length)
    );
  ...
  function updateAimTarget(){
    ...
    for (const [id,obj] of state.active.entries()) {
      obj.el.classList.toggle('aimLock', id === bestId && bestD < range);
    }
  ...
  function bind(){
    ...
    if (els.btnStart && !els.btnStart.__plateBound) {
      els.btnStart.__plateBound = true;
      els.btnStart.addEventListener('click', startGame);
    }

    if (els.btnSkipPractice && !els.btnSkipPractice.__plateBound) {
      els.btnSkipPractice.__plateBound = true;
      els.btnSkipPractice.addEventListener('click', () => {
        if (state.practiceActive) finishPractice(true);
      });
    }

    if (els.btnPause && !els.btnPause.__plateBound) {
      els.btnPause.__plateBound = true;
      els.btnPause.addEventListener('click', () => togglePause(false));
    }

    if (els.btnHint && !els.btnHint.__plateBound) {
      els.btnHint.__plateBound = true;
      els.btnHint.addEventListener('click', () => showHint(true));
    }

    if (els.btnSkill && !els.btnSkill.__plateBound) {
      els.btnSkill.__plateBound = true;
      els.btnSkill.addEventListener('click', rescueSkill);
    }

    if (els.btnAim && !els.btnAim.__plateBound) {
      els.btnAim.__plateBound = true;
      els.btnAim.addEventListener('click', selectAimTarget);
    }

    if (els.btnBack && !els.btnBack.__plateBound) {
      els.btnBack.__plateBound = true;
      els.btnBack.addEventListener('click', goBack);
    }

    if (els.btnReplay && !els.btnReplay.__plateBound) {
      els.btnReplay.__plateBound = true;
      els.btnReplay.addEventListener('click', () => location.reload());
    }

    if (els.btnCooldown && !els.btnCooldown.__plateBound) {
      els.btnCooldown.__plateBound = true;
      els.btnCooldown.addEventListener('click', goCooldown);
    }

    if (els.btnSummaryBack && !els.btnSummaryBack.__plateBound) {
      els.btnSummaryBack.__plateBound = true;
      els.btnSummaryBack.addEventListener('click', goBack);
    }

    // กันพลาด: จับปุ่ม Cooldown จาก id/class/text แม้ HTML ใช้ชื่อไม่ตรง
    if (!DOC.__plateCooldownDelegateBound) {
      DOC.__plateCooldownDelegateBound = true;
      ...
      DOC.addEventListener('click', ev => {
        ...
        const isCooldownBtn =
          ...
          cls.includes('cooldown') ||
          txt.includes('cooldown') ||
          txt.includes('คูลดาวน์') ||
          txt.includes('ทำ cooldown');

        if (!isCooldownBtn) return;
  ...
  function init(){
    ...
    if (els.app) {
      els.app.classList.add('emoji-only-mode','v41-immersive-ready');
    }

    try {
      const disabledAt = Number(sessionStorage.getItem('HHA_API_DISABLED') || '0');

      if (disabledAt && Date.now() - disabledAt < 15 * 60 * 1000) {
        state.apiDisabled = true;
      }
    } catch(e) {}
