...
// v20260513-PLATE-SOLO-SUMMARYFIX2-DOMFIX-POLISH
// ✅ เข้าไฟล์ตรงจาก plate-v1.html ได้
// ✅ Summary ไม่เด้งเองตอนโหลด
// ✅ เล่นได้ / spawn อาหารได้ / update HUD ได้
// ✅ รองรับ Practice / Rush / Boss / Mission / Cooldown / Nutrition Zone
...
(() => {
  ...
  var VERSION = '20260513-PLATE-SOLO-SUMMARYFIX2-DOMFIX-POLISH';
  ...
  function resolvePlateEls(){
    ...
    els.btnHint =
      ...
      ensureEl(
        'btnHint',
        'button',
        DOC.querySelector('.controls-right') || els.app,
        'btn blue',
        'คำใบ้'
      );
    ...
    els.missions =
      ...
      ensureEl(
        'missions',
        'div',
        DOC.querySelector('.goal-panel') || els.app,
        'goal-list'
      );
    ...
    els.meters =
      ...
      ensureEl(
        'meters',
        'div',
        DOC.querySelector('.plate-side') || els.app,
        'meters-list'
      );
    ...
    els.timerFill =
      ...
      ensureEl(
        'timerFill',
        'i',
        DOC.querySelector('.timer-card') || els.app,
        'timer-fill'
      );
    ...
    els.phaseText =
      ...
      ensureEl(
        'phaseText',
        'div',
        DOC.querySelector('.combo-card') || els.app,
        'stat-value',
        'Ready'
      );
  ...
  function balanceScore(){
    const vals = GROUPS.map(g => {
      ...
      return f <= CFG.target
        ? (f / CFG.target) * 100
        : Math.max(0, 100 - (f - CFG.target) * 32);
    });
  ...
  function mostMissingGroup(){
    return GROUPS
      .map(g => ({
        ...g,
        need:CFG.target - (state.fill[g.id] || 0)
      }))
  ...
  function mostOverGroup(){
    return GROUPS
      .map(g => ({
        ...g,
        over:(state.fill[g.id] || 0) - CFG.target
      }))
  ...
  function wouldOverload(food){
    return !!(
      ...
      Object.entries(food.effects).some(([g,v]) =>
        (state.fill[g] || 0) + Number(v || 0) > CFG.target + .25
      )
    );
  }
  ...
  function chooseDailyChallenge(){
    return dailyPick([
      ...
      },
      {
        ...
        goal:'อาหารดีแต่ถ้าหมู่นั้นเต็มแล้วอย่าใส่เพิ่ม',
  ...
  function dailyStatus(){
    ...
    if (d.id === 'balance85') {
      return {
        done:balanceScore() >= 85,
        label:`${balanceScore()}%/85%`
      };
    }

    if (d.id === 'combo10') {
      return {
        done:state.bestCombo >= 10,
        label:`${Math.min(state.bestCombo,10)}/10`
      };
    }

    if (d.id === 'nojunk') {
      return {
        done:state.junkHits <= 1,
        label:`junk ${state.junkHits}/1`
      };
    }
    ...
    if (d.id === 'boss') {
      return {
        ...
        label:state.bossDefeated ? 'ชนะแล้ว' : 'ยังไม่ชนะ'
      };
    }

    if (d.id === 'portion') {
      return {
        done:state.overloads <= 1,
        label:`ล้น ${state.overloads}/1`
      };
    }
  ...
  function renderGoalLock(){
    ...
    const m1 = state.missions[0]
      ? state.missions[0].txt
      : 'ทำจานให้สมดุล';
  ...
  function choosePracticeFood(){
    ...
    if (ph === 1) {
      if (chance(.52)) {
        return {
          ...pick(FOODS.filter(f => !f.junk && f.effects && f.effects.carb))
        };
      }
...
// แสดงบนจานแค่ล่าสุด 10 ชิ้นพอ ไม่ให้จานรกจนเด็กงง
  ...
  function bossAttack(){
    ...
    if (b.id === 'greasy') {
      state.fill.fat = clamp(
        state.fill.fat + (state.bossEnraged ? 1.15 : .9),
        0,
        CFG.target + 2.2
      );
      state.fill.veg = clamp(
        state.fill.veg - (state.bossEnraged ? .35 : .25),
        0,
        CFG.target + 2.2
      );
      ...
      state.fill.carb = clamp(
        state.fill.carb + (state.bossEnraged ? .85 : .65),
        0,
        CFG.target + 2.2
      );
      ...
      state.fill.carb = clamp(
        state.fill.carb + (state.bossEnraged ? 1.28 : 1.05),
        0,
        CFG.target + 2.2
      );
      state.fill.protein = clamp(
        state.fill.protein - (state.bossEnraged ? .3 : .2),
        0,
        CFG.target + 2.2
      );
      ...
      state.fill.veg = clamp(
        state.fill.veg - (state.bossEnraged ? .5 : .35),
        0,
        CFG.target + 2.2
      );
      state.fill.fruit = clamp(
        state.fill.fruit - (state.bossEnraged ? .5 : .35),
        0,
        CFG.target + 2.2
      );
    ...
    const pool = [
      {
        ...
        group:m.id,
        icon:m.icon,
        text:`เติม ${m.icon} ${m.label} ให้ทัน!`,
        sec:7,
        reward:95
      },
      {
  ...
  function startDuel(){
    ...
    const good = {
      ...pick(goodPool.length ? goodPool : FOODS.filter(f => !f.junk))
    };

    const risky = {
      ...pick(riskyPool.length ? riskyPool : FOODS.filter(f => f.junk))
    };
  ...
  function updatePracticeCoach(){
    ...
    if (ph === 1) state.fill.carb = Math.max(state.fill.carb, CFG.target);
  }
  ...
  function rescueSkill(){
    ...
    state.fill[m.id] = Math.min(CFG.target * .7, state.fill[m.id] + .9);
  ...
  function endGame(){
    ...
    const summary = {
      ...
      mode:HHA.mode,
      version:VERSION,
      summaryPolishHint:'v20260513-summaryfix2',
