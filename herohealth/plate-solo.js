...
(function ensurePlateSoloShell(){
  ...
  function foodVisualHtml(food, cls=''){
    /* v40.7: emoji-only mode. No PNG request, no asset 404, use the original emoji visual for now. */
  ...
  function plateFoodVisualHtml(emoji){
    /* v40.7: emoji-only mode for plate icons too. */
...
/* =========================================================
   Plate Solo Mission Fallback
   PATCH v20260513-PLATE-SOLO-MISSION-FALLBACK
   เติมภารกิจ 5 หมู่ ถ้า #missions ยังว่าง
   ========================================================= */

(function(){
  'use strict';

  function ensurePlateMissions(){
    const missions = document.getElementById('missions');
    if (!missions) return;

    if (missions.children.length > 0) return;
    ...
    const groups = [
      {
        ...
        icon:'🥚',
        name:'หมู่ 1 โปรตีน',
        desc:'เนื้อ นม ไข่ ถั่ว'
      },
      {
        key:'carb',
        icon:'🍚',
        name:'หมู่ 2 ข้าว/แป้ง',
        desc:'ข้าว แป้ง เผือก มัน'
      },
      {
        key:'veg',
        icon:'🥦',
      ...
      },
      {
        ...
        icon:'🥑',
        name:'หมู่ 5 ไขมัน',
        desc:'ไขมันดี ปริมาณพอดี'
      }
    ];

    missions.innerHTML = groups.map((g, i) => `
      <div class="goal-item" data-group="${g.key}" id="mission-${g.key}">
        <div class="goal-icon">${g.icon}</div>
        <div>
          <div class="goal-name">${g.name}</div>
          <div class="goal-desc">${g.desc}</div>
        </div>
        <div class="goal-count" id="goalCount${i + 1}">0/1</div>
      </div>
    `).join('');
  }

  window.ensurePlateMissions = ensurePlateMissions;

  document.addEventListener('DOMContentLoaded', ensurePlateMissions);

  if (document.readyState === 'interactive' || document.readyState === 'complete') {
    ensurePlateMissions();
  }

  setTimeout(ensurePlateMissions, 100);
  setTimeout(ensurePlateMissions, 500);
})();
