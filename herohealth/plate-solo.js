...
// v20260514-PLATE-SOLO-PORTION-TARGET-FIX2-COOLDOWN-ZONEFIX
// ✅ Portion targets แยกตามหมู่อาหาร ไม่ใช้ x/4 เหมือนกันหมด
// ✅ อาหาร 1 ชิ้นเติม portion ไม่เท่ากัน ป้องกันเต็มเร็วเกินไป
// ✅ แก้ syntax หลุดจาก patch ก่อนหน้า / แก้ howBanner -> showBanner
// ✅ ปุ่ม Cooldown ส่งเข้า warmup-gate phase=cooldown และหลังจบกลับ Nutrition Zone
// ✅ Summary ไม่เด้งเองตอนโหลด / เล่นซ้ำได้ / DOM-safe / emoji-only

(() => {
  'use strict';

  const VERSION = '20260514-PLATE-SOLO-PORTION-TARGET-FIX2-COOLDOWN-ZONEFIX';
  ...
  const CFG = ({
    easy:      { spawn:1280, life:7100, junk:.10, bossAtk:6700 },
    normal:    { spawn:1160, life:6500, junk:.16, bossAtk:5650 },
    hard:      { spawn:900,  life:5300, junk:.25, bossAtk:4800 },
    challenge: { spawn:760,  life:4500, junk:.32, bossAtk:3950 }
  })[DIFF] || { spawn:1160, life:6500, junk:.16, bossAtk:5650 };
  ...
  // PATCH: เป้าหมายรายหมู่ไม่เท่ากัน และอาหารหนึ่งชิ้นไม่เติม 1.0 เท่ากันทุกหมู่
  const GROUP_TARGETS = {
    protein: 3.5,
    carb:    3.5,
    veg:     5.0,
  ...
  function groupLimit(g){ return groupTarget(g) + .75; }
  function portionValue(g, v){ return Number(v || 0) * Number(GROUP_PORTION_SCALE[g] || .65); }
  ...
  const SAFE_STORE = {
    ok:true,
    get(k, f = null){
      try { const v = localStorage.getItem(k); return v == null ? f : v; }
      catch(e) { this.ok = false; return f; }
    },
    set(k, v){
  ...
  const ids = `
    ...
    compactMeters plateHealthIcon plateHealthFill plateHealthText btnInfoToggle
    btnInfoClose infoDrawer infoDrawerContent feverLayer finalRush aiDirectorChip
    aiDirectorIcon aiDirectorText practiceCoach practiceIcon practiceTitle
    practiceText practiceSec practiceFill btnSkipPractice bossAvatar bossFace
    bossLabel bossSkillFlash bossMechanicBox bossMechanicTitle bossMechanicText
    bossMechanicSec bossMechanicFill lastSaveBox lastSaveText lastSaveSec
    lastSaveFill orderBox orderText orderSec orderFill duelLayer miniEventBox
    miniEventText miniEventSec miniEventFill log flash juiceLayer laneWarn
    startOverlay summaryOverlay dailyBox goalLock rankIcon summaryTitle
    summaryLine starRow bestRow sumScore sumBalance sumCombo sumMission badgeRow
    recommend pShield pFreeze pFever btnStart btnPause btnHint btnSkill btnAim
    btnBack aimReticle btnReplay btnCooldown btnSummaryBack
  `.trim().split(/\s+/);
  ...
  const state = {
    ...
    practiceEnabled:PRACTICE_ON, practiceActive:false, practiceDone:false,
    practiceStarted:0, practiceHits:0, practiceMistakes:0, practicePhase:0,
    startedAt:0, pausedMs:0, pauseStarted:0,
    totalSec:clamp(Number(qs('time','120')) || 120, 60, 180),
    score:0, combo:0, bestCombo:0,
    plateHealth:100, plateHealthMin:100,
    ...
    plateItems:[], active:new Map(), nextId:1,
    lastSpawn:0, lastSpawnLane:-1, spawnLaneHistory:[], wave:0, rush:false,
    feverUntil:0, lastFever:0, freezeUntil:0, shield:0, rescueUsed:false,
    boss:false, bossHp:100, bossDefeated:false, bossType:null, lastBossAtk:0,
    bossEnraged:false, bossWarnActive:false, bossWarnExpire:0, bossCounterCount:0, bossAttackCount:0,
    bossMechanic:null, bossMechanicStats:null, bossMechanicExpire:0, lastBossMechanicAt:0,
    order:null, orderExpire:0, lastOrderAt:0,
    duel:null, duelExpire:0, lastDuelAt:0,
    mini:null, miniStats:null, miniExpire:0, lastMiniAt:0,
    perfectPicks:0, riskyPicks:0, aimPicks:0, lastAimTarget:null,
    adaptiveOn:ADAPTIVE_ON, recentPerf:[], goodStreak:0, badStreak:0,
    directorLevel:0, assistLevel:0, directorMood:'normal', directorLastAt:0,
    missions:[], dailyChallenge:null, stars:0,
    logs:[], lastHintAt:0, bestBefore:null, unlockedBadges:new Set(), newBadges:[], comboMilestones:new Set(),
    nearMissUsed:false, nearMissCount:0, replayGoal:'',
    sessionId:'plate-' + Date.now().toString(36) + '-' + Math.floor(rand()*1e6).toString(36),
    eventQueue:[], lastFlushAt:0, apiDisabled:false
  };
  ...
  function ensureEl(id, tagName = 'div', parent = null, className = '', html = ''){
    ...
    const p = parent || byId('plateApp') || DOC.querySelector('.plate-app') || byId('app') || DOC.body;
    if (p) p.appendChild(el);
    return el;
  }

  function ensureSummaryOverlay(){
    let overlay = byId('summaryOverlay') || byId('summaryModal') || byId('resultModal');
    if (overlay) return overlay;
    overlay = DOC.createElement('section');
    overlay.id = 'summaryOverlay';
    overlay.className = 'summaryOverlay hidden';
    overlay.setAttribute('aria-hidden','true');
    overlay.innerHTML = `
      <div class="summaryCard">
        <div id="rankIcon" class="rankIcon">🍽️</div>
        <h2 id="summaryTitle">Plate Summary</h2>
        <p id="summaryLine">สรุปผลการจัดจานอาหาร</p>
        <div id="starRow" class="starRow"></div>
        <div class="summaryStats">
          <div><b id="sumScore">0</b><small>คะแนน</small></div>
          <div><b id="sumBalance">0%</b><small>สมดุล</small></div>
          <div><b id="sumCombo">0</b><small>คอมโบ</small></div>
          <div><b id="sumMission">0/3</b><small>ภารกิจ</small></div>
        </div>
        <div id="badgeRow" class="badgeRow"></div>
        <div id="recommend" class="recommend"></div>
        <div class="summaryActions">
          <button id="btnReplay" type="button" class="btn primary">🔁 เล่นอีกครั้ง</button>
          <button id="btnCooldown" type="button" class="btn cool" data-action="cooldown">🧘 Cooldown</button>
          <button id="btnSummaryBack" type="button" class="btn ghost">กลับ Nutrition Zone</button>
        </div>
      </div>`;
    (byId('plateApp') || byId('app') || DOC.body).appendChild(overlay);
    return overlay;
  }
  ...
  function resolvePlateEls(){
    ...
    els.app = els.app || byId('plateApp') || DOC.querySelector('.plate-app') || byId('app') || DOC.body;
    ...
    els.arena = els.arena || byId('gameStage') || byId('arena') || ensureEl('arena','div',els.app,'arena');
    els.spawnLayer = els.spawnLayer || byId('playField') || byId('spawnLayer') || ensureEl('spawnLayer','div',els.arena,'spawn-layer');
    els.plate = els.plate || byId('centerPlate') || byId('plate') || ensureEl('plate','div',els.arena,'center-plate');
    ...
    els.phaseBanner = els.phaseBanner || byId('prompt') || byId('phaseBanner') || ensureEl('phaseBanner','div',els.app,'phase-banner');
    els.startOverlay = els.startOverlay || byId('startOverlay');
    els.summaryOverlay = els.summaryOverlay || ensureSummaryOverlay();
  ...
  function installStyles(){
    if (byId('plateSoloPatchStyles')) return;
    const st = DOC.createElement('style');
    st.id = 'plateSoloPatchStyles';
    st.textContent = `
      .hidden{display:none!important}
      .summaryOverlay{position:fixed;inset:0;z-index:9999;display:none;align-items:center;justify-content:center;padding:18px;background:rgba(18,40,58,.48);backdrop-filter:blur(6px)}
      .summaryOverlay.show{display:flex!important}
      .summaryCard{width:min(620px,94vw);max-height:92vh;overflow:auto;border-radius:28px;background:linear-gradient(180deg,#fffef8,#effbff);box-shadow:0 28px 80px rgba(10,30,60,.32);padding:24px;text-align:center;color:#29485f;border:1px solid rgba(120,180,220,.35)}
      .rankIcon{font-size:56px}.summaryCard h2{margin:6px 0 2px;font-size:clamp(26px,4.5vw,42px)}.summaryCard p{margin:0 0 10px;color:#5c7b8f;font-weight:800}
      .starRow{font-size:34px;margin:8px 0}.star{filter:grayscale(1);opacity:.28}.star.on{filter:none;opacity:1}.starLabel{font-size:14px;font-weight:900;color:#5c7b8f}
      .summaryStats{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:14px 0}.summaryStats>div{background:#fff;border-radius:18px;padding:12px;border:1px solid rgba(70,140,190,.18)}.summaryStats b{display:block;font-size:24px}.summaryStats small{font-weight:900;color:#6a8ba0}
      .badgeRow{display:flex;gap:8px;flex-wrap:wrap;justify-content:center;margin:10px 0}.badge{border-radius:999px;padding:7px 10px;background:#fff6d6;border:1px solid rgba(255,190,60,.35);font-weight:900}.badge.new{background:#e8fff2;color:#176d45}
      .childSummary{display:grid;gap:8px;text-align:left}.childLine,.replayChallenge{border-radius:16px;background:#fff;padding:10px 12px;border:1px solid rgba(70,140,190,.16);font-weight:800}.replayChallenge{background:#fff8df}
      .summaryActions{display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin-top:16px}.btn{border:0;border-radius:999px;padding:12px 16px;font-weight:1000;cursor:pointer;background:#ecf6ff;color:#24506b}.btn.primary{background:#4fc3ff;color:#083047}.btn.cool{background:#8be7c2;color:#064330}.btn.ghost{background:#fff;border:1px solid rgba(60,120,170,.22)}
      .foodCard{position:absolute;top:-92px;left:50%;transform:translateX(-50%);z-index:45;border:0;border-radius:24px;padding:10px 12px;min-width:82px;min-height:92px;background:#fff;box-shadow:0 14px 34px rgba(28,68,88,.22);font-weight:1000;color:#244b62;cursor:pointer;touch-action:manipulation;animation:plateFall var(--life,6200ms) linear forwards;will-change:transform,top}.foodCard.junk{background:#fff1ef}.foodCard.goodNow{background:#effff5}.foodCard.dangerFood,.foodCard.riskyFood{background:#fff5d9}.foodCard.power{background:#eef8ff}.foodCard.aimLock{outline:5px solid rgba(255,210,30,.9)}
      .foodVisual{font-size:clamp(34px,8vw,54px);line-height:1}.foodTag{margin-top:4px;border-radius:999px;padding:3px 8px;background:rgba(70,140,190,.12);font-size:12px}.name{font-size:13px;margin-top:2px}.assetPackDebug{display:none}.plateFoodIcon{display:inline-flex;align-items:center;justify-content:center;font-size:22px;margin:1px}.plate-foods{display:flex;flex-wrap:wrap;align-items:center;justify-content:center;gap:2px;max-width:220px;margin:auto}
      .meter-row{border-radius:16px;background:#fff;padding:8px 10px;margin:7px 0;border:1px solid rgba(70,140,190,.15)}.meter-head{display:flex;justify-content:space-between;gap:8px;font-weight:1000}.meter-track{height:11px;border-radius:999px;background:#e9f3f8;overflow:hidden;margin-top:6px}.meter-fill{display:block;height:100%;width:0%;border-radius:999px;background:linear-gradient(90deg,#6edb8f,#ffd76a)}.meter-row.over .meter-fill{background:linear-gradient(90deg,#ff6b6b,#ffb15f)}.meter-need{font-size:12px;font-weight:900;color:#6a8ba0;margin-top:4px}.meter-row.over .meter-need{color:#b33b30}
      .floatText,.scorePop,.comboBoom,.phase-banner.show,.counterFx,.milestoneToast,.nearMissFx{pointer-events:none}.floatText,.scorePop{position:absolute;left:50%;top:42%;transform:translate(-50%,-50%);z-index:95;border-radius:18px;padding:10px 14px;color:#fff;font-weight:1000;box-shadow:0 18px 44px rgba(0,0,0,.22);animation:platePop .9s both}.scorePop{background:#248f5c}.scorePop.bad{background:#d64245}.scorePop.combo,.scorePop.mission,.scorePop.boss,.scorePop.power{background:#7b58dc}.comboBoom,.counterFx,.milestoneToast,.nearMissFx{position:absolute;left:50%;top:40%;transform:translate(-50%,-50%);z-index:96;border-radius:24px;background:#fff7cf;padding:14px 18px;font-weight:1000;box-shadow:0 18px 54px rgba(20,50,80,.22);animation:platePop 1.1s both;text-align:center}.flash.on{position:absolute;inset:0;background:rgba(255,255,255,.55);z-index:70;animation:plateFlash .32s both}
      .boss-warning #arena,.boss-warning .arena,.boss-warning #gameStage{box-shadow:0 0 0 5px rgba(255,82,82,.18),0 0 42px rgba(255,82,82,.28) inset!important}.boss-enraged #arena,.boss-enraged .arena,.boss-enraged #gameStage{animation:plateBossRumble .42s infinite alternate}
      .last-save-active #arena,.last-save-active .arena{box-shadow:0 0 0 5px rgba(255,200,0,.25),0 0 46px rgba(255,125,0,.22) inset!important}
      @keyframes plateFall{0%{top:-96px;transform:translateX(-50%) rotate(var(--rot,0deg))}100%{top:calc(100% + 96px);transform:translateX(calc(-50% + var(--endX,0px))) rotate(calc(var(--rot,0deg) * -1))}}
      @keyframes platePop{0%{opacity:0;transform:translate(-50%,-50%) scale(.72)}18%{opacity:1;transform:translate(-50%,-50%) scale(1.08)}72%{opacity:1;transform:translate(-50%,-70%) scale(1)}100%{opacity:0;transform:translate(-50%,-92%) scale(.9)}}
      @keyframes plateFlash{0%{opacity:0}35%{opacity:1}100%{opacity:0}}@keyframes plateBossRumble{from{transform:translateX(-1px)}to{transform:translateX(1px)}}
    `;
  ...
  function sfx(kind){
    try {
      ...
      const m = {
        good:[660,.055,'sine',.045], perfect:[880,.075,'triangle',.055],
        bad:[150,.08,'sawtooth',.035], boss:[90,.16,'square',.04],
      ...
      o.stop(audioCtx.currentTime + d + .015);
    } catch(e) {}
  }
  ...
  function plateFoodVisualHtml(emoji){
    const safeEmoji = esc(emoji || '🍽️');
    return `<span class="plateFoodIcon assetFallback emojiOnly"><span class="foodEmojiFallback" aria-hidden="true">${safeEmoji}</span></span>`;
  }
  ...
  function feedback(txt, kind = 'good'){
    ...
    d.textContent = txt;
    d.style.background = kind === 'bad' ? 'rgba(214,55,69,.84)' : kind === 'perfect' ? 'rgba(120,80,220,.88)' : 'rgba(36,144,80,.84)';
    (els.arena || DOC.body).appendChild(d);
  ...
  function scorePop(text, kind = 'good'){
    ...
    d.style.top = (42 + rand() * 14).toFixed(1) + '%';
    layer.appendChild(d);
    setTimeout(() => d.remove(), 900);
  }
  ...
  function coinBurst(kind = 'good', count = 7){
    ...
    for (let i = 0; i < count; i++) {
      ...
      c.style.left = (46 + rand()*8).toFixed(1) + '%';
      c.style.top = (45 + rand()*10).toFixed(1) + '%';
      c.style.zIndex = '94';
      c.style.fontSize = '24px';
      c.style.animation = 'platePop .9s both';
      layer.appendChild(c);
  ...
  function comboBoom(combo){
    popFx('comboBoom', combo >= 15 ? `🔥 PERFECT PLATE FEVER x${combo}!` : combo >= 10 ? `🔥 SUPER COMBO x${combo}!` : `⚡ COMBO x${combo}!`, 1000);
  }
  ...
  function plateCrashPenalty(){
    state.plateHealth = 18;
    loseScore(80, '-80 จานเกือบพัง!');
    state.combo = 0;
    shakePlate(); flash(); feedback('🚨 จานเกือบพัง! รีบกู้สมดุล', 'bad'); sfx('bad');
  }
  ...
  function renderMissions(){
    ...
    }).join('') + (state.dailyChallenge ? (() => {
      const ds = dailyStatus();
      return `<span id="mis-daily" class="mission ${ds.done ? 'done' : ''}">${ds.done ? '✅' : '📅'} Daily: ${esc(state.dailyChallenge.text)} <small>${esc(ds.label)}</small></span>`;
    })() : '');
  }
  ...
  function classifyFoodCard(food){
    ...
    if (state.lastSaveActive && state.lastSaveStats) {
      ...
      if (food.effects && food.effects[g] && !wouldOverload(food)) return { cls:'bossWanted goodNow', tag:'กู้จาน!' };
    }
    if (state.bossMechanic) {
      const m = state.bossMechanic;
      if (m.type === 'avoidJunk' && food.junk) return { cls:'bossForbidden junk', tag:'ห้ามโดน!' };
      if (m.type === 'avoidGroup' && food.effects && food.effects[m.group]) return { cls:'bossForbidden dangerFood', tag:'ห้ามตอนนี้!' };
      if (m.type === 'quickFix' && food.effects && food.effects[m.group] && !wouldOverload(food)) return { cls:'bossWanted goodNow', tag:'รีบเก็บ!' };
    }
    if (food.junk) return { cls:'junk', tag:'หลบ!' };
    if (wouldOverload(food)) return { cls:'riskyFood dangerFood', tag:'ล้น!' };
    const m = mostMissingGroup();
    if (food.effects && food.effects[m.id]) return { cls:'goodNow', tag:'ควรเก็บ' };
  ...
  function chooseMovement(food){
    ...
    if (state.mini && state.mini.type === 'junkInvasion' && food.junk) return { cls:'move-junk-dash', lifeScale:.68, label:'junk-invasion' };
    if (food.type === 'power') return { cls:'move-power-float', lifeScale:1.18, label:'power-float' };
    if (food.junk) return state.boss || state.rush ? { cls:'move-junk-dash', lifeScale:.72, label:'junk-fast' } : { cls:'move-junk-wobble', lifeScale:.9, label:'junk-wobble' };
    if (state.boss && chance(.38)) return { cls:'move-boss-swipe', lifeScale:.78, label:'boss-swipe' };
    if (state.rush && chance(.45)) return { cls:'move-zigzag', lifeScale:.82, label:'rush-move' };
  ...
  function foodSizeClass(food, m){
    if (food.type === 'power') return 'size-power';
    if (food.junk) return state.boss || (state.mini && state.mini.type === 'junkInvasion') ? 'size-boss' : 'size-junk';
    if (state.boss && m && m.label === 'boss-swipe') return 'size-boss';
    return 'size-good';
  }

  function chooseSpawnPlacement(food, m){
    const lanes = spawnLanes();
    const cs = lanes.map((pct,i) => ({ pct, i, score:rand() })).map(x => {
  ...
  function cardLife(){
    ...
    return clamp(life * clamp((1 - state.directorLevel * .035) * (1 + state.assistLevel * .16), 1.05, 1.85), 3800, 8800);
  }
  ...
  function choosePracticeFood(){
    ...
    if (ph === 0) {
      const m = mostMissingGroup();
      const pool = FOODS.filter(f => !f.junk && f.effects && f.effects[m.id] && !wouldOverload(f));
      return { ...pick(pool.length ? pool : FOODS.filter(f => !f.junk)) };
    }
    if (ph === 1) {
      if (chance(.52)) return { ...pick(FOODS.filter(f => !f.junk && f.effects && f.effects.carb)) };
      return { ...pick(FOODS.filter(f => !f.junk && f.effects && (f.effects.veg || f.effects.fruit || f.effects.protein))) };
    }
  ...
  function chooseFood(){
    ...
    if (state.mini && state.mini.type === 'healthyRain' && chance(.82)) {
      const pool = FOODS.filter(f => !f.junk && f.effects && (f.effects[m.id] || GROUPS.some(g => state.fill[g.id] < groupTarget(g.id) * .82 && f.effects[g.id])) && !wouldOverload(f));
      if (pool.length) return { ...pick(pool) };
    }

    if (state.mini && state.mini.type === 'missingAlert' && chance(.72)) {
      const g = state.mini.group || m.id;
      const pool = FOODS.filter(f => !f.junk && f.effects && f.effects[g] && !wouldOverload(f));
      if (pool.length) return { ...pick(pool) };
    }

    if (chance(.075) && !state.boss && !state.mini && !state.lastSaveActive) return { ...pick(POWERS) };

    let jr = CFG.junk + (state.wave - 1) * .035 + (state.rush ? .06 : 0) + (state.boss ? .08 : 0) + (state.directorLevel * .025 - state.assistLevel * .035);
  ...
  function spawnFood(forced = null){
    ...
    const place = chooseSpawnPlacement(food, mov);
    const size = foodSizeClass(food, mov);
    ...
    card.style.setProperty('--startX', Math.round((rand()*120)-60) + 'px');
    card.style.setProperty('--endX', Math.round((rand()*150)-75) + 'px');
    card.style.setProperty('--sway', Math.round(22 + rand()*28) + 'px');
  ...
  function clearActiveCards(){
    for (const obj of state.active.values()) { try { obj.el.remove(); } catch(e) {} }
    state.active.clear();
  }
  ...
  function handleHealthy(f){
    ...
    checkBossMechanicOnHealthy(f, overload);
    ...
    if (overload) {
      ...
      feedback('⚠️ หมู่นี้เต็มแล้ว!', 'bad'); sfx('bad'); shakePlate();
    ...
    updateMissionHot(); showHint(false); updateAll();
  ...
  function handleJunk(f){
    ...
    checkMiniOnJunk();
    checkBossMechanicOnJunk();

    if (state.shield > 0) {
      state.shield--;
      addScore(12, 'Shield +12', 'power');
      feedback('🛡️ Shield กันของหลอก!', 'good'); sfx('good');
    ...
    feedback(`❌ หลบ ${f.name}! จานเสียสมดุล`, 'bad'); sfx('bad'); shakePlate(); flash();
    logLine(`${f.emoji} ${f.name}: ของหลอก!`);
    recordPerf('junk', false);
    if (state.practiceActive) state.practiceMistakes++;
    showHint(true); updateAll();
  ...
  function applyPower(f){
    if (f.power === 'shield') {
      ...
      feedback('🛡️ ได้ Shield!', 'perfect'); logLine('ได้ Shield กัน junk');
    } else if (f.power === 'freeze') {
      state.freezeUntil = now() + 3000;
      feedback('❄️ Freeze 3 วิ!', 'perfect'); logLine('Freeze: อาหารหยุดเกิดชั่วคราว');
    } else {
      showHint(true); addScore(10, 'Hint +10', 'power'); feedback('💡 Smart Hint +10', 'good');
    }
    sfx('perfect'); updateAll();
  }
  ...
  function triggerFever(){
    ...
    setTimeout(() => { if (els.app) els.app.classList.remove('fever'); if (els.feverLayer) els.feverLayer.classList.add('hidden'); }, 7600);
    showBanner('🔥 FEVER MODE! คะแนน x2'); feedback('🔥 Fever Mode!', 'perfect'); sfx('fever');
  }
  ...
  function startLastSave(){
    ...
    showBanner('🚨 LAST SAVE!'); feedback('🚨 จานใกล้พัง! กู้ให้ทัน', 'bad'); sfx('boss');
  }
  ...
  function checkLastSaveOnHealthy(food, overload){
    ...
    } else if (overload) damagePlate(8, 'ล้นระหว่าง Last Save');
  }
  ...
  function completeLastSave(success, reason){
    ...
    if (success) {
      ...
      feedback('❤️ กู้จานสำเร็จ!', 'perfect'); coinBurst('boss',14); sfx('perfect');
    } else {
      loseScore(45, '-45 Last Save');
      state.combo = 0;
      state.plateHealth = Math.max(state.plateHealth,18);
      feedback('⚠️ กู้จานไม่สำเร็จ แต่ยังเล่นต่อได้!', 'bad'); shakePlate(); sfx('bad');
    }
  ...
  function updatePhase(tLeft){
    ...
    if (rush !== state.rush) {
      state.rush = rush;
      if (rush) { showBanner('⚡ Rush 8 วิ • คะแนน x2'); logLine('Rush Window! รีบเลือกให้แม่น'); }
    }
  }
  ...
  function startBoss(){
    ...
    if (els.app) { els.app.classList.add('boss-mode','boss-' + state.bossType.id); els.app.classList.remove('boss-defeated'); }
    ...
    showBanner(`${state.bossType.icon} Boss Plate!`); feedback(state.bossType.intro,'bad'); sfx('boss'); logLine(`Boss Phase: ${state.bossType.name}`);
  }

  function animateBoss(kind){
    if (!els.bossAvatar) return;
    els.bossAvatar.classList.remove('hit','attack');
    void els.bossAvatar.offsetWidth;
    els.bossAvatar.classList.add(kind);
    setTimeout(() => { if (els.bossAvatar) els.bossAvatar.classList.remove(kind); }, 520);
  }
  ...
  function maybeBossEnrage(){
    ...
    showBanner('🔥 BOSS ENRAGED!'); feedback('🔥 บอสโกรธแล้ว! โจมตีเร็วขึ้น แต่ counter ได้แรงขึ้น', 'bad'); sfx('boss'); logLine('Boss Enrage: ระวังสกิลถี่ขึ้น!');
  ...
  function startBossWarning(){
    ...
    showBossSkill(txt); showBanner('⚠️ Boss Warning!'); feedback('⚡ เลือกอาหารดีที่ยังไม่ล้น เพื่อสวนกลับ!', 'bad'); sfx('tick');
  ...
  function tryBossCounter(food, overload){
    ...
    showBanner('⚡ COUNTER สำเร็จ!'); feedback('⚡ สวนสกิลบอสสำเร็จ!', 'perfect'); sfx('fever'); logLine('COUNTER! เลือกอาหารดีทันก่อนบอสโจมตี');
  ...
  function tryNearMissRescue(food){
    ...
    feedback('💙 Near Miss! ยังไม่เสียเต็ม ได้ Shield 1 ครั้ง', 'perfect'); sfx('perfect');
  ...
  function bossAttack(){
    ...
    if (!state.bossWarnActive) { startBossWarning(); return; }
    ...
    const b = state.bossType || BOSSES[0];
    animateBoss('attack'); pulseBossArena();
    damagePlate(state.bossEnraged ? 10 : 7, 'บอสโจมตี');
    if (now() - state.lastBossMechanicAt > (state.bossEnraged ? 6500 : 9000)) startBossMechanic();

    if (b.id === 'greasy') {
      state.fill.fat = clamp(state.fill.fat + portionValue('fat', state.bossEnraged ? 1.15 : .9), 0, groupLimit('fat'));
      state.fill.veg = clamp(state.fill.veg - portionValue('veg', state.bossEnraged ? .35 : .25), 0, groupLimit('veg'));
      showBossSkill('👾 Oil Splash! ไขมันพุ่ง'); feedback('👾 บอสเทน้ำมัน! ระวังไขมันล้น', 'bad');
    } else if (b.id === 'sugar') {
      state.fill.carb = clamp(state.fill.carb + portionValue('carb', state.bossEnraged ? .85 : .65), 0, groupLimit('carb'));
      const junk = FOODS.filter(f => f.junk && ['🍰','🧃','🍩','🥤'].includes(f.emoji));
      spawnFood({ ...pick(junk) });
      setTimeout(() => { if (state.running && !state.ended) spawnFood({ ...pick(junk) }); }, state.bossEnraged ? 240 : 350);
      if (state.bossEnraged) setTimeout(() => { if (state.running && !state.ended) spawnFood({ ...pick(junk) }); }, 520);
      showBossSkill('🍭 Sweet Rain! ของหวานตกลงมา'); feedback('🍭 Sugar Storm ปล่อยของหวาน!', 'bad');
    } else if (b.id === 'carbzilla') {
      state.fill.carb = clamp(state.fill.carb + portionValue('carb', state.bossEnraged ? 1.28 : 1.05), 0, groupLimit('carb'));
      state.fill.protein = clamp(state.fill.protein - portionValue('protein', state.bossEnraged ? .3 : .2), 0, groupLimit('protein'));
      showBossSkill('🍚 Carb Burst! คาร์บเพิ่มแรง'); feedback('🍚 Carbzilla ทำให้คาร์บล้น!', 'bad');
    } else {
      state.fill.veg = clamp(state.fill.veg - portionValue('veg', state.bossEnraged ? .5 : .35), 0, groupLimit('veg'));
      state.fill.fruit = clamp(state.fill.fruit - portionValue('fruit', state.bossEnraged ? .5 : .35), 0, groupLimit('fruit'));
      if (!state.duel && !state.order && !state.lastSaveActive) startDuel();
      showBossSkill('🌀 Choice Trap! ต้องเลือกให้ไว'); feedback('🌀 Chaos Chef เปิดกับดักเลือก 1 จาก 2!', 'bad');
    }
  ...
  function damageBoss(dmg){
    ...
    if (state.bossHp <= 0 && !state.bossDefeated) {
      ...
      if (els.app) { els.app.classList.add('boss-defeated'); els.app.classList.remove('boss-warning','boss-enraged'); }
      showBanner('🏆 ชนะบอส! +220'); feedback('👑 Boss Crusher!', 'perfect'); sfx('fever'); logLine('ชนะบอสแล้ว!');
      popFx('comboBoom', '<div class="victoryBurstText">🏆 BOSS DOWN!</div>', 1700);
      coinBurst('boss',18);
  ...
  function startBossMechanic(){
    if (!state.boss || state.bossDefeated || !state.bossType || state.bossMechanic || state.lastSaveActive) return;
    state.lastBossMechanicAt = now();
    const m0 = mostMissingGroup();
    const b = state.bossType;
    const m = b.id === 'greasy'
      ? { type:'avoidGroup', group:'fat', icon:'🥜', title:'👾 Oil Warning!', text:'น้ำมันกำลังล้น! ห้ามเติมไขมันดีช่วงนี้', sec:6, reward:105, penalty:30 }
      : b.id === 'sugar'
        ? { type:'avoidJunk', title:'🍭 Sugar Storm!', text:'พายุน้ำตาลมาแล้ว! ห้ามโดน junk', sec:6, reward:110, penalty:35 }
        : b.id === 'carbzilla'
          ? { type:'avoidGroup', group:'carb', icon:'🍚', title:'🍚 Carb Lock!', text:'Carbzilla ป่วนจาน! ห้ามเติมข้าว/แป้ง', sec:6, reward:105, penalty:30 }
          : { type:'quickFix', group:m0.id, icon:m0.icon, title:'🌀 Chaos Fix!', text:`รีบเติม ${m0.icon} ${m0.label} ให้ถูก 2 ครั้ง`, sec:7, target:2, reward:125, penalty:28 };
    state.bossMechanic = m;
    state.bossMechanicExpire = now() + m.sec * 1000;
    state.bossMechanicStats = { hits:0 };
    if (els.bossMechanicBox) els.bossMechanicBox.classList.add('on');
    if (els.bossMechanicTitle) els.bossMechanicTitle.textContent = m.title;
    if (els.bossMechanicText) els.bossMechanicText.textContent = m.text;
    if (els.bossMechanicSec) els.bossMechanicSec.textContent = m.sec + 's';
    if (els.bossMechanicFill) els.bossMechanicFill.style.width = '100%';
    if (els.app) els.app.classList.add('boss-mechanic-active');
    showBanner(m.title); feedback(m.text, 'bad'); sfx('boss');
  }

  function updateBossMechanic(){
    if (!state.bossMechanic) return;
    const m = state.bossMechanic;
    const left = Math.max(0, state.bossMechanicExpire - now());
    if (els.bossMechanicSec) els.bossMechanicSec.textContent = Math.ceil(left/1000) + 's';
    if (els.bossMechanicFill) els.bossMechanicFill.style.width = clamp(left/(m.sec*1000)*100,0,100) + '%';
    if (left <= 0) {
      if (m.type === 'avoidGroup' || m.type === 'avoidJunk') completeBossMechanic(true, 'หลบสกิลบอสสำเร็จ!');
      else completeBossMechanic(false, 'แก้เกมไม่ทัน!');
    }
  }

  function checkBossMechanicOnHealthy(food, overload){
    if (!state.bossMechanic) return;
    const m = state.bossMechanic;
    if (m.type === 'avoidGroup' && food.effects && food.effects[m.group]) return completeBossMechanic(false, `เผลอเติม ${m.icon} ระหว่างสกิลบอส!`);
    if (m.type === 'quickFix' && !overload && food.effects && food.effects[m.group]) {
      state.bossMechanicStats.hits++;
      if (state.bossMechanicStats.hits >= (m.target || 2)) completeBossMechanic(true, 'แก้เกม Chaos สำเร็จ!');
      else feedback(`ดีมาก! อีก ${(m.target || 2) - state.bossMechanicStats.hits} ครั้ง`, 'good');
    }
  }

  function checkBossMechanicOnJunk(){
    if (state.bossMechanic && state.bossMechanic.type === 'avoidJunk') completeBossMechanic(false, 'โดน junk ระหว่าง Sugar Storm!');
  }

  function completeBossMechanic(success, reason){
    if (!state.bossMechanic) return;
    const m = state.bossMechanic;
    if (success) {
      addScore(m.reward || 100, `Boss Mission +${m.reward || 100}`, 'boss');
      state.combo++;
      state.bestCombo = Math.max(state.bestCombo, state.combo);
      if (state.boss && !state.bossDefeated) damageBoss(14);
      feedback(`✅ ${reason}`, 'perfect'); sfx('perfect'); coinBurst('boss',10);
    } else {
      loseScore(m.penalty || 28, `-${m.penalty || 28} Boss`);
      state.combo = 0;
      if (state.boss && !state.bossDefeated) state.bossHp = clamp(state.bossHp + 7, 0, 100);
      feedback(`⚠️ ${reason}`, 'bad'); sfx('bad'); shakePlate(); pulseBossArena();
    }
    setTimeout(() => {
      if (els.bossMechanicBox) els.bossMechanicBox.classList.remove('on');
      if (els.app) els.app.classList.remove('boss-mechanic-active','boss-mechanic-success','boss-mechanic-fail');
    }, success ? 520 : 700);
    state.bossMechanic = null;
    state.bossMechanicStats = null;
  }
  ...
  function maybeStartMiniEvent(tLeft){
    ...
    if (chance(state.boss ? .42 : .28)) startMiniEvent();
  }
  ...
  function startMiniEvent(){
    ...
    showBanner(`${state.mini.icon} ${state.mini.title}!`); sfx('fever'); logLine(`Mini Event: ${state.mini.title}`);
  }
  ...
  function updateMiniEvent(){
    ...
    if (left <= 0) finishMiniEvent(false);
  }
  ...
  function checkMiniOnHealthy(food, overload){
    ...
    if (state.mini.type === 'healthyRain') {
      ...
      if (!overload && food.effects && food.effects[g] && ++state.miniStats.groupHits >= state.mini.target) finishMiniEvent(true);
    }
  }
  ...
  function finishMiniEvent(success){
    ...
    if (mini.type === 'junkInvasion' && !success) success = state.miniStats && state.miniStats.junk === 0;
    if (success) {
      ...
      feedback(`${mini.icon} Mini Clear! +${reward}`, 'perfect'); sfx('perfect'); logLine(`${mini.title} สำเร็จ +${reward}`);
    } else {
      loseScore(20, '-20 Mini');
      state.combo = 0;
      if (state.boss && !state.bossDefeated) state.bossHp = clamp(state.bossHp + 4, 0, 100);
      feedback(`${mini.icon} Mini Failed`, 'bad'); sfx('bad'); shakePlate(); logLine(`${mini.title} ไม่สำเร็จ`);
    }
    state.mini = null;
  ...
  function maybeStartOrder(tLeft){
    ...
    if (chance(state.boss ? .52 : .38)) startOrder();
  }
  ...
  function startOrder(){
    ...
    showBanner('⚡ ภารกิจด่วน!'); sfx('tick'); logLine('ภารกิจด่วน: ' + state.order.text);
  }
  ...
  function checkOrderOnPick(food, overload, isJunk){
    ...
    const o = state.order;
    if (o.type === 'needGroup') {
      if (!isJunk && food.effects && food.effects[o.group] && !overload) completeOrder(true, 'ทำภารกิจด่วนสำเร็จ!');
      else if (isJunk) completeOrder(false, 'พลาดภารกิจด่วน');
    } else if (o.type === 'avoidGroup') {
      if (food.effects && food.effects[o.group]) completeOrder(false, `เผลอเติม ${o.icon} เพิ่ม`);
    ...
    } else if (o.type === 'noJunk' && isJunk) completeOrder(false, 'โดน junk แล้ว!');
  }
  ...
  function completeOrder(success, reason){
    ...
    if (success) {
      ...
      feedback(`⚡ ${reason} +${reward}`, 'perfect'); sfx('perfect'); logLine(reason + ` +${reward}`);
    } else {
      loseScore(22, '-22 Mission');
      state.combo = 0;
      if (state.boss && !state.bossDefeated) state.bossHp = clamp(state.bossHp + 4, 0, 100);
      feedback(`⚠️ ${reason}`, 'bad'); sfx('bad'); shakePlate(); logLine(reason);
    }
    state.order = null;
  ...
  function maybeStartDuel(tLeft){
    ...
    if (chance(state.boss ? .45 : .28)) startDuel();
  }
  ...
  function startDuel(){
    ...
    const m = mostMissingGroup();
    const o = mostOverGroup();
    const goodPool = FOODS.filter(f => !f.junk && f.effects && f.effects[m.id] && !wouldOverload(f));
    const riskyPool = FOODS.filter(f => f.junk || (o && o.over > .1 && f.effects && f.effects[o.id]) || wouldOverload(f));
    ...
    state.duel = { choices, bestEmoji:good.emoji, secMs:5200 };
    state.duelExpire = now() + state.duel.secMs;
    if (els.duelLayer) {
      els.duelLayer.innerHTML = `<div class="duelBox"><div class="duelTitle">⚡ อาหาร 2 ชิ้นกำลังพุ่งเข้าจาน! เลือกตัวที่เหมาะกว่า!</div><div class="duelChoices">${choices.map((f,i) => `<button class="duelBtn" data-i="${i}" type="button"><div>${foodVisualHtml(f)}<div class="name">${esc(f.name)}</div></div></button>`).join('')}</div><div class="duelTimer"><i id="duelFill"></i></div></div>`;
    ...
    showBanner('⚡ Double Choice!'); sfx('tick'); logLine('Double Choice: เลือกอาหารที่เหมาะที่สุด');
  }
  ...
  function clearDuel(timeout){
    ...
    if (timeout) {
      ...
      feedback('⏱️ ช้าไป! Double Choice หมดเวลา','bad'); sfx('bad'); logLine('Double Choice หมดเวลา');
    }
    state.duel = null;
  ...
  function updateAIDirector(){
    ...
    if (struggling) {
      ...
      state.directorLevel = clamp(state.directorLevel + 1, 0, 4);
      state.assistLevel = clamp(state.assistLevel - 1, 0, 3);
      mood = state.directorLevel >= 3 ? 'hot' : 'challenge';
      icon = state.directorLevel >= 3 ? '🔥' : '⚡';
      msg = state.directorLevel >= 3 ? 'AI Coach: เก่งมาก! เพิ่มความท้าทายแล้ว' : 'AI Coach: จังหวะดีขึ้น เกมจะเร็วขึ้นนิดหนึ่ง';
    }
    ...
    if (els.app) {
      els.app.classList.remove('ai-assist','ai-challenge','ai-hot');
      if (mood === 'assist') els.app.classList.add('ai-assist');
      if (mood === 'challenge') els.app.classList.add('ai-challenge');
      if (mood === 'hot') els.app.classList.add('ai-hot');
    }
  }
  ...
  function updateAll(){
    ...
    updateMeters(); renderPlate(); updatePowers(); updateBoss(); renderMissions(); updatePlateHealthUI();
  }
  ...
  function updatePracticeCoach(){
    ...
    if (ph === 1) state.fill.carb = Math.max(state.fill.carb, groupTarget('carb'));
  }
  ...
  function updatePractice(){
    ...
    expireCards(); updateAll();
  }

  function resetForMainRound(){
    if (els.btnStart && !state.running) { els.btnStart.disabled = false; els.btnStart.classList.remove('hidden'); }
    clearActiveCards();
    Object.assign(state, {
      score:0, combo:0, bestCombo:0,
      plateHealth:100, plateHealthMin:100,
      ...
      fill:Object.fromEntries(GROUPS.map(g => [g.id,0])),
      groupHits:Object.fromEntries(GROUPS.map(g => [g.id,0])),
      plateItems:[], wave:0, rush:false,
      feverUntil:0, lastFever:0, shield:0, freezeUntil:0, rescueUsed:false,
      boss:false, bossHp:100, bossDefeated:false, bossType:null, bossMechanic:null, bossMechanicStats:null,
      order:null, duel:null, mini:null, miniStats:null,
      perfectPicks:0, riskyPicks:0, aimPicks:0,
      recentPerf:[], goodStreak:0, badStreak:0, directorLevel:0, assistLevel:0, directorMood:'normal',
      comboMilestones:new Set(),
      bossEnraged:false, bossWarnActive:false, bossWarnExpire:0, bossCounterCount:0, bossAttackCount:0,
      nearMissUsed:false, nearMissCount:0, replayGoal:'', lastSpawnLane:-1, spawnLaneHistory:[]
    });
    ...
    if (els.app) {
      els.app.classList.remove('boss-mode','boss-greasy','boss-sugar','boss-carbzilla','boss-chaos','boss-defeated','boss-enraged','boss-warning','boss-mechanic-active','ai-assist','ai-challenge','ai-hot','plate-warn','plate-danger','plate-critical','last-save-active');
    }
    updateAll();
  }
  ...
  function startPractice(){
    ...
    updatePracticeCoach(); showBanner(`🧑‍🍳 Practice ${PRACTICE_SEC} วิ`); feedback('ลองก่อน! คะแนนยังไม่คิดจริง', 'good'); sfx('good');
  }
  ...
  function finishPractice(skipped){
    ...
    clearActiveCards(); resetForMainRound();
  ...
  function startGame(){
    hideSummaryOverlay(); resolvePlateEls(); setGameplayViewportVars();
  ...
  function startRealGame(){
    ...
    if (els.practiceCoach) els.practiceCoach.classList.add('hidden');
    if (els.app) els.app.classList.remove('practice-mode');
    showBanner('Wave 1 • เติมจานให้ครบ!'); showHint(true);
  ...
  function loop(){
    ...
    const left = timeLeft();
    updatePhase(left); updateAIDirector();
    if (left <= 0) { endGame(); return; }
    maybeStartMiniEvent(left); maybeStartOrder(left); maybeStartDuel(left);
    updateOrderAndDuel(); updateMiniEvent(); updateBossMechanic(); updateLastSave();
    ...
    updateAll(); requestAnimationFrame(loop);
  }
  ...
  function rescueSkill(){
    ...
    state.shield = Math.max(state.shield,1);
    const m = mostMissingGroup();
    state.fill[m.id] = Math.min(groupTarget(m.id) * .7, (state.fill[m.id] || 0) + .55);
    addScore(30, 'Rescue +30', 'power'); healPlate(18, 'Rescue Skill');
    feedback('✨ Rescue! Freeze + Shield + เติมหมู่ที่ขาด', 'perfect'); sfx('fever'); logLine('ใช้ Rescue Skill แล้ว'); updateAll();
  }
  ...
  function loadBadgeSet(){
    const arr = safeJsonParse(SAFE_STORE.get(badgeKey(),'[]'),[]);
  ...
  function endGame(){
    ...
    state.ended = true;
    state.running = false;
    try { DOC.body.classList.remove('plate-playing','plate-immersive'); } catch(e) {}
    clearActiveCards(); renderMissions();
    ...
    if (els.starRow) {
      els.starRow.innerHTML = `<span class="star ${stars >= 1 ? 'on' : ''}">⭐</span><span class="star ${stars >= 2 ? 'on' : ''}">⭐</span><span class="star ${stars >= 3 ? 'on' : ''}">⭐</span><div class="starLabel">${stars >= 3 ? 'สุดยอด! 3 ดาวแบบ Plate Hero' : stars === 2 ? 'ดีมาก! อีกนิดเดียวถึง 3 ดาว' : stars === 1 ? 'ผ่านแล้ว! รอบหน้าลุ้น 2 ดาว' : 'ลองใหม่ได้! เริ่มจากเติมหมู่ที่ขาดก่อน'}</div>`;
    }
    ...
    if (state.bossEnraged && state.bossDefeated) badges.push('🔥 Enrage Survivor');
    if (state.nearMissUsed) badges.push('💙 Near Miss Saver');
    ...
    if (els.badgeRow) els.badgeRow.innerHTML = badges.map(b => `<span class="badge ${news.includes(b) ? 'new' : ''}">${esc(b)}${news.includes(b) ? ' • NEW' : ''}</span>`).join('');

    state.replayGoal = buildReplayChallenge(stars, bal, done);
    if (els.recommend) {
      els.recommend.innerHTML = `<div class="childSummary"><div class="childLine">🌟 วันนี้เก่ง: <b>${bal >= 85 ? 'คุมจานสมดุลได้ยอดเยี่ยม' : state.bestCombo >= 8 ? 'ทำคอมโบได้ดีมาก' : 'เลือกอาหารได้ดีขึ้นแล้ว'}</b></div><div class="childLine">🎯 ฝึกต่อ: <b>${state.junkHits > 2 ? 'หลบ junk เช่น 🍟 🍰 🧃 ให้มากขึ้น' : `เติม ${mostMissingGroup().icon} ${mostMissingGroup().label} ให้เร็วกว่านี้`}</b></div><div class="childLine">❤️ พลังจานสุดท้าย: <b>${Math.round(state.plateHealth)}%</b></div><div class="childLine">🔁 เป้าหมายรอบหน้า: <b>${stars >= 2 ? 'รอบหน้าลุ้น 3 ดาว' : 'รอบหน้าลองทำ 2 ดาว'}</b></div><div class="replayChallenge">${esc(state.replayGoal)}</div></div>`;
    }

    if (els.btnStart) { els.btnStart.disabled = false; els.btnStart.classList.remove('hidden'); }
    showBanner('สรุปผล • เลือก Cooldown หรือกลับ Nutrition Zone');
    showSummaryOverlay();
    bind();

    const summary = {
      timestampIso:new Date().toISOString(), timestampBangkok:bangkokIso(),
      projectTag:HHA.projectTag, gameId:HHA.gameId, zone:HHA.zone, mode:HHA.mode,
      version:VERSION, summaryPolishHint:'portion-target-fix2-cooldown-zonefix',
      sessionId:state.sessionId, pid:HHA.pid, name:HHA.name, diff:DIFF, view:VIEW,
      scoreFinal:Math.round(state.score), balancePct:bal, comboMax:state.bestCombo,
      hits:state.hits, misses:state.misses, junkHits:state.junkHits, overloads:state.overloads,
      missionsDone:done, stars, bossDefeated:state.bossDefeated, bossEnraged:state.bossEnraged,
      bossCounterCount:state.bossCounterCount, bossAttackCount:state.bossAttackCount,
      nearMissUsed:state.nearMissUsed, nearMissCount:state.nearMissCount, replayGoal:state.replayGoal,
      plateHealthFinal:Math.round(state.plateHealth), lastSaveUsed:state.lastSaveUsed,
      lastSaveSuccess:state.lastSaveSuccess, durationPlayedSec:Math.round(elapsedSec())
    };
  ...
  function buildNutritionZoneUrl(fromTag){
    ...
    // หลังกลับ Nutrition Zone ปุ่มกลับ Hub ใหญ่ยังอ้างอิง hub เดิม
  ...
  function goCooldown(){
    ...
    // สำคัญ: หลัง cooldown กลับ Nutrition Zone ไม่กลับ Hub ทันที
  ...
  function postLog(eventType, extra = {}){
    ...
    state.eventQueue.push({
      table:eventType === 'session_end' ? 'sessions' : 'events', eventType,
      projectTag:HHA.projectTag, gameId:HHA.gameId, zone:HHA.zone, mode:HHA.mode,
      version:VERSION, sessionId:state.sessionId, pid:HHA.pid, name:HHA.name,
      diff:DIFF, view:VIEW, runMode:HHA.runMode, timestampIso:new Date().toISOString(),
      timestampBangkok:bangkokIso(), pageUrl:location.href, userAgent:navigator.userAgent, extra
    });
  ...
  function flushLogs(force = false){
    ...
    try {
      ...
      fetch(endpoint, { method:'POST', headers:{ 'Content-Type':'application/json' }, body, keepalive:!!force })
        .then(res => {
          if (res.status === 401 || res.status === 403) {
            state.apiDisabled = true;
            try { sessionStorage.setItem('HHA_API_DISABLED', String(Date.now())); } catch(e) {}
          }
  ...
  function selectAimTarget(){
    if (!isAimMode()) return;
    updateAimTarget();
    if (state.lastAimTarget && state.active.has(state.lastAimTarget)) { state.aimPicks++; pickFood(state.lastAimTarget); }
    else { feedback('🎯 เล็งอาหารให้ตรงกลางก่อน', 'bad'); sfx('bad'); }
  }

  function bind(){
    resolvePlateEls();
    if (els.btnStart && !els.btnStart.__plateBound) { els.btnStart.__plateBound = true; els.btnStart.addEventListener('click', startGame); }
    if (els.btnSkipPractice && !els.btnSkipPractice.__plateBound) { els.btnSkipPractice.__plateBound = true; els.btnSkipPractice.addEventListener('click', () => { if (state.practiceActive) finishPractice(true); }); }
    if (els.btnPause && !els.btnPause.__plateBound) { els.btnPause.__plateBound = true; els.btnPause.addEventListener('click', () => togglePause(false)); }
    if (els.btnHint && !els.btnHint.__plateBound) { els.btnHint.__plateBound = true; els.btnHint.addEventListener('click', () => showHint(true)); }
    if (els.btnSkill && !els.btnSkill.__plateBound) { els.btnSkill.__plateBound = true; els.btnSkill.addEventListener('click', rescueSkill); }
    if (els.btnAim && !els.btnAim.__plateBound) { els.btnAim.__plateBound = true; els.btnAim.addEventListener('click', selectAimTarget); }
    if (els.btnBack && !els.btnBack.__plateBound) { els.btnBack.__plateBound = true; els.btnBack.addEventListener('click', goBack); }
    if (els.btnReplay && !els.btnReplay.__plateBound) { els.btnReplay.__plateBound = true; els.btnReplay.addEventListener('click', () => location.reload()); }
    if (els.btnCooldown && !els.btnCooldown.__plateBound) { els.btnCooldown.__plateBound = true; els.btnCooldown.addEventListener('click', goCooldown); }
    if (els.btnSummaryBack && !els.btnSummaryBack.__plateBound) { els.btnSummaryBack.__plateBound = true; els.btnSummaryBack.addEventListener('click', goBack); }
    if (els.arena && !els.arena.__plateAimBound) {
      ...
      els.arena.addEventListener('pointerdown', ev => {
        ...
        ev.preventDefault(); selectAimTarget();
      }, { passive:false });
    }
    // robust fallback: เผื่อปุ่ม cooldown ใน HTML ใช้ id/class/data-action อื่น
    if (!DOC.__plateCooldownDelegationBound) {
      DOC.__plateCooldownDelegationBound = true;
      DOC.addEventListener('click', (ev) => {
        const btn = ev.target && ev.target.closest ? ev.target.closest('[data-action="cooldown"], .cooldownBtn, .btnCooldown, #cooldownBtn, #btnCooldown') : null;
        if (!btn) return;
        ev.preventDefault();
        goCooldown();
      }, true);
    ...
    if (!DOC.__plateVisibilityBound) {
      ...
      DOC.addEventListener('visibilitychange', () => { if (DOC.hidden && state.running && !state.paused) togglePause(true); });
    }
  }

  function init(){
    installStyles(); resolvePlateEls(); hideSummaryOverlay();
    state.ended = false; state.running = false;
    setGameplayViewportVars();
    if (els.app) els.app.classList.add('emoji-only-mode','v42-portion-target-ready');
    try {
    ...
    renderDailyBox(); renderGoalLock(); setupAimMode(); renderMissions(); updateAll(); bind();
    logLine('พร้อมแล้ว: เป้าหมายแต่ละหมู่ไม่เท่ากัน และ cooldown กลับ Nutrition Zone แล้ว!');
  }
