...
(() => {
  ...
  var VERSION = '20260509-PLATE-SOLO-V4.0.6-LINE11-SAFE-CACHE-MARKER';
  ...
  const PRACTICE_SEC = clamp(Number(qs('practiceSec','20')) || 20, 10, 30);

  const CFG = ({
    easy:{ spawn:1180, life:5200, junk:.11, target:3.6, bossAtk:6500 },
    normal:{ spawn:940, life:4550, junk:.19, target:4.0, bossAtk:5550 },
    hard:{ spawn:760, life:3850, junk:.27, target:4.3, bossAtk:4650 },
    challenge:{ spawn:620, life:3250, junk:.34, target:4.5, bossAtk:3820 }
  })[DIFF] || { spawn:940, life:4550, junk:.19, target:4.0, bossAtk:5550 };
  ...
  const FOODS = [
    {emoji:'🥚', name:'ไข่', effects:{protein:1}}, {emoji:'🐟', name:'ปลา', effects:{protein:1}},
    {emoji:'🍗', name:'ไก่', effects:{protein:1}}, {emoji:'🫘', name:'ถั่ว', effects:{protein:.7,fat:.35}},
    {emoji:'🥛', name:'นม', effects:{protein:.7,fat:.25}}, {emoji:'🍚', name:'ข้าว', effects:{carb:1}},
    {emoji:'🍞', name:'ขนมปัง', effects:{carb:1}}, {emoji:'🥔', name:'มัน', effects:{carb:1}},
    {emoji:'🍠', name:'เผือก/มัน', effects:{carb:1}}, {emoji:'🥦', name:'บรอกโคลี', effects:{veg:1}},
    {emoji:'🥬', name:'ผักใบ', effects:{veg:1}}, {emoji:'🥕', name:'แครอต', effects:{veg:1}},
    {emoji:'🥒', name:'แตงกวา', effects:{veg:1}}, {emoji:'🍎', name:'แอปเปิล', effects:{fruit:1}},
    {emoji:'🍌', name:'กล้วย', effects:{fruit:1}}, {emoji:'🍊', name:'ส้ม', effects:{fruit:1}},
    {emoji:'🍉', name:'แตงโม', effects:{fruit:1}}, {emoji:'🥜', name:'ถั่ว/ไขมันดี', effects:{fat:1}},
    {emoji:'🥑', name:'อะโวคาโด', effects:{fat:1}}, {emoji:'🫒', name:'น้ำมันดี', effects:{fat:1}},
    {emoji:'🍟', name:'เฟรนช์ฟรายส์', junk:true, effects:{fat:1.2,carb:.5}},
    {emoji:'🍰', name:'เค้กหวาน', junk:true, effects:{carb:1,fat:.6}},
    {emoji:'🧃', name:'น้ำหวาน', junk:true, effects:{carb:1}}, {emoji:'🥤', name:'น้ำอัดลม', junk:true, effects:{carb:1}},
    {emoji:'🍩', name:'โดนัท', junk:true, effects:{carb:.8,fat:.8}}, {emoji:'🍔', name:'เบอร์เกอร์', junk:true, effects:{carb:.8,fat:1,protein:.3}}
  ];
  const POWERS = [{emoji:'🛡️',name:'Shield',power:'shield',type:'power'},{emoji:'❄️',name:'Freeze',power:'freeze',type:'power'},{emoji:'💡',name:'Smart Hint',power:'hint',type:'power'}];
  const BOSSES = [
  ...
  const FOOD_ASSET_BASES = ['./assets/plate/','./assets/food/','./assets/goodjunk/'];
  const FOOD_ASSET_FILES = {
    '🥚':['egg.png','egg-object.png'], '🐟':['fish.png','fish-object.png'], '🍗':['chicken.png','chicken-object.png'],
    '🫘':['beans.png','bean.png','legume.png'], '🥛':['milk.png','milk-box.png'], '🍚':['rice.png','rice-bowl.png'],
    '🍞':['bread.png','toast.png'], '🥔':['potato.png'], '🍠':['sweet-potato.png','yam.png'],
    '🥦':['broccoli.png','broccoli-object.png'], '🥬':['leafy-green.png','vegetable.png','green-veg.png'],
    '🥕':['carrot.png'], '🥒':['cucumber.png'], '🍎':['apple.png','apple-object.png'], '🍌':['banana.png'],
    '🍊':['orange.png'], '🍉':['watermelon.png'], '🥜':['nuts.png','nut.png'], '🥑':['avocado.png'],
    '🫒':['olive-oil.png','oil.png','healthy-oil.png'], '🍟':['fries.png','french-fries.png','junk-fries.png'],
    '🍰':['cake.png','sweet-cake.png'], '🧃':['sugar-drink.png','sugar-juice.png','juice.png','soda.png','fake-healthy.png'],
    '🥤':['soda.png','soft-drink.png','sugar-drink.png'], '🍩':['donut.png','doughnut.png'], '🍔':['burger.png','hamburger.png'],
    '🛡️':['shield.png','power-shield.png'], '❄️':['freeze.png','ice.png','power-freeze.png'], '💡':['hint.png','idea.png','power-hint.png']
  };

  function hashSeed(str){ let h=2166136261>>>0; str=String(str||'plate'); for(let i=0;i<str.length;i++){h^=str.charCodeAt(i); h=Math.imul(h,16777619);} return h>>>0; }
  function mulberry32(seed){ let t=seed>>>0; return function(){ t+=0x6D2B79F5; let r=Math.imul(t^(t>>>15),1|t); r^=r+Math.imul(r^(r>>>7),61|r); return ((r^(r>>>14))>>>0)/4294967296; }; }
  function bangkokYmd(){ try{return new Intl.DateTimeFormat('en-CA',{timeZone:'Asia/Bangkok',year:'numeric',month:'2-digit',day:'2-digit'}).format(new Date());}catch(e){return new Date().toISOString().slice(0,10);} }
  function bangkokIso(){ try{const s=new Intl.DateTimeFormat('sv-SE',{timeZone:'Asia/Bangkok',year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}).format(new Date()); return s.replace(' ','T')+'+07:00';}catch(e){return new Date().toISOString();} }
  ...
  const SAFE_STORE = { ok:true, get(k,f=null){try{const v=localStorage.getItem(k); return v==null?f:v;}catch(e){this.ok=false; return f;}}, set(k,v){try{localStorage.setItem(k,v); return true;}catch(e){this.ok=false; return false;}} };

  function assetCandidatesForEmoji(emoji){ const files=FOOD_ASSET_FILES[String(emoji||'')]||[]; const out=[]; FOOD_ASSET_BASES.forEach(b=>files.forEach(f=>out.push(b+f))); return out; }
  function assetPackNameFromUrl(url){ if(!url)return'emoji'; if(url.includes('/plate/'))return'plate'; if(url.includes('/food/'))return'food'; if(url.includes('/goodjunk/'))return'goodjunk'; return'asset'; }
  WIN.HHAPlateAssetFallback = function(img){ try{const list=String(img.getAttribute('data-assets')||'').split('|').filter(Boolean); const idx=Number(img.getAttribute('data-asset-index')||'0')+1; if(idx<list.length){img.setAttribute('data-asset-index',String(idx)); img.src=list[idx]; const d=img.parentElement&&img.parentElement.querySelector('.assetPackDebug'); if(d)d.textContent=assetPackNameFromUrl(list[idx]); return;} img.style.display='none'; const p=img.parentElement; if(p)p.classList.add('assetFallback'); const fb=img.nextElementSibling; if(fb)fb.style.display='inline'; const d=p&&p.querySelector('.assetPackDebug'); if(d)d.textContent='emoji';}catch(e){} };
  function foodVisualHtml(food, cls=''){ const emoji=esc(food&&food.emoji?food.emoji:'🍽️'); const name=esc(food&&food.name?food.name:'food'); const list=assetCandidatesForEmoji(food&&food.emoji); const src=list[0]||''; if(!src)return `<div class="foodVisual assetFallback ${cls}"><span class="foodEmojiFallback" style="display:inline">${emoji}</span><span class="assetPackDebug">emoji</span></div>`; return `<div class="foodVisual ${cls}"><img class="foodImg" src="${esc(src)}" data-assets="${esc(list.join('|'))}" data-asset-index="0" alt="${name}" loading="eager" draggable="false" onerror="window.HHAPlateAssetFallback&&window.HHAPlateAssetFallback(this)"><span class="foodEmojiFallback">${emoji}</span><span class="assetPackDebug">${esc(assetPackNameFromUrl(src))}</span></div>`; }
  function plateFoodVisualHtml(emoji){ const list=assetCandidatesForEmoji(emoji); const src=list[0]||''; const safeEmoji=esc(emoji||'🍽️'); if(!src)return `<span class="plateFoodIcon assetFallback"><span class="foodEmojiFallback" style="display:inline">${safeEmoji}</span></span>`; return `<span class="plateFoodIcon"><img src="${esc(src)}" data-assets="${esc(list.join('|'))}" data-asset-index="0" alt="${safeEmoji}" loading="lazy" draggable="false" onerror="window.HHAPlateAssetFallback&&window.HHAPlateAssetFallback(this)"><span class="foodEmojiFallback">${safeEmoji}</span></span>`; }
  ...
  const state = {
    ...
    score:0, combo:0, bestCombo:0, plateHealth:100, plateHealthMin:100, lastSaveActive:false, lastSaveUsed:false, lastSaveSuccess:false, lastSaveExpire:0, lastSaveStats:null,
    hits:0, misses:0, junkHits:0, overloads:0, missedGood:0, fill:Object.fromEntries(GROUPS.map(g=>[g.id,0])), groupHits:Object.fromEntries(GROUPS.map(g=>[g.id,0])), plateItems:[], active:new Map(), nextId:1,
  ...
  function practiceElapsedSec(){return Math.max(0,(now()-state.practiceStarted)/1000);} function practiceLeftSec(){return Math.max(0,PRACTICE_SEC-practiceElapsedSec());}
  function isFever(){return now()<state.feverUntil;} function isFreeze(){return now()<state.freezeUntil;}
  function balanceScore(){ const vals=GROUPS.map(g=>{const f=state.fill[g.id]||0; return f<=CFG.target?(f/CFG.target)*100:Math.max(0,100-(f-CFG.target)*32);}); return Math.round(vals.reduce((a,b)=>a+b,0)/vals.length); }
  function mostMissingGroup(){return GROUPS.map(g=>({...g,need:CFG.target-(state.fill[g.id]||0)})).sort((a,b)=>b.need-a.need)[0]||GROUPS[0];}
  function mostOverGroup(){return GROUPS.map(g=>({...g,over:(state.fill[g.id]||0)-CFG.target})).sort((a,b)=>b.over-a.over)[0]||GROUPS[0];}
  ...
  function coinBurst(kind='good',count=7){const layer=els.juiceLayer||els.arena, icons=kind==='boss'?['⭐','✨','🏆','💥']:kind==='power'?['✨','⚡','💫']:['⭐','🟡','✨']; for(let i=0;i<count;i++){const c=DOC.createElement('div'); c.className='coinFx'; c.textContent=pick(icons); const a=Math.PI*2*i/count+rand()*.55,dist=54+rand()*94; c.style.setProperty('--dx',Math.cos(a)*dist+'px'); c.style.setProperty('--dy',Math.sin(a)*dist+'px'); c.style.setProperty('--rot',((rand()*540)-270).toFixed(0)+'deg'); c.style.left=(46+rand()*8).toFixed(1)+'%'; c.style.top=(45+rand()*10).toFixed(1)+'%'; layer.appendChild(c); setTimeout(()=>c.remove(),950);}}
  function comboBoom(combo){const d=DOC.createElement('div'); d.className='comboBoom'; d.textContent=combo>=15?`🔥 PERFECT PLATE FEVER x${combo}!`:combo>=10?`🔥 SUPER COMBO x${combo}!`:`⚡ COMBO x${combo}!`; (els.juiceLayer||els.arena).appendChild(d); setTimeout(()=>d.remove(),1000);}
  function shakePlate(){els.plate.classList.remove('bad'); void els.plate.offsetWidth; els.plate.classList.add('bad');} function flash(){if(els.flash){els.flash.classList.remove('on'); void els.flash.offsetWidth; els.flash.classList.add('on');}}
  function logLine(txt){state.logs.unshift(txt); state.logs=state.logs.slice(0,7); els.log.innerHTML=state.logs.map(x=>`<div class="logLine">${esc(x)}</div>`).join('');}
  ...
  function damagePlate(amount,reason=''){const n=Math.max(0,Number(amount)||0); if(!n)return; state.plateHealth=clamp(state.plateHealth-n,0,100); state.plateHealthMin=Math.min(state.plateHealthMin,state.plateHealth); if(reason)logLine(`❤️ จานเสียหาย: ${reason}`); if(state.plateHealth<=25&&!state.lastSaveActive&&!state.lastSaveUsed&&state.running&&!state.ended&&!state.practiceActive)startLastSave(); if(state.plateHealth<=0&&!state.lastSaveActive)plateCrashPenalty();}
  ...
  function showHint(force=false){if(!force&&now()-state.lastHintAt<2400)return; state.lastHintAt=now(); const m=mostMissingGroup(),o=mostOverGroup(); let msg='เติมหมู่ที่ยังขาดก่อน จะได้คอมโบสูง!'; if(state.lastSaveActive&&state.lastSaveStats)msg=`🚨 Last Save: รีบเติม <b>${state.lastSaveStats.needIcon} ${state.lastSaveStats.needLabel}</b> และห้ามโดน junk!`; else if(o&&o.over>.35)msg=`⚠️ <b>${o.icon} ${o.label}</b> เริ่มล้นแล้ว หยุดเติมก่อน!`; else if(m&&m.need>.65)msg=`รีบเติม <b>${m.icon} ${m.label}</b> จานยังขาดอยู่!`; else if(balanceScore()>=86)msg='เยี่ยม! จานใกล้สมดุลมาก รักษาคอมโบไว้!'; if(state.boss&&!state.bossDefeated)msg+=' ทำถูกต่อเนื่องเพื่อลด HP บอส!'; els.hintBox.innerHTML=msg;}

  function chooseMissions(){const pool=[{id:'veg',txt:'เก็บผัก 4 ครั้ง',type:'group',group:'veg',target:4},{id:'fruit',txt:'เก็บผลไม้ 3 ครั้ง',type:'group',group:'fruit',target:3},{id:'protein',txt:'เติมโปรตีน 3 ครั้ง',type:'group',group:'protein',target:3},{id:'combo',txt:'ทำคอมโบ 8',type:'combo',target:8},{id:'rainbow',txt:'ครบทั้ง 5 หมู่',type:'rainbow',target:5},{id:'lowjunk',txt:'โดน junk ไม่เกิน 2',type:'endLowJunk',target:2},{id:'balance',txt:'จบด้วยสมดุล 80%+',type:'endBalance',target:80},{id:'boss',txt:'ชนะบอสท้ายเกม',type:'boss'}]; const arr=[]; while(arr.length<3&&pool.length)arr.push(pool.splice(Math.floor(rand()*pool.length),1)[0]); return arr;}
  function chooseDailyChallenge(){return dailyPick([{id:'balance85',icon:'⚖️',text:'จบเกมด้วยสมดุล 85%+',goal:'รักษาจานให้พอดี ไม่ล้น',reward:'Daily Balance Star'},{id:'combo10',icon:'🔥',text:'ทำคอมโบสูงสุด 10+',goal:'เลือกถูกต่อเนื่อง อย่ากดมั่ว',reward:'Daily Combo Star'},{id:'nojunk',icon:'🚫',text:'จบเกมโดยโดน junk ไม่เกิน 1',goal:'หลบของทอด ของหวาน น้ำหวาน',reward:'Daily Clean Star'},{id:'boss',icon:'👾',text:'ชนะบอสท้ายเกม',goal:'ทำ Perfect Pick เพื่อลด HP บอส',reward:'Daily Boss Star'},{id:'portion',icon:'📏',text:'จบเกมโดยล้นไม่เกิน 1 ครั้ง',goal:'อาหารดีแต่ถ้าหมู่นั้นเต็มแล้วอย่าใส่เพิ่ม',reward:'Daily Portion Star'},{id:'vegfruit',icon:'🥦',text:'เก็บผัก 4 ครั้ง และผลไม้ 3 ครั้ง',goal:'เติมผักผลไม้ให้ครบ ไม่ขาด',reward:'Daily Rainbow Star'}]);}
  function dailyStatus(){const d=state.dailyChallenge; if(!d)return{done:false,label:''}; if(d.id==='balance85')return{done:balanceScore()>=85,label:`${balanceScore()}%/85%`}; if(d.id==='combo10')return{done:state.bestCombo>=10,label:`${Math.min(state.bestCombo,10)}/10`}; if(d.id==='nojunk')return{done:state.junkHits<=1,label:`junk ${state.junkHits}/1`}; if(d.id==='boss')return{done:!!state.bossDefeated,label:state.bossDefeated?'ชนะแล้ว':'ยังไม่ชนะ'}; if(d.id==='portion')return{done:state.overloads<=1,label:`ล้น ${state.overloads}/1`}; if(d.id==='vegfruit'){const v=state.groupHits.veg||0,f=state.groupHits.fruit||0; return{done:v>=4&&f>=3,label:`ผัก ${Math.min(v,4)}/4 ผลไม้ ${Math.min(f,3)}/3`};} return{done:false,label:''};}
  function missionStatus(m){if(m.type==='group')return{done:(state.groupHits[m.group]||0)>=m.target,val:Math.min(state.groupHits[m.group]||0,m.target),max:m.target}; if(m.type==='combo')return{done:state.bestCombo>=m.target,val:Math.min(state.bestCombo,m.target),max:m.target}; if(m.type==='rainbow'){const c=GROUPS.filter(g=>(state.groupHits[g.id]||0)>0).length; return{done:c>=5,val:c,max:5};} if(m.type==='endLowJunk')return{done:state.junkHits<=m.target,val:Math.min(state.junkHits,m.target),max:m.target}; if(m.type==='endBalance')return{done:balanceScore()>=m.target,val:balanceScore(),max:m.target,pct:true}; if(m.type==='boss')return{done:state.bossDefeated,val:state.bossDefeated?1:0,max:1}; return{done:false,val:0,max:1};}
  function renderMissions(){els.missions.innerHTML=state.missions.map(m=>{const st=missionStatus(m),p=st.pct?`${st.val}%`:`${st.val}/${st.max}`; return`<span id="mis-${m.id}" class="mission ${st.done?'done':''}">${st.done?'✅':'🎯'} ${esc(m.txt)} <small>${p}</small></span>`;}).join('')+(state.dailyChallenge?(()=>{const ds=dailyStatus(); return`<span id="mis-daily" class="mission ${ds.done?'done':''}">${ds.done?'✅':'📅'} Daily: ${esc(state.dailyChallenge.text)} <small>${esc(ds.label)}</small></span>`;})():'');}
  function updateMissionHot(){renderMissions(); state.missions.forEach(m=>{const st=missionStatus(m),el=$('mis-'+m.id); if(el&&!st.done&&st.val>=st.max-1)el.classList.add('hot');});}
  ...
  function startDuel(){state.lastDuelAt=now(); const m=mostMissingGroup(),o=mostOverGroup(),goodPool=FOODS.filter(f=>!f.junk&&f.effects&&f.effects[m.id]&&!wouldOverload(f)),riskyPool=FOODS.filter(f=>f.junk||(o&&o.over>.1&&f.effects&&f.effects[o.id])||wouldOverload(f)),good={...pick(goodPool.length?goodPool:FOODS.filter(f=>!f.junk))},risky={...pick(riskyPool.length?riskyPool:FOODS.filter(f=>f.junk))},choices=chance(.5)?[good,risky]:[risky,good]; state.duel={choices,bestEmoji:good.emoji,secMs:5200}; state.duelExpire=now()+state.duel.secMs; if(els.duelLayer){els.duelLayer.innerHTML=`<div class="duelBox"><div class="duelTitle">⚡ อาหาร 2 ชิ้นกำลังพุ่งเข้าจาน! เลือกตัวที่เหมาะกว่า!</div><div class="duelChoices">${choices.map((f,i)=>`<button class="duelBtn" data-i="${i}"><div>${foodVisualHtml(f)}<div class="name">${esc(f.name)}</div></div></button>`).join('')}</div><div class="duelTimer"><i id="duelFill"></i></div></div>`; Array.from(els.duelLayer.querySelectorAll('.duelBtn')).forEach(btn=>btn.addEventListener('pointerdown',ev=>{ev.preventDefault(); pickDuel(Number(btn.dataset.i));},{passive:false}));} showBanner('⚡ Double Choice!'); sfx('tick'); logLine('Double Choice: เลือกอาหารที่เหมาะที่สุด');}
  function pickDuel(i){if(!state.duel)return; const f=state.duel.choices[i]; clearDuel(false); if(!f)return; if(f.junk)return handleJunk(f); return handleHealthy(f);} function clearDuel(timeout){if(!state.duel)return; if(timeout){state.combo=0; loseScore(18,'-18 ช้าไป!'); feedback('⏱️ ช้าไป! Double Choice หมดเวลา','bad'); sfx('bad'); logLine('Double Choice หมดเวลา');} state.duel=null; if(els.duelLayer)els.duelLayer.innerHTML=''; updateAll();}
  ...
  function rescueSkill(){if(!state.running||state.paused||state.ended||state.rescueUsed)return; state.rescueUsed=true; els.btnSkill.disabled=true; state.freezeUntil=now()+2500; state.shield=Math.max(state.shield,1); const m=mostMissingGroup(); state.fill[m.id]=Math.min(CFG.target*.7,state.fill[m.id]+.9); addScore(30,'Rescue +30','power'); healPlate(18,'Rescue Skill'); feedback('✨ Rescue! Freeze + Shield + เติมหมู่ที่ขาด','perfect'); sfx('fever'); logLine('ใช้ Rescue Skill แล้ว'); updateAll();}

  function playerKey(){return String(qs('pid','anon')||'anon').replace(/[^\w.-]/g,'_');} function bestKey(){return`HHA_PLATE_SOLO_BEST_${playerKey()}_${DIFF}`;} function badgeKey(){return`HHA_PLATE_SOLO_BADGES_${playerKey()}`;} function dailyKey(){return`HHA_PLATE_SOLO_DAILY_${playerKey()}_${DAILY_YMD}_${DIFF}`;}
