...
// BathVR SAFE — PRODUCTION
// + HHA Standard-ish: end summary + back hub + cooldown btn + flush-hardened nav
// + Practice 15s + Calibration/Recenter helper (cVR/Cardboard-friendly)
// + 3-Stage + Boss + FX + AI Predict hooks (prediction-only)
// FULL v20260304-BATH-HHA-PRACTICE-CALIB-COOLDOWN
'use strict';
...
(function(){
  ...
  const practiceEnabled = (qs('practice','1')||'1').trim() !== '0';  // ✅ default ON
  const practiceSec = Math.max(10, parseInt(qs('practiceSec','15'),10) || 15);
  ...
  const btnStart=$('btnStart'), btnRetry=$('btnRetry'), btnPause=$('btnPause'), btnBack=$('btnBack'),
        ...
        btnCooldown=$('btnCooldown'); // ✅ Go Cooldown (daily-first)

  // Practice + Calibration overlays (optional elements in HTML)
  const ovPractice = $('ovPractice');
  const tPractice  = $('tPractice');
  const btnSkipPractice = $('btnSkipPractice');

  const ovCalib = $('ovCalib');
  const btnRecenter = $('btnRecenter');
  const btnCalibClose = $('btnCalibClose');
  ...
  // ---------------- ✅ HHA Standard: local day Bangkok ----------------
  ...
  const ZONE = 'hygiene';
  const COOLDOWN_GATE = '../warmup-gate.html';
  ...
  function buildCooldownUrl(){
    try{
      ...
      u.searchParams.set('kind', 'cooldown');
      u.searchParams.set('pid', pid);
  ...
  async function safeNavigate(url, reason){
    try{ logger.push('nav', { to:url, reason: reason||'' }); }catch(_){}
    try{ await logger.flush('nav'); }catch(_){}
    // tiny delay improves keepalive reliability on some Android WebView
    setTimeout(()=>{ location.href = url; }, 30);
  }

  async function safeBackToHub(){
    let to = hub || '../hub.html';
    try{
    ...
    await safeNavigate(to, 'back_hub');
  }

  async function safeGoCooldown(){
    const to = buildCooldownUrl();
    await safeNavigate(to, 'go_cooldown');
  }
  ...
  const st = {
    ...
    shield:0, timeBonusSec:0, slowUntilMs:0, feverUntilMs:0,
    spawnMs:720, ttlMs:1500,

    hazardRisk:0,
    coachLine:'',

    practice:false,         // ✅ practice mode flag
    practiceEndsAtSec: 0,
  ...
  function setHud(){
    ...
    const limit = st.practice ? practiceSec : (timeLimit + st.timeBonusSec);
    if(tTime) tTime.textContent = String(Math.max(0, Math.ceil(limit - st.elapsed)));
    ...
    if(tStage){
      const tag = st.practice ? 'PRACTICE' : `STAGE ${st.stage}: ${st.stageName}`;
      tStage.textContent = tag;
    }

    if(tBoss && bBoss){
      if(st.bossOn && !st.practice){
    ...
    if(tCoach){
      tCoach.textContent = st.practice ? '🎯 ซ้อม 15 วิ: เก็บคราบให้ไว + เลี่ยงเชื้อ' : (st.coachLine || '');
    }
  }
  ...
  function stageParams(){
    ...
    if(st.practice){
      ...
      ttl   *= 1.25;
      return {
        spawnMs: clamp(Math.round(spawn), 320, 1600),
        ttlMs:   clamp(Math.round(ttl),   900, 3000)
      };
    }

    if(st.stage===1){ spawn *= 1.08; ttl *= 1.15; }
    else if(st.stage===2){ spawn *= 0.95; ttl *= 0.98; }
    else { spawn *= 0.88; ttl *= 0.92; }

    if(pro){ spawn *= 0.90; ttl *= 0.93; }

    const mercy = clamp01((st.germRisk-72)/28);
    ttl *= (1.0 + mercy*0.10);

    if(nowMs() < st.slowUntilMs){ spawn *= 1.25; ttl *= 1.10; }
  ...
  function maybeAdvanceStage(){
    ...
    if(st.stage===1 && st.elapsed >= st.stageEndsAtSec) enterStage(2);
    if(st.stage===2 && st.elapsed >= st.stage2EndsAtSec) enterStage(3);
  }

  // ---------------- boss ----------------
  function startBoss(){
    if(st.bossOn || st.practice) return;
    st.bossOn = true;
    st.spawnedBoss++;
  ...
  function spawn(){
    ...
    if(st.practice){
      wGood = 0.82; wBad = 0.16; wBonus = 0.02;
    }else{
    ...
    if(!st.practice && st.stage===3 && st.bossOn && rng()<0.22){
  ...
  function hit(id, meta){
    ...
    if(it.kind === 'bonus'){
      ...
      if(it.bonusType==='time'){
        ...
        emitBath('power', { type:'soap' });
        logger.push('power', { type:'soap' });
      }
    ...
    if(it.kind === 'good'){
      ...
      const feverBonus = (!st.practice && (nowMs() < st.feverUntilMs)) ? 1 : 0;
      ...
      if(!st.practice && st.stage===1 && st.cleanMeter>=40 && st.elapsed < st.stageEndsAtSec){
        if(!st._s1Reward){
          st._s1Reward = true;
          st.score += 3;
      ...
      if(st.practice){
        // ✅ practice: ไม่ลงโทษแรง (เด็กไม่ท้อ)
        st.germRisk = clamp(st.germRisk + 3.0, 0, 100);
        st.cleanMeter = clamp(st.cleanMeter - 1.2, 0, 100);
        fxPulse('fxBad', 160);
        toast('🦠 ระวังเชื้อ!');
        logger.push('judge', { judge:'bad_hit_practice', kind:'bad', score: st.score, src });
        emitBath('judge', { judge:'bad_hit_practice', kind:'bad', score: st.score, src });
      }else{
        breakCombo('bad_hit');
    ...
    if(!st.practice && st.germRisk >= 100){
      endGame('risk');
    }
  }
  ...
  function timeoutTarget(id){
    ...
    if(it.kind === 'good'){
      ...
      if(!st.practice){
        ...
        st.germRisk = clamp(st.germRisk + 1.6, 0, 100);
        st.cleanMeter = clamp(st.cleanMeter - 0.8, 0, 100);
        logger.push('judge', { judge:'timeout_practice', kind:'good', score: st.score });
        emitBath('judge', { judge:'timeout_practice', kind:'good', score: st.score });
      }
    }else{
  ...
  // ---------------- AI prediction (prediction-only) ----------------
  function computeHazard(){
    if(st.practice){
      st.hazardRisk = Math.round(clamp(st.germRisk,0,100));
      st.coachLine = '🎯 ซ้อม: เก็บคราบก่อน + อย่าแตะเชื้อ';
      return;
    }
    ...
    const factors = [
      {w:0.36*riskNow,  t:'Risk สูง'},
      {w:0.22*meterLow, t:'Meter ต่ำ'},
      {w:0.20*badRate,  t:'โดนเชื้อบ่อย'},
      {w:0.16*missRate, t:'พลาดคราบบ่อย'},
      {w:0.10*clamp01(comboBreakRate), t:'คอมโบหลุดบ่อย'}
    ].sort((a,b)=>b.w-a.w);

    const top = factors.slice(0,2).filter(x=>x.w>0.04).map(x=>x.t);
    let coach = '';
    if(hazard>=75) coach = `⚠️ ระวัง! ${top.join(' + ') || 'โฟกัสคราบก่อน'}`;
    else if(hazard>=45) coach = `👀 ดีอยู่ แต่… ${top[0] || 'รักษาคอมโบ 6+'}`;
    else coach = `👍 เยี่ยม! ทำคอมโบ 6+ รับโบนัส`;

    st.hazardRisk = hazard;
    st.coachLine = coach;

    emitBath('ai', { hazardRisk: hazard, coach });
  }
  ...
  function tick(){
    ...
    if(!st.practice) maybeAdvanceStage();
    ...
    if(((t - st.t0) % 1000) < 90){
      logger.push('time', {
        ...
        mode: st.practice ? 'practice' : 'main',
        stage: st.practice ? 0 : st.stage,
        score: st.score,
        clean: st.clean,
        mistake: st.mistake,
        combo: st.combo,
...
log=${logEndpoint||'—'} | studyId=${studyId||'—'} | phase=${phase||'—'} | conditionGroup=${conditionGroup||'—'}`;
  ...
  // ---------------- ✅ Practice flow ----------------
  function showPracticeOverlay(on){
    if(!ovPractice) return;
    ovPractice.hidden = !on;
  }

  function updatePracticeText(secLeft){
    if(!tPractice) return;
    tPractice.textContent = `ซ้อม ${Math.max(0,Math.ceil(secLeft))} วิ: เก็บคราบ 🟤 ให้ไว อย่าแตะเชื้อ 🦠`;
  }
  ...
  function startPractice(){
    ...
    updatePracticeText(practiceSec);

    logger.push('practice_start', { sec: practiceSec });
    emitBath('practice_start', { sec: practiceSec });
    ...
    tickTimer = setInterval(()=>{
      if(!st.running || st.paused || st.over) return;
      const secLeft = practiceSec - st.elapsed;
      updatePracticeText(secLeft);
      tick();
    }, 80);
  ...
  function endPracticeToMain(){
    // stop practice, hard reset into main start
    logger.push('practice_end', { t: +st.elapsed.toFixed(2), clean: st.clean, mistake: st.mistake });
    emitBath('practice_end', { t: +st.elapsed.toFixed(2), clean: st.clean, mistake: st.mistake });

    showPracticeOverlay(false);
  ...
  function startMainGame(){
    ...
    setHud();
    toast('เริ่มจริง! ทำความสะอาดให้ทัน');
  ...
  function startFlow(){
    // open calibration hint once for cVR-like views
    if(looksLikeCVR(view)) openCalibOnce();
  ...
  // ---------------- ✅ Calibration / recenter helper ----------------
  function looksLikeCVR(v){
    const s = String(v||'').toLowerCase();
    return s.includes('cvr') || s.includes('card') || s.includes('vr');
  }

  function openCalib(on){
    if(!ovCalib) return;
    ovCalib.hidden = !on;
  }

  function openCalibOnce(){
    try{
      const key = `HHA_CALIB_SHOWN::bath::${pid}`;
  ...
  function doRecenter(){
    // try common APIs
    try{
      if(WIN.HHA_VRUI && typeof WIN.HHA_VRUI.recenter === 'function'){
        WIN.HHA_VRUI.recenter();
      }else{
        // fallback event
        emit('hha:recenter', { game:'bath' });
      }
      toast('🎯 Recenter แล้ว!');
      logger.push('recenter', { ok:true });
      emitBath('recenter', { ok:true });
    }catch(_){
      toast('⚠️ Recenter ไม่สำเร็จ');
      logger.push('recenter', { ok:false });
      emitBath('recenter', { ok:false });
    }
  }

  if(btnRecenter){
    btnRecenter.addEventListener('click', doRecenter, {passive:true});
  }
  if(btnCalibClose){
    btnCalibClose.addEventListener('click', ()=> openCalib(false), {passive:true});
  }

  // ---------------- buttons ----------------
  if(btnStart) btnStart.addEventListener('click', startFlow, {passive:true});
  if(btnRetry) btnRetry.addEventListener('click', startFlow, {passive:true});
  if(btnEndRetry) btnEndRetry.addEventListener('click', startFlow, {passive:true});
  ...
  if(btnEndBack){
    btnEndBack.addEventListener('click', (ev)=>{
      // prevent default to ensure flush
      ev.preventDefault();
      safeBackToHub();
    }, {passive:false});
  }
  ...
  if(btnCooldown){
    btnCooldown.addEventListener('click', (ev)=>{
      ev.preventDefault();
      safeGoCooldown();
    }, {passive:false});
  }
  ...
  updateCooldownButton();
  setHud();
  toast(practiceEnabled ? 'พร้อมแล้ว! กด “เริ่มเกม” (มีซ้อม 15 วิ)' : 'พร้อมแล้ว! กด “เริ่มเกม”');
})();