...
// ✅ submit/skip -> save + emit + (optional) log + route next
// FULL v20260305-QUIZC-CONTROLLER-NEXT
...
(function(){
  ...
  // ✅ default next: ไป bath.html เสมอ (แก้ “ตอบแล้วไม่ไปไหน”)
  const nextDefault = '../vr-bath/bath.html';
  const next = (qs('next', nextDefault) || nextDefault).trim();
  ...
  function toast(msg){
    ...
    if(!el){ console.log('[Quiz]', msg); return; }
    el.textContent = msg;
  ...
  function getSelected(){
    ...
    const label = lab ? (lab.textContent||'').trim() : value;
    return { value, label };
  }

  // ถูก = ตัวเลือกที่มีคำว่า น้ำตาล + แบคทีเรีย + กรด (robust)
  function isCorrect(sel){
    if(!sel) return false;
    const t = (sel.label||'') + ' ' + (sel.value||'');
    return (t.includes('น้ำตาล') && t.includes('แบคทีเรีย') && t.includes('กรด'));
  }
  ...
  async function submit(goAnyway){
    ...
    // local save
    try{
      localStorage.setItem(`HHA_QUIZ_C_LAST::${pid}`, JSON.stringify({ ts, pid, ok, sel }));
    }catch(_){}
