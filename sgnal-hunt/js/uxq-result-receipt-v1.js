...
(() => {
  ...
  function addStyle(){
    ...
    style.textContent = '.uxq-submission-receipt{width:min(700px,100%);text-align:left;border:1px solid rgba(110,231,255,.32);border-radius:16px;background:rgba(9,28,56,.64);padding:14px 16px;display:grid;gap:8px}.uxq-submission-receipt[data-tone="good"]{border-color:rgba(119,233,164,.54);background:rgba(39,112,77,.16)}.uxq-submission-receipt[data-tone="warn"]{border-color:rgba(255,209,102,.54);background:rgba(101,73,18,.20)}.uxq-submission-receipt[data-tone="bad"]{border-color:rgba(255,151,166,.46);background:rgba(105,35,53,.18)}.uxq-submission-receipt__kicker{font-size:.73rem;font-weight:900;letter-spacing:.1em;text-transform:uppercase;color:var(--uxq-accent,#6ee7ff)}.uxq-submission-receipt__title{font-weight:900;color:#fff;font-size:1rem}.uxq-submission-receipt__body{color:#d4e0fa;line-height:1.58;font-size:.9rem}.uxq-submission-receipt__codes{display:flex;flex-wrap:wrap;gap:8px;margin-top:2px}.uxq-submission-receipt__code{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:.72rem;color:#c8dcff;background:rgba(3,13,30,.48);border:1px solid rgba(181,205,255,.17);padding:6px 8px;border-radius:8px;word-break:break-all}';
  ...
  function makeCard(item, info, renderKey){
    ...
    addText(card, 'uxq-submission-receipt__kicker', info.tag);
    addText(card, 'uxq-submission-receipt__title', info.title);
    addText(card, 'uxq-submission-receipt__body', info.body);
    const codes = document.createElement('div');
    codes.className = 'uxq-submission-receipt__codes';
    if (item.attemptId) addText(codes, 'uxq-submission-receipt__code', `Attempt: ${item.attemptId}`);
    if (item.eventId) addText(codes, 'uxq-submission-receipt__code', `Event: ${item.eventId}`);
  ...
  function decorate(){
    ...
    if (!result) return;
    const item = getReceipt();
    if (!item) return;
    const info = getMeta(item);
    const renderKey = `${item.receiptId || ''}|${item.state || ''}|${info.tag}`;
    const existing = result.querySelector('.uxq-submission-receipt');
    if (existing?.dataset.renderKey === renderKey) return;
    const card = makeCard(item, info, renderKey);
