...
// V8 — One canonical unlock state + Tutorial once + Strict 60-core Replay Scheduler + 720 Scenario Variants
...
(function () {
  ...
  const ROUND_SIZE = 5;
  const SESSION_KEY = 'uxquest-w1-session-v8';
  const PROGRESS_KEY = 'uxquest-w1-progress-v8';
  const LEGACY_KEYS = [
  ...
  function resetAllW1() {
    ...
    clearLegacyData();
    if (window.UXQProgressV8 && typeof window.UXQProgressV8.reset === 'function') {
      window.UXQProgressV8.reset();
    }
    render();
    scrollToTop();
  }
  ...
  function updateProgressFromRound(starCount) {
    ...
    if (window.UXQProgressV8 && typeof window.UXQProgressV8.writeW1 === 'function') {
      window.UXQProgressV8.writeW1({
