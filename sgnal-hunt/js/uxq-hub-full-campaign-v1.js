/* UX Quest • Full CSAI2601 Campaign Hub v2
 * Extends Mission Control through W15 / B5. All campaign nodes now have
 * playable routes and retain prerequisite-based unlocks.
 */
(() => {
  'use strict';
  ...
  const NODES = [
    ...
    { id:'w7', label:'W7', act:'ACT III • WIREFRAME & HIERARCHY', title:'Wireframe Heist', desc:'จัด Grid, Layout และ Visual Hierarchy ให้ task สำคัญเด่นขึ้นก่อน', icon:'▦', prereq:'b2', href:'./w7-wireframe-heist.html?v=20260704-act3-act5-v1', built:true },
    { id:'b3', label:'B3', act:'MIDTERM BOSS • WEEK 8', title:'UX Blueprint Gauntlet', desc:'ด่านกลางภาค: วิเคราะห์และปกป้อง Blueprint ตั้งแต่ user insight จนถึง wireframe', icon:'◆', prereq:'w7', href:'./b3-ux-blueprint-gauntlet.html?v=20260704-act3-act5-v1', built:true, boss:true },
    { id:'w9', label:'W9', act:'ACT IV • DESIGN SYSTEM', title:'Design System Vault', desc:'สร้าง Pattern Library, UI Kit และกติกาความสม่ำเสมอของหน้าจอ', icon:'◫', prereq:'b3', href:'./w9-design-system-vault.html?v=20260704-act3-act5-v1', built:true },
    { id:'w10', label:'W10', act:'ACT IV • RESPONSIVE & ACCESSIBLE WEB', title:'Responsive Rescue', desc:'ออกแบบ Navigation และ Responsive Layout ที่เข้าถึงได้ในหลายหน้าจอ', icon:'⌁', prereq:'w9', href:'./w10-responsive-rescue.html?v=20260704-act3-act5-v1', built:true },
    { id:'w11', label:'W11', act:'ACT IV • VISUAL ACCESSIBILITY', title:'Contrast Cipher', desc:'ใช้ Color, Typography และ Contrast เพื่อให้ข้อมูลอ่านง่ายและตัดสินใจได้', icon:'◐', prereq:'w10', href:'./w11-contrast-cipher.html?v=20260704-act3-act5-v1', built:true },
    { id:'b4', label:'B4', act:'BOSS GATE • W9–W11', title:'Design System Siege', desc:'รวม UI system, responsive behavior และ visual accessibility ในคดีออกแบบจริง', icon:'✹', prereq:'w11', href:'./b4-design-system-siege.html?v=20260704-act3-act5-v1', built:true, boss:true },
    { id:'w12', label:'W12', act:'ACT V • COMPONENTS & STATES', title:'Component Command', desc:'ออกแบบ Forms, Cards, Icons และ Feedback States ที่ช่วยป้องกัน error', icon:'▣', prereq:'b4', href:'./w12-component-command.html?v=20260704-act3-act5-v1', built:true },
    { id:'w13', label:'W13', act:'ACT V • PROTOTYPE', title:'Prototype Pulse', desc:'สร้าง High-fidelity Prototype และ Microinteraction ที่สื่อ feedback ได้ชัด', icon:'◌', prereq:'w12', href:'./w13-prototype-pulse.html?v=20260704-act3-act5-v1', built:true },
    { id:'w14', label:'W14', act:'ACT V • EVALUATION & ITERATION', title:'Validation Lab', desc:'ใช้ Heuristic, Cognitive Walkthrough และ Usability Test เพื่อปรับรอบใหม่', icon:'✓', prereq:'w13', href:'./w14-validation-lab.html?v=20260704-act3-act5-v1', built:true },
    { id:'b5', label:'B5', act:'FINAL BOSS • WEEK 15', title:'UX Launch Defense', desc:'Final Design Studio: นำเสนอ Portfolio, อธิบายเหตุผล และปกป้องการตัดสินใจออกแบบ', icon:'★', prereq:'w14', href:'./b5-ux-launch-defense.html?v=20260704-act3-act5-v1', built:true, boss:true }
  ];
  ...
  function addStyle(){
    ...
    style.textContent = `
      .campaign-preview{position:relative}.campaign-preview.is-ready{border-color:rgba(117,232,255,.72);box-shadow:0 13px 34px rgba(66,158,255,.16)}.campaign-preview.is-cleared{border-color:rgba(92,229,182,.7)}.campaign-preview--boss{background:linear-gradient(145deg,rgba(41,29,80,.82),rgba(13,31,62,.92))}.campaign-preview .campaign-launch{display:inline-flex;align-items:center;justify-content:center;min-height:38px;padding:9px 12px;border-radius:10px;background:rgba(117,232,255,.15);border:1px solid rgba(117,232,255,.55);color:#e6fbff;text-decoration:none;font-weight:850;font-size:.82rem}.campaign-preview .campaign-launch.is-disabled{opacity:.5;pointer-events:none}.campaign-separator{grid-column:1/-1;margin:8px 0 0;padding:11px 13px;border-left:3px solid rgba(155,140,255,.8);background:rgba(155,140,255,.08);color:#d9d1ff;font-size:.86rem;font-weight:800;border-radius:0 12px 12px 0}
  ...
  function renderNode(node, progress){
    ...
    if (card) {
      card.classList.toggle('is-ready', ready && !completed);
      card.classList.toggle('is-cleared', completed);
    }
    if (state) state.textContent = completed ? 'ผ่านแล้ว' : (ready ? 'พร้อมเริ่ม' : 'ล็อกอยู่');
    if (note) note.textContent = completed ? `ผ่านแล้ว ${stars(item.bestStars)}` : (ready ? `${labelFor(node)} พร้อมเริ่มแล้ว` : `ต้องผ่าน ${node.prereq.toUpperCase()} ที่ 2★ ระดับความพร้อม`);
    if (launch) {
      launch.href = ready ? node.href : '#';
      launch.textContent = completed ? 'เล่นซ้ำ' : (ready ? 'เริ่มภารกิจ' : 'ล็อกอยู่');
      launch.classList.toggle('is-disabled', !ready);
      launch.setAttribute('aria-disabled', String(!ready));
      launch.onclick = ready ? null : (event) => event.preventDefault();
    }
  }

  function setCurrent(progress){
    const next = NODES.find((node) => passed(progress, node.prereq) && !passed(progress, node.id));
    if (!next) return;
