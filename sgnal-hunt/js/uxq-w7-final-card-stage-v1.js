/* CSAI2601 UX Quest • W7 Final Card Stage v1.3
 ...
 * Important: it hides old spans but does NOT blank their text, so Student Ready has nothing to rewrite repeatedly.
...
(() => {
  ...
  function pick(arr,seed,i,used){for(let k=0;k<arr.length;k++){const p=arr[(h(seed)+i+k)%arr.length]; const key=(p[0]+'|'+p[1]).toLowerCase(); if(!used.has(key)){used.add(key);return p;}} return arr[i%arr.length];}
  ...
  function ensureSmallSubtitle(root){
    ...
    $$('span',root).forEach(sp=>{
      ...
      if(!text(sp)) sp.textContent='อ่านสถานการณ์ แล้วเลือกคำตอบที่มีเหตุผลจากหลักฐานมากที่สุด';
    });
    return small;
  }
  function writeOption(btn,i,used){
    ...
    const b=$('b',btn); const small=ensureSmallSubtitle(btn); if(b)b.textContent=p[0]; if(small.textContent!==p[1])small.textContent=p[1];
  }
  function writeCard(card,i,source,used){
    const p=pairFor(source||card,i,used); card.dataset.w7FinalCardStage='1';
    const lane=$('.uxqMiniLane,.uxqDragLane',card); const head=$('strong,b',card); const small=ensureSmallSubtitle(card); if(lane)lane.textContent=letters[i]||String(i+1); if(head)head.textContent=p[0]; if(small.textContent!==p[1])small.textContent=p[1];
  }
