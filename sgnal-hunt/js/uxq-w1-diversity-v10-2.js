...
(function () {
  ...
  console.info('[UXQ W1 V10.2] Schema-compatible engine loaded');
  ...
  function saveProgress() {
    try { localStorage.setItem(CONTENT_KEY, JSON.stringify(progress)); }
    catch (error) { console.warn('[W1 V10] unable to save progress', error); }
  }

  function saveSession() {
    try { localStorage.setItem(SESSION_KEY, JSON.stringify(state)); }
    catch (error) { console.warn('[W1 V10] unable to save session', error); }
  }
  ...
  function updateProgress(starCount) {
    ...
    progress.roundHistory = [...progress.roundHistory, { mode: state.mode, caseIds: [...state.caseIds], stars: starCount, score: state.score, completedAt: new Date().toISOString() }].slice(-24);
    saveProgress();
