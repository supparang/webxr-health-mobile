/* CSAI2601 UX Quest • Production Final Polish v1
 * Final visual cleanup for W1-W15 + B1-B4.
 * Presentation only. Does not change scoring, answers, progress, or Sheet sync.
 */
(function(){
  'use strict';
  var VERSION='uxq-production-final-polish-v1-20260715';
  ...
  style.textContent=`
    ...
    .panel{border-radius:26px!important;box-shadow:0 26px 70px rgba(0,0,0,.30)!important}
    .game{padding:0!important}
    .hud{grid-template-columns:minmax(0,1.8fr) repeat(3,minmax(118px,.42fr))!important;gap:10px!important;padding:12px 14px!important}
    .hud .meter{min-height:64px!important;padding:10px 12px!important}
    .case{padding:34px 32px 18px!important;margin:0!important}
    .case h1{font-size:clamp(2rem,3.3vw,3.25rem)!important;line-height:1.08!important;margin:0 0 10px!important}
    .case p{font-size:clamp(1rem,1.25vw,1.18rem)!important;line-height:1.55!important}
    .question{margin:0 32px 32px!important;padding:24px!important;border-radius:22px!important}
    .question .prompt{font-size:clamp(1.45rem,2vw,2rem)!important;line-height:1.28!important}
    .instruction{font-size:clamp(.98rem,1.15vw,1.08rem)!important;line-height:1.52!important;margin-bottom:16px!important}
    ...
    .option b,button.option b{font-size:clamp(1rem,1.15vw,1.12rem)!important;line-height:1.42!important}
    .option span,button.option span{font-size:.86rem!important;line-height:1.42!important;margin-top:8px!important;color:#9fb1d2!important}
    .verify{margin:16px 0 0!important;padding:18px!important;border-radius:18px!important}
    ...
    .utility .hint{display:flex!important;align-items:center!important;min-height:52px!important;padding:12px 16px!important;border-radius:14px!important}
    .utility button{min-width:132px!important;min-height:52px!important;border-radius:14px!important;font-weight:900!important}
    .feedback{border-radius:16px!important;padding:16px!important}
    .artifact{margin-top:18px!important;border-radius:20px!important;padding:20px!important}
    .briefs{gap:14px!important}
    .brief{min-height:112px!important;padding:18px!important;border-radius:18px!important}
    .actions{margin-top:4px!important}
    .actions .btn{min-height:52px!important;padding:14px 18px!important;border-radius:15px!important}
    ...
    @media(max-width:900px){
      ...
      .panel{border-radius:0!important;border-left:0!important;border-right:0!important}
      .hud{grid-template-columns:1fr 1fr!important}
      ...
      .options{grid-template-columns:1fr!important;grid-auto-rows:auto!important}
      .option,button.option{min-height:0!important;height:auto!important;padding:16px!important}
      .utility{grid-template-columns:1fr!important}
      .utility button{width:100%!important}
      .briefs{grid-template-columns:1fr!important}
      .actions{display:grid!important;grid-template-columns:1fr!important}
      .actions .btn{width:100%!important;text-align:center!important}
    }
    @media(max-width:480px){
      .hud{grid-template-columns:1fr 1fr!important;padding:10px!important}
      .hud .meter{min-height:58px!important;padding:9px!important}
      .case h1{font-size:2rem!important}
      .case p{font-size:1rem!important}
      .question{margin:0 8px 14px!important;padding:15px!important;border-radius:18px!important}
      .question .prompt{font-size:1.35rem!important}
  ...
  function hideByText(){
    document.querySelectorAll('div,span,small,p').forEach(function(el){
      var t=String(el.textContent||'').replace(/\s+/g,' ').trim();
      if(!t)return;
      if(/W1 hard compact|4-card override|Anti-Guess v3 Stable|W7 Final v4|Sheet Only|Mission identity loading|Mission Logo/i.test(t) && el.children.length<3){
