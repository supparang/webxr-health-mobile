// === /sgnal-hunt/js/uxq-hub-v7.js ===
// UX Quest Mission Control • V7
// Reads one canonical W1 state only. W1 never loops Tutorial after a pass.

(function () {
  'use strict';

  const W1_DETAIL_KEY = 'uxquest-w1-progress-v7';
  const W1_SESSION_KEY = 'uxquest-w1-session-v7';
  ...
  function setW2Action(enabled, label) {
    ...
    if (enabled) {
      launch.href = './w2-design-thinking-sprint.html?v=progress-v7';
  ...
  function updateHub() {
    ...
    if (w2.active) {
      setCurrent({
        ...
        href: './w2-design-thinking-sprint.html?v=progress-v7&resume=1',
        cta: 'ทำ W2 ต่อ',
        state: 'resume'
      });
    } else if (w2.cleared) {
      setCurrent({
        ...
        href: './w2-design-thinking-sprint.html?v=progress-v7',
        cta: 'เล่น W2 ซ้ำ',
        state: 'complete'
      });
    } else if (w1.cleared) {
  ...
  function resetAct() {
    ...
    try {
      ...
      console.warn('[UXQ V7] Could not reset local progress.', error);
    }
