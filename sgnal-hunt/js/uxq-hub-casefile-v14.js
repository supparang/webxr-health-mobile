/* UX Quest • Mission Control Act I v14
   Reads local-first progress created by uxq-progress-v1.js.
*/
(() => {
  'use strict';
  ...
  const ROUTES = {
    w1: './w1-ux-crisis-casefile.html?v=act1-v1',
    w2: './w2-design-thinking-sprint.html?v=act1-v1',
    w3: './w3-cognitive-load-escape.html?v=act1-v1',
    b1: './b1-cognitive-storm.html?v=act1-v1'
  };
  const META = {
  ...
  function setClass(el, classes){ if (!el) return; el.classList.remove('path-step--available','path-step--complete','path-step--locked','compact-stage--locked','compact-stage--available','compact-stage--complete','boss-preview--available','boss-preview--complete'); classes.filter(Boolean).forEach(c => el.classList.add(c)); }
  ...
  function render(){
    ...
    setClass($('pathW1'), ['path-step--complete'].filter(() => p.w1).concat(!p.w1 ? ['path-step--available'] : []));
    setClass($('pathW2'), p.w2 ? ['path-step--complete'] : (p.w1 ? ['path-step--available'] : ['path-step--locked']));
    setClass($('pathW3'), p.w3 ? ['path-step--complete'] : (p.w2 ? ['path-step--available'] : ['path-step--locked']));
    setClass($('pathB1'), p.b1 ? ['path-step--complete'] : (p.w3 ? ['path-step--available'] : ['path-step--locked']));
    ...
    setText('nowIcon', next.icon); setText('w1Title', next.title); setText('nowSummary', next.summary); setText('nowReward', next.reward);
    ...
    if (reset) reset.onclick = () => {
      ...
      Object.keys(localStorage).filter(k => k.startsWith('uxq.recent.') || k.startsWith('uxq.run.')).forEach(k => localStorage.removeItem(k));
      render();
    };
  }
