...
(function(){
  ...
  function queue(x){var a=[];try{a=JSON.parse(localStorage.getItem(QUEUE_KEY)||'[]');}catch(e){}a=(Array.isArray(a)?a:[]).concat([x]).slice(-60);try{localStorage.setItem(QUEUE_KEY,JSON.stringify(a));}catch(e){}return a.length;}
  ...
  function patchArtifact(){var art=document.querySelector('.artifact');if(!art)return;var btn=art.querySelector('[data-save-artifact]')||Array.prototype.slice.call(art.querySelectorAll('button')).find(function(b){return /บันทึก note|ส่ง.*Sheet|artifact/i.test(b.textContent);});if(btn){btn.textContent='ส่ง Studio Artifact เข้า Google Sheet';btn.setAttribute('data-production-artifact-submit-v2','1');btn.classList.remove('secondary');}}
  ...
  window.UXQProductionSupportV2=Object.freeze({version:VERSION,run:run});
})();
