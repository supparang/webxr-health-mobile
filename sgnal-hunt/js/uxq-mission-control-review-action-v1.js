/* CSAI2601 UX Quest • Mission Control Review Action v2
 * Completed 3/3 nodes always open a read-only Studio & Reflection review.
 * Runs after the official course-progress render event, without a DOM observer loop.
 */
(() => {
  'use strict';

  const VERSION = '20260726-MISSION-CONTROL-REVIEW-ACTION-V2-EVENT-AUTHORITY';
  ...
  function nodeIdFromCard(card) {
    return clean(
      card?.dataset?.nodeId ||
      card?.dataset?.node ||
      card?.querySelector?.('[data-node-id]')?.dataset?.nodeId ||
      ''
    ).toLowerCase();
  }
  ...
  function reviewUrl(id) {
    ...
    [
      'studentId','studentName','section','classroom','courseId',
      'sheet','api','sid','name','device'
    ].forEach(key => {
  ...
  function isComplete(card) {
    ...
    return /complete\s*3\s*\/\s*3/i.test(badge) ||
      (/mission\s*✓/i.test(status) &&
       /studio\s*✓/i.test(status) &&
       /reflection\s*✓/i.test(status));
  }
  ...
  function apply() {
    document.querySelectorAll('.campaign-preview,[data-node-id],[data-node]').forEach(card => {
      ...
      launch.textContent = 'ดู Studio & Reflection';
      launch.href = reviewUrl(id);
      launch.setAttribute(
        'aria-label',
        `ดู Studio Practice และ Weekly Reflection ของ ${id.toUpperCase()}`
      );
  ...
  /* Official event emitted after uxq-studio-status-v1.js decorates every card. */
  window.addEventListener('uxq-three-part-course-progress', schedule);

  /* Compatibility with earlier status components. */
  ...
  window.UXQMissionControlReviewActionV2 = Object.freeze({
    version: VERSION,
    apply,
    schedule
  });
