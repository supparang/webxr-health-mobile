/* CSAI2601 UX Quest • Debrief Quality Gate v3.1
...
(() => {
  ...
  function hintNode(ta){let h=ta.parentElement.querySelector('.uxqDebriefQualityHint');if(!h){h=document.createElement('div');h.className='uxqDebriefQualityHint';ta.insertAdjacentElement('afterend',h);}return h;}
  function validate(r,focusFirst){let okAll=true,first=null;fields(r).forEach((ta,i)=>{const res=score(value(ta),i);ta.dataset.qualityOk=res.ok?'1':'0';const h=hintNode(ta);const wanted=res.ok?'✅ หลักฐานครบแล้ว พร้อมส่งผลงาน':`⚠ ${res.msg} — ${hint(i)}`;if(h.textContent!==wanted)h.textContent=wanted;h.dataset.ok=res.ok?'1':'0';if(!res.ok){okAll=false;if(!first)first=ta;}});$$('button',r).filter(b=>/บันทึก|save|ส่ง|submit/i.test(text(b))).forEach(b=>{b.disabled=!okAll;b.dataset.qualityBlocked=okAll?'0':'1';});if(!okAll&&focusFirst){first?.focus();toast('ต้องเขียนสิ่งที่เลือก + เหตุผล + หลักฐานจาก case ก่อนบันทึก');}return okAll;}
  function apply(){if(!SCOPE.test(node()))return;const r=root();if(!r||r.dataset.qualityGate==='v3.1')return;r.dataset.qualityGate='v3.1';r.classList.add('uxqDebriefQualityRoot');fields(r).forEach((ta,i)=>{ta.placeholder=hint(i);hintNode(ta).textContent=hint(i);ta.addEventListener('input',()=>validate(r,false));ta.addEventListener('blur',()=>validate(r,false));});r.addEventListener('click',e=>{const btn=e.target.closest('button,a');if(!btn||!/บันทึก|save|ส่ง|submit/i.test(text(btn)))return;if(!validate(r,true)){e.preventDefault();e.stopImmediatePropagation();}},true);validate(r,false);}
