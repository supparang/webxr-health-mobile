/* UX Quest • Mission Control v18
   W1 → W2 → W3 → B1 → W4 User Insight Lab.
*/
(() => {
  'use strict';
  ...
  const ROUTES = {
    ...
    w4: './w4-user-insight-lab.html?v=20260702-w4-lab-v1'
  };

  const META = {
    w1: {
      id:'w1', label:'S1 / W1 • FOUNDATION', title:'UX Detective: Casefile Hunt', icon:'⌕',
      summary:'ปิดคดี UX จาก Evidence → Hypothesis → Fix → User Test และ Final Boss',
      reward:'★ 2 Readiness เพื่อปลดล็อก W2', locked:'Available',
      next:'ผ่าน S1 / W1 Casefile ที่ 2★ Readiness เพื่อปลดล็อก W2'
    },
    w2: {
      id:'w2', label:'W2 • PROCESS', title:'Design Thinking Sprint', icon:'↺',
      summary:'เปลี่ยน pain point เป็น Empathize → Define → Ideate → Prototype → Test',
      reward:'★ 2 Readiness เพื่อปลดล็อก W3', locked:'Pass W1 2★ Readiness',
      next:'ผ่าน W2 ที่ 2★ Readiness เพื่อปลดล็อก W3'
    },
    w3: {
      id:'w3', label:'W3 • PSYCHOLOGY', title:'Cognitive Load Escape', icon:'◌',
      summary:'ลดความรก จัดความสนใจ และทำให้ผู้ใช้คิดน้อยลงโดยไม่ทิ้งข้อมูลสำคัญ',
      reward:'★ 2 Readiness เพื่อปลดล็อก B1', locked:'Pass W2 2★ Readiness',
      next:'ผ่าน W3 ที่ 2★ Readiness เพื่อเข้าสู่ Boss Gate'
    },
    b1: {
      id:'b1', label:'B1 • BOSS GATE', title:'Cognitive Storm', icon:'⚠',
      summary:'กู้ระบบที่ผู้ใช้หาเมนูไม่เจอ เข้าใจสถานะผิด และเลิกใช้งานกลางทาง',
      reward:'เคลียร์ Act I • Understand Humans', locked:'Pass W3 2★ Readiness',
      next:'เคลียร์ B1 เพื่อเปิด Act II • User Insight Lab'
    },
    w4: {
      id:'w4', label:'W4 • ACT II • INSIGHT', title:'User Insight Lab', icon:'◉',
      summary:'สแกนเสียงผู้ใช้ แยก Observation → Insight → HMW และฝ่า Boss Signal ที่ข้อมูลชนกัน',
      reward:'★ 2 Readiness เพื่อยืนยัน User Insight', locked:'Pass B1 2★ Readiness',
      next:'ผ่าน W4 ที่ 2★ Readiness เพื่อยืนยันฐาน Insight ของ Act II'
    }
  };

  const ORDER = ['w1','w2','w3','b1','w4'];
  ...
  function setClass(el, classes){
    if (!el) return;
    el.classList.remove(
      'path-step--available','path-step--cleared','path-step--complete','path-step--locked',
      'compact-stage--locked','compact-stage--available','compact-stage--complete',
      'boss-preview--available','boss-preview--complete'
    );
    classes.filter(Boolean).forEach((item) => el.classList.add(item));
  }
  ...
  function replayPathState(progress, id, available){
    ...
    const prefix = ({ w1:'S1 / W1', w2:'W2', w3:'W3', b1:'B1', w4:'W4' })[id] || id.toUpperCase();
  ...
  function render(){
    ...
    const mastery = Math.max(0, Math.min(3, Math.round((totalStars / 15) * 3)));
    const bestScore = Math.max(0, ...ORDER.map((id) => n(items[id].bestScore)));

    setText('menuProgressText', `${completed}/5 nodes • ${starsText(mastery)}`);
    setText('menuScoreText', `Best score ${bestScore}`);
    setText('actProgressText', `${completed}/5`);
    setText('heroStars', starsText(mastery));
    setText('heroScore', String(bestScore));

    const pathAvailability = { w1:true, w2:p.w1, w3:p.w2, b1:p.w3, w4:p.b1 };
    ...
    setPathTitle('pathW4', 'W4 • User Insight Lab');
    ...
    setText('pathW4State', replayPathState(progress, 'w4', pathAvailability.w4));
    ...
    setClass($('pathW4'), p.w4 ? ['path-step--cleared'] : (p.b1 ? ['path-step--available'] : ['path-step--locked']));
    ...
    setPathLaunch($('pathW4'), 'w4', pathAvailability.w4, 'W4 • User Insight Lab');
    ...
    const nodeW4 = $('nodeW4');
    const w4StateEl = nodeW4?.querySelector('.stage-state');
    const w4NoteEl = nodeW4?.querySelector('.boss-preview__footer span');
    const w4Button = nodeW4?.querySelector('.boss-preview__footer button');
    if (w4StateEl) w4StateEl.textContent = p.w4 ? 'INSIGHT SECURED' : (p.b1 ? 'ACT II READY' : 'ACT II LOCKED');
    if (w4NoteEl) w4NoteEl.textContent = p.w4 ? `Insight secured • ${starsText(n(items.w4.bestStars))}` : (p.b1 ? 'B1 cleared • พร้อมเข้าสู่ User Insight Lab' : 'ต้องเคลียร์ B1 • Cognitive Storm ก่อน');
    setButton(w4Button, p.b1, ROUTES.w4, p.w4 ? 'เล่น Insight Lab อีกครั้ง' : (p.b1 ? 'เข้าสู่ User Insight Lab' : 'Insight Lab Locked'));
    setClass(nodeW4, p.w4 ? ['boss-preview--complete'] : (p.b1 ? ['boss-preview--available'] : []));

    let nextId = 'w1';
    if (p.w1 && !p.w2) nextId = 'w2';
    else if (p.w2 && !p.w3) nextId = 'w3';
    else if (p.w3 && !p.b1) nextId = 'b1';
    else if (p.b1 && !p.w4) nextId = 'w4';
    else if (p.w4) nextId = 'w4';
    const next = META[nextId];
    const status = p.w4
      ? 'ACT II SECURED'
      : (p.b1 ? 'ACT II READY' : (nextId === 'b1' ? 'BOSS READY' : (items[nextId].bestStars ? 'REPLAY OR ADVANCE' : 'MISSION READY')));
    setText('nowStatus', status);
    const dot = $('nowStatusDot');
    if (dot) {
      dot.classList.toggle('is-complete', p.w4);
      dot.classList.toggle('is-resume', p.b1 && !p.w4);
    }
    ...
    setText('nowLaunchText', p.w4 ? 'เล่น Insight Lab ทบทวน' : (items[nextId].bestStars ? 'เล่นอีกครั้ง' : 'เริ่มภารกิจ'));

    const hint = p.w4
      ? 'Act I และ W4 เคลียร์แล้ว — คลิกแต่ละ node เพื่อเล่นซ้ำด้วย Casefile ที่สุ่มใหม่'
      : (p.b1
        ? 'Act I เคลียร์แล้ว — W4 User Insight Lab เปิดให้เล่นแล้ว'
        : next.next);
    setText('pathHint', hint);

    const reset = $('resetProgressBtn');
    if (reset) reset.onclick = () => {
      const ok = window.confirm('รีเซ็ตความก้าวหน้า UX Quest และ Casefile ล่าสุดทั้งหมดใช่หรือไม่? ผลในเบราว์เซอร์นี้จะถูกลบ');
      if (!ok) return;
      if (window.UXQProgress?.resetQuest) window.UXQProgress.resetQuest();
      else if (window.UXQProgress?.resetAct1) window.UXQProgress.resetAct1();
      else render();
    };
  }
