/* UX Quest • Mission Engine v3 • Classroom Data Layer
...
(() => {
  ...
  function init(config){
    ...
    function getRecent(){
      ...
      try {
        const ids = normalizeRecent(JSON.parse(raw));
        recentMemory = ids;
        return ids;
      } catch (error) {
    ...
    function setRecent(ids){
      const next = normalizeRecent(ids);
      recentMemory = next;
      // Remove the legacy value first: it may be a stale/oversized value from an older build.
      storage.removeItem(legacyRecentKey);
      // The shared storage adapter silently falls back to sessionStorage, then memory.
      // Case selection must never prevent the mission from starting.
    ...
    function choose(id){
      ...
      run.answers.push({ questionId: question.id, caseId: question.caseId, caseName: question.caseName, stageKey: question.stageKey, correct: Boolean(option.correct), selected: option.label, earned });
    ...
    function finishRun(){
      ...
      const sync = analyticsApi?.recordMissionComplete?.({
    ...
    function renderResults(result){
      ...
      root.innerHTML = `
        ...
        <p class="uxq-footer-note">${esc(result.syncMessage || '')}</p>
