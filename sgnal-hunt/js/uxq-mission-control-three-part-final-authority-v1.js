/* CSAI2601 UX Quest • Mission Control Three-Part Final Authority v1.1
 * Runs after Mission Control decorators and gives every Studio entry point the
 * same canonical URL. Only contiguous Mission + Studio + Reflection unlocks
 * the next node.
 */
(() => {
  'use strict';

  const VERSION = '20260731-MISSION-CONTROL-ROUTE-UNIFIED-V1.1';
  ...
  function nodeUrl(state, mode) {
    ...
    /* view=studio is the canonical contextual Studio route. It preserves the
       Node header, learner context and 3-part tracker. phase=studio was a
       separate stripped route and must not be generated by Mission Control. */
    if (mode === 'studio') url.searchParams.set('view', 'studio');
    if (mode === 'review') {
      url.searchParams.set('review', '1');
      url.searchParams.set('complete', '1');
    }
    url.searchParams.set('v', 'mission-control-route-unified-v1-1-20260731');
    return url.href;
  }
  ...
  function apply(detail) {
    ...
    /* The large current-action card and the W/B grid card now receive the
       exact same destination from actionFor(). */
