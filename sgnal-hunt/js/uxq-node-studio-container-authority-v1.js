/* CSAI2601 UX Quest • Node Studio Container Authority v2
 * Ensures Sheet-authoritative display on every node and repairs phase=studio.
 */
(() => {
  'use strict';
  ...
  function build() {
    ...
    try {
      ...
      if (wizard) {
        artifact.dataset.uxqBuilding = '0'; artifact.removeAttribute('hidden'); artifact.style.setProperty('display','grid','important');
        wizard.removeAttribute('hidden'); wizard.style.setProperty('display','grid','important');
  ...
  function boot() {
    ...
    new MutationObserver(() => { if (!document.getElementById('uxqStudentStudioFinalV2')) build(); }).observe(ROOT,{childList:true,subtree:true});
  }
