...
/* Fitness Duo - Desktop Stable (with mouse cursor) */
window.APP = (function(){
...
lockSceneClicks(false)  # also fix this
...
// ---- Mouse Raycast Fallback (Desktop) ----
function manualRayHit(evt){
  if (!st.playing) return;
  try{
    ...
    if (intersects.length){
      // Find owning A-Frame element
      let obj = intersects[0].object;
      while (obj && !obj.el) obj = obj.parent;
      if (obj && obj.el){
        const el = obj.el;
        if (!el.__handled){ // guard double-fire
          el.__handled = true;
          // Simulate a 'click'
          el.emit('click');
          // If entity still exists after a tick, remove and score to be safe
          setTimeout(()=>{
            if (el && el.parentNode){
...
function bindMouseFallback(){
  ['click','mousedown','mouseup'].forEach(t=>{
    window.addEventListener(t, manualRayHit, {passive:false});
  });
}
