// Rhythm — Weekly + Chain-3 + Auto-Tune + Fever
...
function init(){
  ...
  // Weekly override (ถ้าส่งจากกระดาน)
  const wtype = q.get("wtype"); // score|combo|accuracy|fever
  const wtarget = q.get("wtarget");
  ...
  function autoTune(base){
    const s=loadSave();
    const acc = Math.min(1, Math.max(0, s.lastAcc||0.75));
    const combo = Math.min(1, (s.lastCombo||8)/32);
    const power = (acc*0.6 + combo*0.4) - 0.5;
    return { hit: Math.max(0.08, base.hit - power*0.05), secs: base.secs + Math.round(power*10) };
  }
  ...
  // เลือก Challenge (Weekly > Auto > ธรรมดา)
  let CH = autoChallenge ? genDailyChallenge(`${daySeed()}|${diff}|${bpm}`) : {type:"none", title:"โหมดธรรมดา", hint:"", bonus:0, cfg:base};
  if(wtype){ CH.type=wtype; CH.title=`Weekly: ${wtype}`; CH.cfg=autoTune(base); CH.bonus=600;
    if(wtype==="accuracy") CH.target=parseFloat(wtarget||"0.85");
    else CH.target=parseInt(wtarget||"2500",10);
  ...
  const chainCfg = [
    {hit:-0.00, secs:-5,  beat:+0},   // stage1
    {hit:-0.02, secs:+0,  beat:+0},   // stage2 เข้มขึ้น
    {hit:-0.04, secs:+5,  beat:+0}    // stage3 เข้มสุด
  ];
  ...
  function applyChainTuning(){
    ...
    state.hitWindow = Math.max(0.06, state.hitWindow + t.hit);
    state.duration += t.secs;
    // (beat ไม่เปลี่ยนในเดโม่นี้เพื่อคงจังหวะที่เลือก)
  }
  ...
  function resetAll(){
    ...
    state.hitWindow=CH.cfg.hit; state.duration=CH.cfg.secs;
  }
  ...
  function endStage(){
    ...
    const ok=cleared(); const bonus= ok?CH.bonus:0;
    totalScore += state.score; totalBonus += bonus;
    const doneText = chain ? `ด่าน ${stageIndex+1}/3 จบ • ได้ ${state.score}+${bonus}` : `จบเกม • score=${state.score+bonus}`;
    setHUD(`${doneText} (${ok?"✅ ผ่าน":"❌ ไม่ผ่าน"})`);
    ...
    if(chain && stageIndex<2){
      ...
      const s=loadSave();
      s.lastScore=totalScore+totalBonus; s.lastCombo=state.best; s.lastAcc= state.accTotal? (state.accHit/state.accTotal):0.0; s.lastChallenge=CH; s.ts=Date.now();
      saveSave(s);
  ...
  function loop(){
    ...
    setHUD();
    if(state.elapsed>=state.duration) return endStage();
  ...
  btnReset.addEventListener('click', ()=>{ state.running=false; cancelAnimationFrame(state.raf); notes.forEach(n=>n.el?.parentNode?.removeChild(n.el)); notes=[]; resetAll(); setHUD("รีเซ็ตแล้ว"); });
