(function () {
  "use strict";
  const GAME_URL = "./word-match-memory.html?v=20260804-lexiconx4";
  ...
  function decorateCard(){
    ...
    if(detail)detail.textContent="Dual Motion Tilt • Academic Vocabulary • CEFR A2–B1+";
  ...
  style.textContent=`
    ...
    .stage-card.memory-stage-card.ready>div:nth-child(2)>strong{letter-spacing:.01em;color:#0f2742}
    .stage-card.passed.memory-stage-card .stage-icon::after{content:" LX";font-size:.42rem;font-weight:900;color:#184d73;position:absolute;margin-top:48px}
  `;
  document.head.appendChild(style);

  document.addEventListener("click",openGame,true);
  document.addEventListener("keydown",openByKeyboard,true);
  new MutationObserver(decorateCard).observe(document.getElementById("screen")||document.body,{childList:true,subtree:true});
  decorateCard();
  setTimeout(autoResume,70);
}());
