(function(){
  'use strict';
  const VERSION='2026-08-07-JOURNEY-SUMMARY-V1';
  ...
  function render(data){
    ...
    content.innerHTML=`
      ...
      <section class="card">
        ...
        <div class="gain"><div class="scorebox"><small>Pre-Challenge</small><strong>${Number(s.pre?.accuracy||0)}%</strong></div><div class="arrow">→</div><div class="scorebox"><small>Post-Challenge</small><strong>${Number(s.post?.accuracy||0)}%</strong></div><div class="gain-badge">${signed(gain)}</div></div>
      </section>
      ...
      <section class="achievement">
        <div class="award"><small>🏅 Achievement</small><strong>${h(s.badge||'LEXICON X Explorer')}</strong><small>จาก Learning Gain และผลเกมที่ Firebase รับรอง</small></div>
        <div class="award"><small>⭐ Strongest Skill</small><strong>${h(s.strongestSkill?.skill||'—')}</strong><small>${s.strongestSkill?`${Number(s.strongestSkill.accuracy||0)}% • ${h(missionNames[s.strongestSkill.stageId]||s.strongestSkill.stageId)}`:'ยังไม่มีข้อมูล'}</small></div>
      </section>
  ...
  async function load(){
    ...
    if(!journey?.endpointReady?.()){content.innerHTML='<section class="card">Firebase Journey Authority ยังไม่ได้ Deploy จึงยังโหลด Journey Summary ไม่ได้</section>';certificateBtn.textContent='กลับ Passport';certificateBtn.disabled=false;certificateBtn.onclick=goPassport;return}
    try{
