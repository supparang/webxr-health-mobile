(function(){
  "use strict";
  const URL="./sentence-builder.html?v=20260803-sentence1";
  const selector='.stage-card[data-stage="sentence_city"].clickable';
  function decorate(){const card=document.querySelector('.stage-card[data-stage="sentence_city"]');if(!card||card.dataset.sentenceDecorated==="1")return;card.dataset.sentenceDecorated="1";card.classList.add("sentence-stage-card");const detail=card.querySelector("small");if(detail&&!detail.textContent.includes("Build"))detail.textContent+=" • Build & Repair";const state=card.querySelector(".stage-state");if(state&&card.classList.contains("ready"))state.innerHTML="สร้างประโยคได้ 🧱";}
