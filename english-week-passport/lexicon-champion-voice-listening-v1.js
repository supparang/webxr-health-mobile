(()=>{
'use strict';
const RELEASE='20260808-LCA47-VOICE-LISTENING-V1';
const $=id=>document.getElementById(id);
let lastMic=null,lastBusy=false,raf=0,checkingTimer=0;
...
function bind(){const mic=$('mic');if(!mic||mic===lastMic)return;lastMic=mic;lastBusy=Boolean(state()?.micBusy);mic.addEventListener('click',()=>{requestAnimationFrame(()=>{if(state()?.micBusy)setListening()})},{capture:false});}
function tick(){bind();const st=state(),busy=Boolean(st?.micBusy);if(lastMic){if(busy&&!lastBusy)setListening();if(!busy&&lastBusy)setChecking();lastBusy=busy}raf=requestAnimationFrame(tick)}
injectStyle();tick();window.LEXICON_VOICE_LISTENING=Object.freeze({release:RELEASE});
})();
