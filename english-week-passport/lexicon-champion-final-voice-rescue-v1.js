(()=>{
'use strict';
const VERSION='2026-08-09-LCA47-FINAL-VOICE-RESCUE-V2-LIVE-SCORE';
...
function liveVoiceScore(){
 const arena=$('arena'); if(!arena)return 0;
 const text=String(arena.innerText||'');
 if(!/Final Boss\s*[•·]\s*Voice Attack/i.test(text))return 0;
 const vals=[...text.matchAll(/(\d{1,3})\s*%/g)].map(m=>Number(m[1])).filter(n=>n>=0&&n<=100);
 return vals.length?Math.max(...vals):0;
}
function finalVoiceComplete(){
 const st=state(); if(!st)return false;
 const live=liveVoiceScore();
 if(live>=52){st.voiceScore=Math.max(Number(st.voiceScore||0),live);st.bossAttack=Math.max(Number(st.bossAttack||0),3);return true;}
 return Number(st.bossAttack)>=3&&Number(st.voiceScore)>=52;
}
...
function passportUrl(receipt){const p=new URLSearchParams({resume:'passport',fromGame:'final_boss',v:'20260809-final-voice-rescue2'});if(receipt)p.set('receipt',receipt);if(q.get('view')==='mobile')p.set('view','mobile');return './index.html?'+p.toString()}
async function rescue(){
 if(running||summaryVisible()||!finalVoiceComplete())return;
 running=true; const A=api(),st=state();
 try{speechSynthesis?.cancel()}catch(_){}
 st.mastery=computeMastery(st); const evidence=A?.getReceipt?.()||{};
 ...
 box?.classList.remove('hidden','saved','error'); if(ret){ret.disabled=true;ret.textContent='กำลังบันทึก Firebase…'}
 if(title)title.textContent=PROD?'กำลังบันทึกผล…':'Final Voice Complete ✓';
 if(detail)detail.textContent=PROD?'Final Voice Rescue V2 • รอ Firebase receipt':'Final Voice Rescue completed';
 ...
 try{
  ...
  const firebaseSaved=response.mode==='firebase'||response.authority?.mode==='firebase'; if(!firebaseSaved)throw new Error(response.firebaseError||'FIREBASE_RECEIPT_REQUIRED');
  st.receipt=response.receiptId||response.resultId||'firebase-saved'; box?.classList.add('saved');
  ...
  if(ret){ret.disabled=!response.passed;ret.textContent=response.passed?'Return to Passport':'ยังไม่ผ่าน • เล่นใหม่'} if(retry)retry.classList.add('hidden');
  if(response.passed){await wait(1800);location.replace(passportUrl(st.receipt))}
 }catch(error){console.error('[LCA Final Voice Rescue V2]',error);box?.classList.add('error');if(title)title.textContent='บันทึก Firebase ไม่สำเร็จ';if(detail)detail.textContent=String(error?.message||error);if(ret){ret.disabled=true;ret.textContent='รอการบันทึก'}retry?.classList.remove('hidden');running=false;}
}
function watch(){if(summaryVisible()){armedAt=0;return}if(gameVisible()&&finalVoiceComplete()){if(!armedAt)armedAt=Date.now();if(Date.now()-armedAt>=500)rescue()}else armedAt=0}
interval=setInterval(watch,100); addEventListener('pagehide',()=>clearInterval(interval),{once:true});
window.LEXICON_CHAMPION_FINAL_VOICE_RESCUE=Object.freeze({version:VERSION,rescue,liveVoiceScore});
console.info('[LEXICON Champion] Final Voice Rescue V2 live-score ready');
})();