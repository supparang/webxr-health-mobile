(function(){
'use strict';
const VERSION='2026-08-18-DURABLE-QUEUE-V1';
...
function reconcile(raw,playerId){
 const p={...defaultProgress(playerId),...(raw||{}),playerId};
 const passed=[...new Set((Array.isArray(p.passed)?p.passed:[]).map(clean).filter(Boolean))];
 ...
 return {...p,passed,bestScores,unlocked,currentStage:unlocked[unlocked.length-1],finalDone:Boolean(p.finalDone||passed.includes('final_boss')),certificateEligible:Boolean(p.certificateEligible||p.postDone),totalScore:Object.values(bestScores).reduce((s,v)=>s+Number(v||0),0),queuedPending:true,updatedAt:nowIso()};
}
function applyQueued(progress,item){
 const p=reconcile(progress,item.playerId);
 if(item.kind==='assessment'){
  if(item.payload.assessmentType==='pre')p.preDone=true;
  if(item.payload.assessmentType==='post'){p.postDone=true;p.certificateEligible=true;}
  return reconcile(p,item.playerId);
 }
 if(item.kind==='game'){
  const stage=clean(item.payload.stageId),total=Math.max(1,Number(item.payload.total||100)),score=Math.max(0,Number(item.payload.score||0)),acc=Math.round(score/total*100);
  p.bestScores={...p.bestScores,[stage]:Math.max(Number(p.bestScores?.[stage]||0),acc)};
  if(acc>=Number(PASS_MARKS[stage]||60)&&!p.passed.includes(stage))p.passed=[...p.passed,stage];
  return reconcile(p,item.playerId);
 }
 return p;
}
function overlayQueue(playerId,authority){
 const q=readQueue(playerId);if(!q.length)return authority;
 let progress=reconcile(authority?.progress||defaultProgress(playerId),playerId);q.forEach(item=>{progress=applyQueued(progress,item)});
 return {...(authority||{}),ok:true,mode:'firebase',sourceOfTruth:'Firestore + durable local queue',progress,durableQueuePending:q.length,queuedPending:true};
}
function enqueue(kind,payload,error){
 const playerId=clean(payload?.playerId);if(!playerId)throw error;
 const q=readQueue(playerId);const id=`queued-${kind}-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
 const item={id,kind,playerId,payload:{...(payload||{})},queuedAt:nowIso(),lastError:String(error?.message||error||'FIREBASE_TRANSIENT_ERROR')};q.push(item);if(!writeQueue(playerId,q))throw error;
 let authority=overlayQueue(playerId,readSnapshot(playerId)||{profile:{playerId,nickname:clean(payload?.nickname)||'Player'},progress:defaultProgress(playerId),assignment:null});saveSnapshot(playerId,authority);
 return {ok:true,mode:'firebase',sourceOfTruth:'Durable local queue pending Firestore',receiptId:id,queued:true,durableQueued:true,persisted:false,authority,progress:authority.progress,passed:kind==='game'?authority.progress.passed.includes(clean(payload?.stageId)):true,accuracy:kind==='game'?Number(authority.progress.bestScores?.[clean(payload?.stageId)]||0):undefined,version:VERSION};
}
let flushing=false,lastFlush=0;
async function flush(playerId){
 const id=clean(playerId);if(!id||flushing||Date.now()-lastFlush<15000)return {ok:false,skipped:true};
 flushing=true;lastFlush=Date.now();let q=readQueue(id),done=0;
 try{
  while(q.length){const item=q[0];try{const r=item.kind==='assessment'?await base.submitAssessment(item.payload):await base.submitGame(item.payload);if(!r?.ok)throw new Error(r?.error||'QUEUE_FLUSH_FAILED');q.shift();writeQueue(id,q);done++;if(r.authority)saveSnapshot(id,r.authority);}catch(error){if(transient(error))break;item.lastError=String(error?.message||error);item.failedAt=nowIso();q[0]=item;writeQueue(id,q);break;}}
 }finally{flushing=false}
 return {ok:true,flushed:done,pending:q.length};
}
async function resume(playerId,nickname){
 const id=clean(playerId);try{const r=await base.resume(id,nickname);saveSnapshot(id,r);const out=overlayQueue(id,r);if(readQueue(id).length)setTimeout(()=>flush(id),1200);return out;}catch(error){const snap=readSnapshot(id);if(transient(error)&&snap)return overlayQueue(id,snap);throw error;}
}
