window.EW_CONFIG = Object.freeze({
  ...
  version: "2026-08-18-EVENT-DAY-PRODUCTION-LOCK-R17",
...
(function(){
  ...
  function reconcile(raw,playerId){const passed=unique(raw?.passed).filter(x=>STAGES.has(x));const bestScores=raw?.bestScores&&typeof raw.bestScores==='object'?{...raw.bestScores}:{};const unlocked=['pre_challenge'];if(raw?.preDone)unlocked.push('word_match');if(passed.includes('word_match'))unlocked.push('category_forest');if(passed.includes('category_forest'))unlocked.push('sentence_city');if(passed.includes('sentence_city'))unlocked.push('word_detective');if(passed.includes('word_detective'))unlocked.push('final_boss');if(passed.includes('final_boss'))unlocked.push('post_challenge');if(raw?.postDone)unlocked.push('certificate');return {...raw,playerId,passed,bestScores,unlocked,currentStage:unlocked[unlocked.length-1],finalDone:Boolean(raw?.finalDone||passed.includes('final_boss')),totalScore:Object.values(bestScores).reduce((sum,v)=>sum+Number(v||0),0),updatedAt:nowIso(),passPolicyVersion:VERSION,gamePassMarks:PASS_MARKS}}
  ...
  function wrap(base){
    ...
    const custom=async payload=>{const playerId=clean(payload?.playerId),stageId=clean(payload?.stageId);if(!playerId||!STAGES.has(stageId))throw new Error('INVALID_GAME_PAYLOAD');const passMark=PASS_MARKS[stageId];const total=Math.max(0,Number(payload?.total||0)),score=Math.max(0,Number(payload?.score||0));const accuracy=total>0?Math.round(score/total*100):0,passed=accuracy>=passMark,id=receiptId(stageId);const db=await ensureFirebase(),progressRef=db.collection('ewp_progress').doc(playerId),resultRef=db.collection('ewp_game_results').doc(id),summaryRef=db.collection('ewp_game_summary').doc(playerId);let nextProgress=null;await db.runTransaction(async tx=>{const snap=await tx.get(progressRef),raw=snap.exists?{playerId,...(snap.data()||{})}:{playerId,passed:[],bestScores:{},preDone:false,postDone:false};const current=reconcile(raw,playerId);if(!current.unlocked.includes(stageId))throw new Error('STAGE_LOCKED');const nextPassed=[...current.passed];if(passed&&!nextPassed.includes(stageId))nextPassed.push(stageId);const nextBest={...current.bestScores,[stageId]:Math.max(Number(current.bestScores?.[stageId]||0),accuracy)};nextProgress=reconcile({...current,passed:nextPassed,bestScores:nextBest},playerId);tx.set(progressRef,nextProgress,{merge:true});tx.set(resultRef,{...payload,playerId,stageId,receiptId:id,accuracy,passMark,passed,submittedAt:nowIso(),sourceVersion:VERSION,authorityMode:'firestore-direct',passPolicy:stageId==='word_match'?'g1-55':'g2-5-60'});tx.set(summaryRef,{playerId,totalScore:nextProgress.totalScore,bestScores:nextProgress.bestScores,passed:nextProgress.passed,currentStage:nextProgress.currentStage,gamePassMarks:PASS_MARKS,updatedAt:nowIso(),sourceVersion:VERSION},{merge:true})});let resumed=null;try{resumed=typeof base.resume==='function'?await base.resume(playerId,payload?.nickname||''):null}catch(_){}return {ok:true,mode:'firebase',sourceOfTruth:'Cloud Firestore Direct Authority + Production Pass Policy',receiptId:id,accuracy,passMark,passed,progress:nextProgress,authority:resumed&&resumed.ok?resumed:{ok:true,mode:'firebase',progress:nextProgress},version:VERSION}};
...
(function(){
  ...
  async function reconcileStatus(){const detail=document.getElementById('statusDetail');if(!detail||!/คะแนนยังไม่ถึงเกณฑ์ผ่าน/.test(detail.textContent||''))return;let identity=null;try{identity=JSON.parse(localStorage.getItem(window.EW_CONFIG?.cacheKeys?.identity||'ew_passport_identity_v1')||'null')}catch(_){}if(!identity?.playerId||!window.EW_AUTHORITY?.resume)return;try{const r=await window.EW_AUTHORITY.resume(identity.playerId,identity.nickname||identity.fullName||'');if(r?.progress?.passed?.includes(stage)){const mark=window.EW_CONFIG?.gamePassMarks?.[stage]||60;detail.textContent=`ผ่านด่านและปลดล็อกขั้นถัดไปแล้ว • เกณฑ์ผ่าน ${mark}%`}}catch(_){}}
...
/* Event-Day Production Lock R17: load final idempotency/queue-integrity/telemetry hardening on every entry page that loads config.js. */
(function(){
  'use strict';
  const VERSION='2026-08-18-HARDENING-AUTOLOADER-R1';
  let attempts=0,loading=false;
  function ready(){return !!window.EW_AUTHORITY && (window.EW_AUTHORITY.extremeFailSafe===true || window.EW_AUTHORITY.eventDayLightMode===true || /EXTREME|ULTRA/i.test(String(window.EW_AUTHORITY?.sourceOfTruth||'')))}
  function load(){
    if(window.EW_EVENT_DAY_HARDENING?.version)return;
    if(!ready()){if(++attempts<200)setTimeout(load,50);return}
    if(loading)return;loading=true;
    const s=document.createElement('script');s.src='./event-day-final-hardening-v1.js?v=20260818-final-hardening-r1';s.async=false;s.dataset.ewHardeningAutoload=VERSION;
    s.onload=()=>{loading=false};s.onerror=()=>{loading=false;if(++attempts<200)setTimeout(load,120)};
    document.head.appendChild(s);
  }
  setTimeout(load,0);
  window.EW_HARDENING_AUTOLOADER=Object.freeze({version:VERSION});
})();