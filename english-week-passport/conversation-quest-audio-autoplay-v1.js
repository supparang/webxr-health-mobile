(()=>{
'use strict';
const VERSION='2026-08-12-CQ-AUDIO-HARD-GATE-V4-SINGLE-OWNER';
...
/* Single-owner contract:
   - Conversation Quest runtime owns automatic Teacher Maya prompt/reply speech.
   - This file only guards audible completion, blocks premature transitions,
     and provides explicit user-triggered replay via the speaker button.
   - No MutationObserver or voiceschanged handler may auto-speak #line. */
if(!synth.__cqSpeechHardGateV4){
  ...
  synth.__cqSpeechHardGateV4=true;
}

if(!window.__CQ_TRANSITION_TIMER_GATE_V4){
  window.__CQ_TRANSITION_TIMER_GATE_V4=true;
...
function replayCurrent(){
  ...
  if(!text||!audioEnabled())return false;
...
function bind(){
  ...
  start?.addEventListener('click',()=>{primed=true;try{synth.resume?.()}catch{}},{capture:true});
  replay?.addEventListener('click',()=>{primed=true;try{synth.resume?.()}catch{}},{capture:true});

  // Explicit replay only. Stop the runtime's own click handler from causing
  // an additional second utterance after this replay.
  ...
  audio?.addEventListener('click',()=>{
    nativeSetTimeout(()=>{
      if(audioEnabled()){primed=true;try{synth.resume?.()}catch{}}
  ...
  window.CQ_AUDIO_AUTOPLAY=Object.freeze({version:VERSION,replayCurrent,waitUntilTeacherDone,get primed(){return primed},get speechBusy(){return speechBusy()}});
}
