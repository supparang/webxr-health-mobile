(function () {
  ...
  const VERSION = "2026-08-04-WORD-MATCH-SMOOTH-TILT-V12-SINGLE-TARGET";
  const DEAD_ZONE = 2.2;
  const MAX_TILT = 15;
  const MAX_SPEED = 720;
  ...
  function clearLegacyFocus() {
    document.querySelectorAll(".memory-card.ew-tilt-focus").forEach(card => card.classList.remove("ew-tilt-focus"));
  }
  ...
  function setTarget(card) {
    ...
    if (state.targetCard === card) {
      if (card) card.classList.add("smooth-target");
      return;
    }
  ...
  function updateAcademicCopy() {
    ...
    const passportBadge = document.querySelector(".memory-top .title + *");
    if (title) title.textContent = "Vocabulary Navigation Lab";
    if (subtitle && !subtitle.dataset.smoothV12) {
      subtitle.dataset.smoothV12 = "1";
      subtitle.textContent = subtitle.textContent.replace("Tilt Snap", "Smooth Tilt").replace("P?", "Passport");
    }
  ...
  window.EW_WORD_MATCH_SMOOTH_TILT = Object.freeze({
    ...
    dwellMs:DWELL_MS,
    interaction:"smooth-tilt-single-target-auto-open",
    singleTarget:true,
    touchFallback:false
  });
}());
