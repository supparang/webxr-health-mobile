...
function decorate(){
  Object.entries(ROUTES).forEach(([stage,route])=>{
    ...
    card.dataset.canonicalRoute='1';card.classList.add('canonical-game-card');
    const icon=card.querySelector('.stage-icon');if(icon)icon.textContent=route.icon;
    const title=card.querySelector('div:nth-child(2)>strong');if(title)title.textContent=route.title;
    const detail=card.querySelector('div:nth-child(2)>small');if(detail){
      const best=(detail.textContent.match(/• ดีที่สุด[^•]*/)||[])[0]||'';
      detail.textContent=route.detail+(best?' '+best:'');
    }
    const state=card.querySelector('.stage-state');if(state&&card.classList.contains('ready'))state.textContent='พร้อมเล่น';
  });
}
