...
const ROUTES={
  ...
  bonus_lens:{title:'Lexicon Lens Hunt',detail:'Bonus Mission • QR Clue • 20 คนแรก/รอบมีรางวัล • ไม่บังคับ Certificate',icon:'📷',bonus:true},
...
function makeInteractive(card){
  if(!card)return;const unlocked=card.classList.contains('ready')||card.classList.contains('passed');
  if(card.classList.contains('clickable')!==unlocked)card.classList.toggle('clickable',unlocked);
  if(unlocked){if(card.tabIndex!==0)card.tabIndex=0;if(card.getAttribute('role')!=='button')card.setAttribute('role','button');if(card.style.pointerEvents!=='auto')card.style.pointerEvents='auto';if(card.hasAttribute('inert'))card.removeAttribute('inert');}
  else{if(card.hasAttribute('tabindex'))card.removeAttribute('tabindex');if(card.hasAttribute('role'))card.removeAttribute('role');}
}
function ensureBonusCard(){
  const source=document.querySelector('.stage-card[data-stage="word_detective"]');if(!source)return null;
  let card=document.querySelector('.stage-card[data-stage="bonus_lens"]');const ready=source.classList.contains('passed');const identity=readIdentity();const best=bonusBest(identity);
  if(!card){card=document.createElement('article');card.dataset.stage='bonus_lens';source.insertAdjacentElement('afterend',card);}
  const wantedClass=`stage-card canonical-game-card bonus-lens-card ${ready?'ready clickable':'locked'}`;setClassName(card,wantedClass);setData(card,'canonicalRoute','1');setData(card,'canonicalDecorated',VERSION);makeInteractive(card);
  const status=best?.score!=null?`โบนัส ${Number(best.score)}% ✓ • เช็กรางวัล`:ready?'พร้อมเล่นโบนัส 🎁':'ผ่าน Game 4 เพื่อเปิด';
  const bestText=best?.score!=null?` • ดีที่สุด ${Number(best.score)}%`:'';
  const wantedHTML=`<div class="stage-icon">📷</div><div><strong>Lexicon Lens Hunt</strong><small>Bonus Mission • QR Clue • 20 คนแรก/รอบมีรางวัล • ไม่บังคับ Certificate${bestText}</small></div><div class="stage-state">${status}</div>`;
  if(card.innerHTML!==wantedHTML)card.innerHTML=wantedHTML;return card;
}
function decorateNow(){
  ...
  Object.entries(ROUTES).forEach(([stage,route])=>{
    ...
    setData(card,'canonicalRoute','1');setData(card,'canonicalDecorated',VERSION);if(!card.classList.contains('canonical-game-card'))card.classList.add('canonical-game-card');makeInteractive(card);
    const icon=card.querySelector('.stage-icon');setText(icon,route.icon);const title=card.querySelector('div:nth-child(2)>strong');setText(title,route.title);const detail=card.querySelector('div:nth-child(2)>small');if(detail){const best=(detail.textContent.match(/• ดีที่สุด[^•]*/)||[])[0]||'';setText(detail,route.detail+(best?' '+best:''));}
...
new MutationObserver(mutations=>{const relevant=mutations.some(m=>m.type==='childList'||(m.type==='attributes'&&m.attributeName==='class'));if(relevant)scheduleDecorate();}).observe(screen,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});
...
decorateNow();window.EW_CANONICAL_ROUTES=Object.freeze({version:VERSION,routes:ROUTES,openStage,currentView});
}());