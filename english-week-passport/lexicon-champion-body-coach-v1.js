(()=>{
'use strict';
const RELEASE='20260809-LCA47-BODY-COACH-V5-UPPER-BODY-READY';
...
.poseStage.coach-command .bodyCoach{right:auto;max-width:58%}.poseStage.coach-command .bodyCoachTitle{min-width:150px;padding:5px 7px}
...
function bodyState(l){if(!Array.isArray(l))return null;const head=vis(l[0])>.18,shoulders=vis(l[11])>.2&&vis(l[12])>.2,arms=(vis(l[13])>.16||vis(l[15])>.12)&&(vis(l[14])>.16||vis(l[16])>.12),torso=vis(l[23])>.16&&vis(l[24])>.16,knees=vis(l[25])>.1&&vis(l[26])>.1;const primary=[head,shoulders,arms,torso],pct=Math.round(primary.filter(Boolean).length/primary.length*100);return{head,shoulders,arms,torso,knees,pct,ready:head&&shoulders&&arms&&torso}}
...
function paint(){if(!bodyModeActive()){clear();return}if(!ensureUI())return;const state=V()?.state,l=state?.latest,bs=bodyState(l),active=Boolean(state?.commandAt);stage.classList.toggle('coach-command',active);if(bs){setChip('bcHead',bs.head);setChip('bcShoulders',bs.shoulders);setChip('bcArms',bs.arms);setChip('bcTorso',bs.torso);setChip('bcKnees',bs.knees,!bs.knees);$('bodyCoachPct').textContent=bs.pct+'%';$('bodyCoachState').textContent=bs.ready?'BODY READY':'BODY SEARCHING';$('bodyCoachHint').textContent=bs.ready?actionHint(state?.poseAction):!bs.head?'ปรับกล้องให้เห็นศีรษะ':!bs.shoulders?'ให้เห็นไหล่ทั้งสองข้าง':!bs.arms?'ให้เห็นแขนอย่างน้อยถึงข้อศอกทั้งสองข้าง':'ถอยอีกนิดให้เห็นลำตัวถึงสะโพก';stage.classList.toggle('body-ready',bs.ready);drawSkeleton(l,bs.ready)}else{const pct=$('bodyCoachPct'),stateEl=$('bodyCoachState'),hint=$('bodyCoachHint');if(pct)pct.textContent='0%';if(stateEl)stateEl.textContent='BODY SEARCHING';if(hint)hint.textContent='ยืนกลางกรอบและให้เห็นช่วงบนชัดเจน';stage.classList.remove('body-ready');drawSkeleton(null,false)}}
function schedule(delay){clearTimeout(timer);timer=setTimeout(run,delay)}
function run(){try{const now=performance.now();if(bodyModeActive()){if(now-lastPaint>=66){paint();lastPaint=now}schedule(50)}else{clear();schedule(300)}}catch(err){console.warn('[LCA Body Coach]',err);schedule(500)}}
function loop(){if(timer)return;run()}function stop(){clearTimeout(timer);timer=0;clear()}
injectStyle();loop();addEventListener('pagehide',stop,{once:true});document.addEventListener('visibilitychange',()=>{if(document.hidden)stop();else loop()});window.LEXICON_BODY_COACH=Object.freeze({release:RELEASE,stop,restart:loop,kneesOptional:true});
})();
