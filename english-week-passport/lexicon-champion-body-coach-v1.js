(()=>{
'use strict';
const RELEASE='20260808-LCA47-BODY-COACH-V2';
...
function drawSkeleton(l,ready){if(!ctx||!canvas)return;resize();const d=Math.min(2,devicePixelRatio||1),w=canvas.width/d,h=canvas.height/d;ctx.setTransform(d,0,0,d,0,0);ctx.clearRect(0,0,w,h);if(!Array.isArray(l))return;ctx.lineCap='round';ctx.lineJoin='round';ctx.lineWidth=2.5;ctx.strokeStyle=ready?'rgba(114,240,192,.92)':'rgba(98,232,255,.78)';for(const [a,b] of CONNECTIONS){if(vis(l[a])<.1||vis(l[b])<.1)continue;ctx.beginPath();ctx.moveTo(l[a].x*w,l[a].y*h);ctx.lineTo(l[b].x*w,l[b].y*h);ctx.stroke()}const ids=[0,11,12,13,14,15,16,23,24,25,26];for(const i of ids){if(vis(l[i])<.1)continue;ctx.beginPath();ctx.fillStyle=ready?'#72f0c0':'#62e8ff';ctx.arc(l[i].x*w,l[i].y*h,i===0?5:3.8,0,Math.PI*2);ctx.fill();ctx.strokeStyle='rgba(8,18,35,.8)';ctx.lineWidth=1.2;ctx.stroke()}}
function tick(){cancelAnimationFrame(raf);const run=()=>{try{ensureUI();const state=V()?.state,l=state?.latest,bs=bodyState(l),active=Boolean(state?.commandAt);if(stage)stage.classList.toggle('coach-command',active);if(stage&&bs){setChip('bcHead',bs.head);setChip('bcShoulders',bs.shoulders);setChip('bcArms',bs.arms);setChip('bcTorso',bs.torso);setChip('bcKnees',bs.knees);$('bodyCoachPct').textContent=bs.pct+'%';$('bodyCoachState').textContent=bs.ready?(active?'BODY READY':'BODY READY'):'BODY SEARCHING';$('bodyCoachHint').textContent=bs.ready?actionHint(state?.poseAction):!bs.head?'ถอย/ปรับกล้องให้เห็นศีรษะ':!bs.shoulders?'ให้เห็นไหล่ทั้งสองข้าง':!bs.arms?'ให้เห็นแขนอย่างน้อยถึงข้อศอกทั้งสองข้าง':!bs.torso?'ถอยอีกนิดให้เห็นสะโพกทั้งสองข้าง':'ถอยอีกนิดให้เห็นเข่าทั้งสองข้าง';stage.classList.toggle('body-ready',bs.ready);drawSkeleton(l,bs.ready)}else if(stage){$('bodyCoachPct')&&($('bodyCoachPct').textContent='0%');$('bodyCoachState')&&($('bodyCoachState').textContent='BODY SEARCHING');$('bodyCoachHint')&&($('bodyCoachHint').textContent='ยืนกลางกรอบและถอยให้เห็นช่วงตัวมากขึ้น');stage.classList.remove('body-ready');drawSkeleton(null,false)}}catch(e){console.warn('[LCA Body Coach]',e)}raf=requestAnimationFrame(run)};raf=requestAnimationFrame(run)}
