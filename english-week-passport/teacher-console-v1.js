(function(){
'use strict';
const VERSION='2026-08-07-TEACHER-CONSOLE-R1';
const cfg=window.EW_CONFIG||{};
const endpoint=String(cfg.firebaseTeacherUrl||'').trim();
...
async function call(action,payload){
  if(!/^https:\/\//.test(endpoint))throw new Error('TEACHER_ENDPOINT_MISSING');
...
function renderOverview(){
  const o=data?.overview||{};const t=o.totals||{};const l=o.learning||{};
  ...
  $('games').innerHTML=(o.games||[]).map(game=>`<div class="game-row"><div><strong>${h(game.title)}</strong><small>${h(game.skill)} • ${Number(game.players||0)} players</small></div><div class="pill">${Number(game.avgBestAccuracy||0)}%<small>Avg</small></div><div class="pill">${Number(game.passed||0)}<small>Pass</small></div><div class="pill">${Number(game.avgAttempts||0)}<small>Try</small></div></div>`).join('');
  $('issues').innerHTML=(o.issues||[]).length?(o.issues||[]).map(issue=>`<div class="issue" data-player="${h(issue.playerId)}"><strong>${h(issue.name)} • ${h(issue.type)}</strong><small>${h(issue.detail)}</small></div>`).join(''):'<div class="good">✓ ไม่พบ Data Health issue จากกฎ R1</div>';
  renderParticipants();
}

function renderParticipants(){
  const query=$('searchInput').value.trim().toLowerCase();const stage=$('stageFilter').value;
  const rows=(data?.participants||[]).filter(row=>{
    const hay=`${row.playerId} ${row.nickname} ${row.fullName} ${row.groupName}`.toLowerCase();
    return (!query||hay.includes(query))&&(!stage||row.stage===stage);
  });
  $('participantBody').innerHTML=rows.map(row=>`<tr data-player="${h(row.playerId)}"><td><strong>${h(row.nickname)}</strong><br><small>${h(row.playerId)}</small></td><td>${h(row.groupName)}</td><td><strong>${h(row.stage)}</strong></td><td>${pct(row.preAccuracy)}</td><td>${pct(row.postAccuracy)}</td><td class="${row.learningGain!=null&&row.learningGain>=0?'good':'warn'}">${signed(row.learningGain)}</td><td>${pct(row.averageGameAccuracy)}</td><td>${Number(row.totalAttempts||0)}</td><td class="${row.reflectionDone?'good':'warn'}">${row.reflectionDone?'✓':'—'}</td><td class="${row.summaryViewed?'good':'warn'}">${row.summaryViewed?'✓':'—'}</td><td class="${row.summaryViewed&&row.certificateEligible?'good':'warn'}">${row.summaryViewed&&row.certificateEligible?'✓':'—'}</td></tr>`).join('');
}

async function refresh(){
  if(!teacherKey)return showLogin();
  setStatus('กำลังโหลด Firebase…',false);$('refreshBtn').disabled=true;
  try{const result=await call('overview');data=result;renderOverview();setStatus(`Firebase live • ${new Date(result.generatedAt||Date.now()).toLocaleTimeString('th-TH')}`,false);scheduleRefresh()}
...
async function exportRows(kind){
  try{setStatus(`กำลังเตรียม CSV ${kind}…`,false);const result=await call('export_rows',{kind});downloadCsv(`lexicon-x-${kind}-${new Date().toISOString().slice(0,10)}.csv`,result.rows||[]);setStatus(`Export ${kind} สำเร็จ`,false)}catch(error){setStatus(`Export ไม่สำเร็จ: ${String(error?.message||error)}`,true)}
}

$('loginBtn').onclick=login;$('teacherKey').onkeydown=e=>{if(e.key==='Enter')login()};$('refreshBtn').onclick=refresh;$('lockBtn').onclick=lock;$('searchInput').oninput=renderParticipants;$('stageFilter').onchange=renderParticipants;$('exportParticipantsBtn').onclick=()=>exportRows('participants');$('exportGamesBtn').onclick=()=>exportRows('games');$('closeReportBtn').onclick=()=>$('reportModal').classList.add('hidden');$('reportModal').addEventListener('click',e=>{if(e.target===$('reportModal'))$('reportModal').classList.add('hidden')});document.addEventListener('click',e=>{const row=e.target.closest?.('[data-player]');if(row?.dataset.player&&row.closest('#participantBody,#issues'))openReport(row.dataset.player)});
...
window.EW_TEACHER_CONSOLE=Object.freeze({VERSION,refresh});
}());
