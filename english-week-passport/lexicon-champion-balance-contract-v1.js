(()=>{
'use strict';
const RELEASE='20260809-LCA47-BALANCE-CONTRACT-V3-SMOOTHER-HOLD';
const V=()=>window.LEXICON_CHAMPION_V47;
const $=id=>document.getElementById(id);
const HOLD_REAL_MS=1050;
...
function speakBarrier(text){const st=V()?.state;if(!st||!text||!('speechSynthesis'in window))return;const epoch=++gateEpoch;speaking=true;st.commandAt=0;st.holdStart=0;const bar=$('holdBar');if(bar)bar.style.width='0%';const status=$('poseStatus');if(status)status.textContent='🎧 ฟังคำสั่งให้จบก่อน';try{speechSynthesis.cancel()}catch(_){}const u=new SpeechSynthesisUtterance(text);u.lang='en-US';u.rate=.82;u.pitch=1;const voice=preferredVoice();if(voice)u.voice=voice;const arm=()=>{if(epoch!==gateEpoch)return;speaking=false;if(status)status.textContent='🧍 เตรียมทำท่า…';clearTimeout(unlockTimer);unlockTimer=setTimeout(()=>{if(epoch!==gateEpoch)return;const s=V()?.state;if(!s)return;s.holdStart=0;s.commandAt=performance.now();if(status)status.textContent='🎯 ทำท่าตามคำสั่งและค้างไว้'},260)};u.onend=arm;u.onerror=()=>setTimeout(arm,100);try{speechSynthesis.speak(u)}catch(_){setTimeout(arm,180)}}
...
function style(){if($('lcaBalanceContractStyle'))return;const s=document.createElement('style');s.id='lcaBalanceContractStyle';s.textContent='#holdBar{transition:width .10s linear!important}.poseStage[data-audio-barrier="1"] .ghost{opacity:.22!important}';document.head.appendChild(s)}
...
poll();addEventListener('pagehide',()=>{clearTimeout(timer);clearTimeout(unlockTimer);gateEpoch++},{once:true});window.LEXICON_CHAMPION_BALANCE_CONTRACT=Object.freeze({release:RELEASE,holdMs:HOLD_REAL_MS,audioBarrier:true,stableHold:true});
})();
