(()=>{
'use strict';
const VERSION='2026-08-09-LCA47-FINAL-VOICE-DIRECT-EXIT-V3-BOUNDED-RECONCILE';
...
function passportUrl(receipt,reconciled=false){const p=new URLSearchParams({resume:'passport',fromGame:'final_boss',v:'20260809-final-voice-direct-exit3'});if(receipt)p.set('receipt',receipt);if(reconciled)p.set('reconcile','1');if(q.get('view')==='mobile')p.set('view','mobile');return './index.html?'+p.toString()}
...
async function complete(score){
  ...
  try{
    ...
    const payload={playerId,nickname,stageId:'final_boss',score:st.mastery,total:100,durationMs:Number(evidence.durationMs||0),clientPoints:Number(st.score||0),answers:[{...evidence,mastery:st.mastery,voiceScore:st.voiceScore,clientPoints:st.score,directExitVersion:VERSION}],sourceVersion:VERSION};
    let response=null;
    try{response=await Promise.race([window.EW_AUTHORITY.submitGame(payload),delay(8000)])}catch(submitError){
      ...
      if(confirmed){
        ...
        try{sessionStorage.setItem(`ew_passport_receipt::${playerId}::final_boss`,JSON.stringify({receipt:synthetic,at:new Date().toISOString(),passed:true,accuracy:st.mastery,missionSet:A?.SET||'',reconciled:true,directExitVersion:VERSION}))}catch(_){}
    ...
    try{sessionStorage.setItem(`ew_passport_receipt::${playerId}::final_boss`,JSON.stringify({receipt,at:new Date().toISOString(),passed:true,accuracy:response.accuracy??st.mastery,missionSet:A?.SET||'',directExitVersion:VERSION}))}catch(_){}
    overlay('✅ Firebase Saved • PASS<br><small>กำลังกลับ Passport…</small>');
    location.replace(passportUrl(receipt));
  }catch(err){
    console.error('[LCA Direct Exit V3]',err);locked=false;
...
console.info('[LEXICON Champion] Final Voice Direct Exit V3 bounded-reconcile ready');
})();