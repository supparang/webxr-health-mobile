...
function refreshMissionWallProgress() {
  missionDB.forEach((mGroup) => {
    ...
    const basePrefix = isUnitFinal(mGroup.id)
      ? "👑 FINAL: "
      : ((mGroup.id % 3 === 0) ? "🔥 BOSS: " : "");
...
function showEndSummary(success, extraLines = []) {
  ...
  const lines = [
    ...
    `Question Diff: ${(currentMission && currentMission._selectedDifficulty ? currentMission._selectedDifficulty : gameDifficulty).toUpperCase()}`
  ].concat(extraLines || []);
...
function getFinalBossPattern(id) {
  ...
  if (id === 15) {
    return {
      code: "FINAL EXAM MIX",
      desc: "สุ่มโจทย์ผสมจาก Interview + Remote Work + Career Path",
      color: "#a29bfe",
      timeAdjust: 0
    };
  }
...
function buildMixedFinalMission(mode = gameDifficulty, aiState = aiDirector) {
  ...
  return {
    ...chosenItem,
    id: 15,
    type: chosenItem.type || chosenGroup.type,
...
function prepareMissionForBossPattern(missionGroup) {
  ...
  if (missionGroup.id === 15) {
    prepared = buildMixedFinalMission(gameDifficulty, aiDirector);
  } else {
    prepared = pickAdaptiveMissionItem(missionGroup.id, gameDifficulty, aiDirector);
  }
...
function loadMission(id) {
  ...
  if (!currentMission) {
    console.warn("No mission item found for session:", id);
  ...
  document.getElementById("ui-title").innerText =
    `🚨 ${titlePrefix}SESSION ${id}: ${currentMission.title.toUpperCase()} [${gameDifficulty.toUpperCase()}]`;

  document.getElementById("ui-title").style.color =
    isFinal ? (currentMission._bossPatternColor || "#f1c40f") : (isBoss ? "#e74c3c" : "#ff4757");

  document.getElementById("ui-desc").innerText = isFinal
    ? `${currentMission.desc} | ${currentMission._bossPattern} | FINAL BOSS HP ${finalBossState.hp}/${finalBossState.maxHp} | AI ${aiDirector.mood} | Q:${(currentMission._selectedDifficulty || gameDifficulty).toUpperCase()}`
    : `${currentMission.desc} | AI ${aiDirector.mood} | Q:${(currentMission._selectedDifficulty || gameDifficulty).toUpperCase()}`;

  document.getElementById("feedback").innerText = isFinal
    ? `👑 UNIT FINAL START! ${currentMission._bossPattern} — ${currentMission._bossPatternDesc}\nAI เลือกโจทย์ระดับ ${(currentMission._selectedDifficulty || gameDifficulty).toUpperCase()}`
  ...
  if (currentMission.type === "speaking") {
    ...
    document.getElementById("speaking-prompt").setAttribute(
      "value",
...
window.playAudio = function() {
  ...
  if ("speechSynthesis" in window) {
    ...
    utterance.rate = gameDifficulty === "hard" ? 1.0 : 0.9;
...
function winMission() {
  ...
  if (isFinal && finalBossState.active && finalBossState.hp > 1) {
    ...
    document.getElementById("feedback").innerText =
      `⚔️ ${currentMission._bossPattern} HIT! HP ที่เหลือ ${finalBossState.hp}/${finalBossState.maxHp}\n${currentMission._bossPatternDesc}`;
    document.getElementById("feedback").style.color = "#f1c40f";
    hideAllScenesAndControls();
  ...
  let finalUnitBonus = (currentMission && isUnitFinal(currentMission.id))
    ? (currentMission.id === 5 ? 3000 : currentMission.id === 10 ? 2700 : 3200)
    : 0;
  ...
  if (consecutiveWins >= 2) {
    ...
    } else if (gameDifficulty === "normal" && aiDirector.pressure >= 2) {
      setDifficulty("hard");
