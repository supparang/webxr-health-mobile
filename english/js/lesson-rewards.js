...
function loadLocalProfile() {
  try {
    ...
    return {
      avatar: candidate,
      name: data && typeof data.name === "string" && data.name.trim()
...
export function syncProfileUI() {
  ...
  if (scoreNameInput && !scoreNameInput.value.trim()) {
    scoreNameInput.value = playerProfile.name;
  }

  const nextGoal = [];
  if (state.currentRewardStreak < 3) {
    nextGoal.push(`Streak ${3 - state.currentRewardStreak} วันเพื่อปลด 🤖`);
  } else if (state.currentRewardStreak < 7) {
    nextGoal.push(`Streak อีก ${7 - state.currentRewardStreak} วันเพื่อปลด 👩‍🚀`);
  }

  if (clearedMissions.length < 5) {
    nextGoal.push(`Clear อีก ${5 - clearedMissions.length} ด่านเพื่อปลด 🦾`);
  }

  if (!(state.currentRewardStreak >= 14 && clearedMissions.length >= 12)) {
    if (state.currentRewardStreak < 14) {
      nextGoal.push(`Streak อีก ${14 - state.currentRewardStreak} วัน`);
    }
    if (clearedMissions.length < 12) {
      nextGoal.push(`Clear อีก ${12 - clearedMissions.length} ด่าน`);
    }
  }
  ...
  ALLOWED_AVATARS.forEach((avatar, i) => {
    ...
    chip.title = unlocked
      ? `เลือก ${avatar}`
      : (AVATAR_META[avatar] ? AVATAR_META[avatar].label : "Locked");
  });
}
...
export async function savePlayerProfile(
  ...
  const nextName = input && input.value.trim()
    ? input.value.trim().slice(0, 24)
    : (playerProfile.name || "Hero");
