...
export function getFinalBossPattern(id) {
  ...
  if (id === 15) {
    return {
      code: "FINAL EXAM MIX",
      desc: "สุ่มโจทย์ผสมจาก Career Path + Remote Work + Graduation Final",
      color: "#a29bfe",
      timeAdjust: 0
    };
  }
...
export function remixChoiceMission(mission) {
  ...
  const correctEntry =
    parsed.find(item => item.originalLetter === mission.answer) || parsed[0];
...
export function getFinalBossMaxHp(id) {
  ...
  let hp = state.gameDifficulty === "easy"
    ? 2
    : (state.gameDifficulty === "hard" ? 4 : 3);
...
export function animateBossActor(mode = "intro") {
  ...
  if (mode === "intro") {
    boss.setAttribute(
      "animation__scale",
      "property: scale; from: 0.7 0.7 0.7; to: 1.12 1.12 1.12; dur: 650; dir: alternate; loop: 1; easing: easeOutElastic"
    );
    return;
  }
  ...
  if (mode === "hit") {
    boss.setAttribute(
      "animation__scale",
      "property: scale; from: 1 1 1; to: 1.22 1.22 1.22; dur: 180; dir: alternate; loop: 1; easing: easeOutQuad"
    );
    return;
  }
  ...
  if (mode === "clear") {
    boss.setAttribute(
      "animation__scale",
      "property: scale; from: 1 1 1; to: 1.55 1.55 1.55; dur: 320; easing: easeOutQuad"
    );
    boss.setAttribute(
      "animation__colorPulse",
      "property: visible; to: false; delay: 260; dur: 20"
    );
  }
}

export function maybeShowFinalBossIntro(id, playBossIntroSfx) {
  if (!finalBossState.active) return;
  if (!isUnitFinal(id)) return;
