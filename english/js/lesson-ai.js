...
function getDifficultyColor(diff = "normal") {
  const level = String(diff || "normal").toLowerCase();
  if (level === "easy") return "#2ed573";
  if (level === "hard") return "#ff6b81";
  return "#f1c40f";
}
...
export function getDifficultyTimeMod() {
  return state.gameDifficulty === "easy"
    ? 15
    : (state.gameDifficulty === "hard" ? -15 : 0);
}
...
export function getAdaptiveSpeakAllowance() {
  const base = state.gameDifficulty === "easy"
    ? 3
    : (state.gameDifficulty === "normal" ? 1 : 0);
...
export function setAIDirector(
  ...
  aiDirector.mood = mood || "STEADY";
  aiDirector.note = note || "AI กำลังดูจังหวะการเล่นของคุณ";
...
export function onMissionLoadedForAI(mission, isUnitFinalFn) {
  ...
  const totalTime =
    getBaseTimeForMissionType(mission.type) +
    getDifficultyTimeMod() +
    getAdaptiveTimeBonus() +
    Number(mission._bossTimeAdjust || 0);

  const allow = getAdaptiveSpeakAllowance();
  const isFinal = typeof isUnitFinalFn === "function" ? isUnitFinalFn(mission.id) : false;
  const bossAdj = isFinal ? getAdaptiveBossHpAdjustment() : 0;

  const pickedDiff = mission._selectedDifficulty || state.gameDifficulty;
  ...
  renderQuestionDiffBadge(pickedDiff);
}
