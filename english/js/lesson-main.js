...
window.selectAvatar = function(avatar) {
  ...
  if (!ok) {
    setFeedback("🔒 Avatar นี้ยังไม่ปลดล็อก", "#ff9f43");
  }
};
...
window.claimReward = async function() {
  ...
  try {
    ...
    setFeedback(`🎁 ${rarity.name} CHEST! Streak ${streak} วัน (+${bonus} Pts)${unlockMsg}`, rarity.color);
  } catch (e) {
...
window.setDifficulty = function(level) {
  ...
  setText("ui-desc", `ความยาก: ${level.toUpperCase()} - ${descText}`);
  renderAIDirector();
...
function loadMission(id) {
  ...
  let titleColor = isFinal ? (state.currentMission._bossPatternColor || "#f1c40f") : (isBoss ? "#e74c3c" : "#ff4757");

  let descText = isFinal
  ...
  if (state.currentMission.type === "speaking") {
    ...
    setText("ui-desc", descText + " (ใช้นิ้วจิ้มที่กล่อง หรือกดปุ่ม A, B, C บนคีย์บอร์ดก็ได้)");
...
window.playAudio = function() {
  ...
  if ("speechSynthesis" in window) {
    ...
    setFeedback("🔊 กำลังฟัง... (ตั้งใจฟังให้ดี!)", "#00e5ff");
  }
};
...
window.startRecognition = function() {
  ...
  if (!SpeechRecognition) {
    setFeedback("⚠️ เบราว์เซอร์ไม่รองรับเสียง (แนะนำให้ใช้ Google Chrome)", "#ff9f43");
    return;
  }
  ...
  setFeedback("🎙️ พูดเลย! (กำลังฟัง...)", "#00e5ff");
  ...
  recognition.onresult = (event) => {
    ...
    if (isMatch) {
      ...
      setFeedback(`คุณพูดว่า: "${text}"\n❌ ${state.currentMission.failMsg}`, "#ff4757");
      takeDamage();
  ...
  recognition.onend = () => {
    if (!state.isGameOver && document.getElementById("mission-speaking-scene").getAttribute("visible") === "true") {
      ...
      if (document.getElementById("feedback").innerText.includes("กำลังฟัง")) {
        setFeedback("⚠️ ไมค์ตัดไป (กรุณากดพูดใหม่อีกครั้ง)", "#ff4757");
      }
    }
  };
  ...
  recognition.onerror = (event) => {
    if (event.error === "no-speech") {
      setFeedback("⚠️ ไม่ได้ยินเสียงเลยครับ ลองกดพูดใหม่อีกครั้ง", "#ff4757");
    } else if (event.error === "not-allowed") {
      setFeedback("⚠️ คุณยังไม่ได้อนุญาตการใช้ไมโครโฟนในเบราว์เซอร์", "#ff4757");
    } else {
      setFeedback(`⚠️ เกิดข้อผิดพลาดของไมค์ (${event.error})`, "#ff4757");
    }
    document.getElementById("btn-speak").disabled = false;
  };
...
window.submitScore = async function() {
  ...
  if (!state.currentUser || !state.db) {
    setFeedback("❌ เกิดข้อผิดพลาด ไม่สามารถเชื่อมต่อฐานข้อมูลได้", "#ff4757");
    return;
  }
  ...
  try {
    ...
    setFeedback("❌ เกิดข้อผิดพลาดในการบันทึก", "#ff4757");
    btn.disabled = false;
...
function takeDamage() {
  ...
  if (state.systemHP <= 0) {
    ...
    setFeedback(`💥 MISSION FAILED: SYSTEM OFFLINE! 💥${levelDownMsg}`, "#ff4757");
    ...
    setFeedback(`❌ โดนโจมตี! SYSTEM HP -${damageAmount}%${levelDownMsg}`, "#ff4757");
  }
}
...
window.returnToHub = function() {
  ...
  setTitleBlock("TECHPATH VR: MAIN HUB", "เลือกภารกิจต่อไป (ใช้นิ้วจิ้ม/คลิกที่ป้ายได้เลย)!", "#00e5ff");
  applyUnitTheme(0);
  hide("btn-next");
  hide("btn-return");
  setFeedback("", "#ffffff");
