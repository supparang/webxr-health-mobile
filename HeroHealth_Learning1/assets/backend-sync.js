...
const QUEUE_KEY='herohealth_backend_queue_v3';
const SENT_KEY='herohealth_backend_sent_v3';
...
function envelope(type,suffix,data,stable=true){const s=state(),p=s.profile||{},st=R.status(s),group=R.groupOf(s),rotationZones=R.zonesFor(s);const core={eventType:type,studentId:String(p.studentId||data.studentId||''),profile:{fullName:p.fullName||'',section:p.section||'',group},platformVersion:C.platformVersion||'',rotationGroup:group,rotationZones,currentStep:st.nextStep,progressPct:st.progressPct,completedCount:st.completedCount,missionComplete:st.missionComplete,...data};const idSeed=stable?{type,suffix,studentId:core.studentId,data,group}:core;return{...core,eventId:`HH-${type}-${suffix}-${hash(idSeed)}`,clientTs:new Date().toISOString()}}
...
function snapshot(){const s=state(),p=s.profile;if(!p)return;const st=R.status(s),group=R.groupOf(s),rotationZones=R.zonesFor(s);
 queue(envelope('profile','profile',{studentId:p.studentId,status:'เข้าสู่ระบบตาม Rotation '+rotationZones.join(' → ')}));
 const games=s.gameResults||{};Object.entries(games).forEach(([key,g])=>{if(!g?.completed)return;const [zone,gameId]=key.split(':');queue(envelope('game',key,{status:'จบ '+gameId,game:{zone,gameId,score:Number(g.score)||Number(s.gameScores?.[key])||0,accuracy:Number(g.accuracy)||0,passed:g.passed===true,completed:true,finishedAt:g.finishedAt||'',sessionId:g.sessionId||'',startedAt:g.startedAt||'',rotationGroup:group,rotationZones}}))});
 ...
 queue(envelope('progress','progress',{status:st.missionComplete?'Mission Complete':'อัปเดตความคืบหน้า',progress:{progressPct:st.progressPct,completedCount:st.completedCount,totalSteps:st.totalSteps,nextStep:st.nextStep,missionComplete:st.missionComplete,rotationGroup:group,rotationZones}}));
}
...
addEventListener('online',flush);addEventListener('storage',e=>{if(e.key===STATE_KEY)setTimeout(snapshot,50)});addEventListener('visibilitychange',()=>{if(!document.hidden){snapshot();heartbeat()}});addEventListener('error',e=>{const s=state();if(!s.profile)return;queue(envelope('error','runtime-'+Date.now(),{message:e.message||'runtime_error',stack:e.error?.stack||'',studentId:s.profile.studentId},false))});
