(()=>{
'use strict';
const VERSION='20260908-REWARD-CHAMPION-R2-STRICT-SIX-GAMES';
...
const REQUIRED_GAMES=Object.keys(GAME_BADGES);
function read(){try{return JSON.parse(localStorage.getItem(KEY)||'{}')}catch(_){return{}}}
function write(s){try{localStorage.setItem(KEY,JSON.stringify(s))}catch(_){}}
function gameDone(s,zone,game){
  if(s?.gameCompleted?.[zone]?.[game]===true)return true;
  if(zone==='fitness'&&game==='balance-hold'){
    return s?.gameCompleted?.fitness?.balance===true||s?.gameCompleted?.fitness?.balancehold===true;
  }
  return false;
}
function allGamesDone(s){
  return REQUIRED_GAMES.every(id=>{
    const [zone,game]=id.split(':');
    return gameDone(s,zone,game);
  });
}
function zoneDone(s,zone){
  return REQUIRED_GAMES.filter(id=>id.startsWith(`${zone}:`)).every(id=>{
    const [,game]=id.split(':');
    return gameDone(s,zone,game);
  });
}
function badgeState(s){
  const games=Object.entries(GAME_BADGES).map(([id,b])=>{const [z,g]=id.split(':');return{id,...b,earned:gameDone(s,z,g)}});
  const zones=Object.entries(ZONE_BADGES).map(([z,b])=>({id:z,...b,earned:zoneDone(s,z)}));
  return{games,zones,champion:allGamesDone(s)};
}
