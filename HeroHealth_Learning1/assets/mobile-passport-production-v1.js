...
function firebaseSessionReady(s){
 if(!isFirebase()||window.__HH_FIREBASE_LOGIN_REQUIRED__===true)return false;
 const sid=queryStudentId();
 if(!sid)return false;
 return String(s?.profile?.studentId||'')===sid&&String(s?.firebaseAuthority?.studentId||'')===sid&&s?.view==='student';
}
...
function ensureMobileCta(s){
 ...
 const authenticated=!isFirebase()||firebaseSessionReady(s);
