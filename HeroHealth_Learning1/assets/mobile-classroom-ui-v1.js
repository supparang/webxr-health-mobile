...
function bind(){const f=document.getElementById('game');if(!f)return;const run=()=>{try{inject(f.contentDocument)}catch(_){}};f.addEventListener('load',()=>{run();setTimeout(run,250);setTimeout(run,1000);setTimeout(run,2500)});run()}
function refresh(){const nd=detectDevice(),nv=detectView();if(nd===device&&nv===view)return;device=nd;view=nv;mark();bind()}
