(()=>{
'use strict';
const VERSION='20260729-SHEET-AUTHORITY-HARD-RESET-V2';
...
function topProgress(api){
  if(api?.live&&isFalse(api.live.missionComplete)){
    return{progressPct:Number(api.live.progressPct)||0,completedCount:Number(api.live.completedCount)||0,totalSteps:9,nextStep:api.live.currentStep||api?.progress?.nextStep||'pretest',missionComplete:false};
  }
  if(api?.progress&&isFalse(api.progress.missionComplete))return api.progress;
  if(api?.progress)return api.progress;
  return api?.authoritativeState?.progress||null;
}
