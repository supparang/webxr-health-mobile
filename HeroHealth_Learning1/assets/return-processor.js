...
function clean(){['hhReturn','task','zone','gameId','passed','score','accuracy','eventId','studentId'].forEach(k=>q.delete(k));const next=location.pathname+(q.toString()?`?${q}`:'')+location.hash;history.replaceState({},'',next)}
const s=load();
...
if(kind==='game'){
 const zone=q.get('zone'),gameId=q.get('gameId'),passed=q.get('passed')==='true';
 if(!passed||!['hygiene','nutrition','fitness'].includes(zone)||!gameId){clean();return}
