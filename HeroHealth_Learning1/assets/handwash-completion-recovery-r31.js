...
function show(text){let n=document.getElementById('hh-handwash-recovery-r31');if(!n){n=document.createElement('div');n.id='hh-handwash-recovery-r31';n.style.cssText='position:fixed;inset:0;z-index:999999;display:grid;place-items:center;padding:26px;background:rgba(2,16,24,.92);color:#fff;font:800 17px/1.55 system-ui;text-align:center;white-space:pre-line';document.body.appendChild(n)}n.textContent=text;}
...
function compactEvents(raw){const arr=Array.isArray(raw?.events)?raw.events:Array.isArray(raw?.eventLog)?raw.eventLog:[];return arr.slice(-80).map((event,index)=>({eventName:String(event?.eventName||event?.event||event?.type||event?.name||'event').slice(0,80),ts:String(event?.ts||event?.timestamp||event?.clientTs||event?.time||'').slice(0,80),stepId:String(event?.stepId||event?.phase||'').slice(0,60),value:typeof event?.value==='number'||typeof event?.value==='boolean'?event.value:String(event?.value??'').slice(0,160),index:index+1}));}
function compactGame(raw,a){
 ...
 return{...out,zone:'hygiene',gameId:'handwash',completed:true,passed:yes(raw?.passed),skillCriteriaMet:yes(raw?.passed),procedureCompleted:true,progressionEligible:true,completionPolicy:'recovered-complete-7-rub-12-phase-r31',analyticsSchemaVersion:String(raw?.analyticsSchemaVersion||'HH-UNIFIED-GAME-ANALYTICS-V2'),analyticsRecoveryRelease:RELEASE,totalSteps:12,completedSteps:12,totalWhoRubSteps:7,whoStepsTotal:7,whoStepsCompleted:7,completedRubSteps:7,totalProcessSteps:5,completedProcessSteps:5,wristsPassed:true,processCompliancePct:100,metricCompletenessPct:a.analyticsPct,steps,stepResults:steps,events:compactEvents(raw),finishedAt:raw?.finishedAt||raw?.timestamp||raw?.endedAt||new Date().toISOString()};
}
async function submitPost(payload){
 const body=JSON.stringify(payload);
 try{
  await fetch(endpoint(),{method:'POST',mode:'no-cors',cache:'no-store',headers:{'Content-Type':'text/plain;charset=utf-8'},body,keepalive:true});
  return true;
 }catch(error){
  console.warn('[Handwash recovery R31] POST failed, trying compact JSONP',error);
  if(body.length>7000)throw error;
  const ack=await jsonp({action:'submit',payload:body},22000);
...
async function pollAuthority(sid){
 ...
 for(let i=0;i<12;i++){
  ...
  await sleep(700+Math.min(2200,i*250));
 }
 throw new Error(last?.error||'authority_not_advanced');
}
async function run(){
 const state=read(STATE_KEY,{}),sid=String(state?.profile?.studentId||'').trim();
 if(!sid||state?.gameCompleted?.hygiene?.handwash===true)return{recovered:false,reason:'not_needed'};
 const raw=read(RESULT_KEY,null);if(!raw)return{recovered:false,reason:'no_local_result'};
 const a=audit(raw);if(!a.valid)return{recovered:false,reason:'result_not_complete',audit:a};
 const rawSid=String(raw.studentId||raw.participantId||raw.pid||'').trim();if(rawSid&&rawSid!=='anon'&&rawSid!==sid)return{recovered:false,reason:'student_mismatch'};
 ...
 show(`ตรวจพบ Handwash ที่ทำครบแล้ว\nท่าถู ${a.rubDone}/7 • กระบวนการ ${a.processDone}/5 • Analytics ${a.analyticsPct}%\nกำลังส่งหลักฐานให้ Google Sheet…`);
 const profile=state.profile||{},eventId=`HH-recover-r31-${sid}-${attempt}`;
 const payload={eventType:'game',eventId,studentId:sid,profile:{fullName:profile.fullName||'',section:profile.section||'',group:profile.group||''},clientTs:new Date().toISOString(),currentStep:'hygiene:handwash',status:'Recovered completed Handwash 7/7 R31',game:compactGame(raw,a)};
...
window.__HH_HANDWASH_RECOVERY_PROMISE__=run().catch(error=>{console.error('[Handwash recovery R31]',error);window.__HH_HANDWASH_RECOVERY_ERROR__=String(error?.message||error);const n=document.getElementById('hh-handwash-recovery-r31');if(n){n.textContent='Google Sheet ยังไม่ยืนยัน Handwash\nระบบจะลองใหม่เมื่อรีเฟรชหน้า';setTimeout(()=>n.remove(),5500)}return{recovered:false,error:String(error?.message||error)}}).finally(()=>{window.__HH_HANDWASH_RECOVERY_ACTIVE__=false});
