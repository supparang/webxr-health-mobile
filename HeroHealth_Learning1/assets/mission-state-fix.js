(()=>{
'use strict';
const VERSION='20260814-MISSION-TIMELINE-RESEARCH-R16';
...
function displayLabel(step){const labels={pretest:'Pre-test — แบบทดสอบก่อนเริ่มภารกิจ','nutrition:groups':'Food Groups AR — จัดอาหารให้ถูก 5 หมู่','nutrition:goodjunk':'GoodJunk AR — เลือกอาหารสุขภาพ','fitness:jumpduck':'JumpDuck AR — กระโดดและหลบ','fitness:balance-hold':'Balance Hold AR — ฝึกการทรงตัว','hygiene:handwash':'Handwash AR — ล้างมือ 7 ขั้นตอน','hygiene:toothbrush':'Toothbrush AR — แปรงฟันให้สะอาด',posttest:'Post-test — แบบทดสอบหลังจบภารกิจ',postExperience:'แบบประเมินหลังเล่น — UX • Engagement • Satisfaction',reflection:'Reflection — สะท้อนการเรียนรู้'};return labels[step.id]||step.label}
function currentGuidance(){return matchMedia('(max-width:700px)').matches?'ภารกิจปัจจุบัน — แตะแถบด้านล่างเพื่อเริ่ม':'ภารกิจปัจจุบัน — กดปุ่มเริ่มด้านบน'}
function patch(){const s=load();if(!s||!s.profile)return;const st=R.status(s),width=st.progressPct+'%';document.querySelectorAll('.progress span').forEach(el=>{if(el.style.width!==width)el.style.width=width});document.querySelectorAll('p').forEach(el=>{if(/% ของภารกิจ/.test(el.textContent||'')){const text=st.progressPct+'% ของภารกิจ';if(el.textContent!==text)el.textContent=text}});const timeline=document.querySelector('.timeline');if(timeline){const current=st.route.findIndex(step=>!R.done(s,step));const html=st.route.map((step,i)=>{const ok=R.done(s,step),now=!ok&&i===current,rowClass=ok?'done':now?'current':'locked',detail=ok?'เสร็จเรียบร้อยแล้ว':now?currentGuidance():'ยังไม่ถึงขั้นนี้',badge=ok?'✓ ผ่านแล้ว':now?'พร้อมเริ่ม':'🔒 ยังไม่เปิด';return `<div class="step ${rowClass}" data-step-id="${esc(step.id)}" aria-current="${now?'step':'false'}"><div class="num">${ok?'✓':i+1}</div><div><b>${esc(displayLabel(step))}</b><div class="small muted">${detail}</div></div><span class="badge ${ok?'ok':now?'warn':'locked'}">${badge}</span></div>`}).join('');const signature=VERSION+'|'+R.groupOf(s)+'|'+(matchMedia('(max-width:700px)').matches?'mobile':'desktop')+'|'+html;if(timeline.dataset.rotationRoute!==signature){timeline.innerHTML=html;timeline.dataset.rotationRoute=signature}}}
