...
if(!config)return;
config.platformVersion=String(config.platformVersion||'HeroHealth')+'-JUMPDUCK-V58-BALANCE-V47';
config.deploymentState=String(config.deploymentState||'QA')+'_JUMPDUCK_RESULT_LAYOUT_V58_BALANCE_BALANCED_BANK_V47';
const fitness=(config.zones||[]).find(zone=>zone&&zone.id==='fitness');
const jumpduck=fitness?.games?.find(game=>game&&game.id==='jumpduck');
if(jumpduck){
  jumpduck.url='../fitness/jumpduck-classroom-v26-ar.html?v=20260731-result-layout-v58';
  jumpduck.status='classroom-core-v5.8-compact-result-layout';
...
if(classroom){
  classroom.description=String(classroom.description||'')+' JumpDuck V5.8 คง MoveNet ทิศทางซ้าย–ขวา Camera Hold Visual Polish และหน้าสรุปภาษาไทยเดิม พร้อมแก้หน้าผลให้เริ่มจากด้านบนทุกครั้ง ลดระยะและขนาดองค์ประกอบตามความสูงหน้าจอ แยกพื้นที่ปุ่มกลับ Passport ออกจากการ์ด และลดการเกิด scrollbar โดยไม่แก้ระบบตรวจจับร่างกายหรือเกณฑ์คะแนน Balance Hold V47 คง 6 ท่าต่อรอบเพื่อเหมาะกับฐาน 10 นาที แต่สุ่มจากลำดับสมดุล 12 รูปแบบ โดยทุกคนได้รับท่ามาตรฐานชุดเดียวกันครบทั้งซ้าย–ขวาและไม่ซ้ำรูปแบบเดิมทันที พร้อมบันทึก Sequence ID และลำดับจริงเพื่อวิเคราะห์งานวิจัย';
}
})();