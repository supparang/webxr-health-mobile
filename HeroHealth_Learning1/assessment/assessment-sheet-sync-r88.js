...
const VERSION='20260730-ASSESSMENT-AUTHORITY-FIRST-R88';
const QUEUE_KEY='HH_ASSESSMENT_SYNC_QUEUE_V8';
const DURABLE_PREFIX='HH_ASSESSMENT_LAST_V8:';
...
function jsonp(url,params,timeoutMs=15000){
  return new Promise((resolve,reject)=>{
    const cb='HHAS88_'+Date.now()+'_'+Math.random().toString(36).slice(2,9);
    const script=document.createElement('script');
    let settled=false;
    const finish=(err,data)=>{
      if(settled)return;settled=true;clearTimeout(timer);script.onerror=null;
      try{delete window[cb]}catch(_){}try{script.remove()}catch(_){}
    ...
    script.async=true;
    script.src=url+(url.includes('?')?'&':'?')+new URLSearchParams({...params,callback:cb,_:String(Date.now()),transport:'jsonp-short-r88',clientVersion:VERSION}).toString();
...
function formPost(url,event){
  return new Promise((resolve,reject)=>{
    try{
      const token='HHAS88FORM_'+Date.now()+'_'+Math.random().toString(36).slice(2,8),iframe=document.createElement('iframe'),form=document.createElement('form');
      iframe.name=token;iframe.style.cssText='position:fixed;width:1px;height:1px;opacity:0;pointer-events:none;border:0;left:-9999px;top:-9999px';iframe.setAttribute('aria-hidden','true');
      ...
      const fields={action:'submit',payload:JSON.stringify(event),transport:'hidden-form-post-r88',clientVersion:VERSION,_:String(Date.now())};
      ...
      (document.body||document.documentElement).appendChild(iframe);(document.body||document.documentElement).appendChild(form);form.submit();
...
function assessmentConfirmed(api,payload){
  ...
  const mode=String(payload.mode||'').toLowerCase();
  const completed=state.completed||api.completed||{};
  const scores=state.scores||api.scores||{};
  const key=mode==='post'?'posttest':'pretest';
  const done=completed[key]===true;
  const score=numericScore(scores[key]);
  const expected=numericScore(payload.score);
  const scoreAccepted=score!==null&&(expected===null||score===expected);
  return done||scoreAccepted;
}
async function verifyStudent(url,payload){
  const api=await jsonp(url,{action:'student',studentId:String(payload.studentId||'').trim(),reconcile:'1',force:'1'},15000);
...
async function sendAndVerify(url,payload){
  ...
  const existing=await authoritySnapshot(url,payload,event.eventId);
...
async function submit(rawPayload){
  const payload=enrich(rawPayload);persist(payload);const url=endpoint();
  ...
  if(result.ok){
    const sid=String(payload.studentId||'').trim(),mode=String(payload.mode||'').toLowerCase();
    writeJson(DURABLE_PREFIX+sid+':'+mode+':CONFIRMED',{attemptId:payload.attemptId,eventId:result.eventId,score:payload.score,total:payload.total,transport:result.transport,confirmationSource:result.confirmationSource,confirmedAt:new Date().toISOString(),version:VERSION});
  ...
  queue(payload);return{...result,configured:true,queued:true,version:VERSION};
}
async function flush(){
  ...
  writeQueue(remaining);return{ok:remaining.length===0,configured:true,sent,remaining:remaining.length,version:VERSION};
}
