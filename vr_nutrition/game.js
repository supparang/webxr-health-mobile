(()=>{
  ...
  const clog=(msg)=>{ const el=$("#clicklog"); el.textContent="clicklog: "+msg; };

  // Prevent hidden overlays from stealing clicks
  const hideSummary=()=>{ const s=$("#summary"); s.classList.add('overlay-hidden'); };
  const showSummary=()=>{ const s=$("#summary"); s.classList.remove('overlay-hidden'); };

  // Robust safeClick binder
  function safeClick(el, fn){
    if(!el) return;
    el.addEventListener('click', (e)=>{ e.preventDefault(); e.stopPropagation(); fn(e); }, false);
  ...
  const I18N={ th:{start:"เริ่มเกม",pause:"พัก",restart:"เริ่มใหม่",score:"คะแนน",time:"เวลา",best:"สถิติ",mode:"โหมด",diff:"ความยาก",combo:"คอมโบ",
                    modeGJ:"ดี vs ขยะ",modeGroups:"จาน 5 หมู่",modeHydration:"Hydration",modePlate:"Build Plate",summary:"สรุปผล"},
  ...
  $("#vrSlot").replaceWith(vrBtn);
  vrBtn.style.zIndex = 2147483647; // ensure clickable
  ...
  function spawn(){
    ...
    group.add(cube); ACTIVE.add(cube);
  ...
  function pickMeta(){
    ...
    const g=foods.groups[Math.floor(Math.random()*foods.length)];
  ...
  function handleGaze(){
    ...
    if(inter.length>0){
      ...
      else if(performance.now()-gazeStart >= DWELL){
  ...
  function start(){
    ...
    (function kick(){ const stuck=(seenT===state.timeLeft)||(seenS===SPAWN_COUNT); if(stuck){ clearTimeout(spH); clearTimeout(tmH); setTimeout(()=>{ spawn(); loop(); timer(); },150); } seenT=state.timeLeft; seenS=SPAWN_COUNT; watchdogH=setTimeout(kick,1200); })();
  }
  ...
  // PUBLIC UI + programmatic bindings (double-bound)
  const UI={
    start: ()=>{ clog('Start clicked'); start(); },
    pause: ()=>{ clog('Pause clicked'); pause(); },
    restart: ()=>{ clog('Restart clicked'); state.running=false; state.paused=false; clearTimeout(spH); clearTimeout(tmH); SPAWN_COUNT=0; start(); },
    setMode: (m)=>{ state.mode=m; localStorage.setItem('ng_mode',m); applyLang(); clog('Mode='+m); },
    setDiff: (d)=>{ state.difficulty=d; localStorage.setItem('ng_diff',d); updateHUD(); clog('Diff='+d); },
    toggleLang: ()=>{ state.lang=(state.lang==='th'?'en':'th'); localStorage.setItem('ng_lang',state.lang); applyLang(); clog('Lang='+state.lang); }
  };
  window.UI=UI;

  // Bind buttons defensively
  safeClick($("#btnStart"), UI.start);
  safeClick($("#btnPause"), UI.pause);
  safeClick($("#btnRestart"), UI.restart);
  safeClick($("#btnRestart2"), UI.restart);
  safeClick($("#btnLang"), UI.toggleLang);
  safeClick($("#mGJ"), ()=>UI.setMode('goodjunk'));
  safeClick($("#mGR"), ()=>UI.setMode('groups'));
  safeClick($("#mHY"), ()=>UI.setMode('hydration'));
  safeClick($("#mPL"), ()=>UI.setMode('plate'));
  safeClick($("#dE"), ()=>UI.setDiff('Easy'));
  safeClick($("#dN"), ()=>UI.setDiff('Normal'));
  safeClick($("#dH"), ()=>UI.setDiff('Hard'));

  // Global click logger (top-most element)
  addEventListener('click', (e)=>{
    const top=document.elementFromPoint(e.clientX,e.clientY);
    clog(`clicked target=#${e.target.id||e.target.tagName} top=#${top && (top.id||top.tagName)} `);
  }, true);
