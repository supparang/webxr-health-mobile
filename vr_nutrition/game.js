(()=>{
  ...
  // Expose read-only state for watchdog
  ...
  window.__APP_READ = ()=>({timeLeft:APP.timeLeft, running:APP.running, spawnCount:window.__SPAWN_COUNT||0});

  const I18N={ th:{start:"เริ่มเกม",pause:"พัก",how:"วิธีเล่น",restart:"เริ่มใหม่",
  ...
  const FEVER_MULT=2.0, FEVER_HIT_BONUS=1, FEVER_TIME_MS=6000; let feverTimer=null;
  (function ensureMission(){ const today = new Date().toISOString().slice(0,10);
    if(!APP.mission || APP.mission.date !== today){ APP.mission = { date: today, goal: 140, achieved: false };
  ...
  function applyLang(){
    ...
    $("#sumTitle").textContent=t("summary"); $("#missionTag").textContent = APP.lang==="th" ? I18N.th.daily : I18N.en.daily;
  ...
  function setMode(m){
    ...
    $("#modeName").textContent=
      APP.mode==="goodjunk"?t("modeGJ"):APP.mode==="groups"?t("modeGroups"):APP.mode==="hydration"?t("modeHydra"):APP.mode==="plate"?t("modePlate"):APP.mode;
  ...
  function spawnOne(){
    ...
    root.appendChild(ent); ACTIVE_ENTS.add(ent); window.__SPAWN_COUNT++;
  ...
  function enterFever(ms=FEVER_TIME_MS){
    ...
    feverTimer._end = performance.now() + ms;
  }
  ...
  function handleHit(ent){
    ...
    if(APP.fever && delta>0){
      delta = Math.round(delta * FEVER_MULT) + FEVER_HIT_BONUS;
      extendFever(250);
    }
  ...
  function startGame(){
    ...
    if(!APP.running){
      ...
      document.body.classList.remove("fever-active"); APP.fever=false; if(feverTimer){clearTimeout(feverTimer);feverTimer=null;}
    ...
    setTimeout(()=>{ if(window.__SPAWN_COUNT===0){ try{ spawnOne(); }catch(e){} } }, 400);
    loop(); timerTick();
  }
  ...
  const REAL = {
    ...
    restart: ()=>{ hideSummary(); APP.running=false; APP.paused=false; clearTimeout(spawnerHandle); window.__SPAWN_COUNT=0; startGame(); },
    how: ()=>{ const html = `
      <div class="how-grid">
