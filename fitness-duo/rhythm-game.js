// Hygiene Rhythm — fixed export & scene gate (packed)
...
function endStage(){ state.running=false; cancelAnimationFrame(state.raf); const ok=cleared(); const bonus=ok?CH.bonus:0; totalScore+=state.score; totalBonus+=bonus;
  ...
  if(chain && stageIndex<2){ stageIndex++; setTimeout(()=>start(),900); } else { const s=loadSave(); s.lastScore=totalScore+totalBonus; s.lastCombo=state.best; s.lastAcc= state.accTotal? (state.accHit/state.accTotal):0.0; s.lastChallenge=CH; s.ts=Date.now(); saveSave(s);
