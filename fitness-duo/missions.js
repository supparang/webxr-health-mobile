...
(function(){
  ...
  function rng(seed){ let h=0; for(let i=0;i<seed.length;i++) h=((h<<5)-h + seed.charCodeAt(i))|0;
    return ()=>{ h|=0; h=h+0x6D2B79F5|0; let t=Math.imul(h^h>>>15,1|h); t^=t+Math.imul(t^t>>>7,61|t); return ((t^t>>>14)>>>0)/4294967296; }; }
  ...
  function isCleared(task, last){
    const v = {
      score: last.score || 0, combo: last.combo || 0, acc: last.acc || 0,
      orbs:  last.orbs  || 0, dodges:last.dodges||0, fever:last.fever||0, play:1
    }[task.metric] || 0;
  ...
  function claim(id){
    ...
    m.claimed[id]=true; save(m); applyReward(t.reward||{});
    return true;
  }
  function getProgress(){
    const m=refresh();
    const res={daily:[], weekly:[], chain:[]};
    function decorate(list){ return list.map(x=>({...x, claimed: !!m.claimed[x.id]})); }
  ...
  window.addEventListener("duo:session", (ev)=>{
    const s=ev.detail||{}; const m=refresh();
    ...
    const idx = m.chain.index; const cur = m.chain.tasks[idx];
