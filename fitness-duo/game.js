...
window.APP = (function(){
  ...
  function startGame({mode='practice'}={}){
    ...
    $('hudMode').textContent = mode==='timed' ? 'จับเวลา' : 'โหมดฝึก';
    $('hudTime').textContent = `เวลา: ${st.timeLeft}`;
  ...
  function openWeeklyMission(){
    ...
    const msg = done ?
      ...
      `สัปดาห์ ${weekId}: ทำคะแนน ${goal}+ in จับเวลา mode to complete.`;
    showToast(msg, 2400);
  }
  function getWeekId(){
    const d = new Date();
...
// Bind menu buttons explicitly (robust on all browsers)
const startBtn = document.getElementById('startTimedBtn');
const practiceBtn = document.getElementById('practiceBtn');
const weeklyBtn = document.getElementById('weeklyBtn');
if (startBtn) startBtn.addEventListener('click', ()=> startGame({mode:'timed'}));
if (practiceBtn) practiceBtn.addEventListener('click', ()=> startGame({mode:'practice'}));
if (weeklyBtn) weeklyBtn.addEventListener('click', ()=> openWeeklyMission());
// Global fallbacks
window.startTimed = ()=> startGame({mode:'timed'});
window.startPractice = ()=> startGame({mode:'practice'});
window.openWeeklyMission = ()=> openWeeklyMission();
...
window.addEventListener('keydown', (e)=>{
  if (e.key === 'Escape' || e.key.toLowerCase() === 'm'){
    if (st.playing){ pause(); } else { backToMenu(); }
  }
});

// Safety: if not playing 1s after load, show menu explicitly
setTimeout(()=>{
  if (!st.playing){
    hud.style.display = 'none';
    resultPanel.style.display = 'none';
