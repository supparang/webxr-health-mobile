/* Fitness Adventure VR — v3 Adaptive+ (packed) */
(()=>{
  ...
  const DIFF={ easy:{speed:1.8,hit:0.42,duration:40,spawnStep:1.05}, normal:{speed:2.3,hit:0.36,duration:52,spawnStep:0.90}, hard:{speed:2.7,hit:0.30,duration:58,spawnStep:0.78} };
  ...
  function endStage(){ state.running=false; if(state.rafId) cancelAnimationFrame(state.rafId); stopMusic();
    ...
    next.appendChild(txt); root.appendChild(next); next.addEventListener('click',()=>{ SFX.next(); const diff=DIFF[selectDiff.value||'easy'];
  ...
  function loop(){ if(!state.running) return; const now=performance.now()/1000; state.elapsed=now-state.startTime;
    ...
    for(const p of state.active){ if(!p||p.judged||!p.inUse) continue; const dt=p.time-state.elapsed; p.el.object3D.position.z=dt*state.speed;
      if(Math.abs(dt)<=state.hitWindow){ if(p.kind==='orb'){ if(state.lane===p.lane){ state.totalOrbs++; state.score+=20; state.combo++; SFX.orb(); feedback("เก็บพลังงาน +20","#22c55e"); checkQuestOnEvent({type:'orb'}); adaptOnJudgement(true); } else { feedback("พลาด Orb","#eab308"); adaptOnJudgement(false); state.combo=0; } }
        else { if(state.lane===p.lane){ state.obHit++; state.lives-=1; setLivesUI(state.lives); SFX.hit(); feedback("ชนสิ่งกีดขวาง -1 ชีวิต","#ef4444"); checkQuestOnEvent({type:'obHit'}); adaptOnJudgement(false); state.combo=0; if(state.lives<=0){ gameOver(); return; } }
          else { state.totalObCleared++; state.score+=10; state.combo++; feedback("หลบสำเร็จ +10","#38bdf8"); checkQuestOnEvent({type:'obClear'}); adaptOnJudgement(true); } }
