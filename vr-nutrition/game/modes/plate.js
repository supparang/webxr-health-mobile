...
export function init(state, hud, diff){
  ...
  state.plateTarget={...base}; state.plateQuota={...base};
  render(hud, state);
}
...
export function pickMeta(diff){
  ...
  const arr=G[g]; return {type:'plate', group:g, char: arr[Math.floor(Math.random()*arr.length)]};
}
export function onHit(meta, systems, state, hud){
  ...
  if(state.plateQuota[meta.group]>0){
    ...
    if(Object.values(state.plateQuota).every(v=>v<=0)){
      systems.score.add(14); // Perfect Plate bonus
      init(state, hud, 'Normal'); // reset new plate
    }
  }else{
