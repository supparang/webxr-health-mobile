...
function feverStart(){ if(FEVER_ACTIVE) return; FEVER_ACTIVE=true; emit('hha:fever',{state:'start',ms:FEVER_MS});
  clearTimeout(feverTimer); feverTimer=setTimeout(function(){feverEnd();},FEVER_MS); }
function feverEnd(){ if(!FEVER_ACTIVE) return; FEVER_ACTIVE=false; emit('hha:fever',{state:'end'});
...
function makeTarget(emoji, good, diff){
  ...
  img.addEventListener('click',function(){
    ...
    if(good){
      var base=20+combo*2; var plus=FEVER_ACTIVE?base*2:base;
