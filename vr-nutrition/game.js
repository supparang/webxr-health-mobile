(() => {
  ...
  function loop(){
    ...
    const baseRate = APP.mode==="goodjunk" ? 740 : 780; // สำหรับโหมดจ้อง
  ...
  // ─── Safe bind UI buttons ───
  (function bindUiSafe(){
    const bind = ()=>{
      const map = [
        ["#btnStart",   ()=>{ clearEmojiMenu(); startGame(); }],
        ["#btnPause",   ()=>{ pauseGame(); }],
        ["#btnRestart", ()=>{ document.getElementById("summary").style.display = "none"; APP.running=false; APP.paused=false; clearTimeout(spawnerHandle); startGame(); }],
        ["#btnHow",     ()=>{ alert(APP.mode==="goodjunk" ? (APP.lang==="th"? i18n.th.howGJ : i18n.en.howGJ)
    ...
    bind();
    let tries = 0; const t = setInterval(()=>{ tries++; bind(); if(tries>10) clearInterval(t); }, 300);
  })();
  ...
  (function setupMouseLaser(){
    ...
    // ป้องกันคลิก UI ถูกกลืนโดยระบบเลเซอร์
    const isUiClick = (ev) => {
      const t = ev.target;
      return !!(t && t.closest && t.closest("#menuBar, .hud, .overlay, .targetBox, button, .btn, .tag"));
    };

    // ใช้ THREE อย่างปลอดภัย
    const THREE_NS = window.THREE || (window.AFRAME && AFRAME.THREE);

    function updateLaser(){
      try{
        // ถ้าเป็น VR หุบเลเซอร์เมาส์
    ...
    // ยิงคลิกเฉพาะเมื่อ "คลิกภายในฉาก" และไม่ใช่ปุ่ม UI
    document.addEventListener("mousedown", (e)=>{
      if(e.button !== 0) return;                    // left only
      if (scene.is && scene.is("vr-mode")) return;  // VR ใช้จ้อง
      if (isUiClick(e)) return;                     // ปล่อยให้ UI ทำงาน
      if (!scene.contains(e.target)) return;        // กดนอกฉาก → ไม่ยุ่ง
