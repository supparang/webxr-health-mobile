(() => {
  ...
  // ---------- HUD / I18N apply ----------
  function applyLang(){
    $("#lblScore").textContent = t("score", APP.lang);
    $("#lblTime").textContent  = t("time",  APP.lang);
    $("#lblBest").textContent  = t("best",  APP.lang);
    $("#lblMode").textContent  = t("mode",  APP.lang);
    $("#lblDiff").textContent  = t("diff",  APP.lang);
    $("#lblCombo").textContent = "x"+APP.combo;
    $("#sumTitle").textContent = t("summary", APP.lang);
    $("#sumTips").textContent  = APP.lang==="th" ? "VR: จ้องไอเท็มจนวงแหวนครบเพื่อเลือก" : "VR: Gaze until ring completes to select";
    $$("[data-i18n=modeGJ]").forEach(e=>e.textContent=t("modeGJ",APP.lang));
    $$("[data-i18n=modeGroups]").forEach(e=>e.textContent=t("modeGroups",APP.lang));
    $$("[data-i18n=modeHydra]").forEach(e=>e.textContent=t("modeHydra",APP.lang));
    $$("[data-i18n=modePlate]").forEach(e=>e.textContent=t("modePlate",APP.lang));
    $$("[data-i18n=start]").forEach(e=>e.textContent=t("start",APP.lang));
    $$("[data-i18n=pause]").forEach(e=>e.textContent=t("pause",APP.lang));
    $$("[data-i18n=how]").forEach(e=>e.textContent=t("how",APP.lang));
    $$("[data-i18n=restart]").forEach(e=>e.textContent=t("restart",APP.lang));
    ...
    $("#modeName").textContent =
      APP.mode==="goodjunk"  ? t("modeGJ",APP.lang) :
      APP.mode==="groups"    ? t("modeGroups",APP.lang) :
      APP.mode==="hydration" ? t("modeHydra",APP.lang)  : t("modePlate",APP.lang);
  }
  function updateHUD(){
  ...
  function renderQuota(){
    const q=APP.plateQuota || {grains:0,veggies:0,protein:0,fruits:0,dairy:0};
    $("#quotaText").textContent = `Grains:${q.grains}  Veg:${q.veggies}  Prot:${q.protein}  Fruit:${q.fruits}  Dairy:${q.dairy}`;
  }
  ...
  const isAdj=(r,c)=> !lastLane?false:(Math.abs(lastLane[0]-r)<=1 && Math.abs(lastLane[1]-c)<=1);
  ...
  function spawnOne(){
    const root=$("#spawnerRoot"); const maxAct=SPAWN_CFG.maxActive[APP.difficulty]||4;
    if(ACTIVE_ENTS.size>=maxAct) return;

    const life=APP.difficulty==="Hard"?1900:APP.difficulty==="Easy"?4200:3000;
    const lifeJ=Math.floor(Math.random()*500-250);
    ...
    if(APP.mode==="goodjunk"){ good=meta.good===true; delta=good?5*APP.combo:-3; if(!good) comboBreak(); }
      ...
      if(meta.group){
        ...
        if(APP.plateQuota[g]>0){ good=true; APP.plateQuota[g]-=1; renderQuota();
          if(plateQuotaDone()){ delta+=12; resetPlateQuota(); speak("จานครบชุด!","Plate complete!"); }
        } else { delta+=1; }
      }
    }
  ...
  let feverTimer=null;
  function enterFever(ms=6000){ if(APP.fever) return; APP.fever=true; if(feverTimer) clearTimeout(feverTimer); feverTimer=setTimeout(()=>APP.fever=false,ms); }
  ...
  const hideToast   = ()=> { const t=$("#toast"); t && t.classList.remove("show"); };

  // ---------- Lifecycle (สำคัญ: คุมคลิก canvas ผ่าน body.class) ----------
  function startGame(){
    if(APP.running && !APP.paused) return;
    if(!APP.running){
  ...
  function pauseGame(){
    ...
    if(APP.paused){ clearTimeout(spawnerHandle); } else { loop(); timerTick(); }
  }
  function endGame(){
    ...
    showSummary();
    document.body.classList.remove("game-running"); // ปิดคลิก canvas ให้ปุ่มทำงานชัวร์
  }
  ...
  // เริ่มต้นยังไม่เล่น → index คุม pointer-events ให้แล้ว

  // ---------- Bind GAME_UI (จริง) เข้ากับ stub ----------
  const REAL = {
    start: startGame,
    pause: pauseGame,
    restart: ()=>{ hideSummary(); APP.running=false; APP.paused=false; clearTimeout(spawnerHandle); SPAWN_COUNT=0; startGame(); },
    how: ()=>{ alert(APP.mode==="goodjunk"?I18N[APP.lang].howGJ:APP.mode==="groups"?I18N[APP.lang].howGroups:APP.mode==="hydration"?I18N[APP.lang].howHydra:I18N[APP.lang].howPlate); },
