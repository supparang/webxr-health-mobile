...
(function(){
  const stages = Math.max(1, parseInt(new URLSearchParams(location.search).get('stages')||'8',10));
  ...
  window.addEventListener('load',()=>{ $start=id('start'); $end=id('end'); $sum=id('sum'); $hud=id('hud'); $sp=id('spawner'); $cursor=id('cursor'); window.BH={start}; setHUD(); });
