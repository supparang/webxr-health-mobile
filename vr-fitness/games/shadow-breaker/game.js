...
function spawnBossByIndex(i){
  ...
  function bossIntro(){
    ...
    a.appendChild(head); a.appendChild(mask); a.appendChild(nameLabel);
    arena.appendChild(a);

    sfxPlay(SFX.boss_roar,200,0.9);
    bossShowUI(true); bossSetHP(BOSS.max); setPhase(1);

    // แสดงชื่อบอสที่ HUD ถ้ามี
    const rBoss = byId('rBoss');
    if(rBoss){ rBoss.textContent = BOSS.name||'BOSS'; }
  }
  ...
  function spawnPad(){
    ...
    let pool;
    if (DIFF === DIFFS.easy){
      pool = PAD_SPEC.filter(p=>!p.bomb); // Easy: ไม่มีระเบิด
    } else {
  ...
  function onBossDefeated(){
    ...
    // ไปบอสถัดไป (โหมดง่ายจะถึงลึกขึ้น)
    bossIndex++;
    if (bossIndex < BOSSES.length){
      setTimeout(()=>spawnBossByIndex(bossIndex), 900);
    } else {
  ...
  function computeStars(){
    ...
    if(maxCombo >= 15) s += 1;               // คอมโบเริ่มต้น
        ...
        <div class="grid">
          <div>
            ...
            <ul>
              ...
              <li>ชกเป้า (วงกลม/สามเหลี่ยม/สี่เหลี่ยม/ห้าเหลี่ยม/หกเหลี่ยม/เพชร) เพื่อเก็บคะแนนและลด HP บอส</li>
              <li>เลี่ยง <b>ระเบิด</b> — ชนแล้วคอมโบจะถูกรีเซ็ต</li>
            </ul>
            ...
            <h3>คะแนน & คอมโบ</h3>
            <ul>
              <li>Perfect > Good (ได้คะแนน/ดาเมจมากกว่า)</li>
              <li>ไม่กด = ไม่หักคะแนน (เฉพาะ Shadow Breaker)</li>
              <li>Bomb: กดแล้วคอมโบจะถูกรีเซ็ต</li>
              <li>Fever: คอมโบถึงเกณฑ์ (Easy=15 / โหมดอื่น=25) จะได้ x1.5</li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul>
              ...
              <li>วงแหวน (Ring): กดตอนขยายก่อนหมดเวลา</li>
              <li>เพชร (Core): แตะด่วนเพื่อคริติคอล</li>
              <li>ผ่านบอสจะไปตัวต่อไปอัตโนมัติ</li>
            </ul>

            <h3>Bank</h3>
            <ul>
              <li>คอมโบสะสม = กด Bank เพื่อแปลงเป็นคะแนนถาวร</li>
