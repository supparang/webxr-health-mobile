// === VR Fitness — Shadow Breaker (Production v1.1.1) ===
...
// ✅ PACK 3A: prune targets (no leak), boss delay, visibility pause/resume
// ✅ PACK 3B: HHA_LAST_SUMMARY + HHA_SUMMARY_HISTORY
// ✅ PACK 3C: backHub passthrough + from=shadow-breaker
// ✅ Pass-through: hub/view/seed/research/studyId/log etc.

'use strict';

const SB_GAME_ID = 'shadow-breaker';
const SB_GAME_VERSION = '1.1.1-prod';

const SB_STORAGE_KEY = 'ShadowBreakerSessions_v1';
const SB_META_KEY    = 'ShadowBreakerMeta_v1';

// HHA Standard summary keys
...
function sbShowFeedback(type){
  ...
  // IMPORTANT: match CSS variants (.feedback-perfect/.feedback-good/...)
...
// ---------- prune targets (PACK 3A) ----------
function sbPruneTargets(){
  sbState.targets = sbState.targets.filter(t => t && t.alive && t.el && t.el.isConnected);
}
...
function sbGetSafeTopPx(){
  // Prefer CSS var --safe-top if exists (e.g. 64px + safe area), fallback to 56
  try{
    const v = getComputedStyle(document.documentElement).getPropertyValue('--safe-top').trim();
    if(v){
      const n = parseFloat(v);
      if(Number.isFinite(n)) return clamp(n, 48, 160);
    }
  }catch(_){}
  return 56;
}
...
function sbPickSpawnXY(sizePx){
  ...
  // SAFE top zone: avoid HUD/boss bar
...
function sbHitTarget(tObj){
  ...
  if(isBoss){
    ...
    // PACK 3A: next boss with tiny delay (avoid overlap)
    if(sbState.bossQueue.length){
      setTimeout(()=> sbSpawnNextBossImmediate(), 450);
    }
  }
...
function sbStartSpawnLoop(){
  ...
  sbState.spawnTimer = setInterval(()=>{
    ...
    // prune periodically to avoid stale list
    sbPruneTargets();

    if(Math.random() < 0.1) return;                // leave gaps sometimes
    if(sbState.bossActive && Math.random()<0.5) return; // prevent clutter during boss
...
function sbMainLoop(now){
  ...
  // PACK 3A: prune every ~1s
  if(!sbState._lastPrune || (now - sbState._lastPrune) > 1000){
    sbState._lastPrune = now;
    sbPruneTargets();
  }
...
// ---------- PACK 3B: Save HHA summary ----------
function sbSaveHhaSummary(rec){
  const summary = {
    gameId: rec.gameId,
    ...
    reason: rec.reason || '',
    seed: rec.seed || '',
    research: rec.research ? 1 : 0,
  };

  try{ localStorage.setItem(HHA_LAST, JSON.stringify(summary)); }catch(_){}

  try{
    const raw = localStorage.getItem(HHA_HIST);
    const arr = raw ? JSON.parse(raw) : [];
    arr.push(summary);
    while(arr.length > 200) arr.shift();
    localStorage.setItem(HHA_HIST, JSON.stringify(arr));
  }catch(_){}
}
...
// pause/resume from hub postMessage (kept)
...
// PACK 3A: visibility pause/resume (mobile/vr reliability)
document.addEventListener('visibilitychange', ()=>{
  ...
  if(document.hidden){
    ...
    if(sbState.paused){
      sbState.paused = false;
      if(sbState.startTime && sbState.pauseAt){
        sbState.startTime += (performance.now() - sbState.pauseAt);
      }
      sbStartSpawnLoop();
      sbEmit('hha:coach', { text: sbI18n[sbLang].resumed });
      requestAnimationFrame(sbMainLoop);
...
if(sbBackHubBtn){
  sbBackHubBtn.addEventListener('click',()=>{
    const hub = (qs('hub','')||'').trim() || '../hub.html';
    const u = new URL(hub, location.href);
    u.searchParams.set('from', SB_GAME_ID); // PACK 3C
