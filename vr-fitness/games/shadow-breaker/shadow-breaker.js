...
// ✅ Boss HUD bar + safe spawn + feedback
// ✅ Research meta + local session logging + CSV export
...
// ---- query params ----
const sbPhase   = (qs('phase','train')||'train').toLowerCase();   // train/study/etc.
...
// ---- Difficulty config ----
const sbDiffCfg = {
  easy:   { spawnMs: 900, bossHp: 6, lifeMs: 2500, bossLifeMs: 7000 },
  normal: { spawnMs: 700, bossHp: 9, lifeMs: 2300, bossLifeMs: 6500 },
  hard:   { spawnMs: 520, bossHp: 12, lifeMs: 2100, bossLifeMs: 6000 },
};
...
const sbI18n = {
  th: {
    ...
    csvNone:'ยังไม่มีข้อมูล session',
  ...
  en: {
    ...
    csvNone:'No session data yet',
    csvFail:'Unable to build CSV',
  }
};
...
// ---- feedback ----
// NOTE: CSS expects: .feedback + .show + .feedback-good/.feedback-perfect/.feedback-miss/.feedback-fever
function sbShowFeedback(type){
  ...
  sbFeedbackEl.classList.add('feedback-' + type);
...
function sbPickSpawnXY(sizePx){
  ...
  // Keep away from HUD + boss bar area (matches CSS safe-top idea)
  const safeTop = 118; // px (HUD + boss bar zone)
...
function sbSpawnTarget(isBoss=false, bossInfo=null){
  ...
  if(el.animate){
    el.animate(
      [{ transform:'scale(0.70)', opacity:0 }, { transform:'scale(1)', opacity:1 }],
...
function sbHitTarget(tObj){
  ...
  if(tObj.hp > 0){
    ...
    sbEmit('hha:score', {
      ...
      combo: sbState.combo,
      hit: sbState.hit, miss: sbState.miss,
      boss: !!isBoss
    });
    return;
  }
  ...
  if(tObj.el && tObj.el.animate){
    const anim = tObj.el.animate(
      [{ transform:'scale(1)', opacity:1 }, { transform:'scale(0.12)', opacity:0 }],
  ...
  if(isBoss){
    ...
    // next boss immediately (arcade feel)
...
// Optional: keyboard SPACE hits center-ish target on PC
...
function sbDownloadCsv(){
  ...
  try{
    const raw = localStorage.getItem(SB_STORAGE_KEY);
    if(!raw){ alert(t.csvNone); return; }
    const arr = JSON.parse(raw);
    if(!Array.isArray(arr) || !arr.length){ alert(t.csvNone); return; }

    const header=[
      'studentId','schoolName','classRoom','groupCode','deviceType','language','note',
      'phase','mode','diff','gameId','gameVersion','sessionId','timeSec',
      'score','hits','perfect','good','miss','accuracy','maxCombo','fever','timeUsedSec',
      'seed','research','reason','createdAt'
    ];
...
window.addEventListener('message',(ev)=>{
  ...
  if(v && !sbState.paused){
    ...
    sbStopSpawnLoop();
    sbEmit('hha:coach', { gameId: SB_GAME_ID, text: 'Paused' });
    ...
    sbStartSpawnLoop();
    sbEmit('hha:coach', { gameId: SB_GAME_ID, text: 'Resumed' });
  }
});
