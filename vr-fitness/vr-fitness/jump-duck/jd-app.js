...
(function(){
  ...
  const total = Math.max(10, parseInt(Q.get('count')||'30',10));
  ...
  function kbHit(which){
    const list = Array.from($sp.children);
    for(const el of list){
      const src = el.getAttribute('src');
      if(which==='HIGH' && src==='#HIGH'){ el.emit('click'); return; }
      if(which==='LOW'  && src==='#LOW'){  el.emit('click'); return; }
    }
  }
  window.addEventListener('load',()=>{
    $start=id('start'); $end=id('end'); $sum=id('sum'); $hud=id('hud'); $sp=id('spawner');
    window.JD={start}; setHUD();
    document.addEventListener('keydown',e=>{
      ...
      if(e.key==='ArrowUp')   kbHit('HIGH');
      if(e.key==='ArrowDown') kbHit('LOW');
