...
(function(){
  const tempoSel = new URLSearchParams(location.search).get('tempo')||'normal';
  ...
  AFRAME.registerComponent('rb-note', {
    ...
    tick:function(t,dt){
      ...
      p.y -= 0.8*(dt/1000);
      if(p.y < 0.6){ // miss and remove
        if(!this.hit){ S.combo=0; setHUD(); }
        this.el.remove();
      }
    },
    tryHit:function(){
      ...
      const diff=Math.abs(y-1.0); // hit line
      let gain=0;
  ...
  function end(){
    S.playing=false;
    $sum.textContent = `Score ${S.score} | Max Combo ~`;
