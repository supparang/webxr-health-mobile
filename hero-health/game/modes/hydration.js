...
export function pickMeta(diff,state){
  const water=Math.random()<0.66;
  return {type:'hydra',water,char:water?'💧':'🧋'};
}
export function onHit(meta,systems,state,hud){
  if(meta.type!=='hydra') return;
  if(meta.water){ state.hyd=Math.min(100,state.hyd+5); systems.score.add(5);}
  else { state.hyd=Math.max(0,state.hyd-6); systems.score.add(-3); state.ctx.sweetMiss=(state.ctx.sweetMiss||0)+1; }
