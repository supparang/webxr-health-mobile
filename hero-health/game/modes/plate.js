...
export function init(state,hud){
  ...
  const wrap=document.getElementById('plateTracker'); if(wrap) wrap.style.display='block'; renderPills(state);
}
function renderPills(state){
  const pills=document.getElementById('platePills'); if(!pills) return; pills.innerHTML='';
  ...
  Object.keys(QUOTA).forEach(k=>{
    ...
    const el=document.createElement('div'); el.className='pill'+(cur>=need?' done':''); el.textContent=`${labels[k]} ${cur}/${need}`;
...
export function onHit(meta,systems,state){
  const k=meta.group; const need=QUOTA[k]; const cur=state.plate[k]||0;
  if(cur<need){
