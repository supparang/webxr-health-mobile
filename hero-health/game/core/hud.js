export class HUD{
  ...
  setHydration(p,z){ const bar=document.getElementById('hydroBar'); const lab=document.getElementById('hydroLabel'); const wrap=document.getElementById('hydroWrap'); if(wrap) wrap.style.display='block'; if(bar){ bar.style.width=Math.max(0,Math.min(100,p))+'%'; } if(lab){ const label=z==='ok'?'พอดี':(z==='low'?'น้อยไป':'มากไป'); lab.textContent=Math.round(p)+'% '+label; } }
